...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
	...
	let first_model_object =
		first_model.as_object_mut().context("first model was not a JSON object")?;
	...
	std::fs::write(&custom_catalog_path, serde_json::to_string(&source_catalog)?)?;
...
"#
	...
	env.insert("CODEX_HOME".to_string(), codex_home.path().display().to_string());
	...
	let codex_utils_pty::SpawnedProcess { session, stdout_rx, stderr_rx, exit_rx } = spawned;
