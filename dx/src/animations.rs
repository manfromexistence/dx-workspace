...
impl ChatState {
	pub fn render_menu_in_area(&mut self, area: Rect, buf: &mut Buffer) {
		...
		// Check if closing animation is complete BEFORE rendering
		if self.menu_is_closing && !self.menu.active_effect.1.running() {
			// Animation finished, clean up immediately
			self.menu_is_closing = false;
			self.show_tachyon_menu = false;
			// If we were in content viewer mode, close it now
			if self.menu.content_viewer_mode {
				self.menu.close_content_viewer();
			}
		}
		...
		if self.show_tachyon_menu || self.menu_is_closing {
			...
			let content_height = if self.menu.content_viewer_mode {
				// Content viewer uses fixed height of 20
				20.min(area.height)
			} else {
				// Regular menu uses 75% of height
				(area.height * 75 / 100).min(32)
			};
			...
			// Clear the area behind the menu first to ensure solid background
			// This prevents content from showing through
			let bg_color = self.menu.theme.card;
			...
			// Now render the menu content on top of the solid background
