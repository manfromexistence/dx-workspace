...
pub(crate) fn new_debug_config_output(
	...
	if let Some(proxy) = session_network_proxy {
		...
		let SessionNetworkProxyRuntime { http_addr, socks_addr } = proxy;
...
fn session_all_proxy_url(http_addr: &str, socks_addr: &str, socks_enabled: bool) -> String {
	if socks_enabled { format!("socks5h://{socks_addr}") } else { format!("http://{http_addr}") }
}
...
fn render_debug_config_lines(stack: &ConfigLayerStack) -> Vec<Line<'static>> {
	...
	if let Some(modes) = requirements_toml.allowed_sandbox_modes.as_ref() {
		let value =
			join_or_empty(modes.iter().copied().map(format_sandbox_mode_requirement).collect::<Vec<_>>());
		requirement_lines.push(requirement_line(
	...
	if let Some(modes) = requirements_toml.allowed_web_search_modes.as_ref() {
		...
		let value = join_or_empty(normalized.iter().map(ToString::to_string).collect::<Vec<_>>());
		requirement_lines.push(requirement_line(
	...
	if let Some(servers) = requirements_toml.mcp_servers.as_ref() {
		...
		requirement_lines.push(requirement_line(
			"mcp_servers",
			value,
			requirements.mcp_servers.as_ref().map(|sourced| &sourced.source),
		));
	}
...
fn render_session_flag_details(config: &TomlValue) -> Vec<Line<'static>> {
	...
	pairs.into_iter().map(|(key, value)| format!("     - {key} = {value}").into()).collect()
}

fn render_mdm_layer_details(layer: &ConfigLayerEntry) -> Vec<Line<'static>> {
	let value =
		layer.raw_toml().map(ToString::to_string).unwrap_or_else(|| format_toml_value(&layer.config));
	if value.is_empty() {
	...
	match value {
		TomlValue::Table(table) => {
			...
			for (key, child) in entries {
				let next_prefix =
					if let Some(prefix) = prefix { format!("{prefix}.{key}") } else { key.to_string() };
...
fn requirement_line(
	...
	let source = source.map(ToString::to_string).unwrap_or_else(|| "<unspecified>".to_string());
	format!("  - {name}: {value} (source: {source})").into()
}

fn join_or_empty(values: Vec<String>) -> String {
	if values.is_empty() { "<empty>".to_string() } else { values.join(", ") }
}
...
fn format_network_constraints(network: &NetworkConstraints) -> String {
	...
	if let Some(dangerously_allow_non_loopback_proxy) = dangerously_allow_non_loopback_proxy {
		parts
			.push(format!("dangerously_allow_non_loopback_proxy={dangerously_allow_non_loopback_proxy}"));
	}
	if let Some(dangerously_allow_all_unix_sockets) = dangerously_allow_all_unix_sockets {
		parts.push(format!("dangerously_allow_all_unix_sockets={dangerously_allow_all_unix_sockets}"));
	}
	...
	if let Some(allow_unix_sockets) = allow_unix_sockets {
		parts.push(format!("allow_unix_sockets=[{}]", allow_unix_sockets.join(", ")));
	}
...
mod tests {
	...
	fn debug_config_output_lists_all_layers_including_disabled() {
		...
		let project_folder =
			if cfg!(windows) { absolute_path("C:\\repo\\.codex") } else { absolute_path("/repo/.codex") };

		let layers = vec![
			ConfigLayerEntry::new(ConfigLayerSource::System { file: system_file }, empty_toml_table()),
			ConfigLayerEntry::new_disabled(
				ConfigLayerSource::Project { dot_codex_folder: project_folder },
				empty_toml_table(),
	...
	fn debug_config_output_lists_requirement_sources() {
		...
		let requirements = ConfigRequirements {
			...
			sandbox_policy: ConstrainedWithSource::new(
				...
				Some(RequirementSource::SystemRequirementsToml { file: requirements_file.clone() }),
			),
			mcp_servers: Some(Sourced::new(
				BTreeMap::from([(
					"docs".to_string(),
					McpServerRequirement {
						identity: McpServerIdentity::Command { command: "codex-mcp".to_string() },
					},
				)]),
		...
		let stack = ConfigLayerStack::new(
			vec![ConfigLayerEntry::new(ConfigLayerSource::User { file: user_file }, empty_toml_table())],
			requirements,
			requirements_toml,
		)
		.expect("config layer stack");
		...
		assert!(
			rendered.contains("allowed_web_search_modes: cached, disabled (source: cloud requirements)")
		);
...
"#,
		...
		let stack = ConfigLayerStack::new(
			vec![ConfigLayerEntry::new(ConfigLayerSource::SessionFlags, session_flags)],
			ConfigRequirements::default(),
			ConfigRequirementsToml::default(),
		)
		.expect("config layer stack");
...
"#;
	...
	fn debug_config_output_normalizes_empty_web_search_mode_list() {
		...
		let rendered = render_to_text(&render_debug_config_lines(&stack));
		assert!(rendered.contains("allowed_web_search_modes: disabled (source: cloud requirements)"));
	}
