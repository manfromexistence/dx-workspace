...
impl Command for SetWindowTitle {
	...
	fn execute_winapi(&self) -> io::Result<()> {
		Err(std::io::Error::other("tried to execute SetWindowTitle using WinAPI; use ANSI instead"))
	}
...
mod tests {
	...
	fn writes_osc_title_with_string_terminator() {
		...
		SetWindowTitle("hello".to_string()).write_ansi(&mut out).expect("encode terminal title");
