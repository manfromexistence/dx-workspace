...
impl CommandPopup {
	pub(crate) fn new(mut prompts: Vec<CustomPrompt>, flags: CommandPopupFlags) -> Self {
		...
		Self { command_filter: String::new(), builtins, prompts, state: ScrollState::new() }
	}

	pub(crate) fn set_prompts(&mut self, mut prompts: Vec<CustomPrompt>) {
		let exclude: HashSet<String> = self.builtins.iter().map(|(n, _)| (*n).to_string()).collect();
		prompts.retain(|p| !exclude.contains(&p.name));
		prompts.sort_by(|a, b| a.name.cmp(&b.name));
	...
	fn filtered(&self) -> Vec<(CommandItem, Option<Vec<usize>>)> {
		...
		let mut push_match =
			|item: CommandItem, display: &str, name: Option<&str>, name_offset: usize| {
				...
				let name_prefix = name_lower.as_ref().is_some_and(|name| name.starts_with(&filter_lower));
			...
			.map(|(item, indices)| {
				let (name, description) = match item {
					...
					CommandItem::UserPrompt(i) => {
						...
						let description =
							prompt.description.clone().unwrap_or_else(|| "send saved prompt".to_string());
...
mod tests {
	...
	fn filter_includes_init_when_typing_prefix() {
		...
		assert!(has_init, "expected '/init' to appear among filtered commands");
	}
	...
	fn prompt_name_collision_with_builtin_is_ignored() {
		...
		assert!(!has_collision_prompt, "prompt with builtin name should be ignored");
	}
	...
	fn prompt_description_uses_frontmatter_metadata() {
		...
		assert_eq!(description, Some("Create feature branch, commit and open draft PR."));
	}
