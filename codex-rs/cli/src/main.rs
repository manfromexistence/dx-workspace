...
struct ConfigureCommand {
    /// Provider to configure (mistral, groq, deepseek, etc.)
    /// If omitted, scans all providers with API keys in .env
    ...
    /// Skip interactive model selection and use the first available model
    #[arg(long)]
    auto: bool,

    /// Scan all providers with API keys and show interactive selection
...
fn run_configure_provider(
    ...
    Box::pin(async move {
        ...
        use codex_core::api_bridge::CoreAuthProvider;
        use codex_core::default_client::build_reqwest_client;
        use codex_core::model_provider_info::built_in_model_providers;
        use std::io::Write;
        use std::io::{self};

        println!("🔧 DX - Provider Configuration");
        println!("{}", "=".repeat(50));

        let providers = built_in_model_providers(None);

        // If scan mode or no provider specified, scan all providers
        if cmd.scan || cmd.provider.is_none() {
            println!("\n🔍 Scanning all providers with API keys...\n");

            let mut available: Vec<(String, codex_core::model_provider_info::ModelProviderInfo)> =
                Vec::new();

            // First pass: check for API keys
            for (provider_id, provider_info) in providers.iter() {
                ...
                if provider_info.api_key().ok().flatten().is_some() {
                    available.push((provider_id.clone(), provider_info.clone()));
                }
            }
            ...
            if available.is_empty() {
                ...
                println!(
                    "\nPlease set API keys in your environment (e.g., MISTRAL_API_KEY, GROQ_API_KEY)"
                );
                return Ok(());
            }

            println!("Found {} provider(s) with API keys", available.len());
            println!("Fetching default models from each provider...\n");

            // Second pass: fetch models from each provider concurrently
            let mut tasks = Vec::new();

            for (provider_id, provider_info) in available {
                let task = tokio::spawn(async move {
                    let api_key: String =
                        provider_info.api_key().ok().flatten().unwrap_or_default();

                    let transport = ReqwestTransport::new(build_reqwest_client());
                    let api_provider = match provider_info.to_api_provider(None) {
                        Ok(p) => p,
                        Err(_) => {
                            return (
                                provider_id.clone(),
                    ...
                    match client
                        .list_models(env!("CARGO_PKG_VERSION"), Default::default())
                        .await
                    {
                        Ok((models, _)) if !models.is_empty() => (
                            provider_id,
                            provider_info.name,
                            Some(models[0].slug.clone()),
                        ),
                        Ok(_) | Err(_) => {
                            let default = get_default_model_for_provider(&provider_id);
                            (provider_id, provider_info.name, Some(default.to_string()))
                        }
                    }
                });
                tasks.push(task);
            ...
            for task in tasks {
                match task.await {
                    Ok(result) => {
                        let (_provider_id, name, model): &(String, String, Option<String>) =
                            &result;
                        if let Some(m) = model {
                            println!("  ✅ {} - {}", name, m);
                        } else {
                            println!("  ❌ {} - (no models available)", name);
                        }
                        results.push(result);
                    }
                    Err(e) => {
            ...
            if results.is_empty() {
                println!("\n❌ Could not fetch models from any provider");
                return Ok(());
            }
            ...
            for (i, (_, name, model)) in results.iter().enumerate() {
                if let Some(m) = model {
                    println!("{}. {} - {}", i + 1, name, m);
                } else {
                    println!("{}. {} - (no models available)", i + 1, name);
                }
            }
            ...
            let (selected_provider, selected_name, _) = &results[idx - 1];
            println!("\n✅ Selected: {}", selected_name);

            // Now configure the selected provider
            let new_cmd = ConfigureCommand {
            ...
            return run_configure_provider(new_cmd).await;
        }

        // Single provider configuration
        let provider_name = cmd.provider.unwrap();
        let provider_info = providers.get(&provider_name).ok_or_else(|| {
            anyhow::anyhow!(
                "Unknown provider: {}\n\nAvailable providers: {}",
                provider_name,
                providers
        ...
        // Check for API key
        let api_key = match provider_info.api_key() {
            Ok(Some(key)) => {
                println!("✅ API key found: {}...", &key[..8.min(key.len())]);
                key
            }
            Ok(None) => {
                println!("ℹ️  No API key required for this provider");
                String::new()
            }
            Err(e) => {
                eprintln!("\n❌ {}", e);
                ...
                if let Some(instructions) = &provider_info.env_key_instructions {
                    eprintln!("\n{}", instructions);
                }
        ...
        let (models, _etag) = match client
            .list_models(env!("CARGO_PKG_VERSION"), Default::default())
            .await
        {
            Ok(result) => result,
            Err(e) => {
                eprintln!("\n⚠️  Could not fetch models from provider: {}", e);
                eprintln!(
                    "This might be because the provider doesn't support the /v1/models endpoint."
                );
                eprintln!("\nUsing a default model instead...");

                let default_model = get_default_model_for_provider(&provider_name);
                update_config(&provider_name, default_model, cmd.set_default).await?;
                return Ok(());
            }
        };

        if models.is_empty() {
            eprintln!("\n⚠️  No models returned from provider");
        ...
        // Select model
        let selected_model = if cmd.auto {
            models[0].slug.clone()
        } else {
            println!("\n📋 Available models:");
            println!("{}", "-".repeat(50));
            ...
            for (i, model) in models.iter().enumerate().take(20) {
                println!("{}. {} - {}", i + 1, model.slug, model.display_name);
            }
            ...
            if input.is_empty() {
                ...
                if idx > 0 && idx <= models.len() {
                    models[idx - 1].slug.clone()
                } else {
                    println!("⚠️  Invalid selection, using default");
                    models[0].slug.clone()
                }
            } else {
                models
                    ...
                    .map(|m| m.slug.clone())
                    .unwrap_or_else(|| {
                        println!("⚠️  Model not found, using default");
                        models[0].slug.clone()
                    })
            }
        };
        ...
        update_config(&provider_name, &selected_model, cmd.set_default).await?;

        Ok(())
    })
}

fn get_default_model_for_provider(provider: &str) -> &'static str {
    match provider {
...
async fn update_config(provider: &str, model: &str, set_default: bool) -> anyhow::Result<()> {
    ...
    println!("\n🚀 You can now run: codex");
    println!("   Or test with: codex exec \"Hello, who are you?\"");
