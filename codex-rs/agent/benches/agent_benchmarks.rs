//! Performance benchmarks for Agent hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/agent-labs/agent/issues/618 (item 7)
...
use agent::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use agent::config::MemoryConfig;
use agent::memory;
use agent::memory::{Memory, MemoryCategory};
use agent::observability::{NoopObserver, Observer};
use agent::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use agent::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "agent architecture"}}
</tool_call>
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about agent agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("agent agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
