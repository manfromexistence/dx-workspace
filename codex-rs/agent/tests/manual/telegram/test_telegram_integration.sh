#!/bin/bash
# Agent Telegram Integration Test Suite
...
if [ -f "target/release/agent" ]; then
    BINARY_SIZE=$(ls -lh target/release/agent | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/agent 2>/dev/null || stat -c%s target/release/agent)
...
CONFIG_PATH="$HOME/.agent/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'agent onboard' first"
    fi
else
    warn "No config file found - run 'agent onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/agent channel doctor 2>&1 || true)
END_TIME=$(date +%s)
...
6. [ ] Check logs for errors:
   RUST_LOG=debug agent channel start
