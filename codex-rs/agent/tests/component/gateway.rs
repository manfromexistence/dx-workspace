...
//! verification) in isolation. The gateway module (`agent::gateway`) exposes
...
use agent::gateway::verify_whatsapp_signature;
...
fn gateway_timeout_is_reasonable() {
    assert_eq!(
        agent::gateway::REQUEST_TIMEOUT_SECS,
        30,
...
fn gateway_rate_limit_window_is_60s() {
    assert_eq!(
        agent::gateway::RATE_LIMIT_WINDOW_SECS,
        60,
