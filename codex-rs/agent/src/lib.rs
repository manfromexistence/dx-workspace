...
Examples:
  agent gateway start              # use config defaults
  agent gateway start -p 8080      # listen on port 8080
  agent gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  agent gateway start -p 0         # random available port")]
    Start {
...
Examples:
  agent gateway restart            # restart with config defaults
  agent gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  agent gateway get-paircode       # show current pairing code
  agent gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  agent channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  agent channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  agent channel bind-telegram agent_user
...
Examples:
  agent channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  agent channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `Agent` workspace
    Openclaw {
...
Examples:
  agent cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York --agent
  agent cron add '*/30 * * * *' 'Check system health' --agent
  agent cron add '*/5 * * * *' 'echo ok'")]
    Add {
        /// Cron expression
        expression: String,
...
Examples:
  agent cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  agent cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  agent cron add-every 60000 'Ping heartbeat'     # every minute
  agent cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  agent cron once 30m 'Run backup in 30 minutes'
  agent cron once 2h 'Follow up on deployment'
  agent cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  agent cron update <task-id> --expression '0 8 * * *'
  agent cron update <task-id> --tz Europe/London --name 'Morning check'
  agent cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  agent hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  agent peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash Agent firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash Agent firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  agent peripheral flash
  agent peripheral flash --port /dev/cu.usbmodem12345
  agent peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash Agent firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
