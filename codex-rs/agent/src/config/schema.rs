...
///
/// Resolution order: `AGENT_WORKSPACE` env → `active_workspace.toml` marker → `~/.agent/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `AGENT_API_KEY` or `API_KEY` env vars.
    ...
    ///
    /// Can also be set via `AGENT_EXTRA_HEADERS` environment variable using
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `AGENT_LOCALE`,
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all Agent-managed HTTP traffic (default).
    #[default]
    Agent,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "agent" | "internal" | "core" => Some(ProxyScope::Agent),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "agent_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "agent".
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `AGENT_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `AGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
...
/// How Agent receives events from Feishu / Lark.
///
...
fn default_estop_state_file() -> String {
    "~/.agent/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, Agent validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to Agent permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to Agent tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to agent dir)
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            ...
            config_path: agent_dir.join("config.toml"),
            api_key: None,
            api_url: None,
            api_path: None,
...
pub(crate) fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir.parent().map(|parent| parent.join(".agent"));
...
/// `DX_CONFIG_DIR` > `DX_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_agent_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_agent_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "DX_CONFIG_DIR",
            Self::EnvWorkspace => "DX_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    ...
    if let Ok(custom_config_dir) = std::env::var("DX_CONFIG_DIR") {
        ...
        if !custom_config_dir.is_empty() {
            let agent_dir = PathBuf::from(shellexpand::tilde(custom_config_dir).as_ref());
            return Ok((
                agent_dir.clone(),
    ...
    if let Some((agent_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_agent_dir).await?
    {
        return Ok((
            agent_dir,
            workspace_dir,
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'agent'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "AGENT_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `AGENT_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in AGENT_EXTRA_HEADERS");
                continue;
            }
...
///
/// When the workspace is created outside of `agent onboard` (e.g., non-tty
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am Agent, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are Agent, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (agent_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_agent_dir, &default_workspace_dir).await?;
        ...
        fs::create_dir_all(&agent_dir)
            .await
            .with_context(|| config_dir_creation_error(&agent_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&agent_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: AGENT_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("AGENT_API_KEY").or_else(|_| std::env::var("API_KEY"))
            && !key.is_empty()
        {
        ...
        // 1) AGENT_PROVIDER always wins when set.
        // 2) AGENT_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        ...
        if let Ok(provider) = std::env::var("AGENT_PROVIDER") {
            ...
            std::env::var("AGENT_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        if let Ok(model) = std::env::var("AGENT_MODEL").or_else(|_| std::env::var("MODEL"))
            && !model.is_empty()
        {
            self.default_model = Some(model);
        }

        // Provider HTTP timeout: AGENT_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("AGENT_PROVIDER_TIMEOUT_SECS")
        ...
        // Extra provider headers: AGENT_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("AGENT_EXTRA_HEADERS") {
        ...
        // Open-skills opt-in flag: AGENT_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("AGENT_OPEN_SKILLS_ENABLED")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid AGENT_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Open-skills directory override: AGENT_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("AGENT_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills prompt mode override: AGENT_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("AGENT_SKILLS_PROMPT_MODE")
            && !mode.trim().is_empty()
        {
            if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                self.skills.prompt_injection_mode = parsed;
            } else {
                tracing::warn!("Ignoring invalid AGENT_SKILLS_PROMPT_MODE (valid: full|compact)");
            }
        }

        // Gateway port: AGENT_GATEWAY_PORT or PORT
        if let Ok(port_str) = std::env::var("AGENT_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
            && let Ok(port) = port_str.parse::<u16>()
        {
            self.gateway.port = port;
        }

        // Gateway host: AGENT_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("AGENT_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
            && !host.is_empty()
        {
            self.gateway.host = host;
        }

        // Allow public bind: AGENT_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("AGENT_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: AGENT_TEMPERATURE
        if let Ok(temp_str) = std::env::var("AGENT_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring AGENT_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!("Ignoring AGENT_TEMPERATURE={temp_str:?}: not a valid number");
                }
            }
        }

        // Reasoning override: AGENT_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) =
            std::env::var("AGENT_REASONING_ENABLED").or_else(|_| std::env::var("REASONING_ENABLED"))
        {
        ...
        if let Ok(raw) = std::env::var("AGENT_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("AGENT_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: AGENT_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("AGENT_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: AGENT_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("AGENT_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: AGENT_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("AGENT_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Web search max results: AGENT_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("AGENT_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: AGENT_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("AGENT_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): AGENT_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("AGENT_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): AGENT_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("AGENT_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: AGENT_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("AGENT_STORAGE_CONNECT_TIMEOUT_SECS")
            && let Ok(timeout_secs) = timeout_secs.parse::<u64>()
            && timeout_secs > 0
        {
            self.storage.provider.config.connect_timeout_secs = Some(timeout_secs);
        }
        // Proxy enabled flag: AGENT_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("AGENT_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: AGENT_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("AGENT_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("AGENT_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("AGENT_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) = std::env::var("AGENT_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("AGENT_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid AGENT_PROXY_SCOPE (valid: environment|agent|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("AGENT_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_agent_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (agent_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_agent_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = agent_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let store = crate::security::SecretStore::new(agent_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/agent"));
        assert!(msg.contains("/etc/agent"));
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:agent");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "agent".to_string()));
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers = parse_extra_headers_env("HTTP-Referer:https://github.com/agent-labs/agent");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/agent-labs/agent");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : agent , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(headers[0], ("X-Title".to_string(), "agent".to_string()));
        assert_eq!(
            headers[1],
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:agent,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "agent".to_string()));
    }
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "agent");
    }
...
provider = "postgres"
dbURL = "postgres://postgres:postgres@localhost:5432/agent"
schema = "public"
table = "memories"
...
"#;
        ...
        assert_eq!(
            ...
            Some("postgres://postgres:postgres@localhost:5432/agent")
        );
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("agent_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "agent_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir = std::env::temp_dir().join(format!("agent_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.agent/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
        };
...
"#;
    ...
    async fn env_override_api_key() {
        ...
        unsafe { std::env::set_var("AGENT_API_KEY", "sk-test-env-key") };
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        unsafe { std::env::remove_var("AGENT_API_KEY") };
    }
    ...
    async fn env_override_provider() {
        ...
        unsafe { std::env::set_var("AGENT_PROVIDER", "anthropic") };
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        unsafe { std::env::remove_var("AGENT_PROVIDER") };
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        unsafe { std::env::remove_var("AGENT_PROVIDER") };
        unsafe { std::env::set_var("AGENT_MODEL_PROVIDER", "openai-codex") };
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        unsafe { std::env::remove_var("AGENT_MODEL_PROVIDER") };
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        unsafe { std::env::set_var("AGENT_OPEN_SKILLS_ENABLED", "true") };
        unsafe { std::env::set_var("AGENT_OPEN_SKILLS_DIR", "/tmp/open-skills") };
        unsafe { std::env::set_var("AGENT_SKILLS_PROMPT_MODE", "compact") };
        config.apply_env_overrides();
        ...
        unsafe { std::env::remove_var("AGENT_OPEN_SKILLS_ENABLED") };
        unsafe { std::env::remove_var("AGENT_OPEN_SKILLS_DIR") };
        unsafe { std::env::remove_var("AGENT_SKILLS_PROMPT_MODE") };
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        unsafe { std::env::set_var("AGENT_OPEN_SKILLS_ENABLED", "maybe") };
        unsafe { std::env::set_var("AGENT_SKILLS_PROMPT_MODE", "invalid") };
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        unsafe { std::env::set_var("AGENT_PROVIDER", "openrouter") };
    ...
    async fn env_override_model() {
        ...
        unsafe { std::env::set_var("AGENT_MODEL", "gpt-4o") };
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        unsafe { std::env::remove_var("AGENT_MODEL") };
    }
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("agent_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".agent").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        unsafe { std::env::set_var("HOME", &temp_home) };
        unsafe { std::env::set_var("AGENT_WORKSPACE", &workspace_dir) };
        ...
        unsafe { std::env::remove_var("AGENT_WORKSPACE") };
    ...
    async fn env_override_workspace() {
        ...
        unsafe { std::env::set_var("AGENT_WORKSPACE", "/custom/workspace") };
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        unsafe { std::env::set_var("AGENT_CONFIG_DIR", &explicit_config_dir) };
        unsafe { std::env::remove_var("AGENT_WORKSPACE") };
        ...
        unsafe { std::env::remove_var("AGENT_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("agent_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".agent").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".agent");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("agent_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".agent");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        persist_active_workspace_config_dir(&marker_config_dir)
            .await
            .unwrap();
        unsafe { std::env::set_var("AGENT_WORKSPACE", &env_workspace_dir) };
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("agent_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".agent");
    ...
    async fn env_override_empty_values_ignored() {
        ...
        unsafe { std::env::set_var("AGENT_PROVIDER", "") };
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        unsafe { std::env::set_var("AGENT_GATEWAY_PORT", "8080") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        unsafe { std::env::remove_var("AGENT_GATEWAY_PORT") };
    }
    ...
    async fn env_override_gateway_host() {
        ...
        unsafe { std::env::set_var("AGENT_GATEWAY_HOST", "0.0.0.0") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        unsafe { std::env::remove_var("AGENT_GATEWAY_HOST") };
    }
    ...
    async fn env_override_temperature() {
        ...
        unsafe { std::env::set_var("AGENT_TEMPERATURE", "0.5") };
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        unsafe { std::env::remove_var("AGENT_TEMPERATURE") };
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        unsafe { std::env::set_var("AGENT_TEMPERATURE", "3.0") };
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        unsafe { std::env::set_var("AGENT_REASONING_ENABLED", "false") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        unsafe { std::env::set_var("AGENT_REASONING_ENABLED", "true") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        unsafe { std::env::remove_var("AGENT_REASONING_ENABLED") };
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        unsafe { std::env::set_var("AGENT_REASONING_ENABLED", "maybe") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        unsafe { std::env::set_var("AGENT_REASONING_EFFORT", "HIGH") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        unsafe { std::env::remove_var("AGENT_REASONING_EFFORT") };
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        unsafe { std::env::set_var("AGENT_CODEX_REASONING_EFFORT", "minimal") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        unsafe { std::env::remove_var("AGENT_CODEX_REASONING_EFFORT") };
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        unsafe { std::env::set_var("AGENT_STORAGE_PROVIDER", "postgres") };
        unsafe { std::env::set_var("AGENT_STORAGE_DB_URL", "postgres://example/db") };
        unsafe { std::env::set_var("AGENT_STORAGE_CONNECT_TIMEOUT_SECS", "15") };
        ...
        unsafe { std::env::remove_var("AGENT_STORAGE_PROVIDER") };
        unsafe { std::env::remove_var("AGENT_STORAGE_DB_URL") };
        unsafe { std::env::remove_var("AGENT_STORAGE_CONNECT_TIMEOUT_SECS") };
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        unsafe { std::env::set_var("AGENT_PROXY_ENABLED", "true") };
        unsafe { std::env::set_var("AGENT_HTTP_PROXY", "http://127.0.0.1:7890") };
        unsafe { std::env::set_var("AGENT_PROXY_SERVICES", "provider.openai, tool.http_request") };
        unsafe { std::env::set_var("AGENT_PROXY_SCOPE", "services") };
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        unsafe { std::env::set_var("AGENT_PROXY_ENABLED", "true") };
        unsafe { std::env::set_var("AGENT_PROXY_SCOPE", "environment") };
        unsafe { std::env::set_var("AGENT_HTTP_PROXY", "http://127.0.0.1:7890") };
        unsafe { std::env::set_var("AGENT_HTTPS_PROXY", "http://127.0.0.1:7891") };
        unsafe { std::env::set_var("AGENT_NO_PROXY", "localhost,127.0.0.1") };
...
[security.estop]
enabled = true
state_file = "~/.agent/estop-state.json"
require_otp_to_resume = true
"#,
        )
        .unwrap();
    ...
    async fn channel_secret_telegram_bot_token_roundtrip() {
        let dir =
            std::env::temp_dir().join(format!("agent_test_tg_bot_token_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn nevis_client_secret_encrypt_decrypt_roundtrip() {
        let dir =
            std::env::temp_dir().join(format!("agent_test_nevis_secret_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
