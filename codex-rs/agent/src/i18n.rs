...
///
/// Checks `AGENT_LOCALE`, then `LANG`, then `LC_ALL`.
/// Returns "en" if none are set or parseable.
pub fn detect_locale() -> String {
    if let Ok(val) = std::env::var("AGENT_LOCALE") {
...
"#,
    ...
    fn detect_locale_from_env() {
        ...
        let saved = std::env::var("AGENT_LOCALE").ok();
        let saved_lang = std::env::var("LANG").ok();

        unsafe { std::env::set_var("AGENT_LOCALE", "ja-JP") };
        assert_eq!(detect_locale(), "ja-JP");

        unsafe { std::env::remove_var("AGENT_LOCALE") };
        unsafe { std::env::set_var("LANG", "zh_CN.UTF-8") };
        assert_eq!(detect_locale(), "zh-CN");

        // Restore.
        match saved {
            Some(v) => unsafe { std::env::set_var("AGENT_LOCALE", v) },
            None => unsafe { std::env::remove_var("AGENT_LOCALE") },
        }
        match saved_lang {
