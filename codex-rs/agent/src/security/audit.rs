...
impl AuditLogger {
    ...
    pub fn new(config: AuditConfig, agent_dir: PathBuf) -> Result<Self> {
        let log_path = agent_dir.join(&config.log_path);
...
mod tests {
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@agent_user".to_string()));
    }
