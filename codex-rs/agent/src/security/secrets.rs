...
// in `~/.agent/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.agent/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(agent_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: agent_dir.join(".secret_key"),
            enabled,
        }
    }
