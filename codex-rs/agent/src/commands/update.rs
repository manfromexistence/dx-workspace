//! `agent update` — self-update pipeline with rollback.
...
const GITHUB_RELEASES_LATEST_URL: &str =
    "https://api.github.com/repos/agent-labs/agent/releases/latest";
const GITHUB_RELEASES_TAG_URL: &str = "https://api.github.com/repos/agent-labs/agent/releases/tags";
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("agent/{current}"))
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("agent_new");
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("agent") {
        bail!("downloaded binary does not appear to be agent");
    }
