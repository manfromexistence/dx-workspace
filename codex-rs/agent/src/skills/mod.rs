...
const OPEN_SKILLS_SYNC_MARKER: &str = ".agent-open-skills-sync";
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("AGENT_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("AGENT_OPEN_SKILLS_DIR").ok();
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                print_info("");
                print_info("  Create one: mkdir -p ~/.agent/workspace/skills/my-skill");
                print_info(
                    "              echo '# My Skill' > ~/.agent/workspace/skills/my-skill/SKILL.md",
                );
                print_info("");
                print_info("  Or install: agent skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.agent");
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("AGENT_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("AGENT_OPEN_SKILLS_DIR");
