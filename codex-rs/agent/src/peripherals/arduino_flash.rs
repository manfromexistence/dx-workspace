//! Flash Agent Arduino firmware via arduino-cli.
//!
...
/// Agent Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash Agent firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("agent_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Agent Arduino firmware...");
    ...
    println!("Agent firmware flashed successfully.");
