...
Examples:
  ...
  dx daemon --host 127.0.0.1  # localhost only")]
    ...
    Estop {
        ...
        /// Level used when engaging estop from `agent estop`.
        #[arg(long, value_enum)]
...
Examples:
  ...
  agent hardware introspect /dev/ttyACM0
  agent hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
...
Examples:
  ...
  agent peripheral add nucleo-f401re /dev/ttyACM0
  agent peripheral add rpi-gpio native
  agent peripheral flash --port /dev/cu.usbmodem12345
...
Examples:
  agent memory stats
  agent memory list
  agent memory list --category core --limit 10
  agent memory get <key>
  agent memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  agent config schema              # print JSON Schema to stdout
  agent config schema > schema.json")]
    Config {
        #[command(subcommand)]
...
Examples:
  agent update                      # download and install latest
  agent update --check              # check only, don't install
  agent update --force              # install without confirmation
  agent update --version 0.6.0      # install specific version")]
    Update {
    ...
    #[command(long_about = "\
Run diagnostic self-tests to verify the Agent installation.
...
Examples:
  agent self-test             # full suite
  agent self-test --quick     # quick checks only (no network)")]
    SelfTest {
    ...
    #[command(long_about = "\
Generate shell completion scripts for `agent`.
...
async fn main_impl() -> Result<()> {
    ...
    // `agent onboard --api-key …` both take the fast path, while a bare
    // `agent onboard` in a terminal launches the wizard.
    ...
    } = &cli.command
    {
        ...
        if reinit {
            ...
            if agent_dir.exists() {
                ...
                let backup_dir = format!("{}.backup.{}", agent_dir.display(), timestamp);

                println!("△ Warning: Reinitializing Agent configuration...");
                println!("   Current config directory: {}", agent_dir.display());
                println!(
                ...
                tokio::fs::rename(&agent_dir, &backup_dir)
                    .await
                    .with_context(|| {
        ...
        if std::env::var("AGENT_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for Agent.");
    ...
    match cli.command {
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting Agent Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting Agent Daemon on {host}:{port}");
            }
        ...
        Commands::Providers => {
            ...
            println!(
                "Agent supports 140+ AI providers with {} models!\n",
                total_models
            );
            ...
            println!("Tip: Use `agent models list --provider <name>` to see available models");
            Ok(())
        }
        ...
        Commands::Plugin { plugin_command } => match plugin_command {
            PluginCommands::List => {
                let host = agent::plugins::host::PluginHost::new(&config.workspace_dir)?;
            ...
            PluginCommands::Install { source } => {
                let mut host = agent::plugins::host::PluginHost::new(&config.workspace_dir)?;
                host.install(&source)?;
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting Agent Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting Agent Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `agent auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `agent auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `agent auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `agent auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `agent auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `agent auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["agent", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["agent", "onboard", "--force"])
    ...
    fn onboard_cli_rejects_removed_interactive_flag() {
        ...
        assert!(Cli::try_parse_from(["agent", "onboard", "--interactive"]).is_err());
    }

    #[test]
    fn onboard_cli_bare_parses() {
        let cli = Cli::try_parse_from(["agent", "onboard"]).expect("bare onboard should parse");
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["agent", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["agent", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["agent", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["agent", "agent", "--message", "hello"])
    ...
    fn agent_command_parses_session_state_file() {
        let cli = Cli::try_parse_from(["agent", "agent", "--session-state-file", "session.json"])
