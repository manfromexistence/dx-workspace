...
const SERVICE_LABEL: &str = "com.agent.daemon";
const WINDOWS_TASK_NAME: &str = "Agent Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "agent.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["agent", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "agent.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["agent", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "agent.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["agent", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✓ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["agent", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/agent");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/agent");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "agent", "default"]))
                {
                    eprintln!(
                        "△ Warning: Could not remove agent from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✓ Service uninstalled (/etc/init.d/agent)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: agent service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "agent.service"]));
...
/// Check if the agent user exists and has expected properties.
...
fn check_agent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "agent"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser agent && delgroup agent",
            "addgroup -S agent && adduser -S -s /sbin/nologin -H -D -G agent agent",
        )
    } else {
        ("userdel agent", "useradd -r -s /sbin/nologin agent")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'agent' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'agent' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/agent" && home != "/nonexistent" {
                    eprintln!(
                        "△ Warning: agent user has home directory '{}' (expected /var/lib/agent or /nonexistent)",
                        home
                    );
                }
...
fn ensure_agent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "agent"]).output();
    if let Ok(output) = output
        && output.status.success()
    {
        return check_agent_user();
    }

    let is_alpine = Path::new("/etc/alpine-release").exists();

    if is_alpine {
        let group_output = Command::new("getent").args(["group", "agent"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "agent"])
                .output()
                .context("Failed to create agent group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create agent group: {}", stderr.trim());
            }
            println!("✓ Created system group: agent");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create agent user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create agent user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "agent"])
            .output()
            .context("Failed to create agent user")?;
    ...
    println!("✓ Created system user: agent");
    Ok(())
}

/// Change ownership of a path to agent:agent
#[cfg(unix)]
fn chown_to_agent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["agent:agent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to agent:agent: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_agent(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_agent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "agent:agent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to agent:agent: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_agent(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
        ...
        && output.status.success()
    {
        ...
        if fields.len() >= 6 {
            return Some(PathBuf::from(fields[5]).join(".agent"));
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'agent' cannot write {} ({details}). \
             Re-run `sudo agent service install` and ensure ownership is agent:agent.",
            path.display(),
        );
    }
...
command_user="agent:agent"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/agent/access.log"
error_log="/var/log/agent/error.log"
...
start_pre() {{
    checkpath --directory --owner agent:agent --mode 0750 /var/lib/agent
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/agent");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo agent service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/agent");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/agent");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_agent(&workspace_dir)?;
        println!(
            "✓ Created directory: {} (owned by agent:agent)",
    ...
    chown_recursive_to_agent(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/agent");
    ...
    run_checked(Command::new("rc-update").args(["add", "agent", "default"]))?;
    println!("✓ Installed OpenRC service: /etc/init.d/agent");
    println!("   Config path: /etc/agent/config.toml");
    println!("   Start with: sudo agent service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("agent-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/agent.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "Agent Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/agent");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/agent"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"agent\""));
        assert!(script.contains("description=\"Agent daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/agent\""));
        assert!(script.contains("command_args=\"--config-dir /etc/agent daemon\""));
        assert!(!script.contains("env AGENT_CONFIG_DIR"));
        assert!(!script.contains("env AGENT_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"agent:agent\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/agent/access.log\""));
        assert!(script.contains("error_log=\"/var/log/agent/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/agent\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner agent:agent"),
            "start_pre must ensure /var/lib/agent exists with correct ownership"
        );
    }
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/agent");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/agent");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/agent");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) = build_openrc_writability_probe_command(Path::new("/etc/agent"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/agent/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/agent/workspace'".to_string(),
                "agent".to_string()
            ]
        );
    }
