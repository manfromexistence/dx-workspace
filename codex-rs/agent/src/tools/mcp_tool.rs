//! Wraps a discovered MCP tool as a agent [`Tool`] so it is dispatched
...
/// A agent [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Agent's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // Agent's security model injects `approved: bool` into built-in tool args.
