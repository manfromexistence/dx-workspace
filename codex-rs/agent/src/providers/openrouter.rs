...
impl Provider for OpenRouterProvider {
    ...
    async fn chat_with_system(
        ...
        let credential = self.credential.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "OpenRouter API key not set. Run `agent onboard` or set OPENROUTER_API_KEY env var."
            )
        })?;
        ...
        let response = self
            ...
            .header("Authorization", format!("Bearer {credential}"))
            .header("HTTP-Referer", "https://github.com/agent-labs/agent")
            .header("X-Title", "Agent")
            .json(&request)
            .send()
            .await?;
