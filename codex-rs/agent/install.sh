...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/agent-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
Agent installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds Agent, configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://agentlabs.ai/install.sh | bash
...
Environment:
  AGENT_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  AGENT_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  AGENT_DOCKER_IMAGE      Docker image tag to build/run (default: agent-bootstrap:local)
  AGENT_API_KEY           Used when --api-key is not provided
  AGENT_PROVIDER          Used when --provider is not provided (default: openrouter)
  AGENT_MODEL             Used when --model is not provided
  AGENT_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  AGENT_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  AGENT_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${AGENT_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${AGENT_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/agent-labs/agent/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="agent-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/agent-labs/agent/releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t agent-prebuilt-XXXXXX)"
  ...
  extracted_bin="$temp_dir/agent"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name agent -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable agent binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/agent"
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/agent"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/agent-pacman.XXXXXX.conf)"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with agent onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} Agent Guided Installer${RESET}"
...
ensure_default_config_and_workspace() {
  ...
  #
  # $1 — config directory  (e.g. ~/.agent or $docker_data_dir/.agent)
  # $2 — workspace directory (e.g. ~/.agent/workspace or $docker_data_dir/workspace)
...
# Agent configuration — generated by install.sh
# Edit this file or run 'agent onboard' to reconfigure.
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${AGENT_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set AGENT_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set AGENT_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${AGENT_DOCKER_IMAGE:-agent-bootstrap:local}"
  fallback_image="ghcr.io/agent-labs/agent:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.agent-docker"
  else
    default_data_dir="$WORK_DIR/.agent-docker"
  fi
  docker_data_dir="${AGENT_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.agent" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official Agent image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.agent:/agent-data/.agent"
  workspace_mount="$docker_data_dir/workspace:/agent-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run agent onboard inside the container to configure."
  fi
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/agent-labs/agent.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${AGENT_CONTAINER_CLI:-docker}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${AGENT_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set AGENT_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t agent-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} Agent Installer${RESET}"
...
if have_cmd agent; then
  EXISTING_VERSION="$(agent --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/agent" ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/agent" --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing Agent installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing Agent installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized Agent data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}agent status${RESET}"
  echo -e "  ${DIM}agent agent -m \"Hello, Agent!\"${RESET}"
  echo -e "  ${DIM}agent gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.agentlabs.ai/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing Agent${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing Agent v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  step_dot "Installing agent to cargo bin"

  # Clean up stale cargo install tracking from the old "agent" package name
  # (renamed to "agentlabs"). Without this, `cargo install agentlabs` from
  # crates.io fails with "binary already exists as part of `agent`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"agent ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'agent' package name"
      cargo uninstall agent 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked
  step_ok "Agent installed"

  # Sync binary to ~/.local/bin so PATH lookups find the fresh version
  if [[ -d "$HOME/.local/bin" ]]; then
    cp -f "$HOME/.cargo/bin/agent" "$HOME/.local/bin/agent" 2>/dev/null && \
...
if [[ -x "$HOME/.cargo/bin/agent" ]]; then
  AGENT_BIN="$HOME/.cargo/bin/agent"
elif [[ -x "$WORK_DIR/target/release/agent" ]]; then
  AGENT_BIN="$WORK_DIR/target/release/agent"
elif have_cmd agent; then
  AGENT_BIN="agent"
fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$AGENT_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    step_dot "Configuring provider: ${PROVIDER}"
    ONBOARD_CMD=("$AGENT_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run agent onboard to retry"
    fi
  elif [[ "$PROVIDER" == "ollama" ]]; then
    step_dot "Configuring Ollama (no API key needed)"
    if "$AGENT_BIN" onboard --provider ollama 2>/dev/null; then
      step_ok "Ollama configured"
    else
      step_fail "Ollama configuration failed — run agent onboard to retry"
    fi
  else
    ...
    if [[ -t 0 && -t 1 ]]; then
      ...
      step_dot "No API key provided — run agent onboard to configure"
    fi
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run agent onboard later)"
elif [[ -z "$AGENT_BIN" ]]; then
  warn "Agent binary not found — cannot configure provider"
fi
...
_native_config_dir="${AGENT_CONFIG_DIR:-$HOME/.agent}"
_native_workspace_dir="${AGENT_WORKSPACE:-$_native_config_dir/workspace}"
...
if [[ -n "$AGENT_BIN" ]]; then
  ...
  if "$AGENT_BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$AGENT_BIN" service restart 2>/dev/null; then
      ...
      for i in 1 2 3 4 5; do
        sleep 2
        if PAIR_CODE=$("$AGENT_BIN" gateway get-paircode 2>/dev/null | grep -oE '[0-9]{6}'); then
          break
        fi
      done
      if [[ -n "$PAIR_CODE" ]]; then
        ...
        echo -e "  ${DIM}Run 'agent gateway get-paircode --new' anytime to generate a fresh code.${RESET}"
      fi
    else
      step_fail "Gateway service restart failed — re-run with agent service start"
    fi
  else
    step_dot "Gateway service not installed (run agent service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$AGENT_BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run agent doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$AGENT_BIN" ]]; then
  INSTALLED_VERSION="$("$AGENT_BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} Agent installed successfully (Agent ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} Agent installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/agent" ]] && ! have_cmd agent; then
  echo
  warn "agent is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
echo -e "${DIM}  Run 'agent gateway get-paircode' to get your pairing code.${RESET}"
