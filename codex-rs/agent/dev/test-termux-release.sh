...
ASSET_URL="https://github.com/agent-labs/agent/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t agent-termux-test-XXXXXX)"
...
# --- Test 5: Contains agent binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^agent$"; then
  pass "Archive contains 'agent' binary"
else
  fail "Archive does not contain 'agent' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/agent-labs/agent/releases/download/${TAG}/SHA256SUMS"
