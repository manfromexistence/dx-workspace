...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('[Agent] Render error:', error, info.componentStack);
  }
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'radial-gradient(ellipse at center, #0a0a20 0%, #050510 70%)' }}>
      ...
      <div className="relative glass-card p-8 w-full max-w-md animate-fade-in-scale">
        ...
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-2xl font-bold text-gradient-blue mb-2">Agent</h1>
          {displayCode ? (
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('agent-unauthorized', handler);
    return () => window.removeEventListener('agent-unauthorized', handler);
  }, [logout]);
