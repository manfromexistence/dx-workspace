...
SYSTEM_PROMPT = """You are Agent, an AI assistant with tool access. Use tools to accomplish tasks.
...
class AgentAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "AgentAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await AgentAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a Agent agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured AgentAgent instance

    Example:
        ```python
        from agent_tools import create_agent, shell, file_read
