"""
Discord bot integration for Agent.
"""
...
class DiscordBot:
    """
    Discord bot powered by Agent agent with LangGraph tool calling.

    Example:
        ```python
        import os
        from agent_tools.integrations import DiscordBot
    ...
    def __init__(
        ...
        if not DISCORD_AVAILABLE:
            raise ImportError(
                ...
                "Install with: pip install agent-tools[discord]"
            )
    ...
    def _setup_events(self):
        @self.client.event
        async def on_ready():
            print(f"Agent Discord Bot ready: {self.client.user}")
