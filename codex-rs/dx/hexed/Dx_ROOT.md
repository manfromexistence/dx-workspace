...
Now coming out the welcome screen and in the place of that welcome screen look at our Dx Binary. In the Dx Binary, you can see that we have a fast splash screen. When we click on the right key, it will show the file browser screen. When we click the left arrow key from the splash screen, it goes to the animation carousel screen.
Now please implement that screen navigation in our codex ty2. To now make sure that don't create a sloppy version of that, just like creating an instance of screen and creating new files, or just updating the DXTUI to integrate the DXTUI string directly inside our codex tui - Now, as DxtUI and CodextUI are a little bit different, please check that even rendering the DxtUI in the CodextUI is even a possible thing. If it's possible, then don't implement it right away. Just give me a detailed plan on how you're going to do it so that I can give you clarifications related to what the real plan actually is. Because the real plan or the actual goal is not straightforward or obisious
