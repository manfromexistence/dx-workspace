...
pub(crate) fn current_diff_render_style_context() -> DiffRenderStyleContext {
	...
	DiffRenderStyleContext { theme, color_level, diff_backgrounds }
}
	...
	fn from(val: DiffSummary) -> Self {
		...
		for (i, row) in collect_rows(&val.changes).into_iter().enumerate() {
			...
			rows.push(Box::new(InsetRenderable::new(
				...
				Insets::tlbr(/*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0),
			)));
		}
	...
	match change {
		...
		FileChange::Update { unified_diff, .. } => {
			if let Ok(patch) = diffy::Patch::from_str(unified_diff) {
				...
				let diff_lang =
					if exceeds_highlight_limits(total_diff_bytes, total_diff_lines) { None } else { lang };
				...
				for h in patch.hunks() {
					...
					for (line_idx, l) in h.lines().iter().enumerate() {
						let syntax_spans =
							hunk_syntax_lines.as_ref().and_then(|syntax_lines| syntax_lines.get(line_idx));
						match l {
							diffy::Line::Insert(text) => {
...
fn style_line_bg_for(kind: DiffLineType, diff_backgrounds: ResolvedDiffBackgrounds) -> Style {
	match kind {
		DiffLineType::Insert => {
			diff_backgrounds.add.map_or_else(Style::default, |bg| Style::default().bg(bg))
		}
		DiffLineType::Delete => {
			diff_backgrounds.del.map_or_else(Style::default, |bg| Style::default().bg(bg))
		}
...
fn style_gutter_for(kind: DiffLineType, theme: DiffTheme, color_level: DiffColorLevel) -> Style {
	match (theme, kind, RichDiffColorLevel::from_diff_color_level(color_level)) {
		...
		(DiffTheme::Light, DiffLineType::Insert, Some(level)) => {
			Style::default().fg(light_gutter_fg(color_level)).bg(light_add_num_bg(level))
		}
		(DiffTheme::Light, DiffLineType::Delete, Some(level)) => {
			Style::default().fg(light_gutter_fg(color_level)).bg(light_del_num_bg(level))
		}
		_ => style_gutter_dim(),
	}
}
...
fn style_del(
	...
	match (theme, color_level, diff_backgrounds.del) {
		...
		| (DiffTheme::Light, DiffColorLevel::Ansi256, Some(bg)) => Style::default().bg(bg),
		(DiffTheme::Dark, DiffColorLevel::TrueColor, Some(bg))
		| (DiffTheme::Dark, DiffColorLevel::Ansi256, Some(bg)) => Style::default().fg(Color::Red).bg(bg),
		(DiffTheme::Light, DiffColorLevel::TrueColor, None)
		| (DiffTheme::Light, DiffColorLevel::Ansi256, None) => Style::default(),
		(DiffTheme::Dark, DiffColorLevel::TrueColor, None)
...
mod tests {
	...
	fn display_width(text: &str) -> usize {
		text.chars().map(|ch| ch.width().unwrap_or(if ch == '\t' { TAB_WIDTH } else { 0 })).sum()
	}
	...
	fn diff_gallery_changes() -> HashMap<PathBuf, FileChange> {
		...
		changes.insert(
			...
			FileChange::Add { content: "HEADER\tVALUE\nrocket\t🚀\ncity\t東京\n".to_string() },
		);
		changes.insert(
	...
	fn snapshot_diff_gallery(name: &str, width: u16, height: u16) {
		let lines =
			create_diff_summary(&diff_gallery_changes(), &PathBuf::from("/"), usize::from(width));
	...
	fn display_path_prefers_cwd_without_git_repo() {
		...
		assert_eq!(rendered, PathBuf::from("tui").join("example.png").display().to_string());
	}
	...
	fn ui_snapshot_apply_update_block() {
		...
		changes.insert(
			...
			FileChange::Update { unified_diff: patch, move_path: None },
		);
	...
	fn ui_snapshot_apply_update_with_rename_block() {
		...
		changes.insert(
			...
			FileChange::Update { unified_diff: patch, move_path: Some(PathBuf::from("new_name.rs")) },
		);
	...
	fn ui_snapshot_apply_multiple_files_block() {
		...
		changes.insert(
			PathBuf::from("a.txt"),
			FileChange::Update { unified_diff: patch_a, move_path: None },
		);

		// File b.txt: newly added with one line
		changes.insert(PathBuf::from("b.txt"), FileChange::Add { content: "new\n".to_string() });
	...
	fn ui_snapshot_apply_delete_block() {
		...
		changes.insert(
			...
			FileChange::Delete { content: "first\nsecond\nthird\n".to_string() },
		);
	...
	fn ui_snapshot_apply_update_block_line_numbers_three_digits_text() {
		...
		let modified = (1..=110)
			.map(|i| if i == 100 { format!("line {i} changed\n") } else { format!("line {i}\n") })
			.collect::<String>();
		let patch = diffy::create_patch(&original, &modified).to_string();
	...
	fn ui_snapshot_apply_update_block_relativizes_path() {
		...
		let mut changes: HashMap<PathBuf, FileChange> = HashMap::new();
		changes.insert(abs_old, FileChange::Update { unified_diff: patch, move_path: Some(abs_new) });
	...
	fn truecolor_dark_theme_uses_configured_backgrounds() {
		...
		assert_eq!(
			style_gutter_for(DiffLineType::Insert, DiffTheme::Dark, DiffColorLevel::TrueColor),
			style_gutter_dim()
		);
		assert_eq!(
			style_gutter_for(DiffLineType::Delete, DiffTheme::Dark, DiffColorLevel::TrueColor),
			style_gutter_dim()
		);
	}
	...
	fn theme_scope_backgrounds_override_truecolor_fallback_when_available() {
		let backgrounds = resolve_diff_backgrounds_for(
			DiffTheme::Dark,
			DiffColorLevel::TrueColor,
			DiffScopeBackgroundRgbs { inserted: Some((1, 2, 3)), deleted: Some((4, 5, 6)) },
		);
		assert_eq!(
	...
	fn ansi16_disables_line_and_gutter_backgrounds() {
		...
		assert_eq!(
			style_gutter_for(DiffLineType::Insert, DiffTheme::Light, DiffColorLevel::Ansi16),
			Style::default().fg(Color::Black)
		);
		assert_eq!(
			style_gutter_for(DiffLineType::Delete, DiffTheme::Light, DiffColorLevel::Ansi16),
			Style::default().fg(Color::Black)
		);
		let themed_backgrounds = resolve_diff_backgrounds_for(
			DiffTheme::Light,
			DiffColorLevel::Ansi16,
			DiffScopeBackgroundRgbs { inserted: Some((8, 9, 10)), deleted: Some((11, 12, 13)) },
		);
		assert_eq!(style_line_bg_for(DiffLineType::Insert, themed_backgrounds), Style::default());
		assert_eq!(style_line_bg_for(DiffLineType::Delete, themed_backgrounds), Style::default());
	}
	...
	fn light_truecolor_theme_uses_readable_gutter_and_line_backgrounds() {
		...
		assert_eq!(
			style_gutter_for(DiffLineType::Insert, DiffTheme::Light, DiffColorLevel::TrueColor),
			Style::default().fg(rgb_color(LIGHT_TC_GUTTER_FG_RGB)).bg(rgb_color(LIGHT_TC_ADD_NUM_BG_RGB))
		);
		assert_eq!(
			style_gutter_for(DiffLineType::Delete, DiffTheme::Light, DiffColorLevel::TrueColor),
			Style::default().fg(rgb_color(LIGHT_TC_GUTTER_FG_RGB)).bg(rgb_color(LIGHT_TC_DEL_NUM_BG_RGB))
		);
	}
	...
	fn light_theme_wrapped_lines_keep_number_gutter_contrast() {
		...
		assert!(lines.len() > 1, "expected wrapped output for gutter style verification");
		assert_eq!(
	...
	fn wt_session_promotes_ansi16_to_truecolor_for_diffs() {
		assert_eq!(
			diff_color_level_for_terminal(StdoutColorLevel::Ansi16, TerminalName::Unknown, true, false,),
			DiffColorLevel::TrueColor
		);
	}
	...
	fn non_windows_terminal_keeps_ansi16_diff_palette() {
		assert_eq!(
			diff_color_level_for_terminal(StdoutColorLevel::Ansi16, TerminalName::WezTerm, false, false,),
			DiffColorLevel::Ansi16
		);
	}
	...
	fn add_diff_uses_path_extension_for_highlighting() {
		...
		changes.insert(
			...
			FileChange::Add { content: "pub fn sum(a: i32, b: i32) -> i32 { a + b }\n".to_string() },
		);

		let lines = create_diff_summary(&changes, &PathBuf::from("/"), 80);
		let has_rgb = lines.iter().any(|line| {
			line.spans.iter().any(|s| matches!(s.style.fg, Some(ratatui::style::Color::Rgb(..))))
		});
		assert!(has_rgb, "add diff for .rs file should produce syntax-highlighted (RGB) spans");
	}
	...
	fn delete_diff_uses_path_extension_for_highlighting() {
		...
		changes.insert(
			...
			FileChange::Delete { content: "def scale(x):\n    return x * 2\n".to_string() },
		);
		...
		assert!(has_rgb, "delete diff for .py file should produce syntax-highlighted (RGB) spans");
	}
	...
	fn wrap_styled_spans_flushes_at_span_boundary() {
		...
		assert!(first_width <= 4, "first line should be at most 4 cols wide, got {first_width}");
	}
	...
	fn wrap_styled_spans_wraps_before_first_overflowing_char() {
		...
		for line in &result {
			assert!(line_width(line) <= 5, "wrapped line exceeded width 5: {line:?}");
		}
	}
	...
	fn large_update_diff_skips_highlighting() {
		...
		let modified: String = (0..line_count)
			.map(|i| if i % 2 == 0 { format!("line {i} changed\n") } else { format!("line {i}\n") })
			.collect();
		let patch = diffy::create_patch(&original, &modified).to_string();
		...
		assert!(lines.len() > 100, "expected many output lines from large diff, got {}", lines.len(),);
	...
	fn rename_diff_uses_destination_extension_for_highlighting() {
		...
		changes.insert(
			...
			FileChange::Update { unified_diff: patch, move_path: Some(PathBuf::from("foo.rs")) },
		);
		...
		assert!(has_rgb, "rename from .xyzzy to .rs should produce syntax-highlighted (RGB) spans");
	}
	...
	fn update_diff_preserves_multiline_highlight_state_within_hunk() {
		...
		let expected_style = expected_multiline
			.get(1)
			.and_then(|line| line.iter().find(|span| span.content.as_ref().contains("world")))
			.map(|span| span.style)
