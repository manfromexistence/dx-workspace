...
async fn test_config() -> Config {
	...
	ConfigBuilder::default().codex_home(codex_home.clone()).build().await.expect("config")
}
...
async fn resumed_initial_messages_render_history() {
	...
	assert!(text_blob.contains("hello from user"), "expected replayed user message",);
	assert!(text_blob.contains("assistant reply"), "expected replayed agent message",);
}
...
async fn thread_snapshot_replay_does_not_duplicate_agent_message_history() {
	...
	chat.handle_codex_event_replay(Event {
		...
		msg: EventMsg::ItemCompleted(ItemCompletedEvent {
			...
			item: TurnItem::AgentMessage(AgentMessageItem {
				id: "msg-1".to_string(),
				content: vec![AgentMessageContent::Text { text: "assistant reply".to_string() }],
				phase: None,
				memory_citation: None,
			}),
		}),
	...
	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected replayed assistant message to render once");
...
async fn session_configured_syncs_widget_config_permissions_and_cwd() {
	...
	assert_eq!(chat.config_ref().permissions.approval_policy.value(), AskForApproval::Never);
	assert_eq!(chat.config_ref().permissions.sandbox_policy.get(), &expected_sandbox);
...
async fn submission_preserves_text_elements_and_local_images() {
	...
	chat.bottom_pane.set_composer_text(text.clone(), text_elements.clone(), local_images.clone());
	...
	assert_eq!(items[0], UserInput::LocalImage { path: local_images[0].clone() });
	assert_eq!(
		items[1],
		UserInput::Text { text: text.clone(), text_elements: text_elements.clone() }
	);
...
async fn submission_with_remote_and_local_images_keeps_local_placeholder_numbering() {
	...
	assert_eq!(items[0], UserInput::Image { image_url: remote_url.clone() });
	assert_eq!(items[1], UserInput::LocalImage { path: local_images[0].clone() });
	assert_eq!(
		items[2],
		UserInput::Text { text: text.clone(), text_elements: text_elements.clone() }
	);
...
async fn enter_with_only_remote_images_submits_user_turn() {
	...
	assert_eq!(items, vec![UserInput::Image { image_url: remote_url.clone() }]);
...
async fn blocked_image_restore_preserves_mention_bindings() {
	...
	assert_eq!(chat.bottom_pane.composer_local_image_paths(), vec![local_images[0].path.clone()],);
	assert_eq!(chat.bottom_pane.take_mention_bindings(), mention_bindings);

	let cells = drain_insert_history(&mut rx);
	let warning =
		cells.last().map(|lines| lines_to_single_string(lines)).expect("expected warning cell");
	assert!(
...
async fn blocked_image_restore_with_remote_images_keeps_local_placeholder_mapping() {
	...
	let text_elements = vec![
		TextElement::new((0..first_placeholder.len()).into(), Some(first_placeholder.to_string())),
		TextElement::new(
...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
	...
	chat.bottom_pane.set_composer_text(existing_text, existing_elements, existing_images.clone());
	...
	assert_eq!(
		...
		vec![first_images[0].clone(), second_images[0].clone(), existing_images[0].clone(),]
	);
}
...
async fn interrupted_turn_restore_keeps_active_mode_for_resubmission() {
	...
	let expected_mode = plan_mask.mode.expect("expected mode kind on plan collaboration mode");
...
async fn remap_placeholders_uses_attachment_labels() {
	...
	let elements = vec![
		TextElement::new((0..placeholder_two.len()).into(), Some(placeholder_two.to_string())),
		TextElement::new(
	...
	assert_eq!(
		...
		vec![
			TextElement::new((0.."[Image #4]".len()).into(), Some("[Image #4]".to_string()),),
			TextElement::new(
	...
	assert_eq!(remapped.remote_image_urls, vec!["https://example.com/a.png".to_string()]);
}
...
async fn steer_rejection_queues_review_follow_up_before_existing_queued_messages() {
	...
	chat.queued_user_messages.push_back(UserMessage::from("queued later"));
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "review follow-up one".to_string(), text_elements: Vec::new() }]
		),
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "review follow-up two".to_string(), text_elements: Vec::new() }]
		),
	...
	assert_eq!(
		...
		vec!["review follow-up one", "review follow-up two", "queued later"]
	);
	assert!(drain_insert_history(&mut rx).is_empty());

	chat.handle_codex_event(Event {
		id: "review-exit".into(),
		msg: EventMsg::ExitedReviewMode(ExitedReviewModeEvent { review_output: None }),
	});
	chat.handle_codex_event(Event {
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "queued later".to_string(), text_elements: Vec::new() }]
		),
...
async fn token_count_none_resets_context_indicator() {
	...
	chat.handle_codex_event(Event {
		...
		msg: EventMsg::TokenCount(TokenCountEvent { info: None, rate_limits: None }),
	});
...
async fn context_indicator_shows_used_tokens_when_window_unknown() {
	...
	let token_usage = TokenUsage { total_tokens, ..TokenUsage::default() };
	...
	chat.handle_codex_event(Event {
		id: "token-usage".into(),
		msg: EventMsg::TokenCount(TokenCountEvent { info: Some(token_info), rate_limits: None }),
	});

	assert_eq!(chat.bottom_pane.context_window_percent(), None);
	assert_eq!(chat.bottom_pane.context_window_used_tokens(), Some(total_tokens));
}
...
async fn helpers_are_available_and_do_not_panic() {
	...
	let thread_manager = Arc::new(codex_core::test_support::thread_manager_with_models_provider(
...
fn assert_no_submit_op(op_rx: &mut tokio::sync::mpsc::UnboundedReceiver<Op>) {
	while let Ok(op) = op_rx.try_recv() {
		assert!(!matches!(op, Op::UserTurn { .. }), "unexpected submit op: {op:?}");
	}
}
...
fn make_token_info(total_tokens: i64, context_window: i64) -> TokenUsageInfo {
	fn usage(total_tokens: i64) -> TokenUsage {
		TokenUsage { total_tokens, ..TokenUsage::default() }
	}
...
async fn rate_limit_snapshot_keeps_prior_credits_when_missing_from_headers() {
	...
	let display =
		chat.rate_limit_snapshots_by_limit_id.get("codex").expect("rate limits should be cached");
	let credits = display.credits.as_ref().expect("credits should persist when headers omit them");

	assert_eq!(credits.balance.as_deref(), Some("17.5"));
	assert!(!credits.unlimited);
	assert_eq!(display.primary.as_ref().map(|window| window.used_percent), Some(80.0));
}
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
	...
	let codex =
		chat.rate_limit_snapshots_by_limit_id.get("codex").expect("codex snapshot should exist");
	let other = chat
		.rate_limit_snapshots_by_limit_id
		.get("codex_other")
	...
	assert_eq!(codex.credits.as_ref().and_then(|credits| credits.balance.as_deref()), Some("5.00"));
...
async fn rate_limit_switch_prompt_shows_once_per_session() {
	...
	assert!(chat.rate_limit_warnings.primary_index >= 1, "warnings not emitted");
	chat.maybe_show_pending_rate_limit_prompt();
	assert!(matches!(chat.rate_limit_switch_prompt, RateLimitSwitchPromptState::Shown));
...
async fn submit_user_message_with_mode_sets_coding_collaboration_mode() {
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn {
			collaboration_mode: Some(CollaborationMode { mode: ModeKind::Default, .. }),
			personality: None,
			..
		} => {}
		other => {
...
async fn reasoning_selection_in_plan_mode_without_effort_change_does_not_open_scope_prompt_event() {
	...
	assert!(
		events.iter().any(|event| matches!(event, AppEvent::UpdateReasoningEffort(Some(_)))),
...
fn plan_mode_prompt_notification_uses_dedicated_type_name() {
	let notification = Notification::PlanModePrompt { title: PLAN_IMPLEMENTATION_TITLE.to_string() };

	assert!(notification.allowed_for(&Notifications::Custom(vec!["plan-mode-prompt".to_string(),])));
	assert!(
		!notification.allowed_for(&Notifications::Custom(vec!["approval-requested".to_string(),]))
	);
	assert_eq!(notification.display(), format!("Plan mode prompt: {PLAN_IMPLEMENTATION_TITLE}"));
}
...
fn user_input_requested_notification_uses_dedicated_type_name() {
	...
	assert!(
		notification.allowed_for(&Notifications::Custom(vec!["user-input-requested".to_string(),]))
	);
	assert!(
		!notification.allowed_for(&Notifications::Custom(vec!["approval-requested".to_string(),]))
	);
	assert_eq!(notification.display(), "Question requested: Reasoning scope");
}
...
async fn agent_turn_complete_does_not_override_pending_plan_mode_prompt_notification() {
	...
	chat.open_plan_implementation_prompt();
	chat.notify(Notification::AgentTurnComplete { response: "done".to_string() });
...
async fn plan_reasoning_scope_popup_plan_only_does_not_update_all_modes_reasoning() {
	...
	assert!(
		events.iter().all(|event| !matches!(event, AppEvent::UpdateReasoningEffort(_))),
...
async fn submit_user_message_with_mode_errors_when_mode_changes_during_running_turn() {
	...
	chat.thread_id = Some(ThreadId::new());
	chat.set_feature_enabled(Feature::CollaborationModes, true);
	let plan_mask = collaboration_modes::mask_for_kind(chat.models_manager.as_ref(), ModeKind::Plan)
...
async fn submit_user_message_with_mode_allows_same_mode_during_running_turn() {
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn {
			collaboration_mode: Some(CollaborationMode { mode: ModeKind::Plan, .. }),
			personality: None,
			..
		} => {}
		other => {
...
async fn submit_user_message_with_mode_submits_when_plan_stream_is_not_active() {
	...
	let default_mode = collaboration_modes::default_mask(chat.models_manager.as_ref())
		.expect("expected default collaboration mode");
	let expected_mode = default_mode.mode.expect("expected default collaboration mode kind");
	chat.submit_user_message_with_mode("Implement the plan.".to_string(), default_mode);
...
async fn plan_implementation_popup_skips_without_proposed_plan() {
	...
	chat.on_plan_update(UpdatePlanArgs {
		explanation: None,
		plan: vec![PlanItemArg { step: "First".to_string(), status: StepStatus::Pending }],
	});
...
"
	...
	chat.bottom_pane.set_composer_text("Please continue.".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "Please continue.".to_string(), text_elements: Vec::new() }]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
...
"
	...
	chat.bottom_pane.set_composer_text("Please revise.".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "Please revise.".to_string(), text_elements: Vec::new() }]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
...
async fn plan_implementation_popup_skips_when_rate_limit_prompt_pending() {
	...
	let popup = render_bottom_popup(&chat, 80);
	assert!(popup.contains("Approaching rate limits"), "expected rate limit popup, got {popup:?}");
	assert!(
		!popup.contains(PLAN_IMPLEMENTATION_TITLE),
...
async fn exec_approval_emits_proposed_command_and_decision_history() {
	...
	chat.handle_codex_event(Event { id: "sub-short".into(), msg: EventMsg::ExecApprovalRequest(ev) });
	...
	let decision = drain_insert_history(&mut rx).pop().expect("expected decision cell in history");
	assert_snapshot!(
...
async fn exec_approval_uses_approval_id_when_present() {
	...
	while let Ok(app_ev) = rx.try_recv() {
		if let AppEvent::SubmitThreadOp { op: Op::ExecApproval { id, decision, .. }, .. } = app_ev {
...
async fn exec_approval_decision_truncates_multiline_and_long_commands() {
	...
	assert!(saw_first_line, "expected modal to show first line of multiline snippet");

	// Deny via keyboard; decision snippet should be single-line and elided with " ..."
	chat.handle_key_event(KeyEvent::new(KeyCode::Char('n'), KeyModifiers::NONE));
	let aborted_multi =
		drain_insert_history(&mut rx).pop().expect("expected aborted decision cell (multiline)");
	assert_snapshot!(
	...
	chat.handle_key_event(KeyEvent::new(KeyCode::Char('n'), KeyModifiers::NONE));
	let aborted_long =
		drain_insert_history(&mut rx).pop().expect("expected aborted decision cell (long)");
	assert_snapshot!(
...
fn pending_steer(text: &str) -> PendingSteer {
	PendingSteer {
		...
		compare_key: PendingSteerCompareKey { message: text.to_string(), image_count: 0 },
	}
}
...
fn complete_user_message(chat: &mut ChatWidget, item_id: &str, text: &str) {
	complete_user_message_for_inputs(
		chat,
		item_id,
		vec![UserInput::Text { text: text.to_string(), text_elements: Vec::new() }],
	);
}
...
fn complete_user_message_for_inputs(chat: &mut ChatWidget, item_id: &str, content: Vec<UserInput>) {
	chat.handle_codex_event(Event {
		...
		msg: EventMsg::ItemCompleted(ItemCompletedEvent {
			thread_id: ThreadId::new(),
			turn_id: "turn-1".to_string(),
			item: TurnItem::UserMessage(UserMessageItem { id: item_id.to_string(), content }),
		}),
	});
}
...
fn end_exec(
	...
	let aggregated = if stderr.is_empty() { stdout.to_string() } else { format!("{stdout}{stderr}") };
...
fn active_blob(chat: &ChatWidget) -> String {
	let lines = chat.active_cell.as_ref().expect("active cell present").display_lines(80);
	lines_to_single_string(&lines)
}

fn get_available_model(chat: &ChatWidget, model: &str) -> ModelPreset {
	let models = chat.models_manager.try_list_models().expect("models lock available");
	models
		.iter()
...
async fn restore_thread_input_state_restores_pending_steers_without_downgrading_them() {
	...
	assert_eq!(chat.queued_user_message_texts(), vec!["already rejected", "queued draft"]);
	assert_eq!(chat.pending_steers.len(), 1);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "pending steer");
}
...
async fn alt_up_edits_most_recent_queued_message() {
	...
	chat.bottom_pane.set_queued_message_edit_binding(crate::key_hint::alt(KeyCode::Up));

	// Simulate a running task so messages would normally be queued.
	chat.bottom_pane.set_task_running(true);

	// Seed two queued messages.
	chat.queued_user_messages.push_back(UserMessage::from("first queued".to_string()));
	chat.queued_user_messages.push_back(UserMessage::from("second queued".to_string()));
	chat.refresh_pending_input_preview();
	...
	assert_eq!(chat.bottom_pane.composer_text(), "second queued".to_string());
	// And the queue should now contain only the remaining (older) item.
	assert_eq!(chat.queued_user_messages.len(), 1);
	assert_eq!(chat.queued_user_messages.front().unwrap().text, "first queued");
}
...
async fn assert_shift_left_edits_most_recent_queued_message_for_terminal(
	...
	chat.bottom_pane.set_queued_message_edit_binding(chat.queued_message_edit_binding);
...
async fn enqueueing_history_prompt_multiple_times_is_stable() {
	...
	chat.bottom_pane.set_composer_text("repeat me".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
...
async fn streaming_final_answer_ctrl_c_interrupt_preserves_background_shells() {
	...
	chat.bottom_pane.set_composer_text("queued submission".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));

	assert_eq!(chat.queued_user_messages.len(), 1);
	assert_eq!(chat.queued_user_messages.front().unwrap().text, "queued submission");
	assert_matches!(op_rx.try_recv(), Err(TryRecvError::Empty));
...
async fn preamble_keeps_working_status_snapshot() {
	...
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw preamble + status widget");
...
async fn unified_exec_begin_restores_working_status_snapshot() {
	...
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw chatwidget");
...
async fn submit_user_message_queues_while_compaction_turn_is_running() {
	...
	assert!(chat.pending_steers.is_empty());
	assert_eq!(chat.queued_user_message_texts(), vec!["queued while compacting"]);
...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
	...
	assert!(chat.pending_steers.is_empty());
	assert_eq!(chat.queued_user_messages.len(), 1);
	assert_eq!(chat.queued_user_messages.front().unwrap().text, "queued before compact turn start");
	assert_matches!(op_rx.try_recv(), Err(TryRecvError::Empty));
}
...
async fn steer_enter_uses_pending_steers_while_turn_is_running_without_streaming() {
	...
	chat.bottom_pane.set_composer_text("queued while running".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	assert!(chat.queued_user_messages.is_empty());
	assert_eq!(chat.pending_steers.len(), 1);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "queued while running");
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { .. } => {}
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
...
async fn steer_enter_uses_pending_steers_while_final_answer_stream_is_active() {
	...
	chat.bottom_pane.set_composer_text("queued while streaming".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	assert!(chat.queued_user_messages.is_empty());
	assert_eq!(chat.pending_steers.len(), 1);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "queued while streaming");
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { .. } => {}
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
...
async fn live_legacy_agent_message_after_item_completed_does_not_duplicate_assistant_message() {
	...
	complete_assistant_message(&mut chat, "msg-live", "hello", Some(MessagePhase::FinalAnswer));
	let inserted = drain_insert_history(&mut rx);
	assert_eq!(inserted.len(), 1);
...
fn rendered_user_message_event_from_inputs_matches_flattened_user_message_shape() {
	...
	let rendered = ChatWidget::rendered_user_message_event_from_inputs(&[
		...
		UserInput::Image { image_url: "https://example.com/remote.png".to_string() },
		UserInput::LocalImage { path: local_image.clone() },
		UserInput::Skill { name: "demo".to_string(), path: PathBuf::from("/tmp/skill/SKILL.md") },
		UserInput::Mention { name: "repo".to_string(), path: "app://repo".to_string() },
		UserInput::Text {
...
async fn item_completed_only_pops_front_pending_steer() {
	...
	assert_eq!(chat.pending_steers.len(), 2);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "first");
	let inserted = drain_insert_history(&mut rx);
	assert_eq!(inserted.len(), 1);
	...
	assert_eq!(chat.pending_steers.len(), 1);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "second");
	let inserted = drain_insert_history(&mut rx);
	assert_eq!(inserted.len(), 1);
...
async fn item_completed_pops_pending_steer_with_local_image_and_text_elements() {
	...
	complete_user_message_for_inputs(
		...
		"user-1",
		vec![
			UserInput::Image { image_url: "data:image/png;base64,placeholder".to_string() },
			UserInput::Text { text, text_elements: Vec::new() },
		],
	);
	...
	assert_eq!(stored_elements, vec![TextElement::new((0..4).into(), Some("note".to_string()))]);
...
async fn submit_user_message_emits_structured_plugin_mentions_from_bindings() {
	...
	chat.bottom_pane.set_plugin_mentions(Some(vec![codex_core::plugins::PluginCapabilitySummary {
	...
	assert_eq!(
		items,
		vec![
			UserInput::Text { text: "$sample".to_string(), text_elements: Vec::new() },
			UserInput::Mention {
...
async fn steer_enter_during_final_stream_preserves_follow_up_prompts_in_order() {
	...
	chat.bottom_pane.set_composer_text("first follow-up".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	chat.bottom_pane.set_composer_text("second follow-up".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	assert!(chat.queued_user_messages.is_empty());
	assert_eq!(chat.pending_steers.len(), 2);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "first follow-up");
	assert_eq!(chat.pending_steers.back().unwrap().user_message.text, "second follow-up");
	...
	assert_eq!(
		first_items,
		vec![UserInput::Text { text: "first follow-up".to_string(), text_elements: Vec::new() }]
	);
	...
	assert_eq!(
		second_items,
		vec![UserInput::Text { text: "second follow-up".to_string(), text_elements: Vec::new() }]
	);
	assert!(drain_insert_history(&mut rx).is_empty());
	...
	assert_eq!(chat.pending_steers.len(), 1);
	assert_eq!(chat.pending_steers.front().unwrap().user_message.text, "second follow-up");
...
"
	...
	let inserted = drain_insert_history(&mut rx);
	assert!(
		inserted.iter().all(|cell| !lines_to_single_string(cell).contains("queued while streaming"))
	);
}
...
async fn esc_interrupt_sends_all_pending_steers_immediately_and_keeps_existing_draft() {
	...
	chat.bottom_pane.set_composer_text("first pending steer".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "first pending steer".to_string(), text_elements: Vec::new() }]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}

	chat.bottom_pane.set_composer_text("second pending steer".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "second pending steer".to_string(), text_elements: Vec::new() }]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}

	chat.queued_user_messages.push_back(UserMessage::from("queued draft".to_string()));
	chat.refresh_pending_input_preview();
	chat.bottom_pane.set_composer_text("still editing".to_string(), Vec::new(), Vec::new());
	...
	assert_eq!(chat.queued_user_messages.len(), 1);
	assert_eq!(chat.queued_user_messages.front().unwrap().text, "queued draft");

	let inserted = drain_insert_history(&mut rx);
	assert!(inserted.iter().any(|cell| lines_to_single_string(cell).contains("first pending steer")));
	assert!(
		inserted.iter().any(|cell| lines_to_single_string(cell).contains("second pending steer"))
	);
}
...
async fn esc_with_pending_steers_overrides_agent_command_interrupt_behavior() {
	...
	chat.bottom_pane.set_composer_text("pending steer".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { .. } => {}
		other => panic!("expected Op::UserTurn, got {other:?}"),
...
async fn manual_interrupt_restores_pending_steer_mention_bindings_to_composer() {
	...
	chat.bottom_pane.set_composer_text_with_mention_bindings(
		...
		vec![TextElement::new((11..17).into(), Some("$figma".to_string()))],
		Vec::new(),
		mention_bindings.clone(),
	);
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			...
			vec![UserInput::Text {
				...
				text_elements: vec![TextElement::new((11..17).into(), Some("$figma".to_string()),)],
			}]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
...
"
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "pending steer".to_string(), text_elements: Vec::new() }]
		),
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
	assert!(drain_insert_history(&mut rx).is_empty());
...
"
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { items, .. } => assert_eq!(
			items,
			vec![UserInput::Text { text: "queued draft".to_string(), text_elements: Vec::new() }]
		),
...
async fn enter_submits_when_plan_stream_is_not_active() {
	...
	chat.bottom_pane.set_composer_text("submitted immediately".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	assert!(chat.queued_user_messages.is_empty());
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { personality: Some(Personality::Pragmatic), .. } => {}
		other => panic!("expected Op::UserTurn, got {other:?}"),
	}
}
...
async fn ctrl_c_closes_realtime_conversation_before_interrupt_or_quit() {
	...
	chat.bottom_pane.set_composer_text("recording meter".to_string(), Vec::new(), Vec::new());

	chat.handle_key_event(KeyEvent::new(KeyCode::Char('c'), KeyModifiers::CONTROL));

	next_realtime_close_op(&mut op_rx);
	assert_eq!(chat.realtime_conversation.phase, RealtimeConversationPhase::Stopping);
...
async fn ctrl_c_cleared_prompt_is_recoverable_via_history() {
	...
	chat.bottom_pane.attach_image(PathBuf::from("/tmp/preview.png"));
	let placeholder = "[Image #1]";
	assert!(
...
async fn exec_history_cell_shows_working_then_completed() {
	...
	assert!(blob.contains("echo done"), "expected command text to be present: {blob:?}");
}
...
async fn exec_history_cell_shows_working_then_failed() {
	...
	let lines = &cells[0];
	let blob = lines_to_single_string(lines);
	assert!(blob.contains("• Ran false"), "expected command and header text present: {blob:?}");
...
async fn exec_end_without_begin_uses_event_command() {
	let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
	let command = vec!["bash".to_string(), "-lc".to_string(), "echo orphaned".to_string()];
...
async fn exec_end_without_begin_flushes_completed_unrelated_exploring_cell() {
	...
	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 2, "completed exploring cell should flush before the orphan entry");
	let first = lines_to_single_string(&cells[0]);
	let second = lines_to_single_string(&cells[1]);
	assert!(first.contains("• Explored"), "expected flushed exploring cell: {first:?}");
	assert!(first.contains("List ls -la"), "expected flushed exploring cell: {first:?}");
	assert!(second.contains("• Ran echo after"), "expected orphan end entry after flush: {second:?}");
	assert!(chat.active_cell.is_none(), "both entries should be finalized");
}
...
async fn overlapping_exploring_exec_end_is_not_misclassified_as_orphan() {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "tracked end inside an exploring cell should not render as an orphan");
	let active = active_blob(&chat);
	assert!(active.contains("List ls -la"), "expected first command still grouped: {active:?}");
	assert!(
...
async fn exec_history_shows_unified_exec_startup_commands() {
	...
	assert!(drain_insert_history(&mut rx).is_empty(), "exec begin should not flush until completion");
...
async fn exec_history_shows_unified_exec_tool_calls() {
	...
	let begin =
		begin_exec_with_source(&mut chat, "call-startup", "ls", ExecCommandSource::UnifiedExecStartup);
...
async fn unified_exec_unknown_end_with_active_exploring_cell_snapshot() {
	...
	let cells = drain_insert_history(&mut rx);
	let history = cells.iter().map(|lines| lines_to_single_string(lines)).collect::<String>();
	let active = active_blob(&chat);
...
async fn unified_exec_end_after_task_complete_is_suppressed() {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected unified exec end after task complete to be suppressed");
}
...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
	...
	let cells = drain_insert_history(&mut rx);
	let combined = cells.iter().map(|lines| lines_to_single_string(lines)).collect::<String>();
...
async fn unified_exec_non_empty_then_empty_snapshots() {
	...
	let active_combined =
		pre_cells.iter().map(|lines| lines_to_single_string(lines)).collect::<String>();
	...
	let mut combined =
		pre_cells.iter().map(|lines| lines_to_single_string(lines)).collect::<String>();
	let post = post_cells.iter().map(|lines| lines_to_single_string(lines)).collect::<String>();
...
async fn slash_init_skips_when_project_doc_exists() {
	...
	assert_eq!(std::fs::read_to_string(existing_path).unwrap(), "existing instructions");
}
...
async fn mode_switch_surfaces_reasoning_change_notification_when_model_stays_same() {
	let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("mistral-large-latest")).await;
	chat.set_feature_enabled(Feature::CollaborationModes, true);
	chat.set_reasoning_effort(Some(ReasoningEffortConfig::High));
	...
	let plan_messages = drain_insert_history(&mut rx)
		...
		.join("\n");
	assert!(
		plan_messages.contains("Model changed to mistral-large-latest medium for Plan mode."),
...
async fn collab_slash_command_opens_picker_and_updates_mode() {
	...
	let popup = render_bottom_popup(&chat, 80);
	assert!(popup.contains("Select Collaboration Mode"), "expected collaboration picker: {popup}");
	...
	chat.bottom_pane.set_composer_text("follow up".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
	match next_submit_op(&mut op_rx) {
		Op::UserTurn {
			collaboration_mode: Some(CollaborationMode { mode: ModeKind::Default, .. }),
...
async fn plan_slash_command_with_args_submits_prompt_in_plan_mode() {
	...
	chat.bottom_pane.set_composer_text("/plan build the plan".to_string(), Vec::new(), Vec::new());
	chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
	...
	assert_eq!(
		items[0],
		UserInput::Text { text: "build the plan".to_string(), text_elements: Vec::new() }
	);
	assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn collaboration_modes_defaults_to_code_on_startup() {
	...
	let cfg = ConfigBuilder::default()
		...
		.cli_overrides(vec![("features.collaboration_modes".to_string(), TomlValue::Boolean(true))])
		.build()
		.await
		.expect("config");
	let resolved_model = codex_core::test_support::get_model_offline(cfg.model.as_deref());
...
async fn experimental_mode_plan_is_ignored_on_startup() {
	...
	let cfg = ConfigBuilder::default()
		.codex_home(codex_home.path().to_path_buf())
		.cli_overrides(vec![
			("features.collaboration_modes".to_string(), TomlValue::Boolean(true)),
			("tui.experimental_mode".to_string(), TomlValue::String("plan".to_string())),
		])
		.build()
		.await
		.expect("config");
...
async fn set_reasoning_effort_updates_active_collaboration_mask() {
	...
	assert_eq!(chat.current_reasoning_effort(), Some(ReasoningEffortConfig::Medium));
	assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn set_reasoning_effort_does_not_override_active_plan_override() {
	...
	assert_eq!(chat.current_reasoning_effort(), Some(ReasoningEffortConfig::High));
	assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn user_turn_includes_personality_from_config() {
	...
	chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { personality: Some(Personality::Friendly), .. } => {}
...
async fn slash_copy_state_tracks_turn_complete_final_reply() {
	...
	assert_eq!(chat.last_copyable_output, Some("Final reply **markdown**".to_string()));
}
...
async fn slash_copy_state_tracks_plan_item_completion() {
	...
	chat.handle_codex_event(Event {
		...
		msg: EventMsg::ItemCompleted(ItemCompletedEvent {
			thread_id: ThreadId::new(),
			turn_id: "turn-1".to_string(),
			item: TurnItem::Plan(PlanItem { id: "plan-1".to_string(), text: plan_text.clone() }),
		}),
	});
	chat.handle_codex_event(Event {
		id: "turn-1".into(),
...
async fn slash_copy_reports_when_no_copyable_output_exists() {
	...
	assert!(
		rendered
			.contains("`/copy` is unavailable before the first Codex output or right after a rollback."),
...
async fn slash_copy_state_is_preserved_during_running_task() {
	...
	assert_eq!(chat.last_copyable_output, Some("Previous completed reply".to_string()));
}
...
async fn slash_rollout_handles_missing_path() {
	...
	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected info message explaining missing path");
	let rendered = lines_to_single_string(&cells[0]);
	assert!(rendered.contains("not available"), "expected missing rollout path message: {rendered}");
}
...
async fn undo_success_events_render_info_messages() {
	...
	chat.handle_codex_event(Event {
		id: "turn-1".to_string(),
		msg: EventMsg::UndoCompleted(UndoCompletedEvent { success: true, message: None }),
	});
...
async fn undo_started_hides_interrupt_hint() {
	...
	let status = chat.bottom_pane.status_widget().expect("status indicator should be active");
	assert!(
...
async fn undo_started_refreshes_default_spinner_project_title() {
	...
	let project =
		chat.last_terminal_title.clone().expect("default title should include a project name");
	chat.terminal_title_animation_origin = Instant::now() + Duration::from_secs(1);
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
	...
	assert!(blob.contains("Add new feature X"), "expected subject in output");
	...
	match evt {
		AppEvent::CodexOp(Op::Review { review_request }) => {
			assert_eq!(
				review_request,
				ReviewRequest {
					target: ReviewTarget::Custom { instructions: "please audit dependencies".to_string() },
					user_facing_hint: None,
				}
			);
		}
...
async fn review_custom_prompt_escape_navigates_back_then_dismisses() {
	...
	chat.handle_key_event(KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
	assert!(chat.is_normal_backtrack_mode(), "expected to be back in normal composer mode");
}
...
async fn review_branch_picker_escape_navigates_back_then_dismisses() {
	...
	let header = render_bottom_first_row(&chat, 60);
	assert!(header.contains("Select a base branch"), "expected branch picker header: {header:?}");
...
fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
	AbsolutePathBuf::try_from(std::env::temp_dir().join("codex-plugin-menu-tests").join(path))
...
fn plugins_test_summary(
	...
	PluginSummary {
		id: id.to_string(),
		name: name.to_string(),
		source: PluginSource::Local { path: plugins_test_absolute_path(&format!("plugins/{name}")) },
		installed,
		enabled,
		install_policy,
...
fn plugins_test_curated_marketplace(plugins: Vec<PluginSummary>) -> PluginMarketplaceEntry {
	PluginMarketplaceEntry {
		...
		interface: Some(MarketplaceInterface { display_name: Some("ChatGPT Marketplace".to_string()) }),
		plugins,
	}
}
...
fn plugins_test_repo_marketplace(plugins: Vec<PluginSummary>) -> PluginMarketplaceEntry {
	PluginMarketplaceEntry {
		...
		interface: Some(MarketplaceInterface { display_name: Some("Repo Marketplace".to_string()) }),
		plugins,
	}
}

fn plugins_test_response(marketplaces: Vec<PluginMarketplaceEntry>) -> PluginListResponse {
	PluginListResponse { marketplaces, remote_sync_error: None, featured_plugin_ids: Vec::new() }
}
...
fn plugins_test_popup_row_position(popup: &str, needle: &str) -> usize {
	popup.find(needle).unwrap_or_else(|| panic!("expected popup to contain {needle}: {popup}"))
}
...
async fn plugin_detail_popup_snapshot_shows_install_actions_and_capability_summaries() {
	...
	let response =
		plugins_test_response(vec![plugins_test_curated_marketplace(vec![summary.clone()])]);
	let cwd = chat.config.cwd.clone();
...
fn selected_permissions_popup_line(popup: &str) -> &str {
	popup
		...
		.unwrap_or_else(|| panic!("expected permissions popup to have a selected preset row: {popup}"))
}
...
async fn apps_popup_stays_loading_until_final_snapshot_updates() {
	let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
	set_chatgpt_auth(&mut chat);
	chat.config.features.enable(Feature::Apps).expect("test config should allow feature update");
...
async fn apps_refresh_failure_keeps_existing_full_snapshot() {
	...
	chat.on_connectors_loaded(Ok(ConnectorsSnapshot { connectors: full_connectors.clone() }), true);
...
async fn apps_refresh_failure_with_cached_snapshot_triggers_pending_force_refetch() {
	...
	chat.connectors_cache =
		ConnectorsCacheState::Ready(ConnectorsSnapshot { connectors: full_connectors.clone() });
...
async fn apps_initial_load_applies_enabled_state_from_config() {
	...
	chat.config.config_layer_stack =
		chat.config.config_layer_stack.with_user_config(&config_toml_path, user_config);
...
async fn experimental_features_toggle_saves_on_exit() {
	...
	assert!(rx.try_recv().is_err(), "expected no updates until saving the popup");

	chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

	let mut updates = None;
	while let Ok(event) = rx.try_recv() {
		if let AppEvent::UpdateFeatureFlags { updates: event_updates } = event {
...
async fn server_overloaded_error_does_not_switch_models() {
	...
	while let Ok(event) = rx.try_recv() {
		if let AppEvent::UpdateModel(model) = event {
			assert_eq!(model, "gpt-5.2-codex", "did not expect model switch on server-overloaded error");
		}
	}
...
async fn preset_matching_accepts_workspace_write_with_extra_roots() {
	let preset =
		builtin_approval_presets().into_iter().find(|p| p.id == "auto").expect("auto preset exists");
...
async fn windows_auto_mode_prompt_requests_enabling_sandbox_feature() {
	...
	let preset =
		builtin_approval_presets().into_iter().find(|preset| preset.id == "auto").expect("auto preset");
...
async fn startup_prompts_for_windows_sandbox_when_agent_requested() {
	...
	assert!(popup.contains("Quit"), "expected startup prompt to offer quit action: {popup}");
}
...
async fn reasoning_popup_shows_extra_high_with_space() {
	...
	let popup = render_bottom_popup(&chat, 120);
	assert!(popup.contains("Extra high"), "expected popup to include 'Extra high'; popup: {popup}");
	assert!(
...
async fn user_shell_command_renders_output_not_exploring() {
	...
	let begin_ls =
		begin_exec_with_source(&mut chat, "user-shell-ls", "ls", ExecCommandSource::UserShell);
	end_exec(&mut chat, begin_ls, "file1\nfile2\n", "", 0);

	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected a single history cell for the user command");
...
async fn disabled_slash_command_while_task_running_snapshot() {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(!cells.is_empty(), "expected an error message history cell to be emitted",);
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
	let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("mistral-large-latest")).await;
...
async fn user_turn_carries_service_tier_after_fast_toggle() {
	...
	match next_submit_op(&mut op_rx) {
		Op::UserTurn { service_tier: Some(Some(ServiceTier::Fast)), .. } => {}
...
async fn fast_status_indicator_is_hidden_for_non_gpt54_model() {
	let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("mistral-large-latest")).await;
	chat.set_service_tier(Some(ServiceTier::Fast));
	set_chatgpt_auth(&mut chat);
...
async fn approvals_popup_shows_disabled_presets() {
	...
	terminal.set_viewport_area(Rect::new(0, 0, width, height));
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("render approvals popup");

	let screen = terminal.backend().vt100().screen().contents();
	let collapsed = screen.split_whitespace().collect::<Vec<_>>().join(" ");
	assert!(collapsed.contains("(disabled)"), "disabled preset label should be shown");
	assert!(
...
async fn approvals_popup_navigation_skips_disabled() {
	...
	assert!(op_rx.try_recv().is_err(), "no actions should be dispatched yet");
...
async fn permissions_selection_emits_history_cell_when_selection_changes() {
	...
	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected one permissions selection history cell");
	let rendered = lines_to_single_string(&cells[0]);
	assert!(
...
async fn permissions_selection_history_snapshot_full_access_to_default() {
	...
	chat.config.notices.hide_full_access_warning = Some(true);
	chat.config.permissions.approval_policy.set(AskForApproval::Never).expect("set approval policy");
	chat
		.config
		.permissions
		.sandbox_policy
...
async fn permissions_selection_emits_history_cell_when_current_is_selected() {
	...
	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected history cell even when selecting current permissions");
	let rendered = lines_to_single_string(&cells[0]);
	assert!(
		rendered.contains("Permissions updated to"),
...
async fn permissions_selection_marks_guardian_approvals_current_after_session_configured() {
	...
	chat.config.notices.hide_full_access_warning = Some(true);
	let _ = chat.config.features.set_enabled(Feature::GuardianApproval, true);
...
async fn permissions_selection_can_disable_guardian_approvals() {
	...
	assert!(
		events
			.iter()
			.any(|event| matches!(event, AppEvent::UpdateApprovalsReviewer(ApprovalsReviewer::User))),
		"expected selecting Default from Guardian Approvals to switch back to manual approval review: {events:?}"
	);
	assert!(
		!events.iter().any(|event| matches!(event, AppEvent::UpdateFeatureFlags { .. })),
...
async fn permissions_full_access_history_cell_emitted_only_after_confirmation() {
	...
	while let Ok(event) = rx.try_recv() {
		match event {
			...
			AppEvent::OpenFullAccessConfirmation { preset, return_to_permissions } => {
	...
	assert_eq!(total_history_cells, 1, "expected one full access history cell total");
...
async fn approval_modal_exec_snapshot() -> anyhow::Result<()> {
	...
	chat.config.permissions.approval_policy.set(AskForApproval::OnRequest)?;
	...
	chat
		.handle_codex_event(Event { id: "sub-approve".into(), msg: EventMsg::ExecApprovalRequest(ev) });
	...
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw approval modal");
	assert!(terminal.backend().vt100().screen().contents().contains("echo hello world"));
	assert_snapshot!("approval_modal_exec", terminal.backend().vt100().screen().contents());
...
async fn approval_modal_exec_without_reason_snapshot() -> anyhow::Result<()> {
	...
	assert_snapshot!("approval_modal_exec_no_reason", terminal.backend().vt100().screen().contents());
...
-> anyhow::Result<()> {
	...
	assert_snapshot!("approval_modal_exec_multiline_prefix_no_execpolicy", contents);
...
async fn approval_modal_patch_snapshot() -> anyhow::Result<()> {
	...
	changes.insert(PathBuf::from("README.md"), FileChange::Add { content: "hello\nworld\n".into() });
	...
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw patch approval modal");
	assert_snapshot!("approval_modal_patch", terminal.backend().vt100().screen().contents());
...
async fn interrupt_restores_queued_messages_into_composer() {
	...
	assert_eq!(chat.bottom_pane.composer_text(), "first queued\nsecond queued");

	// Queue should be cleared and no new user input should have been auto-submitted.
	assert!(chat.queued_user_messages.is_empty());
	assert!(op_rx.try_recv().is_err(), "unexpected outbound op after interrupt");
...
async fn interrupt_prepends_queued_messages_before_existing_composer_text() {
	...
	chat.bottom_pane.set_task_running(true);
	chat.bottom_pane.set_composer_text("current draft".to_string(), Vec::new(), Vec::new());
...
async fn ui_snapshots_small_heights_idle() {
	...
	for h in [1u16, 2, 3] {
		...
		terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw chat idle");
...
async fn ui_snapshots_small_heights_task_running() {
	...
	for h in [1u16, 2, 3] {
		...
		let mut terminal = Terminal::new(TestBackend::new(40, h)).expect("create terminal");
		terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw chat running");
		assert_snapshot!(name, terminal.backend());
	}
}
...
async fn status_widget_and_approval_modal_snapshot() {
	...
	chat.handle_codex_event(Event {
		id: "task-1".into(),
		msg: EventMsg::AgentReasoningDelta(AgentReasoningDeltaEvent { delta: "**Analyzing**".into() }),
	});
	...
	let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(width, height))
		.expect("create terminal");
	terminal.set_viewport_area(Rect::new(0, 0, width, height));
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw status + approval modal");
...
async fn status_widget_active_snapshot() {
	...
	let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(80, height))
		.expect("create terminal");
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw status widget");
...
async fn mcp_startup_header_booting_snapshot() {
	...
	let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(80, height))
		.expect("create terminal");
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw chat widget");
...
async fn guardian_parallel_reviews_keep_remaining_review_visible_after_denial() {
	...
	assert_eq!(chat.current_status.details, Some("rm -rf '/tmp/guardian target 2'".to_string()));
}
...
async fn apply_patch_events_emit_history_cells() {
	...
	let mut changes = HashMap::new();
	changes.insert(PathBuf::from("foo.txt"), FileChange::Add { content: "hello\n".to_string() });
	let ev = ApplyPatchApprovalRequestEvent {
		call_id: "c1".into(),
	...
	chat.handle_codex_event(Event { id: "s1".into(), msg: EventMsg::ApplyPatchApprovalRequest(ev) });
	let cells = drain_insert_history(&mut rx);
	assert!(
		cells.is_empty(),
	...
	changes2.insert(PathBuf::from("foo.txt"), FileChange::Add { content: "hello\n".to_string() });
	...
	chat.handle_codex_event(Event { id: "s1".into(), msg: EventMsg::PatchApplyBegin(begin) });
	let cells = drain_insert_history(&mut rx);
	...
	end_changes.insert(PathBuf::from("foo.txt"), FileChange::Add { content: "hello\n".to_string() });
	...
	chat.handle_codex_event(Event { id: "s1".into(), msg: EventMsg::PatchApplyEnd(end) });
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "no success cell should be emitted anymore");
}
...
async fn apply_patch_manual_approval_adjusts_header() {
	...
	proposed_changes
		.insert(PathBuf::from("foo.txt"), FileChange::Add { content: "hello\n".to_string() });
	chat.handle_codex_event(Event {
		id: "s1".into(),
...
async fn apply_patch_manual_flow_snapshot() {
	...
	assert!(history_before_apply.is_empty(), "expected approval modal to defer history emission");
	...
	let approved_lines = drain_insert_history(&mut rx).pop().expect("approved patch cell");
...
async fn apply_patch_approval_sends_op_with_call_id() {
	...
	let mut changes = HashMap::new();
	changes.insert(PathBuf::from("file.rs"), FileChange::Add { content: "fn main(){}\n".into() });
	let ev = ApplyPatchApprovalRequestEvent {
	...
	let mut found = false;
	while let Ok(app_ev) = rx.try_recv() {
		if let AppEvent::SubmitThreadOp { op: Op::PatchApproval { id, decision }, .. } = app_ev {
...
async fn apply_patch_full_flow_integration_like() {
	...
	let mut changes = HashMap::new();
	changes.insert(PathBuf::from("pkg.rs"), FileChange::Add { content: "".into() });
	chat.handle_codex_event(Event {
		id: "sub-xyz".into(),
		msg: EventMsg::ApplyPatchApprovalRequest(ApplyPatchApprovalRequestEvent {
			call_id: "call-1".into(),
	...
	chat.submit_op(op);
	let forwarded = op_rx.try_recv().expect("expected op forwarded to codex channel");
	match forwarded {
	...
	let mut changes2 = HashMap::new();
	changes2.insert(PathBuf::from("pkg.rs"), FileChange::Add { content: "".into() });
	chat.handle_codex_event(Event {
		id: "sub-xyz".into(),
		msg: EventMsg::PatchApplyBegin(PatchApplyBeginEvent {
			call_id: "call-1".into(),
	...
	let mut end_changes = HashMap::new();
	end_changes.insert(PathBuf::from("pkg.rs"), FileChange::Add { content: "".into() });
	chat.handle_codex_event(Event {
		id: "sub-xyz".into(),
...
async fn apply_patch_untrusted_shows_approval_modal() -> anyhow::Result<()> {
	...
	let mut changes = HashMap::new();
	changes.insert(PathBuf::from("a.rs"), FileChange::Add { content: "".into() });
	chat.handle_codex_event(Event {
		id: "sub-1".into(),
		msg: EventMsg::ApplyPatchApprovalRequest(ApplyPatchApprovalRequestEvent {
			call_id: "call-1".into(),
...
async fn apply_patch_request_shows_diff_summary() -> anyhow::Result<()> {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected approval request to render via modal instead of history");
	...
	assert!(saw_line1 && saw_line2, "expected modal to show per-line diff summary");
...
async fn plan_update_renders_history_cell() {
	...
	let update = UpdatePlanArgs {
		...
		plan: vec![
			PlanItemArg { step: "Explore codebase".into(), status: StepStatus::Completed },
			PlanItemArg { step: "Implement feature".into(), status: StepStatus::InProgress },
			PlanItemArg { step: "Write tests".into(), status: StepStatus::Pending },
		],
	};
	chat.handle_codex_event(Event { id: "sub-1".into(), msg: EventMsg::PlanUpdate(update) });
	let cells = drain_insert_history(&mut rx);
	assert!(!cells.is_empty(), "expected plan update cell to be sent");
	let blob = lines_to_single_string(cells.last().unwrap());
	assert!(blob.contains("Updated Plan"), "missing plan header: {blob:?}");
...
async fn stream_error_updates_status_indicator() {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected no history cell for StreamError event");
	let status = chat.bottom_pane.status_widget().expect("status indicator should be visible");
...
async fn replayed_stream_error_does_not_set_retry_status_or_status_indicator() {
	...
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected no history cell for replayed StreamError event");
...
async fn thread_snapshot_replayed_stream_recovery_restores_previous_status_header() {
	...
	chat.handle_codex_event_replay(Event {
		id: "delta".into(),
		msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent { delta: "hello".to_string() }),
	});
...
async fn resume_replay_interrupted_reconnect_does_not_leave_stale_working_state() {
	...
	chat.replay_initial_messages(vec![
		...
		EventMsg::AgentMessageDelta(AgentMessageDeltaEvent { delta: "hello".to_string() }),
	]);
...
async fn warning_event_adds_warning_history_cell() {
	...
	chat.handle_codex_event(Event {
		id: "sub-1".into(),
		msg: EventMsg::Warning(WarningEvent { message: "test warning message".to_string() }),
	});

	let cells = drain_insert_history(&mut rx);
	assert_eq!(cells.len(), 1, "expected one warning history cell");
	let rendered = lines_to_single_string(&cells[0]);
	assert!(rendered.contains("test warning message"), "warning cell missing content: {rendered}");
}
...
async fn status_line_invalid_items_warn_once() {
	...
	chat.refresh_status_surfaces();
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected invalid status line warning to emit only once");
}
...
async fn terminal_title_invalid_items_warn_once() {
	...
	chat.config.tui_terminal_title =
		Some(vec!["status".to_string(), "bogus_item".to_string(), "bogus_item".to_string()]);
	chat.thread_id = Some(ThreadId::new());
	...
	chat.refresh_status_surfaces();
	let cells = drain_insert_history(&mut rx);
	assert!(cells.is_empty(), "expected invalid terminal title warning to emit only once");
}
...
async fn terminal_title_setup_cancel_reverts_live_preview() {
	...
	assert_eq!(chat.terminal_title_setup_original_items, Some(original.clone()));
...
async fn terminal_title_status_uses_plain_labels_for_transient_states_when_animations_disabled() {
	...
	chat.handle_codex_event(Event {
		id: "undo-1".to_string(),
		msg: EventMsg::UndoStarted(UndoStartedEvent { message: Some("Undoing changes".to_string()) }),
	});
...
async fn default_terminal_title_refreshes_when_spinner_state_changes() {
	...
	let cwd = chat.current_cwd.clone().unwrap_or_else(|| chat.config.cwd.clone());
...
async fn terminal_title_uses_spaces_around_spinner_item() {
	...
	let title = chat.last_terminal_title.clone().expect("expected terminal title");
...
async fn status_line_fast_mode_footer_snapshot() {
	...
	terminal.draw(|f| chat.render(f.area(), f.buffer_mut())).expect("draw fast-mode footer");
...
async fn status_line_model_with_reasoning_includes_fast_for_gpt54_only() {
	...
	chat.set_model("mistral-large-latest");
	chat.refresh_status_surfaces();

	assert_eq!(
		status_line_text(&chat),
		Some("mistral-large-latest xhigh · 100% left · /tmp/project".to_string())
	);
}
...
async fn multiple_agent_messages_in_single_turn_emit_multiple_headers() {
	...
	let cells = drain_insert_history(&mut rx);
	let combined: String = cells.iter().map(|lines| lines_to_single_string(lines)).collect();
...
async fn deltas_then_same_final_message_are_rendered_snapshot() {
	...
	chat.handle_codex_event(Event {
		id: "s1".into(),
		msg: EventMsg::AgentReasoningDelta(AgentReasoningDeltaEvent { delta: "I will ".into() }),
	});
	chat.handle_codex_event(Event {
		id: "s1".into(),
	...
	chat.handle_codex_event(Event {
		id: "s1".into(),
		msg: EventMsg::AgentReasoningDelta(AgentReasoningDeltaEvent { delta: "request.".into() }),
	});
	chat.handle_codex_event(Event {
		id: "s1".into(),
		msg: EventMsg::AgentReasoning(AgentReasoningEvent { text: "request.".into() }),
	});

	// Then stream answer deltas, followed by the exact same final message.
	chat.handle_codex_event(Event {
		id: "s1".into(),
		msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent { delta: "Here is the ".into() }),
	});
	chat.handle_codex_event(Event {
		id: "s1".into(),
		msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent { delta: "result.".into() }),
	});
