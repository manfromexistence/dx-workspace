...
impl RealtimeAudioBehavior {
	...
	fn input_behavior(
		...
		match self {
			...
			Self::PlaybackAware => {
				crate::voice::RealtimeInputBehavior::PlaybackAware { playback_queued_samples }
			}
		}
	}
}
...
impl ChatWidget {
	...
	pub(super) fn rendered_user_message_event_from_parts(
		...
		RenderedUserMessageEvent { message, remote_image_urls, local_images, text_elements }
	}
	...
	pub(super) fn pending_steer_compare_key_from_items(
		...
		PendingSteerCompareKey { message, image_count }
	}
	...
	pub(super) fn rendered_user_message_event_from_inputs(
		...
		for item in items {
			match item {
				UserInput::Text { text, text_elements: current_text_elements } => {
					append_text_with_rebased_elements(
						&mut message,
						&mut text_elements,
						text,
						current_text_elements.iter().map(|element| {
							TextElement::new(element.byte_range, element.placeholder(text).map(str::to_string))
						}),
					)
				}
	...
	pub(super) fn should_render_realtime_user_message_event(&self, event: &UserMessageEvent) -> bool {
		if !self.realtime_conversation.is_live() {
			return false;
		}
	...
	pub(super) fn on_realtime_conversation_started(&mut self, ev: RealtimeConversationStartedEvent) {
	...
	pub(super) fn on_realtime_conversation_closed(&mut self, ev: RealtimeConversationClosedEvent) {
		...
		if !requested
			...
			&& reason != "error"
		{
			self.add_info_message(format!("Realtime voice mode closed: {reason}"), /*hint*/ None);
		}
		self.request_redraw();
	}
	...
	fn start_realtime_local_audio(&mut self) {
		...
		let capture = match crate::voice::VoiceCapture::start_realtime(
			...
			self
				...
				.audio_behavior
				.input_behavior(Arc::clone(&self.realtime_conversation.playback_queued_samples)),
		) {
			Ok(capture) => capture,
			Err(err) => {
				self.realtime_conversation.meter_placeholder_id = None;
				self.remove_transcription_placeholder(&placeholder_id);
				self.fail_realtime_conversation(format!("Failed to start microphone capture: {err}"));
				return;
			}
		};
		...
		match kind {
			...
			RealtimeAudioDeviceKind::Speaker => {
				...
				match crate::voice::RealtimeAudioPlayer::start(
					...
					Err(err) => {
						self.fail_realtime_conversation(format!("Failed to start speaker output: {err}"));
					}
				}
			}
		}
