...
impl ChatWidget {
	...
	fn open_plugins_loading_popup(&mut self) {
		if !self.bottom_pane.replace_selection_view_if_active(
			...
			self.bottom_pane.show_selection_view(self.plugins_loading_popup_params());
		}
	}

	fn open_plugins_popup(&mut self, response: &PluginListResponse) {
		self.bottom_pane.show_selection_view(self.plugins_popup_params(response));
	}
	...
	pub(crate) fn open_plugin_detail_loading_popup(&mut self, plugin_display_name: &str) {
		...
		let _ = self.bottom_pane.replace_selection_view_if_active(PLUGINS_SELECTION_VIEW_ID, params);
	}
		...
		match result {
			Ok(response) => {
				...
				if self.plugin_install_apps_needing_auth.is_empty() {
					...
					self.plugin_install_auth_flow =
						Some(super::PluginInstallAuthFlowState { plugin_display_name, next_app_index: 0 });
	...
	fn open_plugin_install_auth_popup(&mut self) {
		...
		if !self.bottom_pane.replace_selection_view_if_active(PLUGINS_SELECTION_VIEW_ID, params)
	...
	fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
		...
		let app = self.plugin_install_apps_needing_auth.get(flow.next_app_index)?;
		...
		let status_label =
			if is_installed { "Already installed in this session." } else { "Not installed yet." };
		let description = app
			.description
			.as_deref()
			.map(str::trim)
		...
		header.push(Line::from(format!("{} plugin installed.", flow.plugin_display_name).bold()));
		header.push(Line::from(format!("App setup {current}/{total}: {}", app.name).dim()));
		...
		if let Some(install_url) = app.install_url.clone() {
			let install_label = if is_installed { "Manage on ChatGPT" } else { "Install on ChatGPT" };
			items.push(SelectionItem {
				name: install_label.to_string(),
				description: Some("Open the same ChatGPT app management link used by /apps.".to_string()),
				selected_description: Some("Open the app page in your browser.".to_string()),
				actions: vec![Box::new(move |tx| {
					tx.send(AppEvent::OpenUrlInBrowser { url: install_url.clone() });
				})],
				..Default::default()
			});
		} else {
		...
		if is_installed {
			items.push(SelectionItem {
				...
				actions: vec![Box::new(|tx| {
					tx.send(AppEvent::PluginInstallAuthAdvance { refresh_connectors: false });
				})],
				..Default::default()
			});
		} else {
			items.push(SelectionItem {
				...
				description: Some("Trust your confirmation and continue to the next app.".to_string()),
				selected_description: Some("Continue without waiting for refresh to complete.".to_string()),
				actions: vec![Box::new(|tx| {
					tx.send(AppEvent::PluginInstallAuthAdvance { refresh_connectors: true });
				})],
				..Default::default()
			});
		}
	...
	fn plugin_install_auth_app_is_installed(&self, app_id: &str) -> bool {
		self.connectors_for_mentions().is_some_and(|connectors| {
			connectors.iter().any(|connector| connector.id == app_id && connector.is_accessible)
		})
	}
	...
	fn finish_plugin_install_auth_flow(&mut self, abandoned: bool) {
		...
		if abandoned {
			self.add_info_message(
				format!("Skipped remaining app setup for {} plugin.", flow.plugin_display_name),
				Some("The plugin may not be usable until required apps are installed.".to_string()),
			);
		} else {
			self.add_info_message(
				format!("Completed app setup flow for {} plugin.", flow.plugin_display_name),
	...
	fn plugins_loading_popup_params(&self) -> SelectionViewParams {
		...
		header.push(Line::from("This first pass shows the ChatGPT marketplace only.".dim()));
	...
	fn plugin_detail_loading_popup_params(&self, plugin_display_name: &str) -> SelectionViewParams {
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Plugins".bold()));
		header.push(Line::from(format!("Loading details for {plugin_display_name}...").dim()));
		...
		SelectionViewParams {
			...
			items: vec![SelectionItem {
				...
				description: Some("This updates when the plugin detail request finishes.".to_string()),
				is_disabled: true,
				..Default::default()
			}],
			..Default::default()
	...
	fn plugin_install_loading_popup_params(&self, plugin_display_name: &str) -> SelectionViewParams {
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Plugins".bold()));
		header.push(Line::from(format!("Installing {plugin_display_name}...").dim()));
	...
	fn plugin_uninstall_loading_popup_params(
		...
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Plugins".bold()));
		header.push(Line::from(format!("Uninstalling {plugin_display_name}...").dim()));
	...
	fn plugins_popup_params(&self, response: &PluginListResponse) -> SelectionViewParams {
		...
		let total: usize = marketplaces.iter().map(|marketplace| marketplace.plugins.len()).sum();
		...
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Plugins".bold()));
		header.push(Line::from("Browse plugins from the ChatGPT marketplace.".dim()));
		header.push(Line::from(format!("Installed {installed} of {total} available plugins.").dim()));
		if let Some(remote_sync_error) = response.remote_sync_error.as_deref() {
			header.push(Line::from(format!("Using cached marketplace data: {remote_sync_error}").dim()));
		}
		...
		for marketplace in marketplaces {
			...
			for plugin in &marketplace.plugins {
				...
				let selected_description = format!("{status_label}. Press Enter to view plugin details.");
				let search_value =
					format!("{display_name} {} {} {}", plugin.id, plugin.name, marketplace_label);
				let cwd = self.config.cwd.clone();
	...
	fn plugin_detail_popup_params(
		...
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Plugins".bold()));
		header.push(Line::from(format!("{display_name} · {marketplace_label}").bold()));
		header.push(Line::from(status_label.dim()));
		...
		let mut items = vec![SelectionItem {
			...
			selected_description: Some("Return to the plugin list.".to_string()),
			actions: vec![Box::new(move |tx| {
				tx.send(AppEvent::PluginsLoaded { cwd: cwd.clone(), result: Ok(plugins_response.clone()) });
			})],
			..Default::default()
		}];
		...
		if plugin.summary.installed {
			...
			items.push(SelectionItem {
				...
				description: Some("This plugin is not installable from this marketplace.".to_string()),
				is_disabled: true,
				..Default::default()
			});
		} else {
...
fn plugin_status_label(plugin: &PluginSummary) -> &'static str {
	if plugin.installed {
		if plugin.enabled { "Installed" } else { "Installed · Disabled" }
	} else {
...
fn plugin_description(plugin: &PluginSummary) -> Option<String> {
	plugin
		...
		.as_ref()
		.and_then(|interface| {
			interface.short_description.as_deref().or(interface.long_description.as_deref())
		})
		.map(str::trim)
...
fn plugin_detail_description(plugin: &PluginDetail) -> Option<String> {
	plugin
		...
		.as_deref()
		.or_else(|| {
			plugin.summary.interface.as_ref().and_then(|interface| interface.long_description.as_deref())
		})
		.or_else(|| {
			plugin.summary.interface.as_ref().and_then(|interface| interface.short_description.as_deref())
		})
		.map(str::trim)
		.filter(|description| !description.is_empty())
		.map(str::to_string)
...
fn plugin_skill_summary(plugin: &PluginDetail) -> String {
	if plugin.skills.is_empty() {
		...
		plugin.skills.iter().map(|skill| skill.name.as_str()).collect::<Vec<_>>().join(", ")
	}
}
...
fn plugin_app_summary(plugin: &PluginDetail) -> String {
	if plugin.apps.is_empty() {
		...
		plugin.apps.iter().map(|app| app.name.as_str()).collect::<Vec<_>>().join(", ")
	}
}
