...
impl StatusSurfaceSelections {
	fn uses_git_branch(&self) -> bool {
		self.status_line_items.contains(&StatusLineItem::GitBranch)
			|| self.terminal_title_items.contains(&TerminalTitleItem::GitBranch)
	}
}
...
impl ChatWidget {
	...
	fn warn_invalid_status_line_items_once(&mut self, invalid_items: &[String]) {
		if self.thread_id.is_some()
			...
			&& self
				...
				.is_ok()
		{
			let label = if invalid_items.len() == 1 { "item" } else { "items" };
			let message = format!("Ignored invalid status line {label}: {}.", proper_join(invalid_items));
	...
	fn warn_invalid_terminal_title_items_once(&mut self, invalid_items: &[String]) {
				...
				.is_ok()
		{
			let label = if invalid_items.len() == 1 { "item" } else { "items" };
			let message =
				format!("Ignored invalid terminal title {label}: {}.", proper_join(invalid_items));
			self.on_warning(message);
		}
	}
	...
	fn refresh_status_line_from_selections(&mut self, selections: &StatusSurfaceSelections) {
		...
		let line = if parts.is_empty() { None } else { Some(Line::from(parts.join(" · "))) };
	...
	fn refresh_terminal_title_from_selections(&mut self, selections: &StatusSurfaceSelections) {
		...
		let title = selections
			...
			.iter()
			.copied()
			.filter_map(|item| self.terminal_title_value_for_item(item, now).map(|value| (item, value)))
		...
		if self.last_terminal_title == title {
			if should_animate_spinner {
				self.frame_requester.schedule_frame_in(TERMINAL_TITLE_SPINNER_INTERVAL);
			}
			return;
		}
		match title {
	...
	pub(super) fn configured_status_line_items(&self) -> Vec<String> {
		self
			...
			.tui_status_line
			.clone()
			.unwrap_or_else(|| DEFAULT_STATUS_LINE_ITEMS.iter().map(ToString::to_string).collect())
	}
	...
	pub(super) fn configured_terminal_title_items(&self) -> Vec<String> {
		self
			...
			.tui_terminal_title
			.clone()
			.unwrap_or_else(|| DEFAULT_TERMINAL_TITLE_ITEMS.iter().map(ToString::to_string).collect())
	}
	...
	fn status_line_project_root_name(&mut self) -> Option<String> {
		...
		self.status_line_project_root_name_cache =
			Some(CachedProjectRootName { cwd, root_name: root_name.clone() });
		root_name
	}
	...
	fn terminal_title_project_name(&mut self) -> Option<String> {
		...
		Some(Self::truncate_terminal_title_part(project, /*max_chars*/ 24))
	}
	...
	fn sync_status_line_branch_state(&mut self, cwd: &Path) {
		if self.status_line_branch_cwd.as_ref().is_some_and(|path| path == cwd) {
			return;
		}
	...
	pub(super) fn status_line_value_for_item(&mut self, item: &StatusLineItem) -> Option<String> {
		match item {
			...
			StatusLineItem::CurrentDir => {
				Some(format_directory_display(self.status_line_cwd(), /*max_width*/ None))
			}
			...
			StatusLineItem::UsedTokens => {
				...
				if total <= 0 { None } else { Some(format!("{} used", format_tokens_compact(total))) }
			}
			StatusLineItem::ContextRemaining => {
				self.status_line_context_remaining_percent().map(|remaining| format!("{remaining}% left"))
			}
			StatusLineItem::ContextUsed => {
				self.status_line_context_used_percent().map(|used| format!("{used}% used"))
			}
			StatusLineItem::FiveHourLimit => {
				let window =
					self.rate_limit_snapshots_by_limit_id.get("codex").and_then(|s| s.primary.as_ref());
				let label = window
			...
			StatusLineItem::WeeklyLimit => {
				let window =
					self.rate_limit_snapshots_by_limit_id.get("codex").and_then(|s| s.secondary.as_ref());
				let label = window
					.and_then(|window| window.window_minutes)
					.map(get_limits_duration)
			...
			StatusLineItem::TotalInputTokens => {
				Some(format!("{} in", format_tokens_compact(self.status_line_total_usage().input_tokens)))
			}
			StatusLineItem::TotalOutputTokens => {
				Some(format!("{} out", format_tokens_compact(self.status_line_total_usage().output_tokens)))
			}
			StatusLineItem::SessionId => self.thread_id.map(|id| id.to_string()),
			StatusLineItem::FastMode => {
				Some(if matches!(self.config.service_tier, Some(ServiceTier::Fast)) {
					"Fast on".to_string()
				} else {
					"Fast off".to_string()
				})
		...
		match item {
			...
			TerminalTitleItem::Thread => self.thread_name.as_ref().and_then(|name| {
				...
				if trimmed.is_empty() {
					...
					Some(Self::truncate_terminal_title_part(trimmed.to_string(), /*max_chars*/ 48))
				}
			}),
	...
	fn should_animate_terminal_title_spinner_with_selections(
		...
		self.config.animations
			&& selections.terminal_title_items.contains(&TerminalTitleItem::Spinner)
			&& self.terminal_title_has_active_progress()
	}
