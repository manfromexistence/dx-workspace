...
pub(crate) fn set_theme_override(
	...
	if THEME.get().is_some() {
		set_syntax_theme(resolve_theme_with_override(name.as_deref(), codex_home.as_deref()));
	}
...
fn build_default_theme() -> Theme {
	...
	let codex_home = CODEX_HOME.get().and_then(|codex_home| codex_home.as_deref());
...
pub(crate) fn list_available_themes(codex_home: Option<&Path>) -> Vec<ThemeEntry> {
	let mut entries: Vec<ThemeEntry> = BUILTIN_THEME_NAMES
		.iter()
		.map(|name| ThemeEntry { name: name.to_string(), is_custom: false })
		.collect();
			...
			for entry in read_dir.flatten() {
					...
					&& let Some(stem) = path.file_stem().and_then(|s| s.to_str())
				{
					...
					if is_valid_theme && !entries.iter().any(|e| e.name == name) {
						entries.push(ThemeEntry { name, is_custom: true });
					}
				}
			}
		}
...
<key>background</key><string>{deleted_background}</string>
	...
	fn highlight_unknown_lang_falls_back() {
		...
		for line in &lines {
			for span in &line.spans {
				assert_eq!(span.style, Style::default(), "expected default style for unknown language");
			}
		}
	}
	...
	fn highlight_crlf_strips_carriage_return() {
		...
		for (i, line) in lines.iter().enumerate() {
			for span in &line.spans {
				assert!(!span.content.contains('\r'), "line {i} span {:?} contains \\r", span.content,);
			}
		}
	}
	...
	fn convert_style_suppresses_underline() {
		...
		let syn = SyntectStyle {
			foreground: syntect::highlighting::Color { r: 100, g: 200, b: 150, a: 255 },
			background: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 0xFF },
	...
	fn style_conversion_uses_ansi_named_color_when_alpha_is_zero_low_index() {
		let syn = SyntectStyle {
			foreground: syntect::highlighting::Color { r: 0x02, g: 0, b: 0, a: 0 },
			background: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 0xFF },
	...
	fn style_conversion_uses_indexed_color_when_alpha_is_zero_high_index() {
		let syn = SyntectStyle {
			foreground: syntect::highlighting::Color { r: 0x9a, g: 0, b: 0, a: 0 },
			background: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 0xFF },
			font_style: FontStyle::empty(),
		};
		let rt = convert_style(syn);
	...
	fn style_conversion_uses_terminal_default_when_alpha_is_one() {
		let syn = SyntectStyle {
			foreground: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 1 },
			background: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 0xFF },
			font_style: FontStyle::empty(),
		};
		let rt = convert_style(syn);
	...
	fn style_conversion_unexpected_alpha_falls_back_to_rgb() {
		let syn = SyntectStyle {
			foreground: syntect::highlighting::Color { r: 10, g: 20, b: 30, a: 0x80 },
			background: syntect::highlighting::Color { r: 0, g: 0, b: 0, a: 0xFF },
			font_style: FontStyle::empty(),
		};
		let rt = convert_style(syn);
	...
	fn highlight_markdown_preserves_content() {
		...
		assert_eq!(result, code, "markdown highlighting must preserve content exactly");
	}
	...
	fn find_syntax_resolves_languages_and_aliases() {
		...
		for lang in languages {
			assert!(find_syntax(lang).is_some(), "find_syntax({lang:?}) returned None");
		}
		// Common file extensions.
		let extensions = [
			"rs", "py", "js", "ts", "rb", "go", "sh", "md", "yml", "kt", "ex", "hs", "pl", "php", "css",
			"html", "cs",
		];
		for ext in extensions {
			assert!(find_syntax(ext).is_some(), "find_syntax({ext:?}) returned None");
		}
	...
	fn diff_scope_backgrounds_prefer_markup_scope_then_diff_fallback() {
		...
		assert_eq!(
			rgbs,
			DiffScopeBackgroundRgbs { inserted: Some((10, 20, 30)), deleted: Some((40, 50, 60)) }
		);
	}
	...
	fn custom_tmtheme_diff_scope_backgrounds_are_resolved() {
		...
		assert_eq!(
			rgbs,
			DiffScopeBackgroundRgbs { inserted: Some((16, 32, 48)), deleted: Some((64, 80, 96)) }
		);
	}
	...
	fn validate_theme_name_warns_when_custom_file_is_invalid() {
		...
		let warning = validate_theme_name(Some("my-fancy"), Some(dir.path()));
		assert!(warning.is_some(), "should warn when custom .tmTheme exists but cannot be parsed");
		assert!(
			warning.as_deref().is_some_and(|msg| msg.contains("could not be loaded")),
	...
	fn list_available_themes_excludes_invalid_custom_files() {
		...
		assert!(
			entries.iter().any(|entry| entry.name == "valid-custom" && entry.is_custom),
			"expected valid custom theme to be listed"
		);
		assert!(
			!entries.iter().any(|entry| entry.name == "broken-custom" && entry.is_custom),
	...
	fn list_available_themes_returns_stable_sorted_order() {
		...
		let entries = list_available_themes(Some(dir.path()));
		let actual: Vec<(bool, String)> =
			entries.iter().map(|entry| (entry.is_custom, entry.name.clone())).collect();
	...
	fn parse_theme_name_is_exhaustive() {
		...
		assert_eq!(all_variants.len(), 32, "two-face theme count changed — update parse_theme_name");
