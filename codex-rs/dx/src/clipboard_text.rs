...
fn copy_via_osc52(text: &str) -> Result<(), String> {
	...
	let mut tty = OpenOptions::new()
		.write(true)
		.open("/dev/tty")
		.map_err(|e| format!("clipboard unavailable: failed to open /dev/tty for OSC 52 copy: {e}"))?;
	#[cfg(unix)]
	tty
		.write_all(sequence.as_bytes())
		.map_err(|e| format!("clipboard unavailable: failed to write OSC 52 escape sequence: {e}"))?;
	#[cfg(unix)]
	tty
		.flush()
		.map_err(|e| format!("clipboard unavailable: failed to flush OSC 52 escape sequence: {e}"))?;
	#[cfg(windows)]
	stdout()
		.write_all(sequence.as_bytes())
		.map_err(|e| format!("clipboard unavailable: failed to write OSC 52 escape sequence: {e}"))?;
...
fn copy_via_wsl_clipboard(text: &str) -> Result<(), String> {
	...
	if let Err(err) = stdin.write_all(text.as_bytes()) {
		let _ = child.kill();
		let _ = child.wait();
		return Err(format!("clipboard unavailable: failed to write to powershell.exe: {err}"));
	}
	...
	if output.status.success() {
		...
		if stderr.is_empty() {
			...
			Err(format!("clipboard unavailable: powershell.exe exited with status {status}"))
		} else {
