...
impl VoiceCapture {
	pub fn start() -> Result<Self, String> {
		...
		Ok(Self { stream: Some(stream), sample_rate, channels, data, stopped, last_peak })
	}
	...
	pub fn stop(mut self) -> Result<RecordedAudio, String> {
		...
		self.stream.take();
		let data = self.data.lock().map_err(|_| "failed to lock audio buffer".to_string())?.clone();
		Ok(RecordedAudio { data, sample_rate: self.sample_rate, channels: self.channels })
	}
...
impl RecordingMeterState {
	pub(crate) fn new() -> Self {
		...
		Self { history, noise_ema: 0.02, env: 0.0 }
	}
...
pub fn transcribe_async(
	...
	std::thread::spawn(move || {
		...
		let res: Result<String, String> =
			rt.block_on(async move { transcribe_bytes(wav_bytes, context, duration_seconds).await });
		...
		cpal::SampleFormat::F32 => device
			.build_input_stream(
				&config.clone().into(),
				{
					...
					move |input: &[f32], _| {
						let peak = peak_f32(input);
						if !should_send_realtime_input(peak, &input_behavior, &mut allow_input_until) {
...
fn send_realtime_audio_chunk(
	...
	tx.send(AppEvent::CodexOp(Op::RealtimeConversationAudio(ConversationAudioParams {
...
impl RealtimeAudioPlayer {
	pub(crate) fn start(config: &Config, queued_samples: Arc<AtomicUsize>) -> Result<Self, String> {
		let (device, config) = crate::audio_device::select_configured_output_device_and_config(config)?;
		let output_sample_rate = config.sample_rate().0;
		let output_channels = config.channels();
		let queue = Arc::new(Mutex::new(VecDeque::new()));
		let stream =
			build_output_stream(&device, &config, Arc::clone(&queue), Arc::clone(&queued_samples))?;
		stream.play().map_err(|e| format!("failed to start output stream: {e}"))?;
		Ok(Self { _stream: stream, queue, queued_samples, output_sample_rate, output_channels })
	}
	...
	pub(crate) fn enqueue_frame(&self, frame: &RealtimeAudioFrame) -> Result<(), String> {
		...
		let mut guard =
			self.queue.lock().map_err(|_| "failed to lock output audio queue".to_string())?;
		...
		self.queued_samples.fetch_add(converted.len(), Ordering::Relaxed);
...
fn should_send_realtime_input(
	...
	let playback_queued_samples = match input_behavior {
		...
		RealtimeInputBehavior::PlaybackAware { playback_queued_samples } => playback_queued_samples,
	};
...
fn encode_wav_normalized(audio: &RecordedAudio) -> Result<Vec<u8>, String> {
	...
	let (channels, sample_rate, segment) =
		if audio.channels == MODEL_AUDIO_CHANNELS && audio.sample_rate == MODEL_AUDIO_SAMPLE_RATE {
			...
			(MODEL_AUDIO_CHANNELS, MODEL_AUDIO_SAMPLE_RATE, converted.as_slice())
		};

	let mut wav_bytes: Vec<u8> = Vec::new();
	let spec =
		WavSpec { channels, sample_rate, bits_per_sample: 16, sample_format: SampleFormat::Int };
	...
	let gain: f32 = if peak_abs > 0 { target / (peak_abs as f32) } else { 1.0 };

	for &s in segment {
		let v = ((s as f32) * gain).round().clamp(i16::MIN as f32, i16::MAX as f32) as i16;
		writer.write_sample(v).map_err(|_| "failed writing wav sample".to_string())?;
	}
	writer.finalize().map_err(|_| "failed to finalize wav".to_string())?;
	Ok(wav_bytes)
}
...
async fn resolve_auth() -> Result<TranscriptionAuthContext, String> {
	...
	let token = auth.get_token().map_err(|e| format!("failed to get auth token: {e}"))?;
...
async fn transcribe_bytes(
	...
	let (endpoint, request) = if matches!(auth.mode, AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens)
	{
		...
		let part = reqwest::multipart::Part::bytes(wav_bytes)
			...
			.mime_str("audio/wav")
			.map_err(|e| format!("failed to set mime: {e}"))?;
		let mut form = reqwest::multipart::Form::new().text("model", AUDIO_MODEL).part("file", part);
	...
	let resp = request.send().await.map_err(|e| format!("transcription request failed: {e}"))?;
	...
	let v: serde_json::Value = resp.json().await.map_err(|e| format!("failed to parse json: {e}"))?;
	let text = v.get("text").and_then(|t| t.as_str()).unwrap_or("").to_string();

	if text.is_empty() { Err("empty transcription result".to_string()) } else { Ok(text) }
}
...
mod tests {
	...
	fn encode_wav_normalized_outputs_24khz_mono_audio() {
		...
		let samples =
			reader.into_samples::<i16>().collect::<Result<Vec<_>, _>>().expect("samples should decode");
	...
	fn ungated_realtime_input_ignores_playback_backlog() {
		...
		assert!(
			should_send_realtime_input(0, &RealtimeInputBehavior::Ungated, &mut allow_input_until,)
		);
	...
	fn playback_aware_realtime_input_requires_an_interrupt_peak() {
		...
		assert!(!should_send_realtime_input(100, &input_behavior, &mut allow_input_until,));
		assert!(should_send_realtime_input(5_000, &input_behavior, &mut allow_input_until,));
