...
impl OssSelectionWidget<'_> {
	fn new(lmstudio_status: ProviderStatus, ollama_status: ProviderStatus) -> io::Result<Self> {
		let providers = vec![
			...
			ProviderOption { name: "Ollama (Chat)".to_string(), status: ollama_status },
		];

		let mut contents: Vec<Line> = vec![
			Line::from(vec!["? ".fg(Color::Blue), "Select an open-source provider".bold()]),
			Line::from(""),
		...
		contents.push(Line::from(""));
		contents
			.push(Line::from("  Press Enter to select • Ctrl+C to exit").add_modifier(Modifier::DIM));
	...
	pub fn handle_key_event(&mut self, key: KeyEvent) -> Option<String> {
		...
		if self.done { self.selection.clone() } else { None }
	}
	...
	fn handle_select_key(&mut self, key_event: KeyEvent) {
		match key_event.code {
			...
			other => {
				...
				if let Some(opt) =
					self.select_options.iter().find(|opt| Self::normalize_keycode(opt.key) == normalized)
				{
					self.send_decision(opt.provider_id.to_string());
				}
			}
...
impl WidgetRef for &OssSelectionWidget<'_> {
	fn render_ref(&self, area: Rect, buf: &mut Buffer) {
		...
		let [title_area, button_area, description_area] =
		...
		let areas = Layout::horizontal(lines.iter().map(|l| Constraint::Length(l.width() as u16 + 2)))
			.spacing(1)
			.split(button_area);
...
async fn check_port_status(port: u16) -> io::Result<bool> {
	let client =
		reqwest::Client::builder().timeout(Duration::from_secs(2)).build().map_err(io::Error::other)?;
