...
impl<S: EventSource + Default> EventBroker<S> {
	pub fn new() -> Self {
		...
		Self { state: Mutex::new(EventBrokerState::Start), resume_events_tx }
	}
...
impl<S: EventSource + Default + Unpin> TuiEventStream<S> {
	...
	pub fn poll_crossterm_event(&mut self, cx: &mut Context<'_>) -> Poll<Option<TuiEvent>> {
		...
		loop {
			let poll_result = {
				let mut state = self.broker.state.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
...
mod tests {
	...
	async fn key_event_skips_unmapped() {
		...
		handle.send(Ok(Event::Key(KeyEvent::new(KeyCode::Char('a'), KeyModifiers::NONE))));
