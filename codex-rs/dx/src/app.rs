...
fn session_summary(
	...
	Some(SessionSummary { usage_line, resume_command })
}
...
fn emit_skill_load_warnings(app_event_tx: &AppEventSender, errors: &[SkillErrorInfo]) {
	...
	app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(crate::history_cell::new_warning_event(
...
async fn start_plugin_request_client(
	...
	InProcessAppServerClient::start(InProcessClientStartArgs {
		...
		client_name: "codex-tui-dx".to_string(),
...
async fn request_plugins_list(
	...
	let response = request_handle
		.request_typed(ClientRequest::PluginList {
			request_id,
			params: PluginListParams { cwds: Some(vec![cwd]), force_remote_sync: false },
		})
		.await
...
async fn request_plugin_uninstall(
	...
	let response = request_handle
		.request_typed(ClientRequest::PluginUninstall {
			request_id,
			params: PluginUninstallParams { plugin_id, force_remote_sync: false },
		})
		.await
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
	...
	for layer in config
		.config_layer_stack
		.get_layers(ConfigLayerStackOrdering::LowestPrecedenceFirst, /*include_disabled*/ true)
	{
	...
	app_event_tx
		.send(AppEvent::InsertHistoryCell(Box::new(history_cell::new_warning_event(message))));
}
...
async fn emit_custom_prompt_deprecation_notice(app_event_tx: &AppEventSender, codex_home: &Path) {
	...
	let prompt_count = codex_core::custom_prompts::discover_prompts_in(&prompts_dir).await.len();
	if prompt_count == 0 {
		return;
	}
	...
	app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(history_cell::new_deprecation_notice(
...
impl ThreadEventStore {
		...
		match &event.msg {
			...
			EventMsg::ItemCompleted(completed) => {
				if let TurnItem::UserMessage(item) = &completed.item {
					...
					let legacy = Event { id: event.id, msg: item.as_legacy_event() };
	...
	fn snapshot(&self) -> ThreadEventSnapshot {
		ThreadEventSnapshot {
			...
			events: self
				.buffer
				.iter()
				.filter(|event| self.pending_interactive_replay.should_replay_snapshot_event(event))
				.cloned()
				.collect(),
	...
	fn has_pending_thread_approvals(&self) -> bool {
		self.pending_interactive_replay.has_pending_thread_approvals()
	}
}
...
impl ThreadEventChannel {
	...
	fn new_with_session_configured(capacity: usize, event: Event) -> Self {
		...
		Self {
			sender,
			receiver: Some(receiver),
			store: Arc::new(Mutex::new(ThreadEventStore::new_with_session_configured(capacity, event))),
		}
	}
}
...
fn should_show_model_migration_prompt(
	...
	if !available_models.iter().any(|preset| preset.model == target_model && preset.show_in_picker) {
		return false;
	}

	if available_models.iter().any(|preset| preset.model == current_model && preset.upgrade.is_some())
	{
		return true;
	}
...
fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
	match migration_config_key {
		HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG => {
			config.notices.hide_gpt_5_1_codex_max_migration_prompt.unwrap_or(false)
		}
...
fn target_preset_for_upgrade<'a>(
	...
	available_models.iter().find(|preset| preset.model == target_model && preset.show_in_picker)
}
...
fn select_model_availability_nux(
	...
	available_models.iter().find_map(|preset| {
		...
		let shown_count = nux_config.shown_count.get(&preset.model).copied().unwrap_or_default();
...
async fn handle_model_migration_prompt_if_needed(
	...
	}) = upgrade
	{
		...
		let heading_label =
			if target_display_name == model { target_model.clone() } else { target_display_name.clone() };
		...
		match run_model_migration_prompt(tui, prompt_copy).await {
			ModelMigrationOutcome::Accepted => {
				...
				{
					reasoning_effort_mapping.get(&reasoning_effort).cloned().or(config.model_reasoning_effort)
				} else {
...
impl App {
	...
	async fn refresh_in_memory_config_from_disk(&mut self) -> Result<()> {
		let mut config = self.rebuild_config_for_cwd(self.chat_widget.config_ref().cwd.clone()).await?;
	...
	fn apply_runtime_policy_overrides(&mut self, config: &mut Config) {
			...
			&& let Err(err) = config.permissions.approval_policy.set(*policy)
		{
			...
			self
				.chat_widget
				.add_error_message(format!("Failed to carry forward approval policy override: {err}"));
		}
			...
			&& let Err(err) = config.permissions.sandbox_policy.set(policy.clone())
		{
			...
			self
				.chat_widget
				.add_error_message(format!("Failed to carry forward sandbox policy override: {err}"));
		}
	}
	...
	fn try_set_approval_policy_on_config(
		...
		if let Err(err) = config.permissions.approval_policy.set(policy) {
			...
			self.chat_widget.add_error_message(format!("{user_message_prefix}: {err}"));
			return false;
		}
	...
	async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
		...
		let windows_sandbox_changed = updates.iter().any(|(feature, _)| {
			matches!(feature, Feature::WindowsSandbox | Feature::WindowsSandboxElevated)
		});
		...
		let mut builder =
			ConfigEditsBuilder::new(&self.config.codex_home).with_profile(self.active_profile.as_deref());
		...
		for (feature, enabled) in updates {
			...
			if feature == Feature::GuardianApproval {
				...
				if effective_enabled {
					...
					feature_config.approvals_reviewer = guardian_approvals_preset.approvals_reviewer;
					feature_edits.push(ConfigEdit::SetPath {
						segments: scoped_segments("approvals_reviewer"),
						value: guardian_approvals_preset.approvals_reviewer.to_string().into(),
					});
					...
					if profile_approvals_reviewer_configured || self.active_profile.is_none() {
						feature_edits
							.push(ConfigEdit::ClearPath { segments: scoped_segments("approvals_reviewer") });
					}
			...
			builder =
				builder.with_edits(feature_edits).set_feature_enabled(feature_key, effective_enabled);
		}
		...
		if let Err(err) = builder.apply().await {
			...
			self.chat_widget.add_error_message(format!("Failed to update experimental features: {err}"));
			return;
		}
		...
		if sandbox_policy_override.is_some()
			&& let Err(err) =
				self.chat_widget.set_sandbox_policy(self.config.permissions.sandbox_policy.get().clone())
		{
			tracing::error!(
					error = %err,
					"failed to set guardian approvals sandbox policy on chat config"
			);
			self.chat_widget.add_error_message(format!("Failed to enable Guardian Approvals: {err}"));
		}
		...
		if let Some(label) = permissions_history_label {
			self
				.chat_widget
				.add_info_message(format!("Permissions updated to {label}"), /*hint*/ None);
		}
	}

	fn open_url_in_browser(&mut self, url: String) {
		if let Err(err) = webbrowser::open(&url) {
			self.chat_widget.add_error_message(format!("Failed to open browser for {url}: {err}"));
			return;
		}

		self.chat_widget.add_info_message(format!("Opened {url} in your browser."), /*hint*/ None);
	}
	...
	fn fetch_plugins_list(&mut self, cwd: PathBuf) {
		...
		tokio::spawn(async move {
			...
			app_event_tx.send(AppEvent::PluginsLoaded { cwd: cwd_for_event, result });
		});
	}
	...
	fn fetch_plugin_detail(&mut self, cwd: PathBuf, params: PluginReadParams) {
		...
		tokio::spawn(async move {
			...
			app_event_tx.send(AppEvent::PluginDetailLoaded { cwd: cwd_for_event, result });
		});
	}
	...
	fn fetch_plugin_install(
		...
		tokio::spawn(async move {
			...
			let result = request_plugin_install(
				...
				cloud_requirements,
				feedback,
				PluginInstallParams { marketplace_path, plugin_name, force_remote_sync: false },
			)
			.await
	...
	fn abort_all_thread_event_listeners(&mut self) {
		for handle in self.thread_event_listener_tasks.drain().map(|(_, handle)| handle) {
			handle.abort();
		}
	}
		...
		match &event.msg {
			...
			EventMsg::ApplyPatchApprovalRequest(ev) => {
				Some(ThreadInteractiveRequest::Approval(ApprovalRequest::ApplyPatch {
					...
					id: ev.call_id.clone(),
					reason: ev.reason.clone(),
					cwd: self.thread_cwd(thread_id).await.unwrap_or_else(|| self.config.cwd.clone()),
			...
			EventMsg::ElicitationRequest(ev) => {
				if let Some(request) = McpServerElicitationFormRequest::from_event(thread_id, ev.clone()) {
					...
					Some(ThreadInteractiveRequest::Approval(ApprovalRequest::McpElicitation {
						thread_id,
						thread_label,
			...
			EventMsg::RequestPermissions(ev) => {
				Some(ThreadInteractiveRequest::Approval(ApprovalRequest::Permissions {
					thread_id,
					thread_label,
			...
			match self.server.get_thread(thread_id).await {
				Ok(thread) => match thread.submit(op).await {
					...
					Err(err) => {
						self
							.chat_widget
							.add_error_message(format!("Failed to submit op to thread {thread_id}: {err}"));
						false
					}
				},
				Err(err) => {
	...
	async fn refresh_pending_thread_approvals(&mut self) {
		...
		let threads =
			pending_thread_ids.into_iter().map(|thread_id| self.thread_label(thread_id)).collect();
	...
	async fn enqueue_thread_event(&mut self, thread_id: ThreadId, event: Event) -> Result<()> {
		...
		let inactive_interactive_request = if self.active_thread_id != Some(thread_id) {
			self.interactive_request_for_thread_event(thread_id, &event).await
		} else {
			None
		};
		let (sender, store) = {
		...
		if should_send {
			...
			match request {
				...
				ThreadInteractiveRequest::McpServerElicitation(request) => {
					self.chat_widget.push_mcp_server_elicitation_request(request);
				}
			}
		}
	...
	async fn handle_routed_thread_event(&mut self, thread_id: ThreadId, event: Event) -> Result<()> {
	...
	async fn open_agent_picker(&mut self) {
		...
		let has_non_primary_agent_thread =
			self.agent_navigation.has_non_primary_thread(self.primary_thread_id);
		...
		if self.agent_navigation.is_empty() {
			self.chat_widget.add_info_message("No agents available yet.".to_string(), /*hint*/ None);
			return;
		}
	...
	async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
		...
		let Some((receiver, snapshot)) = self.activate_thread_for_replay(thread_id).await else {
			self.chat_widget.add_error_message(format!("Agent thread {thread_id} is already active."));
	...
	async fn start_fresh_session_with_summary_hint(&mut self, tui: &mut tui::Tui) {
		...
		// history.
		self.refresh_in_memory_config_from_disk_best_effort("starting a new thread").await;
		...
		let report = self.server.shutdown_all_threads_bounded(Duration::from_secs(10)).await;
	...
	fn replay_thread_snapshot(&mut self, snapshot: ThreadEventSnapshot, resume_restored_queue: bool) {
		...
		self.chat_widget.restore_thread_input_state(snapshot.input_state);
	...
	pub async fn run(
		...
		let available_models =
			thread_manager.get_models_manager().list_models(RefreshStrategy::Offline).await;
		...
		if config.tui_status_line.as_ref().is_some_and(|cmd| !cmd.is_empty()) {
		...
		chat_widget.maybe_prompt_windows_sandbox_enable(should_prompt_windows_sandbox_nux_at_startup);
		...
		#[cfg(target_os = "windows")]
		{
			let should_check = WindowsSandboxLevel::from_config(&app.config)
				...
				) && !app.config.notices.hide_world_writable_warning.unwrap_or(false);
			if should_check {
	...
	async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
		match event {
			...
			AppEvent::OpenResumePicker => {
				match crate::resume_picker::run_resume_picker(tui, &self.config, /*show_all*/ false).await?
				{
					SessionSelection::Resume(target_session) => {
						...
						let mut resume_config =
							match self.rebuild_config_for_resume_or_fallback(&current_cwd, resume_cwd).await {
								Ok(cfg) => cfg,
								Err(err) => {
									self.chat_widget.add_error_message(format!(
					...
					SessionSelection::Exit | SessionSelection::StartFresh | SessionSelection::Fork(_) => {}
				}
			...
			AppEvent::ForkCurrentSession => {
				...
				self.chat_widget.add_plain_history_lines(vec!["/fork".magenta().into()]);
				if let Some(path) = self.chat_widget.rollout_path() {
					self.refresh_in_memory_config_from_disk_best_effort("forking the thread").await;
			...
			AppEvent::DiffResult(text) => {
				...
				self.overlay = Some(Overlay::new_static_with_lines(pager_lines, "D I F F".to_string()));
				tui.frame_requester().schedule_frame();
			}
			AppEvent::OpenAppLink {
				...
				self.chat_widget.open_app_link_view(crate::bottom_pane::AppLinkViewParams {
					app_id,
					title,
					description,
					instructions,
			...
			AppEvent::OpenPluginDetailLoading { plugin_display_name } => {
				self.chat_widget.open_plugin_detail_loading_popup(&plugin_display_name);
			}
			AppEvent::OpenPluginInstallLoading { plugin_display_name } => {
				self.chat_widget.open_plugin_install_loading_popup(&plugin_display_name);
			}
			AppEvent::OpenPluginUninstallLoading { plugin_display_name } => {
				self.chat_widget.open_plugin_uninstall_loading_popup(&plugin_display_name);
			}
			...
			AppEvent::FetchPluginInstall { cwd, marketplace_path, plugin_name, plugin_display_name } => {
				self.fetch_plugin_install(cwd, marketplace_path, plugin_name, plugin_display_name);
			}
			AppEvent::FetchPluginUninstall { cwd, plugin_id, plugin_display_name } => {
			...
			AppEvent::PluginInstallLoaded {
				...
				if install_succeeded && self.chat_widget.config_ref().cwd == cwd {
					...
					if should_refresh_plugin_detail {
						self.fetch_plugin_detail(cwd, PluginReadParams { marketplace_path, plugin_name });
					}
				}
			}
			...
			AppEvent::OpenFullAccessConfirmation { preset, return_to_permissions } => {
				self.chat_widget.open_full_access_confirmation(preset, return_to_permissions);
			}
			...
			AppEvent::OpenFeedbackNote { category, include_logs } => {
						...
						let event = match result {
							...
							Err(err) => {
								...
								if let Some((code, message)) =
									codex_core::windows_sandbox::elevated_setup_failure_details(&err)
								{
									code_tag = Some(code);
								...
								session_telemetry.counter(
									codex_core::windows_sandbox::elevated_setup_failure_metric_name(&err),
									/*inc*/ 1,
									&tags,
								);
								tracing::error!(
				...
				#[cfg(target_os = "windows")]
				{
					...
					tokio::task::spawn_blocking(move || {
						...
						let event = match codex_core::windows_sandbox_read_grants::grant_read_root_non_elevated(
							...
							Ok(canonical_path) => {
								AppEvent::WindowsSandboxGrantReadRootCompleted { path: canonical_path, error: None }
							}
			...
			AppEvent::WindowsSandboxGrantReadRootCompleted { path, error } => match error {
				Some(err) => {
					self.chat_widget.add_to_history(history_cell::new_error_event(format!("Error: {err}")));
				}
				None => {
					self.chat_widget.add_to_history(history_cell::new_info_event(
			...
			AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
				#[cfg(target_os = "windows")]
				{
					...
					let builder = ConfigEditsBuilder::new(&self.config.codex_home)
						...
						.set_windows_sandbox_mode(if elevated_enabled { "elevated" } else { "unelevated" })
						.clear_legacy_windows_sandbox_keys();
					match builder.apply().await {
						Ok(()) => {
							if elevated_enabled {
								self.config.set_windows_sandbox_enabled(/*value*/ false);
								self.config.set_windows_elevated_sandbox_enabled(/*value*/ true);
							} else {
								self.config.set_windows_sandbox_enabled(/*value*/ true);
								self.config.set_windows_elevated_sandbox_enabled(/*value*/ false);
							}
							self
								.chat_widget
								...
								self.chat_widget.world_writable_warning_details()
							{
								...
								self.app_event_tx.send(AppEvent::OpenWorldWritableWarningConfirmation {
								...
								self.app_event_tx.send(AppEvent::UpdateAskForApprovalPolicy(preset.approval));
								self.app_event_tx.send(AppEvent::UpdateSandboxPolicy(preset.sandbox.clone()));
								let _ = mode;
						...
						Err(err) => {
							...
							self
								.chat_widget
								.add_error_message(format!("Failed to enable the Windows sandbox feature: {err}"));
						}
					}
				}
				#[cfg(not(target_os = "windows"))]
					...
					.await
				{
					...
					Err(err) => {
						...
						if let Some(profile) = profile {
							self
								.chat_widget
								.add_error_message(format!("Failed to save model for profile `{profile}`: {err}"));
						} else {
							self.chat_widget.add_error_message(format!("Failed to save default model: {err}"));
						}
					}
				}
			}
			AppEvent::PluginUninstallLoaded {
				...
				self.chat_widget.on_plugin_uninstall_loaded(cwd.clone(), plugin_display_name, result);
					...
					.await
				{
					...
					Err(err) => {
						...
						if let Some(profile) = profile {
							...
							self
								.chat_widget
								.add_error_message(format!("Failed to save default Fast mode: {err}"));
						}
					}
				}
			}
			AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
				let builder = match kind {
					RealtimeAudioDeviceKind::Microphone => ConfigEditsBuilder::new(&self.config.codex_home)
						.set_realtime_microphone(name.as_deref()),
					RealtimeAudioDeviceKind::Speaker => {
						ConfigEditsBuilder::new(&self.config.codex_home).set_realtime_speaker(name.as_deref())
					}
				};
				...
				match builder.apply().await {
					...
					Err(err) => {
						...
						self
							.chat_widget
							.add_error_message(format!("Failed to save realtime {}: {err}", kind.noun()));
					}
				}
			}
			...
			AppEvent::UpdateSandboxPolicy(policy) => {
				...
				if let Err(err) = self.chat_widget.set_sandbox_policy(policy_for_chat) {
					...
					self.chat_widget.add_error_message(format!("Failed to set sandbox policy: {err}"));
					return Ok(AppRunControl::Continue);
				}
			...
			AppEvent::UpdateApprovalsReviewer(policy) => {
				...
				let segments = if let Some(profile) = profile {
					vec!["profiles".to_string(), profile.to_string(), "approvals_reviewer".to_string()]
				} else {
					vec!["approvals_reviewer".to_string()]
				};
				if let Err(err) = ConfigEditsBuilder::new(&self.config.codex_home)
					.with_profile(profile)
					.with_edits([ConfigEdit::SetPath { segments, value: policy.to_string().into() }])
					.apply()
					.await
				{
					tracing::error!(
					...
					self.chat_widget.add_error_message(format!("Failed to save approvals reviewer: {err}"));
				}
			}
			...
			AppEvent::UpdateWorldWritableWarningAcknowledged(ack) => {
				self.chat_widget.set_world_writable_warning_acknowledged(ack);
			}
			...
			AppEvent::PersistWorldWritableWarningAcknowledged => {
					...
					.await
				{
					...
					self
						.chat_widget
						.add_error_message(format!("Failed to save Agent mode warning preference: {err}"));
				}
			}
			AppEvent::PersistRateLimitSwitchPromptHidden => {
					...
					.await
				{
					...
					self
						.chat_widget
						.add_error_message(format!("Failed to save rate limit reminder preference: {err}"));
				}
			}
			AppEvent::PersistPlanModeReasoningEffort(effort) => {
				...
				let edit = if let Some(effort) = effort {
					ConfigEdit::SetPath { segments, value: effort.to_string().into() }
				} else {
					ConfigEdit::ClearPath { segments }
				};
				if let Err(err) =
					ConfigEditsBuilder::new(&self.config.codex_home).with_edits([edit]).apply().await
				{
					tracing::error!(
							error = %err,
					...
					if let Some(profile) = profile {
						...
						self
							.chat_widget
							.add_error_message(format!("Failed to save Plan mode reasoning effort: {err}"));
					}
				}
			}
			AppEvent::PersistModelMigrationPromptAcknowledged { from_model, to_model } => {
					...
					.await
				{
					...
					self
						.chat_widget
						.add_error_message(format!("Failed to save model migration prompt preference: {err}"));
				}
			}
			...
			AppEvent::SetSkillEnabled { path, enabled } => {
				let edits = [ConfigEdit::SetSkillConfig { path: path.clone(), enabled }];
				match ConfigEditsBuilder::new(&self.config.codex_home).with_edits(edits).apply().await {
					Ok(()) => {
			...
			AppEvent::SetAppEnabled { id, enabled } => {
				let edits = if enabled {
					vec![
						...
						ConfigEdit::ClearPath {
							segments: vec!["apps".to_string(), id.clone(), "disabled_reason".to_string()],
						},
					]
				} else {
					vec![
				...
				match ConfigEditsBuilder::new(&self.config.codex_home).with_edits(edits).apply().await {
					...
					Err(err) => {
						self
							.chat_widget
							.add_error_message(format!("Failed to update app config for {id}: {err}"));
					}
				}
			}
			...
			AppEvent::FullScreenApprovalRequest(request) => match request {
				...
				ApprovalRequest::Exec { command, .. } => {
					...
					self.overlay =
						Some(Overlay::new_static_with_lines(full_cmd_lines, "E X E C".to_string()));
				}
				ApprovalRequest::Permissions { permissions, reason, .. } => {
					let _ = tui.enter_alt_screen();
					...
					{
						lines.push(Line::from(vec!["Permission rule: ".into(), rule_line.cyan()]));
					}
					self.overlay = Some(Overlay::new_static_with_renderables(
				...
				ApprovalRequest::McpElicitation { server_name, message, .. } => {
					let _ = tui.enter_alt_screen();
			...
			AppEvent::UpdateRecordingMeter { id, text } => {
				...
				if updated || self.chat_widget.stop_realtime_conversation_for_deleted_meter(&id) {
					tui.frame_requester().schedule_frame();
				}
			}
			AppEvent::StatusLineSetup { items } => {
				...
				match apply_result {
					...
					Err(err) => {
						...
						self.chat_widget.add_error_message(format!("Failed to save status line items: {err}"));
					}
				}
			}
			...
			AppEvent::SyntaxThemeSelected { name } => {
				...
				match apply_result {
					...
					Err(err) => {
						...
						self.chat_widget.add_error_message(format!("Failed to save theme: {err}"));
					}
				}
			}
		}
	...
	async fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
			...
			self.active_non_primary_shutdown_target(&event.msg)
		{
			...
			if self.active_thread_id == Some(primary_thread_id) {
				self.chat_widget.add_info_message(
					format!("Agent thread {closed_thread_id} closed. Switched back to main thread."),
					/*hint*/ None,
				);
			} else {
				self.clear_active_thread().await;
	...
	async fn handle_thread_created(&mut self, thread_id: ThreadId) -> Result<()> {
		...
		self.thread_event_listener_tasks.insert(thread_id, listener_handle);
		Ok(())
	}
		...
		let editor_cmd = match external_editor::resolve_editor_command() {
			...
			Err(external_editor::EditorError::MissingEditor) => {
				self.chat_widget.add_to_history(history_cell::new_error_event(
					"Cannot open external editor: set $VISUAL or $EDITOR before starting Codex.".to_string(),
				));
			...
			Err(err) => {
				self
					.chat_widget
					.add_to_history(history_cell::new_error_event(format!("Failed to open editor: {err}",)));
				self.reset_external_editor_state(tui);
				return;
			}
		};
	...
	fn request_external_editor_launch(&mut self, tui: &mut tui::Tui) {
		self.chat_widget.set_external_editor_state(ExternalEditorState::Requested);
		self
			.chat_widget
			.set_footer_hint_override(Some(vec![(EXTERNAL_EDITOR_HINT.to_string(), String::new())]));
		tui.frame_requester().schedule_frame();
	}

	fn reset_external_editor_state(&mut self, tui: &mut tui::Tui) {
		self.chat_widget.set_external_editor_state(ExternalEditorState::Closed);
	...
	async fn handle_key_event(&mut self, tui: &mut tui::Tui, key_event: KeyEvent) {
		...
		let allow_agent_word_motion_fallback =
			!self.enhanced_keys_supported && self.chat_widget.composer_text_with_pending().is_empty();
            ...
            && previous_agent_shortcut_matches(key_event, allow_agent_word_motion_fallback)
		{
			if let Some(thread_id) = self
				.agent_navigation
				.adjacent_thread_id(self.current_displayed_thread_id(), AgentNavigationDirection::Previous)
			{
            ...
            && next_agent_shortcut_matches(key_event, allow_agent_word_motion_fallback)
		{
			if let Some(thread_id) = self
				.agent_navigation
				.adjacent_thread_id(self.current_displayed_thread_id(), AgentNavigationDirection::Next)
			{
				let _ = self.select_agent_thread(tui, thread_id).await;
			}
			return;
		...
		match key_event {
			...
			KeyEvent {
				...
				if let Err(err) = self.clear_terminal_ui(tui, /*redraw_header*/ false) {
					...
					self.chat_widget.add_error_message(format!("Failed to clear terminal UI: {err}"));
				} else {
					self.reset_app_ui_state_after_clear();
					self.queue_clear_ui_header(tui);
					tui.frame_requester().schedule_frame();
			...
			KeyEvent { kind: KeyEventKind::Press | KeyEventKind::Repeat, .. } => {
...
mod tests {
	...
	fn normalize_harness_overrides_resolves_relative_add_dirs() -> Result<()> {
		...
		assert_eq!(normalized.additional_writable_roots, vec![base_cwd.join("rel")]);
		Ok(())
	}

	#[test]
	fn startup_waiting_gate_is_only_for_fresh_or_exit_session_selection() {
		assert_eq!(App::should_wait_for_initial_session(&SessionSelection::StartFresh), true);
		assert_eq!(App::should_wait_for_initial_session(&SessionSelection::Exit), true);
		assert_eq!(
	...
	fn startup_waiting_gate_holds_active_thread_events_until_primary_thread_configured() {
		...
		assert_eq!(App::should_handle_active_thread_events(wait_for_initial_session, true), false);

		assert_eq!(App::should_stop_waiting_for_initial_session(wait_for_initial_session, None), false);
		...
		assert_eq!(App::should_handle_active_thread_events(wait_for_initial_session, true), true);
	}
	...
	fn startup_waiting_gate_not_applied_for_resume_or_fork_session_selection() {
		...
		assert_eq!(App::should_handle_active_thread_events(wait_for_resume, true), true);
		...
		assert_eq!(App::should_handle_active_thread_events(wait_for_fork, true), true);
	}
	...
	async fn enqueue_primary_event_delivers_session_configured_before_buffered_approval() -> Result<()>
	{
		...
		app.enqueue_primary_event(session_configured_event.clone()).await?;

		let rx = app.active_thread_rx.as_mut().expect("primary thread receiver should be active");
		...
		app.chat_widget.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));

		while let Ok(app_event) = app_event_rx.try_recv() {
			if let AppEvent::SubmitThreadOp { thread_id: op_thread_id, .. } = app_event {
	...
	async fn routed_thread_event_does_not_recreate_channel_after_reset() -> Result<()> {
		...
		app
			.thread_event_channels
			.insert(thread_id, ThreadEventChannel::new(THREAD_EVENT_CHANNEL_CAPACITY));

		app.reset_thread_event_state();
		app
			.handle_routed_thread_event(
				thread_id,
				Event { id: "stale-event".to_string(), msg: EventMsg::ShutdownComplete },
			)
			.await?;
	...
	async fn reset_thread_event_state_aborts_listener_tasks() {
		...
		started_rx.await.expect("listener task should report it started");
	...
	async fn enqueue_thread_event_does_not_block_when_channel_full() -> Result<()> {
		let mut app = make_test_app().await;
		let thread_id = ThreadId::new();
		app.thread_event_channels.insert(thread_id, ThreadEventChannel::new(1));
		app.set_thread_active(thread_id, true).await;

		let event = Event { id: String::new(), msg: EventMsg::ShutdownComplete };

		app.enqueue_thread_event(thread_id, event.clone()).await?;
		time::timeout(Duration::from_millis(50), app.enqueue_thread_event(thread_id, event))
			.await
	...
	async fn replay_thread_snapshot_restores_draft_and_queued_input() {
		...
		app.chat_widget.apply_external_edit("draft prompt".to_string());
		...
		let expected_input_state =
			app.chat_widget.capture_thread_input_state().expect("expected thread input state");

		app.store_active_thread_receiver().await;

		let snapshot = {
			let channel = app.thread_event_channels.get(&thread_id).expect("thread channel should exist");
			let store = channel.store.lock().await;
		...
		match next_user_turn_op(&mut new_op_rx) {
			Op::UserTurn { items, .. } => assert_eq!(
				items,
				vec![UserInput::Text { text: "queued follow-up".to_string(), text_elements: Vec::new() }]
			),
	...
	async fn replayed_turn_complete_submits_restored_queued_follow_up() {
		...
		app.chat_widget.handle_codex_event(session_configured.clone());
		...
		app.chat_widget.handle_codex_event(Event {
			...
			msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent { delta: "streaming".to_string() }),
		});
		app.chat_widget.apply_external_edit("queued follow-up".to_string());
		app.chat_widget.handle_key_event(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
		let input_state =
			app.chat_widget.capture_thread_input_state().expect("expected queued follow-up state");
	...
	async fn replace_chat_widget_keeps_replacement_terminal_title_cache_when_present() {
		...
		assert_eq!(app.chat_widget.last_terminal_title, Some("codex".to_string()));
	}
	...
	async fn replay_thread_snapshot_restores_pending_pastes_for_submit() {
		...
		app.chat_widget.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

		match next_user_turn_op(&mut new_op_rx) {
			Op::UserTurn { items, .. } => {
				assert_eq!(items, vec![UserInput::Text { text: large, text_elements: Vec::new() }])
			}
	...
	async fn replay_thread_snapshot_restores_collaboration_mode_for_draft_submit() {
		...
		app.chat_widget.handle_codex_event(session_configured.clone());
		app.chat_widget.set_reasoning_effort(Some(ReasoningEffortConfig::High));
		app.chat_widget.set_collaboration_mask(CollaborationModeMask {
			name: "Plan".to_string(),
		...
		app.chat_widget.apply_external_edit("draft prompt".to_string());
		let input_state =
			app.chat_widget.capture_thread_input_state().expect("expected draft input state");

		let (chat_widget, _app_event_tx, _rx, mut new_op_rx) =
			make_chatwidget_manual_with_sender().await;
		app.chat_widget = chat_widget;
		app.chat_widget.handle_codex_event(session_configured);
		app.chat_widget.set_reasoning_effort(Some(ReasoningEffortConfig::Low));
		app.chat_widget.set_collaboration_mask(CollaborationModeMask {
			name: "Default".to_string(),
		...
		match next_user_turn_op(&mut new_op_rx) {
			Op::UserTurn { items, model, effort, collaboration_mode, .. } => {
				assert_eq!(
					items,
					vec![UserInput::Text { text: "draft prompt".to_string(), text_elements: Vec::new() }]
				);
	...
	async fn replay_thread_snapshot_restores_collaboration_mode_without_input() {
		...
		assert_eq!(app.chat_widget.active_collaboration_mode_kind(), ModeKind::Plan);
		assert_eq!(app.chat_widget.current_model(), "gpt-restored");
		assert_eq!(app.chat_widget.current_reasoning_effort(), Some(ReasoningEffortConfig::High));
	}
	...
	async fn replayed_interrupted_turn_restores_queued_input_to_composer() {
		...
		assert_eq!(app.chat_widget.composer_text_with_pending(), "queued follow-up");
		assert!(app.chat_widget.queued_user_message_texts().is_empty());
		assert!(
			new_op_rx.try_recv().is_err(),
	...
	async fn live_turn_started_refreshes_status_line_with_runtime_context_window() {
		let mut app = make_test_app().await;
		app.chat_widget.setup_status_line(vec![crate::bottom_pane::StatusLineItem::ContextWindowSize]);
		...
		assert_eq!(app.chat_widget.status_line_text(), Some("950K window".into()));
	}
	...
	async fn open_agent_picker_keeps_missing_threads_for_replay() -> Result<()> {
		...
		assert_eq!(
			...
			Some(&AgentPickerThreadEntry { agent_nickname: None, agent_role: None, is_closed: true })
		);
	...
	async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
		...
		app.update_feature_flags(vec![(Feature::GuardianApproval, true)]).await;

		assert!(app.config.features.enabled(Feature::GuardianApproval));
		assert!(app.chat_widget.config_ref().features.enabled(Feature::GuardianApproval));
		assert_eq!(app.config.approvals_reviewer, guardian_approvals.approvals_reviewer);
		assert_eq!(app.config.permissions.approval_policy.value(), guardian_approvals.approval_policy);
		assert_eq!(
			app.chat_widget.config_ref().permissions.approval_policy.value(),
			guardian_approvals.approval_policy
		);
		assert_eq!(
			app.chat_widget.config_ref().permissions.sandbox_policy.get(),
	...
	-> Result<()> {
		...
		app.config.config_layer_stack =
			app.config.config_layer_stack.with_user_config(&config_toml_path, user_config);
		app.config.features.set_enabled(Feature::GuardianApproval, true)?;
		app.chat_widget.set_feature_enabled(Feature::GuardianApproval, true);
		app.config.approvals_reviewer = ApprovalsReviewer::GuardianSubagent;
		app.chat_widget.set_approvals_reviewer(ApprovalsReviewer::GuardianSubagent);
		app.config.permissions.approval_policy.set(AskForApproval::OnRequest)?;
		app.config.permissions.sandbox_policy.set(SandboxPolicy::new_workspace_write_policy())?;
		app.chat_widget.set_approval_policy(AskForApproval::OnRequest);
		app.chat_widget.set_sandbox_policy(SandboxPolicy::new_workspace_write_policy())?;

		app.update_feature_flags(vec![(Feature::GuardianApproval, false)]).await;

		assert!(!app.config.features.enabled(Feature::GuardianApproval));
		assert!(!app.chat_widget.config_ref().features.enabled(Feature::GuardianApproval));
		assert_eq!(app.config.approvals_reviewer, ApprovalsReviewer::User);
		assert_eq!(app.config.permissions.approval_policy.value(), AskForApproval::OnRequest);
		assert_eq!(app.chat_widget.config_ref().approvals_reviewer, ApprovalsReviewer::User);
		assert_eq!(app.runtime_approval_policy_override, None);
		assert_eq!(
			op_rx.try_recv(),
			Ok(Op::OverrideTurnContext {
	...
	-> Result<()> {
		...
		app.chat_widget.set_approvals_reviewer(ApprovalsReviewer::User);
	...
	-> Result<()> {
		...
		assert_eq!(
			config_value.as_table().and_then(|table| table.get("approvals_reviewer")),
...
"#;
	...
	-> Result<()> {
		...
		assert!(app.config.features.enabled(Feature::GuardianApproval));
		assert!(app.chat_widget.config_ref().features.enabled(Feature::GuardianApproval));
		assert_eq!(app.config.approvals_reviewer, ApprovalsReviewer::GuardianSubagent);
		assert_eq!(
			app.chat_widget.config_ref().approvals_reviewer,
	...
	async fn refresh_pending_thread_approvals_only_lists_inactive_threads() {
		...
		app.thread_event_channels.insert(main_thread_id, ThreadEventChannel::new(1));
		...
		app.thread_event_channels.insert(agent_thread_id, agent_channel);
		app.agent_navigation.upsert(
			agent_thread_id,
			Some("Robie".to_string()),
			Some("explorer".to_string()),
		...
		assert_eq!(app.chat_widget.pending_thread_approvals(), &["Robie [explorer]".to_string()]);
	...
	async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
		...
		assert_eq!(app.active_non_primary_shutdown_target(&EventMsg::SkillsUpdateAvailable), None);
		Ok(())
	}
	...
	-> Result<()> {
		...
		assert_eq!(app.active_non_primary_shutdown_target(&EventMsg::ShutdownComplete), None);
		Ok(())
	}
	...
	async fn render_clear_ui_header_after_long_transcript_for_snapshot() -> String {
		...
		let agent_cell = |text: &str| -> Arc<dyn HistoryCell> {
			Arc::new(AgentMessageCell::new(vec![Line::from(text.to_string())], true))
				as Arc<dyn HistoryCell>
		};
	...
	async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
		...
		app.chat_widget.set_reasoning_effort(Some(ReasoningEffortConfig::XHigh));
		app.chat_widget.set_service_tier(Some(codex_protocol::config_types::ServiceTier::Fast));
	...
	async fn make_test_app() -> App {
		...
		let server = Arc::new(codex_core::test_support::thread_manager_with_models_provider(
	...
	-> (App, tokio::sync::mpsc::UnboundedReceiver<AppEvent>, tokio::sync::mpsc::UnboundedReceiver<Op>)
	{
	...
	fn select_model_availability_nux_picks_only_eligible_model() {
		...
		let target =
			presets.iter_mut().find(|preset| preset.model == "gpt-5").expect("target preset present");
		target.availability_nux =
			Some(ModelAvailabilityNux { message: "gpt-5 is available".to_string() });
	...
	fn select_model_availability_nux_skips_missing_and_exhausted_models() {
		...
		let gpt_5 =
			presets.iter_mut().find(|preset| preset.model == "gpt-5").expect("gpt-5 preset present");
		gpt_5.availability_nux =
			Some(ModelAvailabilityNux { message: "gpt-5 is available".to_string() });
		let gpt_5_2 =
			presets.iter_mut().find(|preset| preset.model == "gpt-5.2").expect("gpt-5.2 preset present");
		gpt_5_2.availability_nux =
			Some(ModelAvailabilityNux { message: "gpt-5.2 is available".to_string() });
	...
	fn select_model_availability_nux_uses_existing_model_order_as_priority() {
		...
		let first =
			presets.iter_mut().find(|preset| preset.model == "gpt-5").expect("gpt-5 preset present");
		first.availability_nux = Some(ModelAvailabilityNux { message: "first".to_string() });
		let second =
			presets.iter_mut().find(|preset| preset.model == "gpt-5.2").expect("gpt-5.2 preset present");
		second.availability_nux = Some(ModelAvailabilityNux { message: "second".to_string() });
	...
	async fn model_migration_prompt_respects_hide_flag_and_self_target() {
		...
		assert!(!should_show_model_migration_prompt("gpt-5", "gpt-5.1", &seen, &all_model_presets()));
		assert!(!should_show_model_migration_prompt("gpt-5.1", "gpt-5.1", &seen, &all_model_presets()));
	}
	...
	async fn update_reasoning_effort_updates_collaboration_mode() {
		let mut app = make_test_app().await;
		app.chat_widget.set_reasoning_effort(Some(ReasoningEffortConfig::Medium));

		app.on_update_reasoning_effort(Some(ReasoningEffortConfig::High));

		assert_eq!(app.chat_widget.current_reasoning_effort(), Some(ReasoningEffortConfig::High));
		assert_eq!(app.config.model_reasoning_effort, Some(ReasoningEffortConfig::High));
	}
	...
	async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
		...
		ConfigEditsBuilder::new(&app.config.codex_home)
			.with_edits([
				...
				ConfigEdit::SetPath {
					segments: vec!["apps".to_string(), app_id.clone(), "disabled_reason".to_string()],
					value: "user".into(),
				},
			])
			.apply()
		...
		assert_eq!(app_enabled_in_effective_config(&app.config, &app_id), Some(false));
		Ok(())
	}
	...
	-> Result<()> {
		...
		app.refresh_in_memory_config_from_disk_best_effort("starting a new thread").await;
	...
	-> Result<()> {
		...
		let resume_config =
			app.rebuild_config_for_resume_or_fallback(&current_cwd, current_cwd.clone()).await?;
	...
	async fn rebuild_config_for_resume_or_fallback_errors_when_cwd_changes() -> Result<()> {
		...
		let result = app.rebuild_config_for_resume_or_fallback(&current_cwd, next_cwd).await;
	...
	async fn sync_tui_theme_selection_updates_chat_widget_config_copy() {
		...
		assert_eq!(app.chat_widget.config_ref().tui_theme.as_deref(), Some("dracula"));
	}
	...
	async fn fresh_session_config_uses_current_service_tier() {
		...
		assert_eq!(config.service_tier, Some(codex_protocol::config_types::ServiceTier::Fast));
	}
	...
	async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
		...
		let selection = app.confirm_backtrack_from_main().expect("backtrack selection");
		...
		assert_eq!(selection.remote_image_urls, vec!["https://example.com/backtrack.png".to_string()]);
	...
	async fn backtrack_remote_image_only_selection_clears_existing_composer_draft() {
		...
		app.chat_widget.set_composer_text("stale draft".to_string(), Vec::new(), Vec::new());
	...
	async fn replayed_initial_messages_apply_rollback_in_queue_order() {
		...
		let user_messages: Vec<String> = app
			...
			.iter()
			.filter_map(|cell| {
				cell.as_any().downcast_ref::<UserHistoryCell>().map(|cell| cell.message.clone())
			})
			.collect();
		assert_eq!(user_messages, vec!["first prompt".to_string(), "third prompt".to_string()]);
	}
	...
	async fn queued_rollback_syncs_overlay_and_clears_deferred_history() {
		...
		app.transcript_cells = vec![
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("after first")], false))
				as Arc<dyn HistoryCell>,
			Arc::new(UserHistoryCell {
				message: "second".to_string(),
				text_elements: Vec::new(),
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("after second")], false))
				as Arc<dyn HistoryCell>,
		];
	...
	async fn new_session_requests_shutdown_for_previous_conversation() {
		...
		app
			.chat_widget
			.handle_codex_event(Event { id: String::new(), msg: EventMsg::SessionConfigured(event) });
	...
	async fn shutdown_first_exit_returns_immediate_exit_when_shutdown_submit_fails() {
		...
		assert!(matches!(control, AppRunControl::Exit(ExitReason::UserRequested)));
	}
	...
	async fn session_summary_includes_resume_hint() {
		let usage =
			TokenUsage { input_tokens: 10, output_tokens: 2, total_tokens: 12, ..Default::default() };
		...
		assert_eq!(summary.usage_line, "Token usage: total=12 input=10 output=2");
		assert_eq!(
	...
	async fn session_summary_prefers_name_over_id() {
		...
		let summary =
			session_summary(usage, Some(conversation), Some("my-session".to_string())).expect("summary");
		assert_eq!(summary.resume_command, Some("codex resume my-session".to_string()));
	}
}
