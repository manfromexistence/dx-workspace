...
impl AgentNavigationState {
	...
	pub(crate) fn upsert(
		...
		self
			.threads
			.insert(thread_id, AgentPickerThreadEntry { agent_nickname, agent_role, is_closed });
	}
	...
	pub(crate) fn mark_closed(&mut self, thread_id: ThreadId) {
		if let Some(entry) = self.threads.get_mut(&thread_id) {
			...
			self.upsert(
				thread_id, /*agent_nickname*/ None, /*agent_role*/ None, /*is_closed*/ true,
			);
		}
	}
	...
	pub(crate) fn has_non_primary_thread(&self, primary_thread_id: Option<ThreadId>) -> bool {
		self.threads.keys().any(|thread_id| Some(*thread_id) != primary_thread_id)
	}
	...
	pub(crate) fn adjacent_thread_id(
		...
		let current_idx =
			ordered_threads.iter().position(|(thread_id, _)| *thread_id == current_thread_id)?;
	...
	pub(crate) fn picker_subtitle() -> String {
		...
		format!("Select an agent to watch. {} previous, {} next.", previous.content, next.content)
	}
	...
	pub(crate) fn ordered_thread_ids(&self) -> Vec<ThreadId> {
		self.ordered_threads().into_iter().map(|(thread_id, _)| thread_id).collect()
	}
}
...
mod tests {
	...
	fn populated_state() -> (AgentNavigationState, ThreadId, ThreadId, ThreadId) {
		...
		state.upsert(first_agent_id, Some("Robie".to_string()), Some("explorer".to_string()), false);
		state.upsert(second_agent_id, Some("Bob".to_string()), Some("worker".to_string()), false);
	...
	fn upsert_preserves_first_seen_order() {
		...
		state.upsert(first_agent_id, Some("Robie".to_string()), Some("worker".to_string()), true);

		assert_eq!(state.ordered_thread_ids(), vec![main_thread_id, first_agent_id, second_agent_id]);
	}
