...
impl FooterTip {
	fn new(text: impl Into<String>) -> Self {
		Self { text: text.into(), highlight: false }
	}

	fn highlighted(text: impl Into<String>) -> Self {
		Self { text: text.into(), highlight: true }
	}
}
...
impl RequestUserInputOverlay {
	...
	fn options_len(&self) -> usize {
		self.current_question().map(Self::options_len_for_question).unwrap_or(0)
	}
	...
	fn selected_option_index(&self) -> Option<usize> {
		...
		self.current_answer().and_then(|answer| answer.options_state.selected_idx)
	}
	...
	pub(super) fn notes_ui_visible(&self) -> bool {
		...
		let idx = self.current_index();
		self.current_answer().is_some_and(|answer| answer.notes_visible || self.notes_has_content(idx))
	}
	...
	pub(super) fn options_required_height(&self, width: u16) -> u16 {
		...
		let mut state = self.current_answer().map(|answer| answer.options_state).unwrap_or_default();
	...
	fn capture_composer_draft(&self) -> ComposerDraft {
		ComposerDraft {
			...
			local_image_paths: self.composer.local_images().into_iter().map(|img| img.path).collect(),
	...
	fn restore_current_draft(&mut self) {
		self.composer.set_placeholder_text(self.notes_placeholder().to_string());
		self.composer.set_footer_hint_override(Some(Vec::new()));
		let Some(answer) = self.current_answer() else {
			self.composer.set_text_content(String::new(), Vec::new(), Vec::new());
	...
	fn wrap_footer_tips(&self, width: u16, tips: Vec<FooterTip>) -> Vec<Vec<FooterTip>> {
		...
		for tip in tips {
			...
			let extra =
				if current.is_empty() { tip_width } else { separator_width.saturating_add(tip_width) };
			...
			if current.is_empty() {
				...
				used = used.saturating_add(separator_width).saturating_add(tip_width);
			}
			current.push(tip);
		}
	...
	fn reset_for_request(&mut self) {
		self.answers = self
			...
			.iter()
			.map(|question| {
				let has_options = question.options.as_ref().is_some_and(|options| !options.is_empty());
	...
	fn options_len_for_question(
		...
		let options_len = question.options.as_ref().map(std::vec::Vec::len).unwrap_or(0);
		if Self::other_option_enabled_for_question(question) { options_len + 1 } else { options_len }
	}

	fn other_option_enabled_for_question(
		question: &codex_protocol::request_user_input::RequestUserInputQuestion,
	) -> bool {
		question.is_other && question.options.as_ref().is_some_and(|options| !options.is_empty())
	}
	...
	fn submit_answers(&mut self) {
		...
		self.app_event_tx.send(AppEvent::CodexOp(Op::UserInputAnswer {
			...
			response: RequestUserInputResponse { answers: answers.clone() },
		}));
	...
	fn unanswered_confirmation_rows(&self) -> Vec<GenericDisplayRow> {
		let selected =
			self.confirm_unanswered.as_ref().and_then(|state| state.selected_idx).unwrap_or(0);
		let entries = [
	...
	fn notes_input_height(&self, width: u16) -> u16 {
		...
		self.composer.desired_height(width.max(1)).clamp(min_height, min_height.saturating_add(5))
	}

	fn apply_submission_to_draft(&mut self, text: String, text_elements: Vec<TextElement>) {
		let local_image_paths =
			self.composer.local_images().into_iter().map(|img| img.path).collect::<Vec<_>>();
		if let Some(answer) = self.current_answer_mut() {
		...
		self.composer.set_text_content(text, text_elements, local_image_paths);
		self.composer.move_cursor_to_end();
		self.composer.set_footer_hint_override(Some(Vec::new()));
	}
	...
	fn handle_composer_input_result(&mut self, result: InputResult) -> bool {
		match result {
			...
			| InputResult::Queued { text, text_elements } => {
				if self.has_options() && matches!(self.focus, Focus::Notes) && !text.trim().is_empty() {
					let options_len = self.options_len();
					if let Some(answer) = self.current_answer_mut() {
						answer.options_state.clamp_selection(options_len);
					}
	...
	fn handle_confirm_unanswered_key_event(&mut self, key_event: KeyEvent) {
		...
		match key_event.code {
			...
			KeyCode::Char('1') | KeyCode::Char('2') => {
				let idx = if matches!(key_event.code, KeyCode::Char('1')) { 0 } else { 1 };
	...
	fn handle_key_event(&mut self, key_event: KeyEvent) {
		...
		match self.focus {
			...
			Focus::Notes => {
				...
				self.ensure_selected_for_notes();
				if matches!(key_event.code, KeyCode::Char(_) | KeyCode::Backspace | KeyCode::Delete)
...
mod tests {
	...
	fn test_sender() -> (AppEventSender, tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
	...
	fn expect_interrupt_only(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
		...
		assert!(rx.try_recv().is_err(), "unexpected AppEvents before interrupt completion");
	}
		...
		RequestUserInputQuestion {
			...
			options: Some(vec![
				...
				RequestUserInputQuestionOption {
					...
					description: "Pick the most relevant crate and validate the current behavior first."
						.to_string(),
				},
				RequestUserInputQuestionOption {
					label: "Review the diff".to_string(),
					description: "Summarize the changes and highlight the most important risks and gaps."
						.to_string(),
				},
			]),
		}
	...
	fn request_event(
		...
		RequestUserInputEvent { call_id: "call-1".to_string(), turn_id: turn_id.to_string(), questions }
	}
	...
	fn enter_commits_default_selection_on_non_last_option_question() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_with_options("q1", "Pick one"), question_with_options("q2", "Pick two")],
			),
			tx,
			true,
			false,
		...
		assert!(rx.try_recv().is_err(), "unexpected AppEvent before full submission");
		...
		expected
			.insert("q1".to_string(), RequestUserInputAnswer { answers: vec!["Option 1".to_string()] });
		expected
			.insert("q2".to_string(), RequestUserInputAnswer { answers: vec!["Option 1".to_string()] });
	...
	fn options_notes_focus_hides_question_navigation_tip() {
		...
		let tips = overlay.footer_tips();
		let tip_texts = tips.iter().map(|tip| tip.text.as_str()).collect::<Vec<_>>();
		assert_eq!(tip_texts, vec!["tab or esc to clear notes", "enter to submit answer",]);
	}
	...
	fn freeform_shows_ctrl_p_and_ctrl_n_question_navigation_tip() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_with_options("q1", "Area"), question_without_options("q2", "Goal")],
			),
			tx,
			true,
			false,
		...
		assert_eq!(
			tip_texts,
			vec!["enter to submit all", "ctrl + p / ctrl + n change question", "esc to interrupt",]
		);
	}
	...
	fn switching_to_options_resets_notes_focus_when_notes_hidden() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_without_options("q1", "Notes"), question_with_options("q2", "Pick one")],
			),
			tx,
			true,
			false,
	...
	fn switching_from_freeform_with_text_resets_focus_and_keeps_last_option_empty() {
		...
		overlay.composer.set_text_content("freeform notes".to_string(), Vec::new(), Vec::new());
		...
		assert!(rx.try_recv().is_err(), "unexpected AppEvent before confirmation submit");
	...
	fn esc_drops_committed_answers() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_with_options("q1", "First"), question_without_options("q2", "Second")],
			),
			tx,
			true,
			false,
		...
		overlay.handle_key_event(KeyEvent::from(KeyCode::Enter));
		assert!(rx.try_recv().is_err(), "unexpected AppEvent before interruption");
	...
	fn tab_in_notes_clears_notes_and_hides_ui() {
		...
		overlay.handle_key_event(KeyEvent::from(KeyCode::Tab));
		overlay.composer.set_text_content("Some notes".to_string(), Vec::new(), Vec::new());
	...
	fn freeform_requires_enter_with_text_to_mark_answered() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_without_options("q1", "Notes"), question_without_options("q2", "More")],
			),
			tx,
			true,
			false,
	...
	fn freeform_commit_resets_when_draft_changes() {
		...
		overlay.composer.set_text_content("Committed".to_string(), Vec::new(), Vec::new());
		overlay.composer.move_cursor_to_end();
		overlay.handle_key_event(KeyEvent::from(KeyCode::Enter));
		assert_eq!(overlay.answers[0].answer_committed, true);
		let _ = rx.try_recv();
		...
		overlay.composer.set_text_content("Edited".to_string(), Vec::new(), Vec::new());
		overlay.composer.move_cursor_to_end();
		overlay.move_question(true);
		assert_eq!(overlay.answers[0].answer_committed, false);
	...
	fn notes_are_captured_for_selected_option() {
		...
		overlay.composer.set_text_content("Notes for option 2".to_string(), Vec::new(), Vec::new());
		overlay.composer.move_cursor_to_end();
		...
		assert_eq!(
			answer.answers,
			vec!["Option 2".to_string(), "user_note: Notes for option 2".to_string(),]
		);
	}
	...
	fn is_other_adds_none_of_the_above_and_submits_it() {
		let (tx, mut rx) = test_sender();
		let mut overlay = RequestUserInputOverlay::new(
			request_event("turn-1", vec![question_with_options_and_other("q1", "Pick one")]),
			tx,
			true,
			false,
			false,
		...
		assert_eq!(other_row.description.as_deref(), Some(OTHER_OPTION_DESCRIPTION));
		...
		overlay.composer.set_text_content("Custom answer".to_string(), Vec::new(), Vec::new());
		overlay.composer.move_cursor_to_end();
		let draft = overlay.capture_composer_draft();
		if let Some(answer) = overlay.current_answer_mut() {
			answer.draft = draft;
		...
		assert_eq!(
			answer.answers,
			vec![OTHER_OPTION_LABEL.to_string(), "user_note: Custom answer".to_string(),]
		);
	}
	...
	fn large_paste_is_preserved_when_switching_questions() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_without_options("q1", "First"), question_without_options("q2", "Second")],
			),
			tx,
			true,
			false,
	...
	fn pending_paste_placeholder_survives_submission_and_back_navigation() {
		...
		let mut overlay = RequestUserInputOverlay::new(
			request_event(
				"turn-1",
				vec![question_with_options("q1", "First"), question_with_options("q2", "Second")],
			),
			tx,
			true,
			false,
	...
	fn request_user_input_tight_height_snapshot() {
		...
		insta::assert_snapshot!("request_user_input_tight_height", render_snapshot(&overlay, area));
	}
	...
	fn layout_allocates_all_wrapped_options_when_space_allows() {
		...
		let height = question_height.saturating_add(options_height).saturating_add(extras);
	...
	fn footer_wraps_tips_without_splitting_individual_tips() {
		...
		for tips in lines {
			let used = tips.iter().enumerate().fold(0usize, |acc, (idx, tip)| {
				...
				let extra = if idx == 0 { tip_width } else { separator_width.saturating_add(tip_width) };
	...
	fn request_user_input_wrapped_options_snapshot() {
		...
		let options_height = overlay.options_required_height(width);
		let height =
			1u16.saturating_add(question_height).saturating_add(options_height).saturating_add(8);
	...
	fn request_user_input_long_option_text_snapshot() {
		let (tx, _rx) = test_sender();
		let overlay = RequestUserInputOverlay::new(
			request_event("turn-1", vec![question_with_very_long_option_text("q1", "Status")]),
			tx,
			true,
			false,
			false,
		...
		insta::assert_snapshot!("request_user_input_long_option_text", render_snapshot(&overlay, area));
	}
	...
	fn selected_long_wrapped_option_stays_visible() {
		let (tx, _rx) = test_sender();
		let mut overlay = RequestUserInputOverlay::new(
			request_event("turn-1", vec![question_with_long_scroll_options("q1", "Scroll")]),
			tx,
			true,
			false,
			false,
	...
	fn request_user_input_footer_wrap_snapshot() {
		...
		let height = overlay.desired_height(width);
		let area = Rect::new(0, 0, width, height);
		insta::assert_snapshot!("request_user_input_footer_wrap", render_snapshot(&overlay, area));
	}
	...
	fn request_user_input_freeform_snapshot() {
		...
		let area = Rect::new(0, 0, 120, 10);
		insta::assert_snapshot!("request_user_input_freeform", render_snapshot(&overlay, area));
	}
