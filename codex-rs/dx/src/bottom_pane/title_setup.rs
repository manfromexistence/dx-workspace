	...
	T: AsRef<str>,
{
	...
	ids.map(|id| id.as_ref().parse::<TerminalTitleItem>()).collect::<Result<Vec<_>, _>>().ok()
}
...
impl TerminalTitleSetupView {
	...
	pub(crate) fn new(title_items: Option<&[String]>, app_event_tx: AppEventSender) -> Self {
		let selected_items = title_items
			...
			.unique()
			.collect_vec();
		let selected_set = selected_items.iter().copied().collect::<std::collections::HashSet<_>>();
		...
		Self {
			...
			.instructions(vec![
				"Use ↑↓ to navigate, ←→ to move, space to select, enter to confirm, esc to cancel.".into(),
			])
			.items(items)
			.enable_ordering()
			.on_preview(|items| {
				let items = parse_terminal_title_items(
					items.iter().filter(|item| item.enabled).map(|item| item.id.as_str()),
				)?;
...
mod tests {
	...
	fn renders_title_setup_popup() {
		...
		let selected =
			["project".to_string(), "spinner".to_string(), "status".to_string(), "thread".to_string()];
	...
	fn parse_terminal_title_items_preserves_order() {
		let items = parse_terminal_title_items(["project", "spinner", "status", "thread"].into_iter());
		assert_eq!(
			items,
			Some(vec![
