...
impl BottomPane {
	...
	pub(crate) fn set_context_window(&mut self, percent: Option<i64>, used_tokens: Option<i64>) {
		...
		self.composer.set_context_window(percent, self.context_window_used_tokens);
		self.request_redraw();
	}
	...
	pub(crate) fn replace_selection_view_if_active(
		...
		let is_match = self.view_stack.last().is_some_and(|view| view.view_id() == Some(view_id));
		if !is_match {
			return false;
		}
	...
	pub(crate) fn push_mcp_server_elicitation_request(
			...
			&& let Some(install_url) = tool_suggestion.install_url.clone()
		{
			let suggestion_type = match tool_suggestion.suggest_type {
				mcp_server_elicitation::ToolSuggestionType::Install => AppLinkSuggestionType::Install,
				mcp_server_elicitation::ToolSuggestionType::Enable => AppLinkSuggestionType::Enable,
			};
			let is_installed =
				matches!(tool_suggestion.suggest_type, mcp_server_elicitation::ToolSuggestionType::Enable);
	...
	pub(crate) fn is_in_paste_burst(&self) -> bool {
		...
		self.view_stack.last().is_some_and(|view| view.is_in_paste_burst())
	...
	pub(crate) fn on_history_entry_response(
		...
		let updated = self.composer.on_history_entry_response(log_id, offset, entry);
	...
	pub(crate) fn take_recent_submission_images_with_placeholders(
		...
		self.composer.take_recent_submission_images_with_placeholders()
	}
...
mod tests {
	...
	fn overlay_not_shown_above_approval_modal() {
		...
		assert!(!r0.contains("Working"), "overlay should not render above modal");
	}
	...
	fn composer_shown_after_denied_while_task_running() {
		...
		assert!(pane.view_stack.is_empty(), "no active modal view after denial");
		...
		assert!(row0.contains("Working"), "expected Working header after denial on row 0: {row0:?}");
		...
		assert!(found_composer, "expected composer visible under status line");
	}
	...
	fn esc_with_skill_popup_does_not_interrupt_task() {
		...
		pane.insert_str("$");
		assert!(pane.composer.popup_active(), "expected skill popup after typing `$`");
		...
		assert!(!pane.composer.popup_active(), "expected Esc to dismiss skill popup");
	}
	...
	fn esc_with_slash_command_popup_does_not_interrupt_task() {
		...
		pane.insert_str("/");
		assert!(pane.composer.popup_active(), "expected command popup after typing `/`");
	...
	fn esc_release_after_dismissing_agent_picker_does_not_interrupt_task() {
		...
		pane.show_selection_view(SelectionViewParams {
			...
			items: vec![SelectionItem { name: "Main".to_string(), ..Default::default() }],
			..Default::default()
		});
		...
		assert!(pane.no_modal_or_popup_active(), "expected Esc press to dismiss the agent picker");
	}
	...
	fn esc_routes_to_handle_key_event_when_requested() {
		...
		impl BottomPaneView for EscRoutingView {
			fn handle_key_event(&mut self, _key_event: KeyEvent) {
				self.handle_calls.set(self.handle_calls.get().saturating_add(1));
			}

			fn on_ctrl_c(&mut self) -> CancellationEvent {
				self.on_ctrl_c_calls.set(self.on_ctrl_c_calls.get().saturating_add(1));
				CancellationEvent::Handled
			}
	...
	fn release_events_are_ignored_for_active_view() {
		...
		let handle_calls = Rc::new(Cell::new(0));
		pane.push_view(Box::new(CountingView { handle_calls: Rc::clone(&handle_calls) }));
