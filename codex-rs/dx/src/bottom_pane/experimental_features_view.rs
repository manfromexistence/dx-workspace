...
impl ExperimentalFeaturesView {
	pub(crate) fn new(features: Vec<ExperimentalFeatureItem>, app_event_tx: AppEventSender) -> Self {
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Experimental features".bold()));
		header
			.push(Line::from("Toggle experimental features. Changes are saved to config.toml.".dim()));
...
impl BottomPaneView for ExperimentalFeaturesView {
	fn handle_key_event(&mut self, key_event: KeyEvent) {
		match key_event {
			...
			KeyEvent { code: KeyCode::Char('k'), modifiers: KeyModifiers::NONE, .. } => self.move_up(),
			...
			KeyEvent { code: KeyCode::Char('j'), modifiers: KeyModifiers::NONE, .. } => self.move_down(),
	...
	fn on_ctrl_c(&mut self) -> CancellationEvent {
		...
		if !self.features.is_empty() {
			let updates = self.features.iter().map(|item| (item.feature, item.enabled)).collect();
...
impl Renderable for ExperimentalFeaturesView {
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		let header_height = self.header.desired_height(content_area.width.saturating_sub(4));
