...
impl CollaborationModeIndicator {
	fn label(self, show_cycle_hint: bool) -> String {
		let suffix = if show_cycle_hint { format!(" ({MODE_CYCLE_HINT})") } else { String::new() };
		match self {
...
pub(crate) fn toggle_shortcut_mode(
	...
	let base_mode = if is_empty { FooterMode::ComposerEmpty } else { FooterMode::ComposerHasDraft };
...
pub(crate) fn esc_hint_mode(current: FooterMode, is_task_running: bool) -> FooterMode {
	if is_task_running { current } else { FooterMode::EscHint }
}
...
pub(crate) fn single_line_footer_layout(
	...
	if show_queue_hint {
		...
		let mode_only_state = LeftSideState { hint: SummaryHintKind::None, show_cycle_hint: false };
	...
	if let Some(collaboration_mode_indicator) = collaboration_mode_indicator {
		...
		{
			return (
				SummaryLeft::Custom(left_side_line(Some(collaboration_mode_indicator), mode_only_state)),
				true, // show_context
			);
		}
...
fn right_aligned_x(area: Rect, content_width: u16) -> Option<u16> {
	...
	Some(
		area.x.saturating_add(area.width).saturating_sub(content_width).saturating_sub(right_padding),
	)
}
	...
	match props.mode {
		...
		FooterMode::ComposerEmpty => {
			let state = LeftSideState {
				hint: if show_shortcuts_hint { SummaryHintKind::Shortcuts } else { SummaryHintKind::None },
				show_cycle_hint,
			};
...
pub(crate) fn passive_footer_status_line(props: &FooterProps) -> Option<Line<'static>> {
	...
	let mut line = if props.status_line_enabled { props.status_line_value.clone() } else { None };
...
mod tests {
	...
	fn draw_footer_frame<B: Backend>(
		...
		terminal
			.draw(|f| {
				...
				let passive_status_line =
					if status_line_active { passive_footer_status_line(props) } else { None };
				let left_mode_indicator =
					if status_line_active { None } else { collaboration_mode_indicator };
				...
				let right_line = if status_line_active {
					...
					if can_show_left_with_context(area, left_width, full_width) { full } else { compact }
				} else {
					Some(context_window_line(props.context_window_percent, props.context_window_used_tokens))
				};
				let right_width = right_line.as_ref().map(|line| line.width() as u16).unwrap_or(0);
				...
				if matches!(props.mode, FooterMode::ComposerEmpty | FooterMode::ComposerHasDraft) {
					if status_line_active {
	...
	fn footer_status_line_truncates_to_keep_mode_indicator() {
		...
		assert!(collapsed.contains("Plan mode"), "mode indicator should remain visible");
		assert!(
