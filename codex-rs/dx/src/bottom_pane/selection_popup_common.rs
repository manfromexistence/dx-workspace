...
pub(crate) fn wrap_styled_line<'a>(line: &'a Line<'a>, width: u16) -> Vec<Line<'a>> {
	...
	let opts = RtOptions::new(width).initial_indent(Line::from("")).subsequent_indent(Line::from(""));
...
fn wrap_indent(row: &GenericDisplayRow, desc_col: usize, max_width: u16) -> usize {
	...
	let indent = row.wrap_indent.unwrap_or_else(|| {
		if row.description.is_some() || row.disabled_reason.is_some() { desc_col } else { 0 }
	});
...
fn wrap_two_column_row(row: &GenericDisplayRow, desc_col: usize, width: u16) -> Vec<Line<'static>> {
	...
	for idx in 0..rows {
		...
		if let Some(desc) = desc_lines.get(idx) {
			...
			let gap = if left_used == 0 { desc_col } else { desc_col.saturating_sub(left_used).max(2) };
			if gap > 0 {
...
fn wrap_standard_row(row: &GenericDisplayRow, desc_col: usize, width: u16) -> Vec<Line<'static>> {
	...
	word_wrap_line(&full_line, options).into_iter().map(line_to_owned).collect()
}
...
fn adjust_start_for_wrapped_selection_visibility(
	...
	while start_idx < sel {
		let desc_col = compute_desc_col(rows_all, start_idx, desc_measure_items, width, col_width_mode);
...
fn render_rows_inner(
	...
	let desc_col =
		compute_desc_col(rows_all, start_idx, desc_measure_items, area.width, col_width_mode);
	...
	for (i, row) in rows_all.iter().enumerate().skip(start_idx).take(max_items) {
		...
		for line in wrapped {
			...
			line.render(Rect { x: area.x, y: cur_y, width: area.width, height: 1 }, buf);
...
pub(crate) fn render_rows_with_col_width_mode(
	...
	render_rows_inner(area, buf, rows_all, state, max_results, empty_message, col_width_mode)
}
...
pub(crate) fn render_rows_single_line(
	...
	let visible_items = max_results.min(rows_all.len()).min(area.height.max(1) as usize);
	...
	let desc_col =
		compute_desc_col(rows_all, start_idx, visible_items, area.width, ColumnWidthMode::AutoVisible);

	let mut cur_y = area.y;
	let mut rendered_lines: u16 = 0;
	for (i, row) in rows_all.iter().enumerate().skip(start_idx).take(visible_items) {
		if cur_y >= area.y + area.height {
			break;
		}
		...
		full_line.render(Rect { x: area.x, y: cur_y, width: area.width, height: 1 }, buf);
		cur_y = cur_y.saturating_add(1);
		rendered_lines = rendered_lines.saturating_add(1);
	}
...
pub(crate) fn measure_rows_height_stable_col_widths(
	...
	measure_rows_height_inner(rows_all, state, max_results, width, ColumnWidthMode::AutoAllRows)
}
...
fn measure_rows_height_inner(
	...
	let desc_col =
		compute_desc_col(rows_all, start_idx, visible_items, content_width, col_width_mode);
