...
enum PromptSelectionAction {
	...
	Submit { text: String, text_elements: Vec<TextElement> },
}
...
impl Default for ChatComposerConfig {
	fn default() -> Self {
		Self { popups_enabled: true, slash_commands_enabled: true, image_paste_enabled: true }
	}
}
...
impl ChatComposerConfig {
	...
	pub(crate) const fn plain_text() -> Self {
		Self { popups_enabled: false, slash_commands_enabled: false, image_paste_enabled: false }
	}
}
...
impl VoiceState {
	fn new(key_release_supported: bool) -> Self {
		Self { key_release_supported, ..Default::default() }
	}
}
...
impl ChatComposer {
	...
	pub(crate) fn take_mention_bindings(&mut self) -> Vec<MentionBinding> {
		...
		for (id, mention) in elements {
			...
			{
				ordered.push(MentionBinding { mention: binding.mention, path: binding.path });
			}
		}
	...
	fn layout_areas(&self, area: Rect) -> [Rect; 4] {
		...
		let footer_hint_height =
			self.custom_footer_height().unwrap_or_else(|| footer_height(&footer_props));
		let footer_spacing = Self::footer_spacing(footer_hint_height);
		let footer_total_height = footer_hint_height + footer_spacing;
		let popup_constraint = match &self.active_popup {
			ActivePopup::Command(popup) => Constraint::Max(popup.calculate_required_height(area.width)),
			ActivePopup::File(popup) => Constraint::Max(popup.calculate_required_height()),
			ActivePopup::Skill(popup) => Constraint::Max(popup.calculate_required_height(area.width)),
	...
	fn footer_spacing(footer_hint_height: u16) -> u16 {
		if footer_hint_height == 0 { 0 } else { FOOTER_SPACING_HEIGHT }
	}
	...
	pub(crate) fn is_empty(&self) -> bool {
		self.textarea.is_empty() && self.attached_images.is_empty() && self.remote_image_urls.is_empty()
	}
	...
	pub(crate) fn footer_flash_visible(&self) -> bool {
		self.footer_flash.as_ref().is_some_and(|flash| Instant::now() < flash.expires_at)
	}
	...
	pub(crate) fn set_text_content(
		...
		self.set_text_content_with_mention_bindings(text, text_elements, local_image_paths, Vec::new());
	}
	...
	pub(crate) fn clear_for_ctrl_c(&mut self) -> Option<String> {
		...
		let local_image_paths = self.attached_images.iter().map(|img| img.path.clone()).collect();
	...
	pub(crate) fn local_image_paths(&self) -> Vec<PathBuf> {
		self.attached_images.iter().map(|img| img.path.clone()).collect()
	}
	...
	fn prune_attached_images_for_submission(&mut self, text: &str, text_elements: &[TextElement]) {
		...
		let image_placeholders: HashSet<&str> =
			text_elements.iter().filter_map(|elem| elem.placeholder(text)).collect();
		self.attached_images.retain(|img| image_placeholders.contains(img.placeholder.as_str()));
	}
	...
	pub fn take_recent_submission_images_with_placeholders(&mut self) -> Vec<LocalImageAttachment> {
		...
		images
			.into_iter()
			.map(|img| LocalImageAttachment { placeholder: img.placeholder, path: img.path })
			.collect()
	}
	...
	pub fn show_quit_shortcut_hint(&mut self, key: KeyBinding, has_focus: bool) {
		self.quit_shortcut_expires_at =
			Instant::now().checked_add(super::QUIT_SHORTCUT_TIMEOUT).or_else(|| Some(Instant::now()));
	...
	fn next_large_paste_placeholder(&mut self, char_count: usize) -> String {
		...
		*next_suffix += 1;
		if *next_suffix == 1 { base } else { format!("{base} #{next_suffix}") }
	}
		...
		match key_event {
				...
				if let Some(sel) = popup.selected_item() {
					...
					match sel {
						CommandItem::Builtin(cmd) => {
							...
							let starts_with_cmd =
								first_line.trim_start().starts_with(&format!("/{}", cmd.command()));
							if !starts_with_cmd {
			...
			KeyEvent { code: KeyCode::Enter, modifiers: KeyModifiers::NONE, .. } => {
						...
						expand_if_numeric_with_positional_args(prompt, first_line, &text_elements)
				{
					...
					self.textarea.set_text_clearing_elements("");
					return (
						InputResult::Submitted { text: expanded.text, text_elements: expanded.text_elements },
						true,
					);
				}
						...
						CommandItem::UserPrompt(idx) => {
							if let Some(prompt) = popup.prompt(idx) {
								match prompt_selection_action(
									...
									PromptSelectionAction::Submit { text, text_elements } => {
	...
	fn clamp_to_char_boundary(text: &str, pos: usize) -> usize {
		...
		if p < text.len() && !text.is_char_boundary(p) {
			p = text.char_indices().map(|(i, _)| i).take_while(|&i| i <= p).last().unwrap_or(0);
		}
		p
	}
	...
	fn handle_non_ascii_char(&mut self, input: KeyEvent, now: Instant) -> (InputResult, bool) {
		if self.disable_paste_burst {
			...
			self.pending_pastes.retain(|(placeholder, _)| text_after.contains(placeholder));
			return (InputResult::None, true);
		}
		if let KeyEvent { code: KeyCode::Char(ch), .. } = input {
			...
			if let Some(decision) = self.paste_burst.on_plain_char_no_hold(now) {
				match decision {
					...
					CharDecision::BeginBuffer { retro_chars } => {
						...
						if let Some(grab) =
							self.paste_burst.decide_begin_buffer(now, before, retro_chars as usize)
						{
	...
	fn trim_text_elements(
		...
		elements
			...
			.filter_map(|elem| {
				...
				Some(TextElement::new(ByteRange { start: new_start, end: new_end }, placeholder))
			})
			.collect()
	}
	...
	pub(crate) fn expand_pending_pastes(
		...
		for (placeholder, actual) in pending_pastes {
			pending_by_placeholder.entry(placeholder.as_str()).or_default().push_back(actual.as_str());
		}
		...
		for elem in elements {
			...
			if let Some(actual) = replacement {
				...
				rebuilt_elements
					.push(TextElement::new(ByteRange { start: new_start, end: new_end }, placeholder));
			}
			cursor = end;
		}
	...
	fn mentions_enabled(&self) -> bool {
		let skills_ready = self.skills.as_ref().is_some_and(|skills| !skills.is_empty());
		let plugins_ready = self.plugins.as_ref().is_some_and(|plugins| !plugins.is_empty());
		let connectors_ready = self.connectors_enabled
			&& self.connectors_snapshot.as_ref().is_some_and(|snapshot| !snapshot.connectors.is_empty());
	...
	fn current_prefixed_token(
		...
		if safe_cursor < text.len() && !text.is_char_boundary(safe_cursor) {
			...
			safe_cursor =
				text.char_indices().map(|(i, _)| i).take_while(|&i| i <= cursor_offset).last().unwrap_or(0);
		}
		...
		let at_whitespace = if safe_cursor < text.len() {
			text[safe_cursor..].chars().next().map(char::is_whitespace).unwrap_or(false)
		} else {
			false
		};
		...
		let token_left = if start_left < end_left { Some(&text[start_left..end_left]) } else { None };

		// Right candidate: token immediately after any whitespace from the cursor.
		let ws_len_right: usize =
			after_cursor.chars().take_while(|c| c.is_whitespace()).map(char::len_utf8).sum();
		...
		let token_right =
			if start_right < end_right { Some(&text[start_right..end_right]) } else { None };
		...
		if after_cursor.starts_with(prefix) {
			let prefix_starts_token = before_cursor.chars().next_back().is_none_or(char::is_whitespace);
			return if prefix_starts_token { right_prefixed.or(left_prefixed) } else { left_prefixed };
		}
	...
	fn insert_selected_path(&mut self, path: &str) {
		...
		let inserted =
			if needs_quotes && !path.contains('"') { format!("\"{path}\"") } else { path.to_string() };
		...
		self.textarea.replace_range(start_idx..end_idx, &format!("{inserted} "));
	...
	fn insert_selected_mention(&mut self, insert_text: &str, path: Option<&str>) {
		...
		if let (Some(path), Some(mention)) = (path, Self::mention_name_from_insert_text(insert_text)) {
			self.mention_bindings.insert(id, ComposerMentionBinding { mention, path: path.to_string() });
		}

		self.textarea.insert_str(" ");
		let new_cursor = start_idx.saturating_add(insert_text.len()).saturating_add(1);
		self.textarea.set_cursor(new_cursor);
	}
	...
	fn mention_name_from_insert_text(insert_text: &str) -> Option<String> {
		...
		if name.as_bytes().iter().all(|byte| is_mention_name_char(*byte)) {
			Some(name.to_string())
		} else {
			None
		}
	...
	fn snapshot_mention_bindings(&self) -> Vec<MentionBinding> {
		...
		for (id, mention) in self.current_mention_elements() {
			...
			{
				ordered
					.push(MentionBinding { mention: binding.mention.clone(), path: binding.path.clone() });
			}
		}
		ordered
	}
	...
	fn bind_mentions_from_snapshot(&mut self, mention_bindings: Vec<MentionBinding>) {
		...
		for binding in mention_bindings {
			...
			let Some(range) = find_next_mention_token_range(text.as_str(), token.as_str(), scan_from)
			else {
				continue;
			};
			...
			if let Some(id) = id {
				self
					.mention_bindings
					.insert(id, ComposerMentionBinding { mention: binding.mention, path: binding.path });
				scan_from = range.end;
			}
		}
	}
	...
	fn prepare_submission_text(
		...
		let original_local_image_paths =
			self.attached_images.iter().map(|img| img.path.clone()).collect::<Vec<_>>();
			...
			&& let Some((name, _rest, _rest_offset)) = parse_slash_name(&text)
		{
			...
			if !treat_as_plain_text {
				...
				let is_known_prompt = name
					...
					.map(|prompt_name| self.custom_prompts.iter().any(|prompt| prompt.name == prompt_name))
					.unwrap_or(false);
		...
		if self.slash_commands_enabled() {
			let expanded_prompt = match expand_custom_prompt(&text, &text_elements, &self.custom_prompts)
			{
		...
		if actual_chars > MAX_USER_INPUT_TEXT_CHARS {
			...
			self
				.app_event_tx
				.send(AppEvent::InsertHistoryCell(Box::new(history_cell::new_error_event(message))));
			self.set_text_content_with_mention_bindings(
				original_input.clone(),
				original_text_elements,
				original_local_image_paths,
	...
	fn handle_submission_with_time(
		...
		let in_slash_context = self.slash_commands_enabled()
			&& (matches!(self.active_popup, ActivePopup::Command(_))
				|| self.textarea.text().lines().next().unwrap_or("").starts_with('/'));
		if !self.disable_paste_burst
			&& self.paste_burst.is_active()
			&& !in_slash_context
		...
		if !in_slash_context
			...
			&& self.paste_burst.newline_should_insert_instead_of_submit(now)
		{
			self.textarea.insert_str("\n");
		...
		if let Some((text, text_elements)) = self.prepare_submission_text(/*record_history*/ true) {
			if should_queue {
				(InputResult::Queued { text, text_elements }, true)
			} else {
				// Do not clear attached_images here; ChatWidget drains them via take_recent_submission_images().
				(InputResult::Submitted { text, text_elements }, true)
			}
		} else {
	...
	fn try_dispatch_slash_command_with_args(&mut self) -> Option<InputResult> {
		...
		Some(InputResult::CommandWithArgs(cmd, trimmed_rest.to_string(), args_elements))
	}
	...
	pub(crate) fn prepare_inline_args_submission(
		...
		let mut args_elements =
			Self::slash_command_args_elements(prepared_rest, prepared_rest_offset, &prepared_elements);
	...
	fn handle_key_event_without_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
		...
		match key_event {
			...
			| KeyEvent {
				...
				if self.history.should_handle_navigation(self.textarea.text(), self.textarea.cursor()) {
	...
	fn handle_input_basic_with_time(&mut self, input: KeyEvent, now: Instant) -> (InputResult, bool) {
		...
		let crossterm::event::KeyEvent { code, modifiers, .. } = input;
		match code {
			KeyCode::Char(_) => {
				let has_ctrl_or_alt = has_ctrl_or_alt(modifiers);
				if has_ctrl_or_alt {
	...
	fn reconcile_deleted_elements(&mut self, elements_before: Vec<String>) {
		let elements_after: HashSet<String> = self.textarea.element_payloads().into_iter().collect();

		let mut removed_any_image = false;
		for removed in elements_before.into_iter().filter(|payload| !elements_after.contains(payload)) {
			self.pending_pastes.retain(|(ph, _)| ph != &removed);

			if let Some(idx) = self.attached_images.iter().position(|img| img.placeholder == removed) {
	...
	fn handle_shortcut_overlay_key(&mut self, key_event: &KeyEvent) -> bool {
		...
		let next =
			toggle_shortcut_mode(self.footer_mode, self.quit_shortcut_hint_visible(), self.is_empty());
	...
	fn footer_mode(&self) -> FooterMode {
		let base_mode =
			if self.is_empty() { FooterMode::ComposerEmpty } else { FooterMode::ComposerHasDraft };
	...
	fn custom_footer_height(&self) -> Option<u16> {
		...
		self.footer_hint_override.as_ref().map(|items| if items.is_empty() { 0 } else { 1 })
	}
	...
	pub(crate) fn sync_popups(&mut self) {
		...
		let browsing_history =
			self.history.should_handle_navigation(self.textarea.text(), self.textarea.cursor());
		...
		if browsing_history {
			if self.current_file_query.is_some() {
				self.app_event_tx.send(AppEvent::StartFileSearch(String::new()));
		...
		self.dismissed_file_popup_token = None;
		if matches!(self.active_popup, ActivePopup::File(_) | ActivePopup::Skill(_)) {
			self.active_popup = ActivePopup::None;
		}
	}
	...
	fn slash_command_element_range(&self, first_line: &str) -> Option<Range<usize>> {
		...
		if self.is_known_slash_name(name) { Some(0..element_end) } else { None }
	}
	...
	fn is_known_slash_name(&self, name: &str) -> bool {
		...
		{
			return self.custom_prompts.iter().any(|prompt| prompt.name == prompt_name);
		}
		false
	}
	...
	fn looks_like_slash_prefix(&self, name: &str, rest_after_name: &str) -> bool {
		...
		self
			.custom_prompts
			.iter()
			.any(|prompt| fuzzy_match(&format!("{PROMPTS_CMD_PREFIX}:{}", prompt.name), name).is_some())
	}
	...
	fn sync_file_search_popup(&mut self, query: String) {
		...
		if query.is_empty() {
			...
			self.app_event_tx.send(AppEvent::StartFileSearch(query.clone()));
		}
		...
		let plugin_namespaces: HashSet<String> =
			self.plugins.as_ref().map_or_else(HashSet::new, |plugins| {
				plugins
					...
					.filter_map(|plugin| {
						let (plugin_name, _) =
							plugin.config_name.split_once('@').unwrap_or((plugin.config_name.as_str(), ""));
						let plugin_name = plugin_name.trim();
						if plugin_name.is_empty() { None } else { Some(plugin_name.to_ascii_lowercase()) }
					})
					.collect()
			});
		let plugin_display_names: HashSet<String> =
			self.plugins.as_ref().map_or_else(HashSet::new, |plugins| {
				plugins.iter().map(|plugin| plugin.display_name.to_ascii_lowercase()).collect()
			});
...
impl ChatComposer {
		...
		match vc.stop() {
			Ok(audio) => {
				...
				let duration_seconds =
					if samples_per_second > 0.0 { total_samples / samples_per_second } else { 0.0 };
	...
	fn spawn_recording_meter(
		...
		let task = move || {
			...
			loop {
				...
				tx.send(crate::app_event::AppEvent::UpdateRecordingMeter { id: id.clone(), text });
	...
	fn spawn_transcribing_spinner(&mut self, id: String) {
		...
		self.spinner_stop_flags.insert(id.clone(), Arc::clone(&stop));
...
fn find_next_mention_token_range(text: &str, token: &str, from: usize) -> Option<Range<usize>> {
	...
	while index < bytes.len() {
		...
		if bytes.get(end).is_none_or(|byte| !is_mention_name_char(*byte)) {
...
impl Renderable for ChatComposer {
	fn cursor_pos(&self, area: Rect) -> Option<(u16, u16)> {
		// Calculate the actual cursor position from the textarea
		let [_composer_rect, _remote_images_rect, textarea_rect, _popup_rect] = self.layout_areas(area);
		let state = self.textarea_state.borrow();
		self.textarea.cursor_pos_with_state(textarea_rect, *state)
	}
	...
	fn desired_height(&self, width: u16) -> u16 {
		...
		let remote_images_height: u16 =
			self.remote_images_lines(inner_width).len().try_into().unwrap_or(u16::MAX);
		let remote_images_separator = u16::from(remote_images_height > 0);
		self.textarea.desired_height(inner_width)
			+ remote_images_height
			+ remote_images_separator
			+ 2 + match &self.active_popup {
	...
	pub(crate) fn render_with_mask(&self, area: Rect, buf: &mut Buffer, mask_char: Option<char>) {
		...
		match &self.active_popup {
			...
			ActivePopup::None => {
				...
				let footer_hint_height = custom_height.unwrap_or_else(|| footer_height(&footer_props));
				let footer_spacing = Self::footer_spacing(footer_hint_height);
				...
				let left_mode_indicator =
					if status_line_active { None } else { self.collaboration_mode_indicator };
				let mut left_width = if self.footer_flash_visible() {
					self.footer_flash.as_ref().map(|flash| flash.line.width() as u16).unwrap_or(0)
				} else if let Some(items) = self.footer_hint_override.as_ref() {
					footer_hint_items_width(items)
				} else if status_line_active {
					truncated_status_line.as_ref().map(|line| line.width() as u16).unwrap_or(0)
				} else {
					footer_line_width(
						&footer_props,
						left_mode_indicator,
				...
				let right_line = if status_line_active {
					let full = mode_indicator_line(self.collaboration_mode_indicator, show_cycle_hint);
					let compact =
						mode_indicator_line(self.collaboration_mode_indicator, /*show_cycle_hint*/ false);
					let full_width = full.as_ref().map(|l| l.width() as u16).unwrap_or(0);
					if can_show_left_with_context(hint_rect, left_width, full_width) { full } else { compact }
				} else {
				...
				if status_line_active
					...
					&& let Some(line) = combined_status_line
						.as_ref()
						.map(|line| truncate_line_with_ellipsis_if_overflow(line.clone(), max_left as usize))
				{
				...
				let has_override = self.footer_flash_visible() || self.footer_hint_override.is_some();
				...
				let show_right = if matches!(
					footer_props.mode,
					FooterMode::EscHint | FooterMode::QuitShortcutReminder | FooterMode::ShortcutOverlay
				) {
					false
				} else {
					single_line_layout
		...
		Block::default().borders(Borders::ALL).style(style).render_ref(composer_rect, buf);
		if !remote_images_rect.is_empty() {
			Paragraph::new(self.remote_images_lines(remote_images_rect.width))
				.style(style)
				.render_ref(remote_images_rect, buf);
		}
		// Removed the › prompt symbol

		let mut state = self.textarea_state.borrow_mut();
		if let Some(mask_char) = mask_char {
			self.textarea.render_ref_masked(textarea_rect, buf, &mut state, mask_char);
		} else {
		...
		if self.textarea.text().is_empty() {
			let text = if self.input_enabled {
				...
				self.input_disabled_placeholder.as_deref().unwrap_or("Input disabled.").to_string()
			};
	...
	match mode {
		PromptSelectionMode::Completion => {
			if !named_args.is_empty() {
				...
				return PromptSelectionAction::Insert { text, cursor: Some(cursor) };
			}
			if has_numeric {
...
mod tests {
	...
	fn footer_hint_row_is_separated_from_composer() {
		...
		let composer =
			ChatComposer::new(true, sender, false, "Ask Codex to do anything".to_string(), false);
		...
		assert!(hint_row_idx > 0, "expected a spacing row above the footer hints",);
	...
	fn footer_flash_overrides_footer_hint_override() {
		...
		for x in 0..area.width {
			bottom_row.push(buf[(x, area.height - 1)].symbol().chars().next().unwrap_or(' '));
		}
		assert!(
	...
	fn footer_mode_snapshots() {
		...
		snapshot_composer_state("footer_mode_overlay_then_external_esc_hint", true, |composer| {
			let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Char('?'), KeyModifiers::NONE));
			composer.set_esc_backtrack_hint(true);
		});
	...
	fn esc_hint_stays_hidden_with_draft_content() {
		...
		let sender = AppEventSender::new(tx);
		let mut composer =
			ChatComposer::new(true, sender, true, "Ask Codex to do anything".to_string(), false);
	...
	fn clear_for_ctrl_c_preserves_pending_paste_history_entry() {
		...
		assert_eq!(composer.pending_pastes, vec![(placeholder.clone(), large.clone())]);

		composer.clear_for_ctrl_c();
		assert!(composer.is_empty());

		let history_entry =
			composer.history.navigate_up(&composer.app_event_tx).expect("expected history entry");
		let text_elements =
			vec![TextElement::new((0..placeholder.len()).into(), Some(placeholder.clone()))];
		assert_eq!(
			history_entry,
	...
	fn clear_for_ctrl_c_preserves_remote_offset_image_labels() {
		...
		let text_elements =
			vec![TextElement::new((0.."[Image #2]".len()).into(), Some("[Image #2]".to_string()))];
		...
		assert_eq!(expected_elements[0].placeholder(&expected_text), Some("[Image #2]"));
	...
	fn apply_history_entry_preserves_local_placeholders_after_remote_prefix() {
		...
		assert_eq!(restored_elements[0].placeholder(&restored_text), Some("[Image #2]"));
	...
	fn question_mark_does_not_toggle_during_paste_burst() {
		...
		composer.paste_burst.begin_with_retro_grabbed(String::new(), Instant::now());
	...
	fn set_connector_mentions_refreshes_open_mention_popup() {
		...
		let mention = popup.selected_mention().expect("expected connector mention to be selected");
	...
	fn set_plugin_mentions_refreshes_open_mention_popup() {
		...
		let mention = popup.selected_mention().expect("expected plugin mention to be selected");
	...
	fn mention_items_hide_plugin_owned_skill_and_app_duplicates() {
		...
		composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
			...
			description: Some(
				"Connect Google Calendar for scheduling, availability, and event management.".to_string(),
			),
			has_skills: true,
			mcp_server_names: vec!["google-calendar".to_string()],
			app_connector_ids: vec![codex_core::plugins::AppConnectorId("google_calendar".to_string())],
		}]));
		...
		assert_eq!(mentions[0].path, Some("plugin://google-calendar@debug".to_string()));
	}
	...
	fn plugin_mention_popup_snapshot() {
		snapshot_composer_state("plugin_mention_popup", false, |composer| {
			...
			composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
				...
				display_name: "Sample Plugin".to_string(),
				description: Some(
					"Plugin that includes the Figma MCP server and Skills for common workflows".to_string(),
				),
				has_skills: true,
				mcp_server_names: vec!["sample".to_string()],
	...
	fn test_current_at_token_basic_cases() {
		let test_cases = vec![
			...
			("@", 1, Some("".to_string()), "Only @ symbol triggers empty query"),
	...
	fn test_current_at_token_whitespace_boundaries() {
		let test_cases = vec![
			...
			("test　@İstanbul", 8, Some("İstanbul".to_string()), "@ token after full-width space"),
			("@ЙЦУ　@诶", 10, Some("诶".to_string()), "Full-width space between Unicode tokens"),
	...
	fn test_current_at_token_ignores_mid_word_at() {
		...
		let test_cases =
			vec![(at_pos, "Cursor at mid-word @"), (input.len(), "Cursor at end of word containing @")];
...
程序员 与 Unicode 同行";
	...
	fn ascii_burst_treats_enter_as_newline() {
		...
		for ch in ['t', 'h', 'e', 'r', 'e'] {
			now += step;
			let _ = composer
				.handle_input_basic_with_time(KeyEvent::new(KeyCode::Char(ch), KeyModifiers::NONE), now);
		}
	...
	fn slash_command_disabled_while_task_running_keeps_text() {
		...
		let mut composer =
			ChatComposer::new(true, sender, false, "Ask Codex to do anything".to_string(), false);
		composer.set_task_running(true);
		composer.textarea.set_text_clearing_elements("/review these changes");
	...
	fn voice_transcription_disabled_treats_space_as_normal_input() {
		...
		let sender = AppEventSender::new(tx);
		let mut composer =
			ChatComposer::new(true, sender, false, "Ask Codex to do anything".to_string(), true);
	...
	fn replace_transcription_stops_spinner_for_placeholder() {
		...
		let flag = Arc::new(AtomicBool::new(false));
		composer.spinner_stop_flags.insert(id.clone(), Arc::clone(&flag));
	...
	fn set_text_content_stops_all_transcription_spinners() {
		...
		composer.spinner_stop_flags.insert("voice-1".to_string(), Arc::clone(&flag_one));
		composer.spinner_stop_flags.insert("voice-2".to_string(), Arc::clone(&flag_two));
	...
	fn extract_args_supports_quoted_paths_single_arg() {
		...
		assert_eq!(
			args,
			vec![PromptArg { text: "docs/My File.md".to_string(), text_elements: Vec::new() }]
		);
	}
	...
	fn extract_args_supports_mixed_quoted_and_unquoted() {
		...
		assert_eq!(
			args,
			vec![
				PromptArg { text: "with spaces".to_string(), text_elements: Vec::new() },
				PromptArg { text: "simple".to_string(), text_elements: Vec::new() }
			]
		);
	}
	...
	fn slash_plan_args_preserve_text_elements() {
		...
		match result {
			InputResult::CommandWithArgs(cmd, args, text_elements) => {
				...
				assert_eq!(text_elements[0].placeholder(&args), Some(placeholder.as_str()));
			}
	...
	fn test_multiple_pastes_submission() {
		...
		assert_eq!(
			states,
			vec![
				(format!("[Pasted Content {} chars]", test_cases[0].0.chars().count()), 1),
				(format!("[Pasted Content {} chars] and ", test_cases[0].0.chars().count()), 1),
				(
					format!(
	...
	fn test_placeholder_deletion() {
		...
		let states: Vec<_> = test_cases
			...
			.map(|(content, is_large)| {
				...
				(composer.textarea.text().to_string(), composer.pending_pastes.len(), current_pos)
			})
			.collect();
		...
		composer.handle_key_event(KeyEvent::new(KeyCode::Backspace, KeyModifiers::NONE));
		deletion_states.push((composer.textarea.text().to_string(), composer.pending_pastes.len()));
		...
		assert_eq!(
			deletion_states,
			vec![(" and [Pasted Content 1006 chars]".to_string(), 1), (" and ".to_string(), 0),]
		);
	}
	...
	fn deleting_duplicate_length_pastes_removes_only_target() {
		...
		composer.handle_paste(paste.clone());
		assert_eq!(composer.textarea.text(), format!("{placeholder_base}{placeholder_second}"));
	...
	fn test_partial_placeholder_deletion() {
		...
		let states: Vec<_> = test_cases
			...
			.map(|pos_from_end| {
				composer.handle_paste(paste.clone());
				composer.textarea.set_cursor(placeholder.len() - pos_from_end);
				composer.handle_key_event(KeyEvent::new(KeyCode::Backspace, KeyModifiers::NONE));
				let result =
					(composer.textarea.text().contains(&placeholder), composer.pending_pastes.len());
	...
	fn attach_image_and_submit_includes_local_image_paths() {
		...
		match result {
			InputResult::Submitted { text, text_elements } => {
				...
				assert_eq!(text_elements[0].byte_range, ByteRange { start: 0, end: "[Image #1]".len() });
			}
			_ => panic!("expected Submitted"),
		}
	...
	fn submit_captures_recent_mention_bindings_before_clearing_textarea() {
		...
		let (result, _) = composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
		assert!(matches!(result, InputResult::Submitted { .. }));
		assert_eq!(composer.take_recent_submission_mention_bindings(), mention_bindings);
	...
	fn history_navigation_restores_remote_only_submissions() {
		...
		let remote_image_urls =
			vec!["https://example.com/one.png".to_string(), "https://example.com/two.png".to_string()];
		composer.set_remote_image_urls(remote_image_urls.clone());

		let (submitted_text, submitted_elements) =
			composer.prepare_submission_text(true).expect("remote-only submission should be prepared");
	...
	fn large_paste_with_leading_whitespace_trims_and_shifts_elements() {
		...
		match result {
			InputResult::Submitted { text, text_elements } => {
				...
				assert_eq!(
					text_elements[0].byte_range,
					ByteRange { start: trimmed.len() + 1, end: trimmed.len() + 1 + "[Image #1]".len() }
				);
			}
			_ => panic!("expected Submitted"),
		}
	...
	fn suppressed_submission_restores_pending_paste_payload() {
		...
		let large_content = "x".repeat(LARGE_PASTE_CHAR_THRESHOLD + 5);
		composer.handle_paste(large_content.clone());
		let placeholder = composer.pending_pastes.first().expect("expected pending paste").0.clone();
	...
	fn deleting_one_of_duplicate_image_placeholders_removes_one_entry() {
		...
		assert_eq!(0, new_text.matches(&placeholder2).count(), "second placeholder was relabeled");
		assert_eq!(1, new_text.matches("[Image #1]").count(), "remaining placeholder relabeled to #1");
		assert_eq!(
			vec![AttachedImage { path: path2, placeholder: "[Image #1]".to_string() }],
	...
	fn deleting_reordered_image_one_renumbers_text_in_place() {
		...
		let text_elements = vec![
			TextElement::new(
				ByteRange { start: start2, end: start2 + placeholder2.len() },
				Some(placeholder2),
			),
			TextElement::new(
				ByteRange { start: start1, end: start1 + placeholder1.len() },
		...
		assert_eq!(composer.textarea.text(), format!("Test {placeholder1} test "));
		assert_eq!(
			vec![AttachedImage { path: path2, placeholder: placeholder1 }],
			composer.attached_images,
	...
	fn custom_prompt_submission_expands_arguments() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt USER=Alice BRANCH=main");
	...
	fn custom_prompt_submission_preserves_image_placeholder_unquoted() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt IMG=");
		composer.textarea.set_cursor(composer.textarea.text().len());
		...
		match result {
			InputResult::Submitted { text, text_elements } => {
				...
				assert_eq!(
					text_elements,
					vec![TextElement::new(
						ByteRange { start: "Review ".len(), end: "Review ".len() + placeholder.len() },
						Some(placeholder),
					)]
				);
			}
	...
	fn slash_path_input_submits_without_command_error() {
		...
		composer.textarea.set_text_clearing_elements("/Users/example/project/src/main.rs");
	...
	fn slash_with_leading_space_submits_as_text() {
		...
		composer.textarea.set_text_clearing_elements(" /this-looks-like-a-command");
	...
	fn custom_prompt_invalid_args_reports_error() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt USER=Alice stray");
	...
	fn custom_prompt_missing_required_args_reports_error() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt USER=Alice");
		...
		assert!(found_error, "expected missing args error history cell to be sent");
	}
	...
	fn popup_prompt_submission_prunes_unused_image_attachments() {
		...
		assert!(composer.take_recent_submission_images_with_placeholders().is_empty());
	}
	...
	fn numeric_prompt_auto_submit_expands_pending_pastes() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt ");
		composer.textarea.set_cursor(composer.textarea.text().len());
		let large_content = "x".repeat(LARGE_PASTE_CHAR_THRESHOLD + 5);
		composer.handle_paste(large_content.clone());
	...
	fn queued_prompt_submission_prunes_unused_image_attachments() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:my-prompt foo ");
		composer.textarea.set_cursor(composer.textarea.text().len());
		composer.attach_image(PathBuf::from("/tmp/unused.png"));
		composer.set_task_running(true);
	...
	fn prompt_expansion_over_character_limit_reports_error_and_restores_draft() {
		...
		composer.textarea.set_text_clearing_elements(&original_input);
	...
	fn numeric_prompt_positional_args_does_not_error() {
		...
		composer.textarea.set_text_clearing_elements("/prompts:elegant hi");
		let (result, _needs_redraw) =
			composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	...
	fn selecting_custom_prompt_with_no_args_inserts_template() {
		...
		type_chars_humanlike(&mut composer, &['/', 'p', 'r', 'o', 'm', 'p', 't', 's', ':', 'p']);
		let (result, _needs_redraw) =
			composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
	...
	fn burst_paste_fast_small_buffers_and_flushes_on_stop() {
		...
		for _ in 0..count {
			let _ = composer
				.handle_input_basic_with_time(KeyEvent::new(KeyCode::Char('a'), KeyModifiers::NONE), now);
	...
	fn burst_paste_fast_large_inserts_placeholder_on_flush() {
		...
		for _ in 0..count {
			let _ = composer
				.handle_input_basic_with_time(KeyEvent::new(KeyCode::Char('x'), KeyModifiers::NONE), now);
			now += step;
		}
	...
	fn apply_external_edit_rebuilds_text_and_attachments() {
		...
		composer
			.attached_images
			.push(AttachedImage { placeholder: placeholder.clone(), path: PathBuf::from("img.png") });
		composer.pending_pastes.push(("[Pasted]".to_string(), "data".to_string()));

		composer.apply_external_edit(format!("Edited {placeholder} text"));

		assert_eq!(composer.current_text(), format!("Edited {placeholder} text"));
		assert!(composer.pending_pastes.is_empty());
		assert_eq!(composer.attached_images.len(), 1);
	...
	fn current_text_with_pending_expands_placeholders() {
		...
		composer.textarea.insert_element(&placeholder);
		composer.pending_pastes.push((placeholder.clone(), "hello".to_string()));
	...
	fn apply_external_edit_limits_duplicates_to_occurrences() {
		...
		assert_eq!(composer.current_text(), format!("{placeholder} extra {placeholder}"));
		assert_eq!(composer.attached_images.len(), 1);
	}
	...
	fn prepare_submission_keeps_remote_offset_local_placeholder_numbering() {
		...
		let base_elements =
			vec![TextElement::new((0.."[Image #2]".len()).into(), Some("[Image #2]".to_string()))];
		composer.set_text_content(base_text, base_elements, vec![PathBuf::from("/tmp/local.png")]);

		let (submitted_text, submitted_elements) =
			composer.prepare_submission_text(true).expect("remote+local submission should be generated");
	...
	fn prepare_submission_with_only_remote_images_returns_empty_text() {
		...
		composer.set_remote_image_urls(vec!["https://example.com/one.png".to_string()]);
		let (submitted_text, submitted_elements) =
			composer.prepare_submission_text(true).expect("remote-only submission should be generated");
		assert_eq!(submitted_text, "");
		assert!(submitted_elements.is_empty());
	}
	...
	fn delete_selected_remote_image_relabels_local_placeholders() {
		...
		let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Up, KeyModifiers::NONE));
		let _ = composer.handle_key_event(KeyEvent::new(KeyCode::Delete, KeyModifiers::NONE));
		assert_eq!(composer.remote_image_urls(), vec!["https://example.com/one.png".to_string()]);
