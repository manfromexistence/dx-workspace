...
pub fn parse_prompt_inputs(
	...
	for token in parse_tokens_with_elements(rest, text_elements) {
		...
		map.insert(
			key.to_string(),
			PromptArg { text: value.to_string(), text_elements: value_elements },
		);
	}
	Ok(map)
}
...
pub fn expand_custom_prompt(
	...
	if !required.is_empty() {
		let inputs = parse_prompt_inputs(rest, &local_elements)
			.map_err(|error| PromptExpansionError::Args { command: format!("/{name}"), error })?;
		let missing: Vec<String> = required.into_iter().filter(|k| !inputs.contains_key(k)).collect();
		if !missing.is_empty() {
			return Err(PromptExpansionError::MissingArgs { command: format!("/{name}"), missing });
		}
...
fn apply_replacements_to_token(token: String, replacements: &[ElementReplacement]) -> PromptArg {
	if replacements.is_empty() {
		return PromptArg { text: token, text_elements: Vec::new() };
	}
	...
	while cursor < token.len() {
		...
		if start < end {
			out_elements
				.push(TextElement::new(ByteRange { start, end }, replacement.placeholder.clone()));
		}
		cursor = start_in_token + replacement.sentinel.len();
	}

	PromptArg { text: out, text_elements: out_elements }
}
...
fn append_arg_with_elements(
	...
	out_elements.extend(arg.text_elements.iter().map(|elem| {
		elem.map_range(|range| ByteRange { start: start + range.start, end: start + range.end })
	}));
}
...
mod tests {
	...
	fn escaped_placeholder_is_ignored() {
		...
		assert_eq!(prompt_argument_names("literal $$USER and $REAL"), vec!["REAL".to_string()]);
	}
	...
	fn escaped_placeholder_remains_literal() {
		...
		assert_eq!(
			out,
			Some(PromptExpansion { text: "literal $$USER".to_string(), text_elements: Vec::new() })
		);
	}
	...
	fn positional_args_treat_placeholder_with_spaces_as_single_token() {
		...
		let text_elements =
			vec![TextElement::new(ByteRange { start, end }, Some(placeholder.to_string()))];

		let args = parse_positional_args(&rest, &text_elements);
		assert_eq!(
			args,
			vec![
				PromptArg { text: "alpha".to_string(), text_elements: Vec::new() },
				PromptArg {
					text: placeholder.to_string(),
					text_elements: vec![TextElement::new(
						ByteRange { start: 0, end: placeholder.len() },
						Some(placeholder.to_string()),
					)],
				},
				PromptArg { text: "beta".to_string(), text_elements: Vec::new() }
			]
		);
	}
	...
	fn key_value_args_treat_placeholder_with_spaces_as_single_token() {
		...
		assert_eq!(
			args.get("NOTE"),
			Some(&PromptArg { text: "hello".to_string(), text_elements: Vec::new() })
		);
	}
		...
		assert_eq!(
			...
			vec![
				...
				PromptArg {
					...
					text_elements: vec![TextElement::new(
						ByteRange { start: "see ".len(), end: "see ".len() + placeholder.len() },
						Some(placeholder.to_string()),
					)],
				},
				PromptArg { text: "beta".to_string(), text_elements: Vec::new() }
	...
	fn key_value_args_allow_placeholder_inside_quotes() {
		...
		assert_eq!(
			args.get("NOTE"),
			Some(&PromptArg { text: "ok".to_string(), text_elements: Vec::new() })
		);
	}
}
