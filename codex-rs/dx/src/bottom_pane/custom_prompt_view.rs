...
impl Renderable for CustomPromptView {
	...
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		if let Some(context_label) = &self.context_label {
			let context_area = Rect { x: area.x, y: input_y, width: area.width, height: 1 };
		...
		// Input line
		let input_area = Rect { x: area.x, y: input_y, width: area.width, height: input_height };
		if input_area.width >= 2 {
			...
			if text_area_height > 0 {
				...
				if self.textarea.text().is_empty() {
					Paragraph::new(Line::from(self.placeholder.clone().dim())).render(textarea_rect, buf);
				}
			}
		}
