...
impl StatusLineItem {
	...
	pub(crate) fn description(&self) -> &'static str {
		match self {
			...
			StatusLineItem::ContextUsed => "Percentage of context window used (omitted when unknown)",
			...
			StatusLineItem::SessionId => "Current session identifier (omitted until session starts)",
...
mod tests {
	...
	fn preview_uses_runtime_values() {
		...
		assert_eq!(preview_data.line_for_items(&items), Some(Line::from("gpt-5 · /repo")));
	}
	...
	fn preview_omits_items_without_runtime_values() {
		...
		assert_eq!(preview_data.line_for_items(&items), Some(Line::from("gpt-5")));
	}
