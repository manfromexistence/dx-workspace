...
impl AppLinkView {
	...
	fn toggle_enabled(&mut self) {
		...
		self
			.app_event_tx
			.send(AppEvent::SetAppEnabled { id: self.app_id.clone(), enabled: self.is_enabled });
		if self.is_tool_suggestion() {
			self.resolve_elicitation(ElicitationAction::Accept);
			self.complete = true;
		}
	...
	fn link_content_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		if let Some(description) =
			self.description.as_deref().map(str::trim).filter(|description| !description.is_empty())
		{
		...
		if !instructions.is_empty() {
			...
			for line in
				wrap("Newly installed apps can take a few minutes to appear in /apps.", usable_width)
			{
				lines.push(Line::from(line.into_owned()));
			}
			if !self.is_installed {
				for line in wrap("After installed, use $ to insert this app into the prompt.", usable_width)
				{
					lines.push(Line::from(line.into_owned()));
				}
			}
	...
	fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		for line in
			wrap("Complete app setup on ChatGPT in the browser window that just opened.", usable_width)
		{
			lines.push(Line::from(line.into_owned()));
		}
		for line in wrap(
	...
	fn action_rows(&self) -> Vec<GenericDisplayRow> {
		self
			...
			.enumerate()
			.map(|(index, label)| {
				let prefix = if self.selected_action == index { '›' } else { ' ' };
				GenericDisplayRow { name: format!("{prefix} {}. {label}", index + 1), ..Default::default() }
			})
			.collect()
	}
	...
	fn handle_key_event(&mut self, key_event: KeyEvent) {
		match key_event {
			...
			KeyEvent { code: KeyCode::Char(c), modifiers: KeyModifiers::NONE, .. } => {
				if let Some(index) =
					c.to_digit(10).and_then(|digit| digit.checked_sub(1)).map(|index| index as usize)
...
impl crate::render::renderable::Renderable for AppLinkView {
	fn desired_height(&self, width: u16) -> u16 {
		...
		let content_rows =
			Paragraph::new(content_lines).wrap(Wrap { trim: false }).line_count(content_width).max(1)
				as u16;
...
mod tests {
	...
	fn installed_app_has_toggle_action() {
		...
		assert_eq!(view.action_labels(), vec!["Manage on ChatGPT", "Disable app", "Back"]);
	}
	...
	fn toggle_action_sends_set_app_enabled_and_updates_label() {
		...
		assert_eq!(view.action_labels(), vec!["Manage on ChatGPT", "Enable app", "Back"]);
	}
