	...
	pub(crate) fn new(text: String) -> Self {
		...
		Self {
			...
			mention_bindings: decoded
				.mentions
				.into_iter()
				.map(|mention| MentionBinding { mention: mention.mention, path: mention.path })
				.collect(),
...
impl ChatComposerHistory {
	...
	fn populate_history_at_index(
		...
		if global_idx >= self.history_entry_count {
			// Local entry.
			if let Some(entry) = self.local_history.get(global_idx - self.history_entry_count).cloned() {
				self.last_history_text = Some(entry.text.clone());
				return Some(entry);
			}
			...
			let op = Op::GetHistoryEntryRequest { offset: global_idx, log_id };
...
mod tests {
	...
	fn duplicate_submissions_are_not_recorded() {
		...
		assert_eq!(history.local_history.last().unwrap(), &HistoryEntry::new("hello".to_string()));
		...
		assert_eq!(history.local_history.last().unwrap(), &HistoryEntry::new("world".to_string()));
	}
	...
	fn navigation_with_async_fetch() {
		...
		assert_eq!(Op::GetHistoryEntryRequest { log_id: 1, offset: 2 }, history_request1);
		...
		assert_eq!(Op::GetHistoryEntryRequest { log_id: 1, offset: 1 }, history_request_2);
	...
	fn reset_navigation_resets_cursor() {
		...
		let mut history = ChatComposerHistory::new();
		history.set_metadata(1, 3);
		history.fetched_history.insert(1, HistoryEntry::new("command2".to_string()));
		history.fetched_history.insert(2, HistoryEntry::new("command3".to_string()));

		assert_eq!(Some(HistoryEntry::new("command3".to_string())), history.navigate_up(&tx));
		assert_eq!(Some(HistoryEntry::new("command2".to_string())), history.navigate_up(&tx));
