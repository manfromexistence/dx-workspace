...
impl Menu {
	...
	fn enter_submenu_with_mode(&mut self, index: usize, direct: bool) {
		...
		// Special handling for Keyboard Shortcuts submenu (index 1)
			...
			// Don't add extra "Back" - the submenu data already includes it
			let submenu_items: Vec<(String, String)> =
				self.submenus[index].iter().map(|(a, b)| (a.to_string(), b.to_string())).collect();
	...
	/// Enter models list submenu
	pub fn enter_models_submenu(&mut self) {
		use super::submenus::providers;

		let submenu_items: Vec<(String, String)> =
			providers::get_models_submenu().iter().map(|(a, b)| (a.to_string(), b.to_string())).collect();
	...
	pub fn select_current_item(&mut self) -> bool {
		// Check if current item is a submenu marker
		if self.selected_menu_item < self.menu_items.len() {
			let item = &self.menu_items[self.selected_menu_item];
			if item.1 == "SUBMENU:models-list" {
				self.enter_models_submenu();
				return true;
			}
		}
	...
	pub fn get_selected_model_id(&self) -> Option<String> {
		if self.selected_menu_item < self.menu_items.len() {
			let item = &self.menu_items[self.selected_menu_item];
			if item.1.starts_with("MODEL:") {
				// Format is "MODEL:id|Provider", extract just the id
				let model_part = item.1.strip_prefix("MODEL:").unwrap();
				let model_id = model_part.split('|').next().unwrap();
