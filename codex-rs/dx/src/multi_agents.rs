...
pub(crate) fn format_agent_picker_item_name(
	...
	let agent_nickname = agent_nickname.map(str::trim).filter(|nickname| !nickname.is_empty());
...
pub(crate) fn waiting_begin(ev: CollabWaitingBeginEvent) -> PlainHistoryCell {
	...
	let title = match receiver_agents.as_slice() {
		[receiver] => {
			title_with_agent("Waiting for", agent_label_from_ref(receiver), /*spawn_request*/ None)
		}
...
pub(crate) fn waiting_end(ev: CollabWaitingEndEvent) -> PlainHistoryCell {
	let CollabWaitingEndEvent { call_id: _, sender_thread_id: _, agent_statuses, statuses } = ev;
...
fn agent_label_spans(agent: AgentLabel<'_>) -> Vec<Span<'static>> {
	...
	let nickname = agent.nickname.map(str::trim).filter(|nickname| !nickname.is_empty());
...
fn prompt_line(prompt: &str) -> Option<Line<'static>> {
	...
	if trimmed.is_empty() {
		...
		Some(Line::from(Span::from(truncate_text(trimmed, COLLAB_PROMPT_PREVIEW_GRAPHEMES))))
	}
}
...
fn merge_wait_receivers(
	...
	let mut seen = receiver_agents.iter().map(|agent| agent.thread_id).collect::<HashSet<_>>();
...
fn wait_complete_lines(
	...
	let entries = if agent_statuses.is_empty() {
		...
		let seen = entries.iter().map(|entry| entry.thread_id).collect::<HashSet<_>>();
	...
	entries
		.into_iter()
		.map(|entry| {
			let CollabAgentStatusEntry { thread_id, agent_nickname, agent_role, status } = entry;
...
mod tests {
	...
	fn collab_events_snapshot() {
		...
		let robie_id =
			ThreadId::from_string("00000000-0000-0000-0000-000000000002").expect("valid robie thread id");
		let bob_id =
		...
		statuses.insert(robie_id, AgentStatus::Completed(Some("39916800".to_string())));
	...
	fn agent_shortcut_matches_option_arrow_word_motion_fallbacks_only_when_allowed() {
		...
		assert!(next_agent_shortcut_matches(KeyEvent::new(KeyCode::Right, KeyModifiers::ALT), false,));
		assert!(previous_agent_shortcut_matches(
	...
	fn cell_to_text(cell: &PlainHistoryCell) -> String {
		cell.display_lines(200).iter().map(line_to_text).collect::<Vec<_>>().join("\n")
	}
