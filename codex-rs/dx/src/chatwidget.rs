...
impl UnifiedExecWaitStreak {
	fn new(process_id: String, command_display: Option<String>) -> Self {
		Self { process_id, command_display: command_display.filter(|display| !display.is_empty()) }
	}
...
fn is_standard_tool_call(parsed_cmd: &[ParsedCommand]) -> bool {
	!parsed_cmd.is_empty()
		&& parsed_cmd.iter().all(|parsed| !matches!(parsed, ParsedCommand::Unknown { .. }))
}
...
impl RateLimitWarningState {
	fn take_warnings(
		...
		let reached_secondary_cap = matches!(secondary_used_percent, Some(percent) if percent == 100.0);
		...
		if let Some(secondary_used_percent) = secondary_used_percent {
			...
			if let Some(threshold) = highest_secondary {
				let limit_label =
					secondary_window_minutes.map(get_limits_duration).unwrap_or_else(|| "weekly".to_string());
		...
		if let Some(primary_used_percent) = primary_used_percent {
			...
			if let Some(threshold) = highest_primary {
				let limit_label =
					primary_window_minutes.map(get_limits_duration).unwrap_or_else(|| "5h".to_string());
				let remaining_percent = 100.0 - threshold;
				warnings.push(format!(
                    "Heads up, you have less than {remaining_percent:.0}% of your {limit_label} limit left. Run /status for a breakdown."
                ));
...
fn rate_limit_error_kind(info: &CodexErrorInfo) -> Option<RateLimitErrorKind> {
	match info {
		...
		CodexErrorInfo::ResponseTooManyFailedAttempts { http_status_code: Some(429) } => {
...
impl PendingGuardianReviewStatus {
	...
	fn status_indicator_state(&self) -> Option<StatusIndicatorState> {
		let details = if self.entries.len() == 1 {
			...
			let mut lines =
				self.entries.iter().take(3).map(|entry| format!("• {}", entry.detail)).collect::<Vec<_>>();
		...
		Some(StatusIndicatorState { header, details: Some(details), details_max_lines })
	}
}
...
pub struct ChatWidget {
	...
	// Scrollbar support for message history
...
fn remap_placeholders_for_message(message: UserMessage, next_label: &mut usize) -> UserMessage {
	let UserMessage { text, text_elements, local_images, remote_image_urls, mention_bindings } =
		message;
	if local_images.is_empty() {
		return UserMessage { text, text_elements, local_images, remote_image_urls, mention_bindings };
	}
	...
	for attachment in local_images {
		...
		*next_label += 1;
		...
		remapped_images
			.push(LocalImageAttachment { placeholder: new_placeholder, path: attachment.path });
	}
	...
	for mut elem in elements {
		...
		let replacement =
			placeholder.and_then(|ph| mapping.get(ph)).map(String::as_str).unwrap_or(original);
...
fn merge_user_messages(messages: Vec<UserMessage>) -> UserMessage {
	...
	let total_remote_images =
		messages.iter().map(|message| message.remote_image_urls.len()).sum::<usize>();
...
impl ChatWidget {
	...
	fn update_task_running_state(&mut self) {
		self.bottom_pane.set_task_running(self.agent_turn_running || self.mcp_startup_status.is_some());
	...
	fn stream_controllers_idle(&self) -> bool {
		self.stream_controller.as_ref().map(|controller| controller.queued_lines() == 0).unwrap_or(true)
			&& self
	...
	fn set_status(
		...
		let details = details.filter(|details| !details.is_empty()).map(|details| {
		...
		self.current_status =
			StatusIndicatorState { header: header.clone(), details: details.clone(), details_max_lines };
		...
		if title_uses_status
			|| (title_uses_spinner && self.terminal_title_status_kind == TerminalTitleStatusKind::Undoing)
		{
			self.refresh_terminal_title();
		}
	}
	...
	fn on_session_configured(&mut self, event: codex_protocol::protocol::SessionConfiguredEvent) {
		self.bottom_pane.set_history_metadata(event.history_log_id, event.history_entry_count);
		...
		if let Err(err) = self.config.permissions.approval_policy.set(event.approval_policy) {
			tracing::warn!(%err, "failed to sync approval_policy from SessionConfigured");
			self.config.permissions.approval_policy = Constrained::allow_only(event.approval_policy);
		}
		if let Err(err) = self.config.permissions.sandbox_policy.set(event.sandbox_policy.clone()) {
		...
		let session_info_cell = history_cell::new_session_info(
			...
			self.auth_manager.auth_cached().and_then(|auth| auth.account_plan_type()),
			show_fast_status,
		);
		...
		self.submit_op(Op::ListSkills { cwds: Vec::new(), force_reload: true });
	...
	fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
		...
		tokio::spawn(async move {
			...
			let send_name_and_id = |name: String| {
				...
				.into();
				app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(PlainHistoryCell::new(vec![line]))));
			};
			let send_id_only = || {
				let line: Line<'static> =
					vec!["• ".dim(), "Thread forked from ".into(), forked_from_id_text.clone().cyan()].into();
				app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(PlainHistoryCell::new(vec![line]))));
			};
	...
	pub(crate) fn open_feedback_note(
		...
		if let Some(chatgpt_user_id) =
			self.auth_manager.auth_cached().and_then(|auth| auth.get_chatgpt_user_id())
		{
	...
	fn show_feedback_note(
		...
		let rollout = if include_logs { self.current_rollout_path.clone() } else { None };
	...
	fn on_plan_item_completed(&mut self, text: String) {
		...
		let plan_text = if text.trim().is_empty() { streamed_plan } else { text };
		...
		let finalized_streamed_cell = if let Some(mut controller) = self.plan_stream_controller.take() {
			controller.finalize()
		} else {
			None
		};
	...
	fn on_task_started(&mut self) {
		...
		self.turn_sleep_inhibitor.set_turn_running(/*turn_running*/ true);
		...
		self.pending_status_indicator_restore = false;
		self.bottom_pane.set_interrupt_hint_visible(/*visible*/ true);
		self.terminal_title_status_kind = TerminalTitleStatusKind::Working;
		self.set_status_header(String::from("Working"));
		self.full_reasoning_buffer.clear();
		self.reasoning_buffer.clear();
	...
	fn on_task_complete(&mut self, last_agent_message: Option<String>, from_replay: bool) {
		...
		self.turn_sleep_inhibitor.set_turn_running(/*turn_running*/ false);
		self.update_task_running_state();
		...
		self
			.notify(Notification::AgentTurnComplete { response: last_agent_message.unwrap_or_default() });
	...
	fn open_plan_implementation_prompt(&mut self) {
		...
		self.notify(Notification::PlanModePrompt { title: PLAN_IMPLEMENTATION_TITLE.to_string() });
	}
	...
	fn pop_next_queued_user_message(&mut self) -> Option<UserMessage> {
		if self.rejected_steers_queue.is_empty() {
			...
			Some(merge_user_messages(self.rejected_steers_queue.drain(..).collect()))
		}
	}

	fn pop_latest_queued_user_message(&mut self) -> Option<UserMessage> {
		self.queued_user_messages.pop_back().or_else(|| self.rejected_steers_queue.pop_back())
	}
	...
	pub(crate) fn enqueue_rejected_steer(&mut self) -> bool {
		...
		self.rejected_steers_queue.push_back(pending_steer.user_message);
	...
	fn handle_steer_rejected_error(&mut self, codex_error_info: &CodexErrorInfo) -> bool {
		matches!(codex_error_info, CodexErrorInfo::ActiveTurnNotSteerable { .. })
		...
		let items = vec![
			SelectionItem {
				...
				actions: vec![Box::new(|tx| {
					tx.send(AppEvent::UpdateFeatureFlags { updates: vec![(Feature::Collab, true)] });
					tx.send(AppEvent::InsertHistoryCell(Box::new(history_cell::new_warning_event(
	...
	pub(crate) fn set_token_info(&mut self, info: Option<TokenUsageInfo>) {
		match info {
			...
			None => {
				self.bottom_pane.set_context_window(/*percent*/ None, /*used_tokens*/ None);
	...
	fn context_remaining_percent(&self, info: &TokenUsageInfo) -> Option<i64> {
		info
			...
			.map(|window| info.last_token_usage.percent_of_context_window_remaining(window))
	}
	...
	pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
		if let Some(mut snapshot) = snapshot {
			let limit_id = snapshot.limit_id.clone().unwrap_or_else(|| "codex".to_string());
			let limit_label = snapshot.limit_name.clone().unwrap_or_else(|| limit_id.clone());
			...
			let warnings = if is_codex_limit {
				self.rate_limit_warnings.take_warnings(
					snapshot.secondary.as_ref().map(|window| window.used_percent),
					snapshot.secondary.as_ref().and_then(|window| window.window_minutes),
			...
			if high_usage
				...
				&& !matches!(self.rate_limit_switch_prompt, RateLimitSwitchPromptState::Shown)
			{
				self.rate_limit_switch_prompt = RateLimitSwitchPromptState::Pending;
			}

			let display = rate_limit_snapshot_display_for_limit(&snapshot, limit_label, Local::now());
			self.rate_limit_snapshots_by_limit_id.insert(limit_id, display);
	...
	fn on_mcp_startup_update(&mut self, ev: McpStartupUpdateEvent) {
		...
		if let Some(current) = &self.mcp_startup_status {
			...
			let mut starting: Vec<_> = current
				...
				.filter_map(
					|(name, state)| {
						if matches!(state, McpStartupStatus::Starting) { Some(name) } else { None }
					},
				)
				.collect();
			starting.sort();
			if let Some(first) = starting.first() {
				...
				let max_to_show = 3;
				let mut to_show: Vec<String> =
					starting.iter().take(max_to_show).map(ToString::to_string).collect();
				if starting.len() > max_to_show {
					to_show.push("…".to_string());
				}
				let header = if total > 1 {
					format!("Starting MCP servers ({completed}/{total}): {}", to_show.join(", "))
				} else {
	...
	fn on_interrupted_turn(&mut self, reason: TurnAbortReason) {
		...
		if send_pending_steers_immediately {
			let pending_steers: Vec<UserMessage> =
				self.pending_steers.drain(..).map(|pending| pending.user_message).collect();
	...
	fn drain_pending_messages_for_restore(&mut self) -> Option<UserMessage> {
		...
		to_merge.extend(self.pending_steers.drain(..).map(|steer| steer.user_message));
	...
	fn restore_user_message_to_composer(&mut self, user_message: UserMessage) {
		let UserMessage { text, local_images, remote_image_urls, text_elements, mention_bindings } =
			user_message;
	...
	pub(crate) fn restore_thread_input_state(&mut self, input_state: Option<ThreadInputState>) {
		...
		if let Some(input_state) = input_state {
			...
			if let Some(composer) = input_state.composer {
				let local_image_paths = composer.local_images.into_iter().map(|img| img.path).collect();
				...
				self.bottom_pane.set_composer_pending_pastes(composer.pending_pastes);
			} else {
	...
	fn on_exec_approval_request(&mut self, _id: String, ev: ExecApprovalRequestEvent) {
		let ev2 = ev.clone();
		self.defer_or_handle(|q| q.push_exec_approval(ev), |s| s.handle_exec_approval_now(ev2));
	}
	...
	fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
			...
			match tool {
				"shell" | "exec_command" => match action.get("command") {
					...
					Some(serde_json::Value::Array(command)) => {
						let args = command.iter().map(serde_json::Value::as_str).collect::<Option<Vec<_>>>()?;
						shlex::try_join(args.iter().copied()).ok().or_else(|| Some(args.join(" ")))
					}
					_ => None,
				},
				"apply_patch" => {
					let files = action
						...
						.map(|files| files.iter().filter_map(serde_json::Value::as_str).collect::<Vec<_>>())
						.unwrap_or_default();
					...
					Some(if files.len() == 1 {
						...
						format!("apply_patch touching {change_count} changes across {} files", files.len())
					})
				}
				...
				"mcp_tool_call" => {
					let tool_name = action.get("tool_name").and_then(serde_json::Value::as_str)?;
					let label = action
						.get("connector_name")
						.and_then(serde_json::Value::as_str)
			...
			&& let Some(detail) = guardian_action_summary(action)
		{
			...
			self.bottom_pane.ensure_status_indicator();
			self.bottom_pane.set_interrupt_hint_visible(/*visible*/ true);
			self.pending_guardian_review_status.start_or_update(ev.id.clone(), detail);
		...
		let cell = if let Some(command) = guardian_command(&action) {
			...
			match tool {
				...
				Some("mcp_tool_call") => {
					let server =
						action.get("server").and_then(serde_json::Value::as_str).unwrap_or("unknown server");
					let tool_name =
						action.get("tool_name").and_then(serde_json::Value::as_str).unwrap_or("unknown tool");
	...
	fn on_elicitation_request(&mut self, ev: ElicitationRequestEvent) {
		let ev2 = ev.clone();
		self.defer_or_handle(|q| q.push_elicitation(ev), |s| s.handle_elicitation_request_now(ev2));
	}

	fn on_request_user_input(&mut self, ev: RequestUserInputEvent) {
		let ev2 = ev.clone();
		self.defer_or_handle(|q| q.push_user_input(ev), |s| s.handle_request_user_input_now(ev2));
	}
	...
	fn on_exec_command_output_delta(&mut self, ev: ExecCommandOutputDeltaEvent) {
		...
		let Some(cell) =
			self.active_cell.as_mut().and_then(|c| c.as_any_mut().downcast_mut::<ExecCell>())
		else {
			return;
		};
	...
	fn on_terminal_interaction(&mut self, ev: TerminalInteractionEvent) {
		...
		if ev.stdin.is_empty() {
			...
			if self.unified_exec_wait_streak.as_ref().is_some_and(|wait| wait.process_id == ev.process_id)
			{
				self.flush_unified_exec_wait_streak();
			}
			self.add_to_history(history_cell::new_unified_exec_interaction(command_display, ev.stdin));
		}
	}

	fn on_patch_apply_begin(&mut self, event: PatchApplyBeginEvent) {
		self.add_to_history(history_cell::new_patch_event(event.changes, &self.config.cwd));
	}

	fn on_view_image_tool_call(&mut self, event: ViewImageToolCallEvent) {
		self.flush_answer_stream_with_separator();
		self.add_to_history(history_cell::new_view_image_tool_call(event.path, &self.config.cwd));
		self.request_redraw();
	}
	...
	fn on_image_generation_end(&mut self, event: ImageGenerationEndEvent) {
		...
		let saved_path = event.saved_path.map(|saved_path| {
			Url::from_file_path(Path::new(&saved_path)).map(|url| url.to_string()).unwrap_or(saved_path)
		});
	...
	fn on_exec_command_end(&mut self, ev: ExecCommandEndEvent) {
		if is_unified_exec_source(ev.source) {
			if let Some(process_id) = ev.process_id.as_deref()
				&& self.unified_exec_wait_streak.as_ref().is_some_and(|wait| wait.process_id == process_id)
			{
				self.flush_unified_exec_wait_streak();
			}
	...
	fn track_unified_exec_process_begin(&mut self, ev: &ExecCommandBeginEvent) {
		...
		if let Some(existing) =
			self.unified_exec_processes.iter_mut().find(|process| process.key == key)
		{
	...
	fn track_unified_exec_process_end(&mut self, ev: &ExecCommandEndEvent) {
		...
		self.unified_exec_processes.retain(|process| process.key != key);
	...
	fn sync_unified_exec_footer(&mut self) {
		let processes =
			self.unified_exec_processes.iter().map(|process| process.command_display.clone()).collect();
	...
	fn track_unified_exec_output_chunk(&mut self, call_id: &str, chunk: &[u8]) {
		let Some(process) =
			self.unified_exec_processes.iter_mut().find(|process| process.call_id == call_id)
		else {
			return;
		};
	...
	fn on_web_search_end(&mut self, ev: WebSearchEndEvent) {
		self.flush_answer_stream_with_separator();
		let WebSearchEndEvent { call_id, query, action } = ev;
		let mut handled = false;
		if let Some(cell) =
			self.active_cell.as_mut().and_then(|cell| cell.as_any_mut().downcast_mut::<WebSearchCell>())
	...
	fn on_get_history_entry_response(
		...
		let codex_protocol::protocol::GetHistoryEntryResponseEvent { offset, log_id, entry } = event;
		self.bottom_pane.on_history_entry_response(log_id, offset, entry.map(|e| e.text));
	}
	...
	fn on_undo_started(&mut self, event: UndoStartedEvent) {
		self.bottom_pane.ensure_status_indicator();
		self.bottom_pane.set_interrupt_hint_visible(/*visible*/ false);
		let message = event.message.unwrap_or_else(|| "Undo in progress...".to_string());
	...
	fn on_undo_completed(&mut self, event: UndoCompletedEvent) {
		...
		let message = message.unwrap_or_else(|| {
			if success { "Undo completed successfully.".to_string() } else { "Undo failed.".to_string() }
		});
		if success {
	...
	fn on_agent_message_item_completed(&mut self, item: AgentMessageItem) {
		...
		self.finalize_completed_assistant_message((!message.is_empty()).then_some(message.as_str()));
	...
	pub(crate) fn handle_exec_end_now(&mut self, ev: ExecCommandEndEvent) {
		...
		let is_unified_exec_interaction = matches!(source, ExecCommandSource::UnifiedExecInteraction);
		let end_target = match self.active_cell.as_ref() {
			Some(cell) => match cell.as_any().downcast_ref::<ExecCell>() {
				Some(exec_cell) if exec_cell.iter_calls().any(|call| call.call_id == ev.call_id) => {
		...
		match end_target {
			...
			ExecEndTarget::OrphanHistoryWhileActiveExec => {
				...
				debug_assert!(completed, "new orphan exec cell should contain {}", ev.call_id);
				self.needs_final_message_separator = true;
				self.app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(orphan)));
				self.request_redraw();
			}
	...
	pub(crate) fn handle_exec_approval_now(&mut self, ev: ExecApprovalRequestEvent) {
		...
		self.bottom_pane.push_approval_request(request, &self.config.features);
		self.request_redraw();
	}
	...
	pub(crate) fn handle_elicitation_request_now(&mut self, ev: ElicitationRequestEvent) {
		...
		self.notify(Notification::ElicitationRequested { server_name: ev.server_name.clone() });
		...
		if let Some(request) = McpServerElicitationFormRequest::from_event(thread_id, ev.clone()) {
			self.bottom_pane.push_mcp_server_elicitation_request(request);
		} else {
	...
	pub(crate) fn handle_exec_begin_now(&mut self, ev: ExecCommandBeginEvent) {
		...
		let is_wait_interaction = matches!(ev.source, ExecCommandSource::UnifiedExecInteraction)
			&& ev.interaction_input.as_deref().map(str::is_empty).unwrap_or(true);
		let command_display = ev.command.join(" ");
		let should_suppress_unified_wait = is_wait_interaction
			&& self.last_unified_wait.as_ref().is_some_and(|wait| wait.is_duplicate(&command_display));
	...
	pub(crate) fn handle_mcp_end_now(&mut self, ev: McpToolCallEndEvent) {
		...
		let McpToolCallEndEvent { call_id, invocation, duration, result } = ev;
	...
	pub fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
		let ChatWidgetInit {
			config,
			frame_requester,
			app_event_tx,
		...
		let model_for_header = model.clone().unwrap_or_else(|| DEFAULT_MODEL_DISPLAY_NAME.to_string());
		...
		widget
			.bottom_pane
			.set_voice_transcription_enabled(widget.config.features.enabled(Feature::VoiceTranscription));
		widget.bottom_pane.set_realtime_conversation_enabled(widget.realtime_conversation_enabled());
		widget
			.bottom_pane
			.set_audio_device_selection_enabled(widget.realtime_audio_device_selection_enabled());
		widget.bottom_pane.set_status_line_enabled(!widget.configured_status_line_items().is_empty());
		widget.bottom_pane.set_collaboration_modes_enabled(/*enabled*/ true);
		widget.sync_fast_command_enabled();
		widget.sync_personality_command_enabled();
		widget.sync_plugins_command_enabled();
		widget.bottom_pane.set_queued_message_edit_binding(widget.queued_message_edit_binding);
		#[cfg(target_os = "windows")]
	...
	pub fn new_with_op_sender(common: ChatWidgetInit, codex_op_tx: UnboundedSender<Op>) -> Self {
		let ChatWidgetInit {
			config,
			frame_requester,
			app_event_tx,
	...
	pub(crate) fn new_from_existing(
		...
		let model_override = model.as_deref();
		let header_model = model.clone().unwrap_or_else(|| session_configured.model.clone());
		let active_collaboration_mask =
			Self::initial_collaboration_mask(&config, models_manager.as_ref(), model_override);
		let header_model = active_collaboration_mask
			.as_ref()
	...
	pub fn handle_key_event(&mut self, key_event: KeyEvent) {
		match key_event {
			...
			KeyEvent {
				code: KeyCode::PageUp, kind: KeyEventKind::Press | KeyEventKind::Repeat, ..
			} => {
				self.scroll_up(self.viewport_height.get().saturating_sub(1).max(1));
				return;
			}
			KeyEvent {
				...
				self.scroll_down(self.viewport_height.get().saturating_sub(1).max(1));
				return;
			}
			// Scrollbar controls - Ctrl+Home/End
			KeyEvent { code: KeyCode::Home, modifiers, kind: KeyEventKind::Press, .. }
				if modifiers.contains(KeyModifiers::CONTROL) =>
			{
				self.scroll_to_top();
				return;
			}
			KeyEvent { code: KeyCode::End, modifiers, kind: KeyEventKind::Press, .. }
				if modifiers.contains(KeyModifiers::CONTROL) =>
			{
				self.scroll_to_bottom();
				return;
			}
			// Scrollbar controls - Ctrl+Up/Down (one line at a time)
			KeyEvent {
				code: KeyCode::Up,
				modifiers,
				kind: KeyEventKind::Press | KeyEventKind::Repeat,
		...
		match key_event {
			KeyEvent { code: KeyCode::BackTab, kind: KeyEventKind::Press, .. }
				if self.collaboration_modes_enabled()
					&& !self.bottom_pane.is_task_running()
					&& self.bottom_pane.no_modal_or_popup_active() =>
			{
			...
			_ => match self.bottom_pane.handle_key_event(key_event) {
				InputResult::Submitted { text, text_elements } => {
					let local_images = self.bottom_pane.take_recent_submission_images_with_placeholders();
					...
					let should_submit_now = self.is_session_configured() && !self.is_plan_streaming_in_tui();
	...
	pub(crate) fn attach_image(&mut self, path: PathBuf) {
		if !self.current_model_supports_images() {
			self
				.add_to_history(history_cell::new_warning_event(self.image_inputs_not_supported_message()));
			self.request_redraw();
			return;
		}
	...
	fn dispatch_command(&mut self, cmd: SlashCommand) {
		if !cmd.available_during_task() && self.bottom_pane.is_task_running() {
			let message = format!("'/{}' is disabled while a task is in progress.", cmd.command());
			self.add_to_history(history_cell::new_error_event(message));
		...
		match cmd {
			...
			SlashCommand::Rename => {
				self.session_telemetry.counter("codex.thread.rename", /*inc*/ 1, &[]);
			...
			SlashCommand::Plan => {
				...
				if let Some(mask) = collaboration_modes::plan_mask(self.models_manager.as_ref()) {
					...
					self.add_info_message("Plan mode unavailable right now.".to_string(), /*hint*/ None);
				}
			}
			SlashCommand::Collab => {
				if !self.collaboration_modes_enabled() {
			...
			SlashCommand::ElevateSandbox => {
				#[cfg(target_os = "windows")]
				{
					...
					let Some(preset) =
						builtin_approval_presets().into_iter().find(|preset| preset.id == "auto")
					else {
						// Avoid panicking in interactive UI; treat this as a recoverable
						// internal error.
						self
							.add_error_message("Internal error: missing the 'auto' approval preset.".to_string());
						return;
					};

					if let Err(err) = self.config.permissions.approval_policy.can_set(&preset.approval) {
			...
			SlashCommand::SandboxReadRoot => {
				self
					.add_error_message("Usage: /sandbox-add-read-dir <absolute-directory-path>".to_string());
			}
			...
			SlashCommand::Rollout => {
				if let Some(path) = self.rollout_path() {
					...
					self
						.add_info_message("Rollout path is not available yet.".to_string(), /*hint*/ None);
				}
			}
				...
				self.app_event_tx.send(AppEvent::CodexEvent(Event {
					...
					msg: EventMsg::ApplyPatchApprovalRequest(ApplyPatchApprovalRequestEvent {
						...
						changes: HashMap::from([
							(PathBuf::from("/tmp/test.txt"), FileChange::Add { content: "test".to_string() }),
							(
								PathBuf::from("/tmp/test2.txt"),
								FileChange::Update { unified_diff: "+test\n-test2".to_string(), move_path: None },
							),
						]),
						reason: None,
		...
		match cmd {
			...
			SlashCommand::Rename if !trimmed.is_empty() => {
				...
				let Some((prepared_args, _prepared_elements)) =
					self.bottom_pane.prepare_inline_args_submission(/*record_history*/ false)
				else {
					return;
				};
				...
				self.request_redraw();
				self.app_event_tx.send(AppEvent::CodexOp(Op::SetThreadName { name }));
				self.bottom_pane.drain_pending_submission_state();
			}
			SlashCommand::Plan if !trimmed.is_empty() => {
				...
				let Some((prepared_args, prepared_elements)) =
					self.bottom_pane.prepare_inline_args_submission(/*record_history*/ true)
				else {
					return;
				};
				let local_images = self.bottom_pane.take_recent_submission_images_with_placeholders();
			...
			SlashCommand::Review if !trimmed.is_empty() => {
				...
				self.submit_op(Op::Review {
					review_request: ReviewRequest {
						target: ReviewTarget::Custom { instructions: prepared_args },
			...
			SlashCommand::SandboxReadRoot if !trimmed.is_empty() => {
				...
				self.app_event_tx.send(AppEvent::BeginWindowsSandboxGrantReadRoot { path: prepared_args });
				self.bottom_pane.drain_pending_submission_state();
			}
	...
	fn show_rename_prompt(&mut self) {
		let tx = self.app_event_tx.clone();
		let has_name = self.thread_name.as_ref().is_some_and(|name| !name.is_empty());
		let title = if has_name { "Rename thread" } else { "Name thread" };
	...
	pub(crate) fn handle_paste_burst_tick(&mut self, frame_requester: FrameRequester) -> bool {
		if self.bottom_pane.flush_paste_burst_if_due() {
			...
			frame_requester
				.schedule_frame_in(crate::bottom_pane::ChatComposer::recommended_paste_flush_delay());
			true
		} else {
			false
		}
	...
	fn submit_user_message(&mut self, user_message: UserMessage) {
		...
		for image_url in &remote_image_urls {
			items.push(UserInput::Image { image_url: image_url.clone() });
		}

		for image in &local_images {
			items.push(UserInput::LocalImage { path: image.path.clone() });
		}

		if !text.is_empty() {
			items.push(UserInput::Text { text: text.clone(), text_elements: text_elements.clone() });
		}

		let mentions = collect_tool_mentions(&text, &HashMap::new());
		let bound_names: HashSet<String> =
			mention_bindings.iter().map(|binding| binding.mention.clone()).collect();
		...
		if let Some(skills) = self.bottom_pane.skills() {
			skill_names_lower = skills.iter().map(|skill| skill.name.to_ascii_lowercase()).collect();

			for binding in &mention_bindings {
				let path = binding.path.strip_prefix("skill://").unwrap_or(binding.path.as_str());
				let path = Path::new(path);
				if let Some(skill) = skills.iter().find(|skill| skill.path_to_skills_md.as_path() == path)
		...
		if let Some(plugins) = self.plugins_for_mentions() {
			for binding in &mention_bindings {
				let Some(plugin_config_name) =
					binding.path.strip_prefix("plugin://").filter(|id| !id.is_empty())
				else {
					continue;
				};
				...
				if let Some(plugin) = plugins.iter().find(|plugin| plugin.config_name == plugin_config_name)
				{
		...
		if let Some(apps) = self.connectors_for_mentions() {
			for binding in &mention_bindings {
				let Some(app_id) = binding.path.strip_prefix("app://").filter(|id| !id.is_empty()) else {
					continue;
				};
				...
				if let Some(app) = apps.iter().find(|app| app.id == app_id && app.is_enabled) {
					items.push(UserInput::Mention { name: app.name.clone(), path: binding.path.clone() });
				}
			}
			...
			for app in app_mentions {
				...
				items.push(UserInput::Mention { name: app.name.clone(), path: format!("app://{app_id}") });
			}
		}

		let effective_mode = self.effective_collaboration_mode();
		let collaboration_mode = if self.collaboration_modes_enabled() {
			self.active_collaboration_mask.as_ref().map(|_| effective_mode.clone())
		} else {
			None
		};
		...
		if !text.is_empty() {
			...
			self.codex_op_tx.send(Op::AddToHistory { text: history_text }).unwrap_or_else(|e| {
		...
		if render_in_history && !text.is_empty() {
			let local_image_paths = local_images.into_iter().map(|img| img.path).collect::<Vec<_>>();
	...
	fn restore_blocked_image_submission(
		...
		self.add_to_history(history_cell::new_warning_event(self.image_inputs_not_supported_message()));
		self.request_redraw();
	}
	...
	fn replay_initial_messages(&mut self, events: Vec<EventMsg>) {
		for msg in events {
			if matches!(msg, EventMsg::SessionConfigured(_) | EventMsg::ThreadNameUpdated(_)) {
				continue;
			}
			// `id: None` indicates a synthetic/fake id coming from replay.
			self.dispatch_event_msg(/*id*/ None, msg, Some(ReplayKind::ResumeInitialMessages));
		}
	}
	...
	fn dispatch_event_msg(
		...
		let is_resume_initial_replay = matches!(replay_kind, Some(ReplayKind::ResumeInitialMessages));
		...
		match msg {
			...
			EventMsg::AgentMessage(AgentMessageEvent { .. })
				if matches!(replay_kind, Some(ReplayKind::ThreadSnapshot)) && !self.is_review_mode => {}
			...
			EventMsg::Error(ErrorEvent { message, codex_error_info }) => {
				if codex_error_info.as_ref().is_some_and(|info| self.handle_steer_rejected_error(info)) {
					...
					match kind {
						...
						RateLimitErrorKind::UsageLimit | RateLimitErrorKind::Generic => self.on_error(message),
					}
				} else {
			...
			EventMsg::CollabAgentSpawnBegin(CollabAgentSpawnBeginEvent {
				...
				self
					...
					.insert(call_id, multi_agents::SpawnRequestSummary { model, reasoning_effort });
			}
			...
			EventMsg::ThreadRolledBack(rollback) => {
				...
				self.last_copyable_output = None;
				if from_replay {
					self.app_event_tx.send(AppEvent::ApplyThreadRollback { num_turns: rollback.num_turns });
				}
			}
			...
			EventMsg::ItemCompleted(event) => {
				...
				if !from_replay && let codex_protocol::items::TurnItem::UserMessage(item) = &item {
					...
					if self.pending_steers.front().is_some_and(|pending| pending.compare_key == compare_key) {
		...
		if let Some(output) = review.review_output {
			...
			if output.findings.is_empty() {
				...
				if explanation.is_empty() {
					...
					self.app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(body_cell)));
				}
			}
	...
	fn refresh_pending_input_preview(&mut self) {
		let queued_messages: Vec<String> =
			self.queued_user_messages.iter().map(|m| m.text.clone()).collect();
		let pending_steers: Vec<String> =
			self.pending_steers.iter().map(|steer| steer.user_message.text.clone()).collect();
		let rejected_steers: Vec<String> =
			self.rejected_steers_queue.iter().map(|message| message.text.clone()).collect();
		self.bottom_pane.set_pending_input_preview(queued_messages, pending_steers, rejected_steers);
	}
	...
	pub(crate) fn add_status_output(&mut self) {
		...
		let total_usage = token_info.map(|ti| &ti.total_token_usage).unwrap_or(&default_usage);
		...
		let rate_limit_snapshots: Vec<RateLimitSnapshotDisplay> =
			self.rate_limit_snapshots_by_limit_id.values().cloned().collect();
	...
	fn open_status_line_setup(&mut self) {
		...
		let view = StatusLineSetupView::new(
			...
			StatusLinePreviewData::from_iter(
				StatusLineItem::iter()
					.filter_map(|item| self.status_line_value_for_item(&item).map(|value| (item, value))),
			),
			self.app_event_tx.clone(),
		);
		self.bottom_pane.show_view(Box::new(view));
	...
	fn open_theme_picker(&mut self) {
		...
		let terminal_width = self.last_rendered_width.get().and_then(|width| u16::try_from(width).ok());
	...
	fn status_line_context_remaining_percent(&self) -> Option<i64> {
		...
		let default_usage = TokenUsage::default();
		let usage =
			self.token_info.as_ref().map(|info| &info.last_token_usage).unwrap_or(&default_usage);
	...
	fn status_line_total_usage(&self) -> TokenUsage {
		self.token_info.as_ref().map(|info| info.total_token_usage.clone()).unwrap_or_default()
	}
	...
	fn clean_background_terminals(&mut self) {
		...
		self.add_info_message("Stopping all background terminals.".to_string(), /*hint*/ None);
	}
	...
	fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
		...
		tokio::spawn(async move {
			...
			let should_schedule_force_refetch = !force_refetch && !accessible_result.codex_apps_ready;
			let accessible_connectors = accessible_result.connectors;

			app_event_tx.send(AppEvent::ConnectorsLoaded {
				result: Ok(ConnectorsSnapshot { connectors: accessible_connectors.clone() }),
				is_final: false,
			});
			...
			app_event_tx.send(AppEvent::ConnectorsLoaded { result, is_final: true });

			if should_schedule_force_refetch {
				app_event_tx.send(AppEvent::RefreshConnectors { force_refetch: true });
			}
		});
	}
	...
	fn should_prefetch_rate_limits(&self) -> bool {
		...
		self.auth_manager.auth_cached().as_ref().is_some_and(CodexAuth::is_chatgpt_auth)
	}

	fn lower_cost_preset(&self) -> Option<ModelPreset> {
		let models = self.models_manager.try_list_models().ok()?;
		models.iter().find(|preset| preset.show_in_picker && preset.model == NUDGE_MODEL_SLUG).cloned()
	}

	fn rate_limit_switch_prompt_hidden(&self) -> bool {
		self.config.notices.hide_rate_limit_model_nudge.unwrap_or(false)
	}
	...
	fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
		...
		let items = vec![
			...
			SelectionItem {
				...
				description: Some("Hide future rate limit reminders about switching models.".to_string()),
				selected_description: None,
				is_current: false,
	...
	pub(crate) fn open_realtime_audio_popup(&mut self) {
		let items = [RealtimeAudioDeviceKind::Microphone, RealtimeAudioDeviceKind::Speaker]
			.into_iter()
			.map(|kind| {
				let description =
					Some(format!("Current: {}", self.current_realtime_audio_selection_label(kind)));
				let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
	...
	pub(crate) fn open_realtime_audio_device_selection(&mut self, kind: RealtimeAudioDeviceKind) {
		match list_realtime_audio_device_names(kind) {
			...
			Err(err) => {
				self.add_error_message(format!("Failed to load realtime {} devices: {err}", kind.noun()));
			}
		}
	}
	...
	fn open_realtime_audio_device_selection_with_names(
		...
		header.push(Line::from("Saved devices apply to realtime voice only.".dim()));
	...
	pub(crate) fn open_realtime_audio_restart_prompt(&mut self, kind: RealtimeAudioDeviceKind) {
		...
		header
			.push(Line::from("Configuration is saved. Restart local audio to use it immediately.".dim()));
	...
	pub(crate) fn open_model_popup_with_presets(&mut self, presets: Vec<ModelPreset>) {
		let presets: Vec<ModelPreset> =
			presets.into_iter().filter(|preset| preset.show_in_picker).collect();
		...
		let (mut auto_presets, other_presets): (Vec<ModelPreset>, Vec<ModelPreset>) =
			presets.into_iter().partition(|preset| Self::is_auto_model(&preset.model));
		...
		if !other_presets.is_empty() {
			...
			let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
				tx.send(AppEvent::OpenAllModelsPopup { models: all_models.clone() });
			})];

			let is_current = !items.iter().any(|item| item.is_current);
			let description =
				Some(format!("Choose a specific model and reasoning level (current: {current_label})"));
		...
		let header =
			self.model_menu_header("Select Model", "Pick a quick auto mode or browse all models.");
		self.bottom_pane.show_selection_view(SelectionViewParams {
			footer_hint: Some(standard_popup_hint_line()),
			items,
			header,
	...
	pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
		...
		for preset in presets.into_iter() {
			let description = (!preset.description.is_empty()).then_some(preset.description.to_string());
			let is_current = preset.model.as_str() == self.current_model();
			let single_supported_effort = preset.supported_reasoning_efforts.len() == 1;
			let preset_for_action = preset.clone();
			let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
				let preset_for_event = preset_for_action.clone();
				tx.send(AppEvent::OpenReasoningPopup { model: preset_for_event });
			})];
			items.push(SelectionItem {
	...
	pub(crate) fn open_collaboration_modes_popup(&mut self) {
		...
		let current_kind =
			self.active_collaboration_mask.as_ref().and_then(|mask| mask.mode).or_else(|| {
				collaboration_modes::default_mask(self.models_manager.as_ref()).and_then(|mask| mask.mode)
			});
		let items: Vec<SelectionItem> = presets
			...
			.map(|mask| {
				...
				SelectionItem { name, is_current, actions, dismiss_on_select: true, ..Default::default() }
			})
			.collect();
	...
	pub(crate) fn open_plan_reasoning_scope_prompt(
		...
		let reasoning_phrase = match effort {
			...
			Some(selected_effort) => {
				format!("{} reasoning", Self::reasoning_effort_label(selected_effort).to_lowercase())
			}
			None => "the selected reasoning".to_string(),
		};
		let plan_only_description = format!("Always use {reasoning_phrase} in Plan mode.");
		let plan_reasoning_source = if let Some(plan_override) = self.config.plan_mode_reasoning_effort
		{
			format!(
		...
		let all_modes_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
			...
			tx.send(AppEvent::UpdatePlanModeReasoningEffort(effort));
			tx.send(AppEvent::PersistPlanModeReasoningEffort(effort));
			tx.send(AppEvent::PersistModelSelection { model: model.clone(), effort });
		})];
		...
		self
			.notify(Notification::PlanModePrompt { title: PLAN_MODE_REASONING_SCOPE_TITLE.to_string() });
	}
	...
	pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
		...
		let warn_effort =
			if supported.iter().any(|option| option.effort == ReasoningEffortConfig::XHigh) {
				Some(ReasoningEffortConfig::XHigh)
			} else if supported.iter().any(|option| option.effort == ReasoningEffortConfig::High) {
		...
		for effort in ReasoningEffortConfig::iter() {
			if supported.iter().any(|option| option.effort == effort) {
				choices.push(EffortChoice { stored: Some(effort), display: effort });
			}
		}
		if choices.is_empty() {
			choices.push(EffortChoice { stored: Some(default_effort), display: default_effort });
		}
		...
		let highlight_choice = if is_current_model {
			if in_plan_mode {
				self.config.plan_mode_reasoning_effort.or(self.effective_reasoning_effort())
			} else {
		...
		let initial_selected_idx =
			choices.iter().position(|choice| choice.stored == selection_choice).or_else(|| {
				selection_choice
		...
		for choice in choices.iter() {
			...
			let selected_description = if show_warning {
				warning_text.as_ref().map(|warning_message| {
					description
						.as_ref()
						.map_or_else(|| warning_message.clone(), |d| format!("{d}\n{warning_message}"))
				})
			} else {
				None
			};
		...
		let mut header = ColumnRenderable::new();
		header.push(Line::from(format!("Select Reasoning Level for {model_slug}").bold()));
	...
	fn apply_model_and_effort_without_persist(
		...
		self.app_event_tx.send(AppEvent::UpdateReasoningEffort(effort));
	}

	fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
		self.apply_model_and_effort_without_persist(model.clone(), effort);
		self.app_event_tx.send(AppEvent::PersistModelSelection { model, effort });
	}
	...
	pub(crate) fn open_permissions_popup(&mut self) {
		...
		let guardian_disabled_reason = |enabled: bool| {
			...
			self.config.features.can_set(&next_features).err().map(|err| err.to_string())
		};
		...
		for preset in presets.into_iter() {
			...
			let base_description = Some(preset.description.replace(" (Identical to Agent mode)", ""));
			let approval_disabled_reason =
				match self.config.permissions.approval_policy.can_set(&preset.approval) {
					Ok(()) => None,
					Err(err) => Some(err.to_string()),
				};
			let default_disabled_reason =
				approval_disabled_reason.clone().or_else(|| guardian_disabled_reason(false));
				...
				#[cfg(target_os = "windows")]
				{
					if WindowsSandboxLevel::from_config(&self.config) == WindowsSandboxLevel::Disabled {
						...
						if codex_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
							...
							vec![Box::new(move |tx| {
								tx.send(AppEvent::OpenWindowsSandboxEnablePrompt { preset: preset_clone.clone() });
							})]
						}
	...
	fn preset_matches_current(
		...
		match (current_sandbox, &preset.sandbox) {
			...
			(
				SandboxPolicy::ReadOnly { network_access: current_network_access, .. },
				SandboxPolicy::ReadOnly { network_access: preset_network_access, .. },
			) => current_network_access == preset_network_access,
			(
				SandboxPolicy::WorkspaceWrite { network_access: current_network_access, .. },
				SandboxPolicy::WorkspaceWrite { network_access: preset_network_access, .. },
			) => current_network_access == preset_network_access,
			_ => false,
		}
	}
	...
	pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
		if self.config.notices.hide_world_writable_warning.unwrap_or(false) {
			return None;
		}
	...
	pub(crate) fn open_full_access_confirmation(
		...
		header_children.push(Box::new(Paragraph::new(vec![info_line]).wrap(Wrap { trim: false })));
		...
		let mut accept_and_remember_actions =
			Self::approval_preset_actions(approval, sandbox, selected_name, ApprovalsReviewer::User);
	...
	pub(crate) fn open_world_writable_warning_confirmation(
		...
		let info_line = if failed_scan {
			Line::from(vec![
				...
				format!("The Windows sandbox cannot guarantee protection in {mode_label}.").fg(Color::Red),
			])
		} else {
			Line::from(vec![
	...
	pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
		...
		let items = vec![
			SelectionItem {
				...
				description: None,
				actions: vec![Box::new(move |tx| {
					accept_otel.counter("codex.windows_sandbox.elevated_prompt_accept", /*inc*/ 1, &[]);
					tx.send(AppEvent::BeginWindowsSandboxElevatedSetup { preset: preset.clone() });
				})],
				dismiss_on_select: true,
				..Default::default()
			},
			SelectionItem {
				...
				actions: vec![Box::new(move |tx| {
					...
					tx.send(AppEvent::BeginWindowsSandboxLegacySetup { preset: legacy_preset.clone() });
				})],
				dismiss_on_select: true,
				..Default::default()
			},
			SelectionItem {
				...
				description: None,
				actions: vec![Box::new(move |tx| {
					quit_otel.counter("codex.windows_sandbox.elevated_prompt_quit", /*inc*/ 1, &[]);
		...
		let items = vec![
			SelectionItem {
				...
				actions: vec![Box::new({
					...
					move |tx| {
						otel.counter("codex.windows_sandbox.fallback_retry_elevated", /*inc*/ 1, &[]);
						tx.send(AppEvent::BeginWindowsSandboxElevatedSetup { preset: preset.clone() });
					}
				})],
				dismiss_on_select: true,
			...
			SelectionItem {
				...
				actions: vec![Box::new({
					...
					move |tx| {
						otel.counter("codex.windows_sandbox.fallback_use_legacy", /*inc*/ 1, &[]);
						tx.send(AppEvent::BeginWindowsSandboxLegacySetup { preset: preset.clone() });
					}
				})],
				dismiss_on_select: true,
				..Default::default()
			...
			SelectionItem {
				...
				description: None,
				actions: vec![Box::new(move |tx| {
					quit_otel.counter("codex.windows_sandbox.fallback_prompt_quit", /*inc*/ 1, &[]);
					tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
				})],
				dismiss_on_select: true,
				..Default::default()
	...
	pub(crate) fn clear_windows_sandbox_setup_status(&mut self) {
		self.bottom_pane.set_composer_input_enabled(/*enabled*/ true, /*placeholder*/ None);
		self.bottom_pane.hide_status_indicator();
		self.request_redraw();
	}
	...
	pub(crate) fn set_feature_enabled(&mut self, feature: Feature, enabled: bool) -> bool {
		...
		if feature == Feature::RealtimeConversation {
			...
			self.bottom_pane.set_realtime_conversation_enabled(realtime_conversation_enabled);
			self
				.bottom_pane
		...
		#[cfg(target_os = "windows")]
		if matches!(feature, Feature::WindowsSandbox | Feature::WindowsSandboxElevated) {
			self.bottom_pane.set_windows_degraded_sandbox_active(
				codex_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
					&& matches!(
						WindowsSandboxLevel::from_config(&self.config),
	...
	pub(crate) fn world_writable_warning_hidden(&self) -> bool {
		self.config.notices.hide_world_writable_warning.unwrap_or(false)
	}
	...
	fn current_realtime_audio_selection_label(&self, kind: RealtimeAudioDeviceKind) -> String {
		self.current_realtime_audio_device_name(kind).unwrap_or_else(|| "System default".to_string())
	}

	fn sync_fast_command_enabled(&mut self) {
		self.bottom_pane.set_fast_command_enabled(self.fast_mode_enabled());
	}
	...
	fn sync_plugins_command_enabled(&mut self) {
		self.bottom_pane.set_plugins_command_enabled(self.config.features.enabled(Feature::Plugins));
	}
	...
	fn active_mode_kind(&self) -> ModeKind {
		self.active_collaboration_mask.as_ref().and_then(|mask| mask.mode).unwrap_or(ModeKind::Default)
	}
	...
	fn model_display_name(&self) -> &str {
		let model = self.current_model();
		if model.is_empty() { DEFAULT_MODEL_DISPLAY_NAME } else { model }
	}
	...
	fn collaboration_mode_label(&self) -> Option<&'static str> {
		...
		active_mode.is_tui_visible().then_some(active_mode.display_name())
	}
	...
	fn connectors_enabled(&self) -> bool {
		self.config.features.apps_enabled_cached(Some(self.auth_manager.as_ref()))
	}
	...
	fn apply_session_info_cell(&mut self, cell: history_cell::SessionInfoCell) {
		...
		let merged_header = if let Some(active) = self.active_cell.take() {
			if active.as_any().is::<history_cell::SessionHeaderHistoryCell>() {
	...
	pub(crate) fn add_mcp_output(&mut self) {
		let mcp_manager =
			McpManager::new(Arc::new(PluginsManager::new(self.config.codex_home.clone())));
		if mcp_manager.effective_servers(&self.config, /*auth*/ None).is_empty() {
	...
	fn open_connectors_loading_popup(&mut self) {
		if !self.bottom_pane.replace_selection_view_if_active(
			...
			self.bottom_pane.show_selection_view(self.connectors_loading_popup_params());
		}
	}
	...
	fn connectors_popup_params(
		...
		let installed = connectors.iter().filter(|connector| connector.is_accessible).count();
		let mut header = ColumnRenderable::new();
		header.push(Line::from("Apps".bold()));
		header.push(Line::from("Use $ to insert an installed app into your prompt.".dim()));
		header.push(Line::from(format!("Installed {installed} of {total} available apps.").dim()));
		let initial_selected_idx = selected_connector_id.and_then(|selected_connector_id| {
			connectors.iter().position(|connector| connector.id == selected_connector_id)
		});
	...
	fn refresh_connectors_popup_if_open(&mut self, connectors: &[connectors::AppInfo]) {
		let selected_connector_id =
			if let (Some(selected_index), ConnectorsCacheState::Ready(snapshot)) = (
				self.bottom_pane.selected_index_for_active_view(CONNECTORS_SELECTION_VIEW_ID),
				&self.connectors_cache,
			) {
				snapshot.connectors.get(selected_index).map(|connector| connector.id.as_str())
			} else {
				None
			};
	...
	fn connectors_popup_hint_line() -> Line<'static> {
		Line::from(vec!["Press ".into(), key_hint::plain(KeyCode::Esc).into(), " to close.".into()])
	}
	...
	fn connector_status_label(connector: &connectors::AppInfo) -> &'static str {
		if connector.is_accessible {
			if connector.is_enabled { "Installed" } else { "Installed · Disabled" }
		} else {
			"Can be installed"
		}
	}
	...
	fn quit_shortcut_active_for(&self, key: KeyBinding) -> bool {
		self.quit_shortcut_key == Some(key)
			&& self.quit_shortcut_expires_at.is_some_and(|expires_at| Instant::now() < expires_at)
	}
	...
	fn arm_quit_shortcut(&mut self, key: KeyBinding) {
		self.quit_shortcut_expires_at =
			Instant::now().checked_add(QUIT_SHORTCUT_TIMEOUT).or_else(|| Some(Instant::now()));
		self.quit_shortcut_key = Some(key);
	...
	pub(crate) fn submit_user_message_with_mode(
		...
		{
			self
				.add_error_message("Cannot switch collaboration mode while a turn is running.".to_string());
			return;
		}
	...
	pub(crate) fn set_composer_text(
		...
		self.bottom_pane.set_composer_text(text, text_elements, local_image_paths);
	}
	...
	pub(crate) fn queued_user_message_texts(&self) -> Vec<String> {
		self
			...
			.chain(self.queued_user_messages.iter().map(|message| message.text.clone()))
			.collect()
	}
		...
		if self.connectors_enabled() {
			...
			for tool in ev.tools.values() {
				...
				let Some(connector_id) =
					meta.get("connector_id").and_then(serde_json::Value::as_str).filter(|id| !id.is_empty())
				else {
					continue;
				};
				connectors_by_id.entry(connector_id.to_string()).or_insert_with(|| {
					let name = meta
						.get("connector_name")
			...
			self
				.bottom_pane
				.set_connectors_snapshot(Some(ConnectorsSnapshot { connectors: app_connectors }));
		}
	...
	pub(crate) fn on_connectors_loaded(
		...
		match result {
			Ok(mut snapshot) => {
				...
				snapshot.connectors = connectors::with_app_enabled_state(snapshot.connectors, &self.config);
			...
			Err(err) => {
				...
				if let ConnectorsCacheState::Ready(snapshot) = &self.connectors_cache {
					...
					self.bottom_pane.set_connectors_snapshot(Some(snapshot.clone()));
				} else if let Some(snapshot) = partial_snapshot {
					warn!("failed to load full apps list; falling back to installed apps snapshot: {err}");
					self.refresh_connectors_popup_if_open(&snapshot.connectors);
					self.connectors_cache = ConnectorsCacheState::Ready(snapshot.clone());
					self.bottom_pane.set_connectors_snapshot(Some(snapshot));
				} else {
	...
	pub(crate) async fn show_review_branch_picker(&mut self, cwd: &Path) {
		...
		let current_branch =
			current_branch_name(cwd).await.unwrap_or_else(|| "(detached HEAD)".to_string());
			...
			items.push(SelectionItem {
				...
				actions: vec![Box::new(move |tx3: &AppEventSender| {
					tx3.send(AppEvent::CodexOp(Op::Review {
						review_request: ReviewRequest {
							target: ReviewTarget::BaseBranch { branch: branch.clone() },
							user_facing_hint: None,
						},
					}));
				})],
		...
		for entry in commits {
			...
			items.push(SelectionItem {
				...
				actions: vec![Box::new(move |tx3: &AppEventSender| {
					tx3.send(AppEvent::CodexOp(Op::Review {
						review_request: ReviewRequest {
							target: ReviewTarget::Commit { sha: sha.clone(), title: Some(subject.clone()) },
							user_facing_hint: None,
						},
					}));
				})],
	...
	pub(crate) fn show_review_custom_prompt(&mut self) {
		...
		let view = CustomPromptView::new(
			...
			Box::new(move |prompt: String| {
				...
				tx.send(AppEvent::CodexOp(Op::Review {
					review_request: ReviewRequest {
						target: ReviewTarget::Custom { instructions: trimmed },
						user_facing_hint: None,
					},
				}));
			}),
	...
	pub(crate) fn token_usage(&self) -> TokenUsage {
		self.token_info.as_ref().map(|ti| ti.total_token_usage.clone()).unwrap_or_default()
	}
	...
	pub fn scroll_up(&self, lines: usize) {
		...
		tracing::debug!("Scroll up: {} -> {} (lines: {})", current, new_pos, lines);
		self.frame_requester.schedule_frame();
	}
	...
	pub fn scroll_down(&self, lines: usize) {
		...
		let new_pos = (current + lines).min(max_scroll);
		self.scroll_position.set(new_pos);
		tracing::debug!(
			"Scroll down: {} -> {} (lines: {}, max: {}, content: {}, viewport: {})",
			current,
			new_pos,
			lines,
			max_scroll,
...
impl Notification {
	fn display(&self) -> String {
		match self {
			Notification::AgentTurnComplete { response } => Notification::agent_turn_preview(response)
				.unwrap_or_else(|| "Agent turn complete".to_string()),
			Notification::ExecApprovalRequested { command } => {
				format!("Approval requested: {}", truncate_text(command, /*max_graphemes*/ 30))
			}
			...
			Notification::UserInputRequested { question_count, summary } => {
