...
impl App {
	...
	fn bootstrap(&mut self) -> anyhow::Result<Data> {
		...
		// Set up LLM status callback for toast notifications
		let status_tx = self.bridge.chat_state.llm_status_tx.clone();
		let mut llm = (*self.bridge.chat_state.llm).clone();
		llm.set_status_callback(move |message: String| {
		...
		tokio::spawn(async move {
			if let Err(e) = llm.initialize().await {
				...
				eprintln!("LLM initialization error: {}", e);
			}
		});
