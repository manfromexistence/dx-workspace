...
async fn run_session_picker(
	...
	let filter_cwd = if show_all { None } else { std::env::current_dir().ok() };
...
impl PickerState {
		...
		match key.code {
			...
			KeyCode::Enter => {
				if let Some(row) = self.filtered_rows.get(self.selected) {
					...
					self.inline_error =
						Some(format!("Failed to read session metadata from {}", path.display()));
					self.request_frame();
				}
			}
			KeyCode::Up => {
	...
	fn start_initial_load(&mut self) {
		...
		self.pagination.loading = LoadingState::Pending(PendingLoad { request_token, search_token });
		self.request_frame();
	...
	async fn handle_background_event(&mut self, event: BackgroundEvent) -> Result<()> {
		match event {
			BackgroundEvent::PageLoaded { request_token, search_token, page } => {
	...
	fn ingest_page(&mut self, page: ThreadsPage) {
		...
		self.pagination.num_scanned_files =
			self.pagination.num_scanned_files.saturating_add(page.num_scanned_files);
	...
	async fn update_thread_names(&mut self) {
		...
		let names = find_thread_names_by_ids(&self.codex_home, &missing_ids).await.unwrap_or_default();
	...
	fn apply_filter(&mut self) {
		let base_iter = self.all_rows.iter().filter(|row| self.row_matches_filter(row));
...
fn head_to_row(item: &ThreadItem) -> Row {
	...
	let updated_at = item.updated_at.as_deref().and_then(parse_timestamp_str).or(created_at);
...
fn paths_match(a: &Path, b: &Path) -> bool {
	if let (Ok(ca), Ok(cb)) =
		(path_utils::normalize_for_path_comparison(a), path_utils::normalize_for_path_comparison(b))
	{
		return ca == cb;
	}
	a == b
...
fn parse_timestamp_str(ts: &str) -> Option<DateTime<Utc>> {
	chrono::DateTime::parse_from_rfc3339(ts).map(|dt| dt.with_timezone(&Utc)).ok()
}
...
fn render_list(
		...
		rows[start..end].iter().zip(labels[start..end].iter()).enumerate()
	{
		...
		let branch_span = if !visibility.show_branch {
			...
			Some(Span::from(format!("{empty:<width$}", empty = "-", width = max_branch_width)).dim())
		} else {
		...
		let cwd_span = if !visibility.show_cwd {
			...
			Some(Span::from(format!("{empty:<width$}", empty = "-", width = max_cwd_width)).dim())
		} else {
...
fn human_time_ago(ts: DateTime<Utc>) -> String {
	...
	if secs < 60 {
		let n = secs.max(0);
		if n == 1 { format!("{n} second ago") } else { format!("{n} seconds ago") }
	} else if secs < 60 * 60 {
		let m = secs / 60;
		if m == 1 { format!("{m} minute ago") } else { format!("{m} minutes ago") }
	} else if secs < 60 * 60 * 24 {
		let h = secs / 3600;
		if h == 1 { format!("{h} hour ago") } else { format!("{h} hours ago") }
	} else {
		let d = secs / (60 * 60 * 24);
		if d == 1 { format!("{d} day ago") } else { format!("{d} days ago") }
	}
}
...
fn render_column_headers(
	...
	if visibility.show_created {
		let label = format!("{text:<width$}", text = "Created at", width = metrics.max_created_width);
		spans.push(Span::from(label).bold());
		spans.push("  ".into());
	}
	if visibility.show_updated {
		let label = format!("{text:<width$}", text = "Updated at", width = metrics.max_updated_width);
		spans.push(Span::from(label).bold());
		spans.push("  ".into());
	}
	if visibility.show_branch {
		let label = format!("{text:<width$}", text = "Branch", width = metrics.max_branch_width);
		spans.push(Span::from(label).bold());
		spans.push("  ".into());
	}
	if visibility.show_cwd {
		let label = format!("{text:<width$}", text = "CWD", width = metrics.max_cwd_width);
		spans.push(Span::from(label).bold());
		spans.push("  ".into());
	}
...
fn calculate_column_metrics(rows: &[Row], include_cwd: bool) -> ColumnMetrics {
	fn right_elide(s: &str, max: usize) -> String {
		...
		let tail_len = max - 1;
		let tail: String = s.chars().rev().take(tail_len).collect::<String>().chars().rev().collect();
		format!("…{tail}")
	}
	...
	ColumnMetrics { max_created_width, max_updated_width, max_branch_width, max_cwd_width, labels }
}
...
fn column_visibility(
	...
	let show_created =
		if show_both { metrics.max_created_width > 0 } else { sort_key == ThreadSortKey::CreatedAt };
	let show_updated =
		if show_both { metrics.max_updated_width > 0 } else { sort_key == ThreadSortKey::UpdatedAt };

	ColumnVisibility { show_created, show_updated, show_branch, show_cwd }
}
...
mod tests {
	...
	fn page(
		...
		ThreadsPage { items, next_cursor, num_scanned_files, reached_scan_cap }
	}
	...
	fn row_uses_tail_timestamp_for_updated_at() {
		...
		let expected_created =
			chrono::DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z").unwrap().with_timezone(&Utc);
		let expected_updated =
			chrono::DateTime::parse_from_rfc3339("2025-01-01T01:00:00Z").unwrap().with_timezone(&Utc);
	...
	fn resume_search_error_snapshot() {
		...
		state.inline_error =
			Some(String::from("Failed to read session metadata from /tmp/missing.jsonl"));
	...
	fn pageless_scrolling_deduplicates_and_keeps_order() {
		...
		state.ingest_page(page(
			...
			Some(cursor_from_str("2025-01-02T00-00-00|00000000-0000-0000-0000-000000000000")),
			2,
			false,
		));
		...
		state.ingest_page(page(
			...
			Some(cursor_from_str("2025-01-01T00-00-00|00000000-0000-0000-0000-000000000001")),
			2,
			false,
		));

		state.ingest_page(page(
			vec![make_item("/tmp/d.jsonl", "2024-12-31T23:00:00Z", "very old")],
			None,
			1,
			false,
		));

		let previews: Vec<_> = state.filtered_rows.iter().map(|row| row.preview.as_str()).collect();
	...
	fn ensure_minimum_rows_prefetches_when_underfilled() {
		...
		state.ingest_page(page(
			...
			Some(cursor_from_str("2025-01-03T00-00-00|00000000-0000-0000-0000-000000000000")),
			2,
			false,
		));
	...
	async fn page_navigation_uses_view_rows() {
		...
		state.handle_key(KeyEvent::new(KeyCode::PageDown, KeyModifiers::NONE)).await.unwrap();
		assert_eq!(state.selected, 5);

		state.handle_key(KeyEvent::new(KeyCode::PageDown, KeyModifiers::NONE)).await.unwrap();
		assert_eq!(state.selected, 10);

		state.handle_key(KeyEvent::new(KeyCode::PageUp, KeyModifiers::NONE)).await.unwrap();
		assert_eq!(state.selected, 5);
	}
	...
	async fn up_at_bottom_does_not_scroll_when_visible() {
		...
		state.handle_key(KeyEvent::new(KeyCode::Up, KeyModifiers::NONE)).await.unwrap();
	...
	async fn set_query_loads_until_match_and_respects_scan_cap() {
		...
		state.reset_pagination();
		state.ingest_page(page(
			vec![make_item("/tmp/start.jsonl", "2025-01-01T00:00:00Z", "alpha")],
			Some(cursor_from_str("2025-01-02T00-00-00|00000000-0000-0000-0000-000000000000")),
			1,
			false,
		));
		...
		state
			.handle_background_event(BackgroundEvent::PageLoaded {
				...
				page: Ok(page(
					...
					Some(cursor_from_str("2025-01-03T00-00-00|00000000-0000-0000-0000-000000000001")),
					5,
					false,
				)),
			})
		...
		state
			.handle_background_event(BackgroundEvent::PageLoaded {
				...
				page: Ok(page(
					vec![make_item("/tmp/match.jsonl", "2025-01-03T00:00:00Z", "target log")],
					Some(cursor_from_str("2025-01-04T00-00-00|00000000-0000-0000-0000-000000000002")),
					7,
					false,
				)),
			})
