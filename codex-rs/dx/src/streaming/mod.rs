...
impl StreamState {
	...
	pub(crate) fn step(&mut self) -> Vec<Line<'static>> {
		self.queued_lines.pop_front().map(|queued| queued.line).into_iter().collect()
	}
	...
	pub(crate) fn drain_n(&mut self, max_lines: usize) -> Vec<Line<'static>> {
		...
		self.queued_lines.drain(..end).map(|queued| queued.line).collect()
	}
	/// Drains all queued lines from the front of the queue.
	pub(crate) fn drain_all(&mut self) -> Vec<Line<'static>> {
		self.queued_lines.drain(..).map(|queued| queued.line).collect()
	}
	...
	pub(crate) fn oldest_queued_age(&self, now: Instant) -> Option<Duration> {
		self.queued_lines.front().map(|queued| now.saturating_duration_since(queued.enqueued_at))
	}
	/// Appends committed lines to the queue with a shared enqueue timestamp.
	pub(crate) fn enqueue(&mut self, lines: Vec<Line<'static>>) {
		let now = Instant::now();
		self.queued_lines.extend(lines.into_iter().map(|line| QueuedLine { line, enqueued_at: now }));
	}
}
