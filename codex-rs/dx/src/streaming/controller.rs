...
impl PlanStreamController {
	...
	/// render against the same session cwd that was active when streaming started.
	pub(crate) fn new(width: Option<usize>, cwd: &Path) -> Self {
		Self { state: StreamState::new(width, cwd), header_emitted: false, top_padding_emitted: false }
	}
	...
	pub(crate) fn on_commit_tick(&mut self) -> (Option<Box<dyn HistoryCell>>, bool) {
		let step = self.state.step();
		(self.emit(step, /*include_bottom_padding*/ false), self.state.is_idle())
	}
	...
	fn emit(
		...
		Some(Box::new(history_cell::new_proposed_plan_stream(out_lines, is_stream_continuation)))
	}
}
