...
// The standalone `codex-tui-dx` binary prints a short help message before the
...
mod voice {
	...
	pub(crate) enum RealtimeInputBehavior {
		Ungated,
		PlaybackAware { playback_queued_samples: Arc<AtomicUsize> },
	}
...
pub async fn run_main(
	...
	let (sandbox_mode, approval_policy) = if cli.full_auto {
		...
		(cli.sandbox_mode.map(Into::<SandboxMode>::into), cli.approval_policy.map(Into::into))
	};

	// Map the legacy --search flag to the canonical web_search mode.
	if cli.web_search {
		cli.config_overrides.raw_overrides.push("web_search=\"live\"".to_string());
	}
	...
	if let Err(err) =
		codex_core::personality_migration::maybe_migrate_personality(&codex_home, &config_toml).await
	{
	...
	let cloud_requirements =
		cloud_requirements_loader(cloud_auth_manager, chatgpt_base_url, codex_home.to_path_buf());

	let model_provider_override = if cli.oss {
		let resolved =
			resolve_oss_provider(cli.oss_provider.as_deref(), &config_toml, cli.config_profile.clone());
		...
		if let Some(provider) = resolved {
			...
			if provider == "__CANCELLED__" {
				return Err(std::io::Error::other("OSS provider selection was cancelled by user"));
			}
			Some(provider)
		}
	} else {
	...
	let config =
		load_config_or_exit(cli_kv_overrides.clone(), overrides.clone(), cloud_requirements.clone())
			.await;
	...
	let log_file = log_file_opts.open(log_dir.join("codex-tui-dx.log"))?;
	...
	let env_filter = || {
		EnvFilter::try_from_default_env().unwrap_or_else(|_| {
			EnvFilter::new("codex_core=info,codex_tui_dx=info,codex_rmcp_client=info")
		})
	};
	...
	if cli.oss && model_provider_override.is_some() {
		...
		let provider_id = match model_provider_override.as_ref() {
			...
			None => {
				...
				return Err(std::io::Error::other("OSS provider not set but oss flag was used"));
			}
		};
...
async fn run_ratatui_app(
	...
	let session_selection = if use_fork {
		if let Some(id_str) = cli.fork_session_id.as_deref() {
			...
			match path {
				Some(path) => {
					let thread_id =
						match resolve_session_thread_id(path.as_path(), is_uuid.then_some(id_str)).await {
							Some(thread_id) => thread_id,
							None => return missing_session_exit(id_str, "fork"),
						};
					resume_picker::SessionSelection::Fork(resume_picker::SessionTarget { path, thread_id })
				}
				None => return missing_session_exit(id_str, "fork"),
			}
			...
			.await
			{
				Ok(page) => match page.items.first() {
					Some(item) => {
						match resolve_session_thread_id(item.path.as_path(), /*id_str_if_uuid*/ None).await {
							Some(thread_id) => {
		...
		match path {
			Some(path) => {
				...
				resume_picker::SessionSelection::Resume(resume_picker::SessionTarget { path, thread_id })
			}
			None => return missing_session_exit(id_str, "resume"),
		}
	} else if cli.resume_last {
		let provider_filter = vec![config.model_provider_id.clone()];
		let filter_cwd = if cli.resume_show_all { None } else { Some(config.cwd.as_path()) };
	...
	if let Some(w) =
		crate::render::highlight::set_theme_override(config.tui_theme.clone(), find_codex_home().ok())
	{
	...
	let Cli { prompt, images, no_alt_screen, .. } = cli;
...
pub(crate) async fn resolve_session_thread_id(
	...
	match id_str_if_uuid {
		...
		None => read_session_meta_line(path).await.ok().map(|meta_line| meta_line.meta.id),
	}
}
...
mod tests {
	...
	async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
		ConfigBuilder::default().codex_home(temp_dir.path().to_path_buf()).build().await
	}
	...
	async fn windows_shows_trust_prompt_without_sandbox() -> std::io::Result<()> {
		...
		assert!(should_show, "Trust prompt should be shown when project trust is undecided");
		Ok(())
	}
	...
	async fn windows_shows_trust_prompt_with_sandbox() -> std::io::Result<()> {
		...
		if cfg!(target_os = "windows") {
			...
			assert!(should_show, "Non-Windows should still show trust prompt when project is untrusted");
		}
		Ok(())
	}
	#[tokio::test]
	async fn untrusted_project_skips_trust_prompt() -> std::io::Result<()> {
		...
		let temp_dir = TempDir::new()?;
		let mut config = build_config(&temp_dir).await?;
		config.active_project = ProjectConfig { trust_level: Some(TrustLevel::Untrusted) };
	...
	fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
		let model = config.model.clone().unwrap_or_else(|| "gpt-5.1".to_string());
		TurnContextItem {
			turn_id: None,
			trace_id: None,
			cwd,
	...
	async fn read_session_cwd_prefers_latest_turn_context() -> std::io::Result<()> {
		...
		let cwd =
			read_session_cwd(&config, ThreadId::new(), &rollout_path).await.expect("expected cwd");
	...
	async fn should_prompt_when_meta_matches_current_but_latest_turn_differs() -> std::io::Result<()>
	{
		...
		let rollout_path = temp_dir.path().join("rollout.jsonl");
		let session_meta = SessionMeta { cwd: current.clone(), ..SessionMeta::default() };
		let lines = vec![
			RolloutLine {
				timestamp: "t0".to_string(),
...
"#
		...
		let trusted_overrides = ConfigOverrides { cwd: Some(trusted.clone()), ..Default::default() };
		let trusted_config = ConfigBuilder::default()
			.codex_home(codex_home.clone())
			.harness_overrides(trusted_overrides.clone())
			.build()
			.await?;
		assert_eq!(trusted_config.permissions.approval_policy.value(), AskForApproval::OnRequest);

		let untrusted_overrides = ConfigOverrides { cwd: Some(untrusted), ..trusted_overrides };
		let untrusted_config = ConfigBuilder::default()
			.codex_home(codex_home)
			.harness_overrides(untrusted_overrides)
			.build()
			.await?;
		assert_eq!(untrusted_config.permissions.approval_policy.value(), AskForApproval::UnlessTrusted);
		Ok(())
	}
	...
	async fn read_session_cwd_falls_back_to_session_meta() -> std::io::Result<()> {
		...
		let rollout_path = temp_dir.path().join("rollout.jsonl");
		let session_meta = SessionMeta { cwd: session_cwd.clone(), ..SessionMeta::default() };
	...
	async fn read_session_cwd_prefers_sqlite_when_thread_id_present() -> std::io::Result<()> {
		...
		let runtime =
			codex_state::StateRuntime::init(config.codex_home.clone(), config.model_provider_id.clone())
				.await
				.map_err(std::io::Error::other)?;
		runtime.mark_backfill_complete(None).await.map_err(std::io::Error::other)?;
		...
		runtime.upsert_thread(&metadata).await.map_err(std::io::Error::other)?;

		let cwd = read_session_cwd(&config, thread_id, &rollout_path).await.expect("expected cwd");
