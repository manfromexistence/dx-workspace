...
fn experimental_tooltips() -> Vec<&'static str> {
	FEATURES.iter().filter_map(|spec| spec.stage.experimental_announcement()).collect()
}
...
fn pick_paid_tooltip<R: Rng + ?Sized>(rng: &mut R, fast_mode_enabled: bool) -> &'static str {
	if fast_mode_enabled || rng.random_bool(0.5) { paid_app_tooltip() } else { FAST_TOOLTIP }
}
...
pub(crate) mod announcement {
	...
	pub(crate) fn fetch_announcement_tip() -> Option<String> {
		ANNOUNCEMENT_TIP.get().cloned().flatten().and_then(|raw| parse_announcement_tip_toml(&raw))
	}
	...
	fn init_announcement_tip_in_thread() -> Option<String> {
		thread::spawn(blocking_init_announcement_tip).join().ok().flatten()
	}
	...
	fn blocking_init_announcement_tip() -> Option<String> {
		...
		let client = reqwest::blocking::Client::builder().no_proxy().build().ok()?;
		let response =
			client.get(ANNOUNCEMENT_TIP_URL).timeout(Duration::from_millis(2000)).send().ok()?;
	...
	impl AnnouncementTip {
		...
		fn version_matches(&self, version: &str) -> bool {
			self.version_regex.as_ref().is_none_or(|regex| regex.is_match(version))
		}
...
to_date = "2000-01-01"
		...
		assert_eq!(Some("latest match".to_string()), parse_announcement_tip_toml(toml));
...
content = "This is a test announcement"
		...
		assert_eq!(Some("This is a test announcement".to_string()), parse_announcement_tip_toml(toml));
	}
}
