...
impl AuthModeWidget {
	...
	fn move_highlight(&mut self, delta: isize) {
		...
		let current_index =
			options.iter().position(|option| *option == self.highlighted_mode).unwrap_or(0);
		let next_index = (current_index as isize + delta).rem_euclid(options.len() as isize) as usize;
	...
	fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
		let mut lines: Vec<Line> = vec![
			...
			Line::from(vec!["  ".into(), "or connect an API key for usage-based billing".into()]),
			"".into(),
		];
		...
		let create_mode_item = |idx: usize,
		 ...
		 -> Vec<Line<'static>> {
			...
			let line2 = if is_selected {
				Line::from(format!("     {description}")).fg(Color::Cyan).add_modifier(Modifier::DIM)
			} else {
		...
		for (idx, option) in self.displayed_sign_in_options().into_iter().enumerate() {
			match option {
				SignInOption::ChatGpt => {
					lines.extend(create_mode_item(idx, option, "Sign in with ChatGPT", chatgpt_description));
				}
				SignInOption::DeviceCode => {
	...
	fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
		...
		if self.animations_enabled {
			...
			self.request_frame.schedule_frame_in(std::time::Duration::from_millis(100));
			...
			&& !state.auth_url.is_empty()
		{
			...
			lines.push("".into());
			lines.push(Line::from(vec!["  ".into(), state.auth_url.as_str().cyan().underlined()]));
			lines.push("".into());
	...
	fn render_chatgpt_success(&self, area: Rect, buf: &mut Buffer) {
		let lines = vec!["✓ Signed in with your ChatGPT account".fg(Color::Green).into()];
	...
	fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
		let [intro_area, input_area, footer_area] =
			Layout::vertical([Constraint::Min(4), Constraint::Length(3), Constraint::Min(2)]).areas(area);

		let mut intro_lines: Vec<Line> = vec![
			Line::from(vec!["> ".into(), "Use your own OpenAI API key for usage-based billing".bold()]),
			"".into(),
		...
		if state.prepopulated_from_env {
			...
			intro_lines
				.push("  Paste a different key if you prefer to use another account.".dim().into());
			intro_lines.push("".into());
		}
		Paragraph::new(intro_lines).wrap(Wrap { trim: false }).render(intro_area, buf);
		...
		let mut footer_lines: Vec<Line> =
			vec!["  Press Enter to save".dim().into(), "  Press Esc to go back".dim().into()];
		if let Some(error) = &self.error {
			footer_lines.push("".into());
			footer_lines.push(error.as_str().red().into());
		}
		Paragraph::new(footer_lines).wrap(Wrap { trim: false }).render(footer_area, buf);
	}
...
mod tests {
	...
	fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
		...
		let widget = AuthModeWidget {
			...
			auth_manager: AuthManager::shared(codex_home_path, false, AuthCredentialsStoreMode::File),
	...
	fn api_key_flow_disabled_when_chatgpt_forced() {
		...
		assert!(matches!(&*widget.sign_in_state.read().unwrap(), SignInState::PickMode));
	}
	...
	fn mark_url_hyperlink_sanitizes_control_chars() {
		...
		assert!(sym.contains(sanitized), "symbol should contain sanitized URL, got: {sym:?}");
