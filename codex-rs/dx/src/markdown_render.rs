	...
	I: Iterator<Item = Event<'a>>,
{
	...
	fn text(&mut self, text: CowStr<'a>) {
		...
		if self.in_code_block && !self.needs_newline {
			let has_content =
				self.current_line_content.as_ref().map(|line| !line.spans.is_empty()).unwrap_or_else(
					|| self.text.lines.last().map(|line| !line.spans.is_empty()).unwrap_or(false),
				);
			if has_content {
				self.push_line(Line::default());
			}
	...
	fn start_item(&mut self) {
		...
		let marker = if let Some(last_index) = self.list_indices.last_mut() {
			match last_index {
				None => {
					Some(vec![Span::styled(" ".repeat(width - 1) + "- ", self.styles.unordered_list_marker)])
				}
				Some(index) => {
					*index += 1;
		...
		self.indent_stack.push(IndentContext::new(indent_prefix, marker, /*is_list*/ true));
		self.needs_newline = false;
	}
	...
	fn pop_link(&mut self) {
		if let Some(link) = self.link.take() {
			if link.show_destination {
				...
				let style = self.inline_styles.last().copied().unwrap_or_default().patch(self.styles.code);
	...
	fn suppressing_local_link_label(&self) -> bool {
		self.link.as_ref().and_then(|link| link.local_target_display.as_ref()).is_some()
	}
	...
	fn push_line(&mut self, line: Line<'static>) {
		self.flush_current_line();
		let blockquote_active =
			self.indent_stack.iter().any(|ctx| ctx.prefix.iter().any(|s| s.content.contains('>')));
		let style = if blockquote_active { self.styles.blockquote } else { line.style };
...
fn strip_local_path_prefix<'a>(path_text: &'a str, cwd_text: &str) -> Option<&'a str> {
	...
	path_text.strip_prefix(cwd_text).and_then(|rest| rest.strip_prefix('/'))
}
...
mod tests {
	...
	fn wraps_plain_text_when_width_provided() {
		...
		assert_eq!(
			lines,
			vec!["This is a simple".to_string(), "sentence that".to_string(), "should wrap.".to_string(),]
		);
	}
	...
	fn wraps_list_items_preserving_indent() {
		...
		let lines = lines_to_strings(&rendered);
		assert_eq!(lines, vec!["- first second".to_string(), "  third fourth".to_string(),]);
	}
	...
	fn does_not_wrap_code_blocks() {
		...
		let lines = lines_to_strings(&rendered);
		assert_eq!(lines, vec!["fn main() { println!(\"hi from a long line\"); }".to_string(),]);
	}
	...
	fn fenced_code_info_string_with_metadata_highlights() {
		...
		for info in &["rust,no_run", "rust no_run", "rust title=\"demo\""] {
			...
			assert!(has_rgb, "info string \"{info}\" should still produce syntax highlighting");
		}
	}
