		...
		match (key.code, key.modifiers) {
			...
			(KeyCode::Char('v'), KeyModifiers::CONTROL) => {
				...
				if let Ok(clipboard_content) = cli_clipboard::get_contents() {
					...
					// Preserve newlines - don't replace them
					// Normalize line endings to \n
					let sanitized = clipboard_content.replace("\r\n", "\n").replace('\r', "\n");
			...
			// Shift/Ctrl/Alt+Enter adds a newline (for multi-line input)
			// MUST come before plain Enter to avoid being shadowed
			(KeyCode::Enter, mods)
			...
			// Enter (without modifiers) submits the message
			(KeyCode::Enter, _) if !self.content.trim().is_empty() => {
...
pub enum InputAction {
	...
	NextHistory,
	Changed,
	Pasted, // New action to indicate paste occurred
}
