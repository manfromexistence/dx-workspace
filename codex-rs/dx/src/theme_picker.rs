...
const NARROW_PREVIEW_ROWS: [PreviewRow; 4] = [
	...
	PreviewRow { line_no: 13, kind: PreviewDiffKind::Added, code: "    format!(\"Hello, {name}!\")" },
	PreviewRow { line_no: 14, kind: PreviewDiffKind::Context, code: "}" },
];
...
const WIDE_PREVIEW_ROWS: [PreviewRow; 8] = [
	...
	PreviewRow { line_no: 35, kind: PreviewDiffKind::Added, code: "        .trim()" },
	PreviewRow { line_no: 36, kind: PreviewDiffKind::Context, code: "}" },
];
...
fn centered_offset(available: u16, content: u16, min_frame: u16) -> u16 {
	...
	let frame = if free >= min_frame.saturating_mul(2) { min_frame } else { 0 };
...
fn render_preview(
	...
	for (idx, row) in preview_rows.iter().enumerate() {
		...
		first_line.render(Rect::new(area.x.saturating_add(left_pad), y, render_width, 1), buf);
		y += 1;
	}
}
...
fn subtitle_available_width(terminal_width: Option<u16>) -> usize {
	...
	if let Some((list_width, _side_width)) =
		side_by_side_layout_widths(content_width, SideContentWidth::Half, WIDE_PREVIEW_MIN_WIDTH)
	{
		list_width as usize
	} else {
...
fn theme_picker_subtitle(codex_home: Option<&Path>, terminal_width: Option<u16>) -> String {
	...
	let themes_dir_display =
		themes_dir.as_deref().map(|path| format_directory_display(path, /*max_width*/ None));
...
pub(crate) fn build_theme_picker_params(
	...
	let items: Vec<SelectionItem> = entries
		...
		.map(|(idx, entry)| {
			let display_name =
				if entry.is_custom { format!("{} (custom)", entry.name) } else { entry.name.clone() };
			...
			SelectionItem {
				...
				actions: vec![Box::new(move |tx| {
					tx.send(AppEvent::SyntaxThemeSelected { name: name_for_action.clone() });
				})],
				..Default::default()
			}
		})
...
mod tests {
	...
	fn wide_preview_renders_all_lines_with_vertical_center_and_left_inset() {
		...
		let first_row = *numbered_rows.first().expect("expected at least one preview row");
		let last_row = *numbered_rows.last().expect("expected at least one preview row");
		assert!(first_row > 0, "expected top padding before centered preview");
		assert!(last_row < 19, "expected bottom padding after centered preview");
		...
		let markers: Vec<char> = lines.iter().filter_map(|line| preview_line_marker(line)).collect();
		assert!(markers.contains(&'+'), "expected wide preview to include at least one addition line");
		assert!(markers.contains(&'-'), "expected wide preview to include at least one removal line");
	}
	...
	fn narrow_preview_renders_single_add_and_single_remove_in_four_lines() {
		...
		let numbered_lines: Vec<usize> =
			lines.iter().filter_map(|line| preview_line_number(line)).collect();
		let markers: Vec<char> = lines.iter().filter_map(|line| preview_line_marker(line)).collect();
	...
	fn deleted_preview_code_uses_dim_overlay_like_real_diff_renderer() {
		...
		assert!(modifiers.contains(Modifier::DIM), "expected deleted preview code to be dimmed");
	}
	...
	fn unavailable_configured_theme_falls_back_to_configured_or_default_selection() {
		...
		let selected_idx =
			params.initial_selected_idx.expect("expected selected index for active fallback theme");
