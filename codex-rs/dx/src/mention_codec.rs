...
pub(crate) fn encode_history_mentions(text: &str, mentions: &[LinkedMention]) -> String {
	...
	for mention in mentions {
		mentions_by_name.entry(mention.mention.as_str()).or_default().push_back(mention.path.as_str());
	}
...
pub(crate) fn decode_history_mentions(text: &str) -> DecodedHistoryText {
	...
	while index < bytes.len() {
			...
			&& let Some((name, path, end_index)) = parse_history_linked_mention(text, bytes, index)
		{
			out.push(TOOL_MENTION_SIGIL);
			out.push_str(name);
			mentions.push(LinkedMention { mention: name.to_string(), path: path.to_string() });
			index = end_index;
			continue;
		}
	...
	DecodedHistoryText { text: out, mentions }
}
...
fn is_tool_path(path: &str) -> bool {
	path.starts_with("app://")
		...
		|| path.rsplit(['/', '\\']).next().is_some_and(|name| name.eq_ignore_ascii_case("SKILL.md"))
}
...
mod tests {
	...
	fn decode_history_mentions_restores_visible_tokens() {
		...
		assert_eq!(
			decoded.mentions,
			vec![
				LinkedMention { mention: "figma".to_string(), path: "app://figma-1".to_string() },
				LinkedMention { mention: "sample".to_string(), path: "plugin://sample@test".to_string() },
				LinkedMention { mention: "figma".to_string(), path: "/tmp/figma/SKILL.md".to_string() },
			]
		);
	}
	...
	fn decode_history_mentions_restores_plugin_links_with_at_sigil() {
		let decoded =
			decode_history_mentions("Use [@sample](plugin://sample@test) and [$figma](app://figma-1).");
	...
	fn encode_history_mentions_links_bound_mentions_in_order() {
		...
		let encoded = encode_history_mentions(
			text,
			&[
				LinkedMention { mention: "figma".to_string(), path: "app://figma-app".to_string() },
				LinkedMention { mention: "sample".to_string(), path: "plugin://sample@test".to_string() },
				LinkedMention { mention: "figma".to_string(), path: "/tmp/figma/SKILL.md".to_string() },
			],
		);
