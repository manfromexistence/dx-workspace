...
impl App {
	...
	pub(crate) fn apply_backtrack_rollback(&mut self, selection: BacktrackSelection) {
		...
		if self.backtrack.pending_rollback.is_some() {
			self.chat_widget.add_error_message("Backtrack rollback already in progress.".to_string());
			return;
		}
		...
		self.backtrack.pending_rollback =
			Some(PendingBacktrackRollback { selection, thread_id: self.chat_widget.thread_id() });
	...
	fn step_backtrack_and_highlight(&mut self, tui: &mut tui::Tui) {
		...
		let next_selection = if self.backtrack.nth_user_message == usize::MAX {
			...
			self.backtrack.nth_user_message.saturating_sub(1).min(last_index)
		};
	...
	fn step_forward_backtrack_and_highlight(&mut self, tui: &mut tui::Tui) {
		...
		let next_selection = if self.backtrack.nth_user_message == usize::MAX {
			...
			self.backtrack.nth_user_message.saturating_add(1).min(last_index)
		};
	...
	fn overlay_forward_event(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
			...
			&& let Some(Overlay::Transcript(t)) = &mut self.overlay
		{
			...
			{
				tui.frame_requester().schedule_frame_in(std::time::Duration::from_millis(50));
			}
			if close_overlay {
	...
	fn overlay_step_backtrack_forward(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
		if self.backtrack.base_id.is_some() {
	...
	fn sync_overlay_after_transcript_trim(&mut self) {
		...
		if self.backtrack.overlay_preview_active {
			...
			let next_selection = if total_users == 0 {
				...
				self.backtrack.nth_user_message.min(total_users.saturating_sub(1))
			};
			self.apply_backtrack_selection_internal(next_selection);
		}
...
fn nth_user_position(
	...
	user_positions_iter(cells).enumerate().find_map(|(i, idx)| (i == nth).then_some(idx))
}
...
fn user_positions_iter(
	...
	let start =
		cells.iter().rposition(|cell| type_of(cell) == session_start_type).map_or(0, |idx| idx + 1);
...
mod tests {
	...
	fn trim_transcript_for_first_user_drops_user_and_newer_cells() {
		let mut cells: Vec<Arc<dyn HistoryCell>> = vec![
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("assistant")], true)) as Arc<dyn HistoryCell>,
		];
	...
	fn trim_transcript_preserves_cells_before_selected_user() {
		let mut cells: Vec<Arc<dyn HistoryCell>> = vec![
			Arc::new(AgentMessageCell::new(vec![Line::from("intro")], true)) as Arc<dyn HistoryCell>,
			Arc::new(UserHistoryCell {
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("after")], false)) as Arc<dyn HistoryCell>,
		];
		trim_transcript_cells_to_nth_user(&mut cells, 0);

		assert_eq!(cells.len(), 1);
		let agent = cells[0].as_any().downcast_ref::<AgentMessageCell>().expect("agent cell");
		let agent_lines = agent.display_lines(u16::MAX);
		assert_eq!(agent_lines.len(), 1);
		let intro_text: String =
			agent_lines[0].spans.iter().map(|span| span.content.as_ref()).collect();
	...
	fn trim_transcript_for_later_user_keeps_prior_history() {
		let mut cells: Vec<Arc<dyn HistoryCell>> = vec![
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("between")], false)) as Arc<dyn HistoryCell>,
			Arc::new(UserHistoryCell {
			...
			Arc::new(AgentMessageCell::new(vec![Line::from("tail")], false)) as Arc<dyn HistoryCell>,
		];
		trim_transcript_cells_to_nth_user(&mut cells, 1);

		assert_eq!(cells.len(), 3);
		let agent_intro = cells[0].as_any().downcast_ref::<AgentMessageCell>().expect("intro agent");
		let intro_lines = agent_intro.display_lines(u16::MAX);
		let intro_text: String =
			intro_lines[0].spans.iter().map(|span| span.content.as_ref()).collect();
		assert_eq!(intro_text, "• intro");

		let user_first = cells[1].as_any().downcast_ref::<UserHistoryCell>().expect("first user");
		assert_eq!(user_first.message, "first");

		let agent_between =
			cells[2].as_any().downcast_ref::<AgentMessageCell>().expect("between agent");
		let between_lines = agent_between.display_lines(u16::MAX);
		let between_text: String =
			between_lines[0].spans.iter().map(|span| span.content.as_ref()).collect();
	...
	fn trim_drop_last_n_user_turns_applies_rollback_semantics() {
		...
		let first_user = cells[0].as_any().downcast_ref::<UserHistoryCell>().expect("first user");
	...
	fn trim_drop_last_n_user_turns_allows_overflow() {
		...
		assert!(changed);
		assert_eq!(cells.len(), 1);
		let intro = cells[0].as_any().downcast_ref::<AgentMessageCell>().expect("intro agent");
