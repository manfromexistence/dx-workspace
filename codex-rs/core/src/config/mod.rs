...
pub fn set_default_oss_provider(codex_home: &Path, provider: &str) -> std::io::Result<()> {
    ...
    // DX FORK: Relaxed validation - allow any provider that exists in built_in_model_providers
    let known_providers = built_in_model_providers(None);
...
impl Config {
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let model_provider_id = model_provider
            ...
            .unwrap_or_else(|| "openai".to_string()); // Changed from "local-llm" to "openai"
        let model_provider = model_providers
            .get(&model_provider_id)
            .ok_or_else(|| {
                let message = format!("Model provider `{model_provider_id}` not found");
