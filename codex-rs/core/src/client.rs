...
use crate::tools::spec::create_tools_json_for_chat_api;
...
impl ModelClientSession {
    ...
    fn build_chat_completions_request(
        ...
        // Chat Completions uses a different tool format:
        request.tools = create_tools_json_for_chat_api(&prompt.tools)?;
    ...
    /// Streams a turn via the Chat Completions API (OpenAI-compatible providers).
    #[allow(clippy::too_many_arguments)]
    #[instrument(
    ...
    pub async fn stream(
        ...
        match wire_api {
            ...
            WireApi::AnthropicMessages => Err(CodexErr::UnsupportedOperation(
                "Anthropic Messages API is not yet implemented in this fork".into(),
            )),
            WireApi::GeminiGenerateContent => Err(CodexErr::UnsupportedOperation(
