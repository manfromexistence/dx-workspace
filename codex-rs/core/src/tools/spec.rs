...
/// Returns JSON values that are compatible with OpenAI's Chat Completions API.
pub fn create_tools_json_for_chat_api(
    tools: &[ToolSpec],
) -> crate::error::Result<Vec<serde_json::Value>> {
    let mut tools_json = Vec::new();
    ...
    for tool in tools {
        match tool {
            ...
            ToolSpec::Freeform(tool) => {
                let syntax = &tool.format.syntax;
                let definition = &tool.format.definition;
                let combined_desc = format!(
                    "{}\n\nFormat:\n{}\n\nDocumentation:\n{}",
                    tool.description, syntax, definition
                );
                    ...
                    "function": {
                        ...
                        "parameters": {
                            ...
                            "properties": {
                                "input": {
                                    "type": "string",
                                    "description": "The command, query, or script input. Must follow the specified format.",
                                }
                            },
                            "required": ["input"],
                            "additionalProperties": false,
