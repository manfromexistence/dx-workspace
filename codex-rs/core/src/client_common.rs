...
impl ResponseStream {
    pub fn from_sse<S, F>(stream: S, transform: F) -> Self
    where
        S: Stream<Item = std::result::Result<Bytes, reqwest::Error>> + Send + Unpin + 'static,
        F: Fn(&str, &str) -> Vec<ResponseEvent> + Send + Sync + 'static,
    {
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = sse_stream.next().await {
                match event {
                    Ok(event) => {
                        for ev in transform(&event.event, &event.data) {
                    ...
                    Err(e) => {
                        let err = CodexErr::Stream(format!("SSE stream error: {}", e), None);
    ...
    pub fn from_gemini_sse<S, F>(stream: S, transform: F) -> Self
    where
        S: Stream<Item = std::result::Result<Bytes, reqwest::Error>> + Send + Unpin + 'static,
        F: Fn(&serde_json::Value) -> Vec<ResponseEvent> + Send + Sync + 'static,
    {
        let (tx, rx) = mpsc::channel(100);
        tokio::spawn(async move {
            let mut sse_stream = stream.eventsource();
            while let Some(event) = sse_stream.next().await {
                match event {
                    ...
                    Err(e) => {
                        let err = CodexErr::Stream(format!("Gemini SSE stream error: {}", e), None);
                        if tx.send(Err(err)).await.is_err() {
                            return;
                        }
                    }
