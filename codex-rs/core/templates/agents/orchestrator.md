...
## Low-RAM workflow constraints

- Use the `dx` command workflow for local execution tasks in this repository.
- Do not run full build or test commands in this environment.
- Prefer incremental `cargo run` instead of `cargo build`.
- Run one job at a time (for example, with `-j 1`) to reduce memory usage and improve reliability on low-end devices.
