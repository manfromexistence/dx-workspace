...
impl Renderable for Box<dyn HistoryCell> {
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		let y = if area.height == 0 {
			...
			let overflow = paragraph.line_count(area.width).saturating_sub(usize::from(area.height));
...
impl HistoryCell for UserHistoryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		let wrap_width = width
			.saturating_sub(LIVE_PREFIX_COLS + 1 /* keep a one-column right margin for wrapping */)
			.max(1);
		...
		let wrapped_remote_images = if self.remote_image_urls.is_empty() {
			...
			Some(adaptive_wrap_lines(
				self
					...
					.iter()
					.enumerate()
					.map(|(idx, _url)| remote_image_display_line(element_style, idx.saturating_add(1))),
		...
		if let Some(wrapped_remote_images) = wrapped_remote_images {
			lines.extend(prefix_lines(wrapped_remote_images, "  ".into(), "  ".into()));
		...
		if let Some(wrapped_message) = wrapped_message {
			lines.extend(prefix_lines(wrapped_message, "› ".bold().dim(), "  ".into()));
		}
...
impl ReasoningSummaryCell {
	...
	pub(crate) fn new(header: String, content: String, cwd: &Path, transcript_only: bool) -> Self {
		Self { _header: header, content, cwd: cwd.to_path_buf(), transcript_only }
	}
	...
	fn lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let summary_lines = lines
			.into_iter()
			.map(|mut line| {
				line.spans = line.spans.into_iter().map(|span| span.patch_style(summary_style)).collect();
				line
			})
			.collect::<Vec<_>>();
...
impl HistoryCell for ReasoningSummaryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		if self.transcript_only { Vec::new() } else { self.lines(width) }
	}
...
impl UpdateAvailableHistoryCell {
	pub(crate) fn new(latest_version: String, update_action: Option<UpdateAction>) -> Self {
		Self { latest_version, update_action }
	}
}
...
impl HistoryCell for UpdateAvailableHistoryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let content = text![
			...
			"https://github.com/openai/codex/releases/latest".cyan().underlined(),
		];

		let inner_width = content.width().min(usize::from(width.saturating_sub(4))).max(1);
...
impl HistoryCell for UnifiedExecInteractionCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let input_lines: Vec<Line<'static>> =
			self.stdin.lines().map(|line| Line::from(line.to_string())).collect();
	...
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		for process in &self.processes {
			...
			for (idx, chunk) in process.recent_chunks.iter().enumerate() {
				let chunk_prefix = if idx == 0 { chunk_prefix_first } else { chunk_prefix_next };
				...
				if !remainder.is_empty() && budget > truncation_suffix_width {
					...
					out.push(vec![chunk_prefix.dim(), shorter.dim(), truncation_suffix.dim()].into());
				} else {
...
pub fn new_approval_decision_cell(
	...
	let (symbol, summary): (Span<'static>, Vec<Span<'static>>) = match decision {
		...
		NetworkPolicyAmendment { network_policy_amendment } => match network_policy_amendment.action {
		...
		Denied => {
			...
			let summary = match actor {
				ApprovalDecisionActor::User => {
					vec![actor.subject().into(), "did not approve".bold(), " codex to run ".into(), snippet]
				}
				ApprovalDecisionActor::Guardian => {
					vec!["Request ".into(), "denied".bold(), " for codex to run ".into(), snippet]
				}
			};
			("✗ ".red(), summary)
		}
		Abort => {
			...
			(
				"✗ ".red(),
				vec![actor.subject().into(), "canceled".bold(), " the request to run ".into(), snippet],
			)
		}
	};

	Box::new(PrefixedWrappedHistoryCell::new(Line::from(summary), symbol, "  "))
}
...
pub fn new_guardian_denied_patch_request(
	...
	let mut summary = vec!["Request ".into(), "denied".bold(), " for codex to apply ".into()];
	if files.len() == 1 {
	...
	Box::new(PrefixedWrappedHistoryCell::new(Line::from(summary), "✗ ".red(), "  "))
}

pub fn new_guardian_denied_action_request(summary: String) -> Box<dyn HistoryCell> {
	let line =
		Line::from(vec!["Request ".into(), "denied".bold(), " for ".into(), Span::from(summary).dim()]);
...
pub(crate) fn new_review_status_line(message: String) -> PlainHistoryCell {
	PlainHistoryCell { lines: vec![Line::from(message.cyan())] }
}
...
fn with_border_internal(
	...
	let max_line_width = lines
		.iter()
		.map(|line| {
			line.iter().map(|span| UnicodeWidthStr::width(span.content.as_ref())).sum::<usize>()
		})
		.max()
		.unwrap_or(0);
	let content_width = forced_inner_width.unwrap_or(max_line_width).max(max_line_width);
	...
	for line in lines.into_iter() {
		let used_width: usize =
			line.iter().map(|span| UnicodeWidthStr::width(span.content.as_ref())).sum();
...
impl HistoryCell for TooltipHistoryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let wrap_width = usize::from(width.max(1)).saturating_sub(indent_width).max(1);
		let mut lines: Vec<Line<'static>> = Vec::new();
		append_markdown(
			&format!("**Tip:** {}", self.tip),
			Some(wrap_width),
...
pub(crate) fn new_session_info(
	...
	let SessionConfiguredEvent { model, reasoning_effort, .. } = event;
	...
	if is_first_event {
		...
		let help_lines: Vec<Line<'static>> = vec![
			"  To get started, describe a task or try one of these commands:".dim().into(),
			Line::from(""),
			Line::from(vec![
				"  ".into(),
				"/init".into(),
		...
		if config.show_tooltips
			&& let Some(tooltips) = tooltip_override
				.or_else(|| {
					tooltips::get_tooltip(auth_plan, matches!(config.service_tier, Some(ServiceTier::Fast)))
				})
...
pub(crate) fn new_user_prompt(
	...
	UserHistoryCell { message, text_elements, local_image_paths, remote_image_urls }
}
...
impl SessionHeaderHistoryCell {
	...
	pub(crate) fn new_with_style(
		...
		Self { version, model, model_style, reasoning_effort, show_fast_status, directory }
	}
...
impl HistoryCell for SessionHeaderHistoryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		// Title line rendered inside the box: ">_ OpenAI Codex codex-rs/tui/src (vX)"
		...
		let model_label =
			format!("{model_label:<label_width$}", model_label = "model:", label_width = label_width);
		...
		let lines =
			vec![make_row(title_spans), make_row(Vec::new()), make_row(model_spans), make_row(dir_spans)];
...
impl McpToolCallCell {
	pub(crate) fn new(call_id: String, invocation: McpInvocation, animations_enabled: bool) -> Self {
		Self {
			call_id,
			invocation,
...
impl HistoryCell for McpToolCallCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		if !detail_lines.is_empty() {
			let initial_prefix: Span<'static> =
				if inline_invocation { "  └ ".dim() } else { "    ".into() };
...
fn web_search_header(completed: bool) -> &'static str {
	if completed { "Searched" } else { "Searching the web" }
}
...
pub(crate) fn new_web_search_call(
	...
	let mut cell =
		WebSearchCell::new(call_id, query, Some(action), /*animations_enabled*/ false);
	cell.complete();
	cell
}
...
fn try_new_completed_mcp_tool_call_with_image_output(
	...
	let image = result.as_ref().ok()?.content.iter().find_map(decode_mcp_image)?;
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
	let lines: Vec<Line<'static>> = vec![
		...
		Line::from(vec![
			"    See the ".into(),
			"\u{1b}]8;;https://developers.openai.com/codex/mcp\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
...
pub(crate) fn new_mcp_tools_output(
	...
	for (server, cfg) in servers {
		...
		let auth_status =
			auth_statuses.get(server.as_str()).copied().unwrap_or(McpAuthStatus::Unsupported);
		...
		let server_templates: Vec<ResourceTemplate> =
			resource_templates.get(server.as_str()).cloned().unwrap_or_default();
...
impl HistoryCell for RequestUserInputResultCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let answered = self
			...
			.iter()
			.filter(|question| {
				self.answers.get(&question.id).is_some_and(|answer| !answer.answers.is_empty())
			})
			.count();
		...
		for question in &self.questions {
			...
			let mut question_lines =
				wrap_with_prefix(&question.question, width, "  • ".into(), "    ".into(), Style::default());
			...
			if let Some(note) = note {
				let (label, continuation, style) = if question.options.is_some() {
					("    note: ".dim(), "          ".dim(), Style::default().fg(Color::Cyan))
				} else {
					("    answer: ".dim(), "            ".dim(), Style::default().fg(Color::Cyan))
				};
...
pub(crate) fn new_proposed_plan(plan_markdown: String, cwd: &Path) -> ProposedPlanCell {
	ProposedPlanCell { plan_markdown, cwd: cwd.to_path_buf() }
}
...
impl HistoryCell for ProposedPlanCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		append_markdown(&self.plan_markdown, Some(wrap_width), Some(self.cwd.as_path()), &mut body);
		if body.is_empty() {
...
impl HistoryCell for PlanUpdateCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let note = self.explanation.as_ref().map(|s| s.trim()).filter(|t| !t.is_empty());
...
pub(crate) fn new_patch_event(
	...
	PatchHistoryCell { changes, cwd: cwd.to_path_buf() }
}
...
impl FinalMessageSeparator {
	...
	pub(crate) fn new(
		...
		Self { elapsed_seconds, runtime_metrics }
	}
}
impl HistoryCell for FinalMessageSeparator {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		vec![Line::from_iter([label, "─".repeat((width as usize).saturating_sub(label_width))]).dim()]
	}
}
...
pub(crate) fn runtime_metrics_label(summary: RuntimeMetricsSummary) -> Option<String> {
	...
	if summary.tool_calls.count > 0 {
		...
		parts.push(format!("Local tools: {} {calls} ({duration})", summary.tool_calls.count));
	}
	if summary.api_calls.count > 0 {
		...
		parts.push(format!("Inference: {} {calls} ({duration})", summary.api_calls.count));
	}
	if summary.websocket_calls.count > 0 {
		let duration = format_duration_ms(summary.websocket_calls.duration_ms);
		parts.push(format!("WebSocket: {} events send ({duration})", summary.websocket_calls.count));
	}
	if summary.streaming_events.count > 0 {
		...
		parts.push(format!("{stream_label}: {} {events} ({duration})", summary.streaming_events.count));
	}
	if summary.websocket_events.count > 0 {
		let duration = format_duration_ms(summary.websocket_events.duration_ms);
		parts.push(format!("{} events received ({duration})", summary.websocket_events.count));
	}
	...
	if summary.responses_api_engine_iapi_tbt_ms > 0 || summary.responses_api_engine_service_tbt_ms > 0
	{
...
mod tests {
	...
	fn unified_exec_interaction_cell_renders_input() {
		...
		assert_eq!(
			lines,
			vec!["↳ Interacted with background terminal · echo hello", "  └ ls", "    pwd",],
		);
	}
	...
	fn final_message_separator_hides_short_worked_label_and_includes_runtime_metrics() {
		let summary = RuntimeMetricsSummary {
			tool_calls: RuntimeMetricTotals { count: 3, duration_ms: 2_450 },
			api_calls: RuntimeMetricTotals { count: 2, duration_ms: 1_200 },
			streaming_events: RuntimeMetricTotals { count: 6, duration_ms: 900 },
			websocket_calls: RuntimeMetricTotals { count: 1, duration_ms: 700 },
			websocket_events: RuntimeMetricTotals { count: 4, duration_ms: 1_200 },
	...
	fn ps_output_chunk_leading_whitespace_snapshot() {
		let cell = new_unified_exec_processes_output(vec![UnifiedExecProcessDetails {
			...
			recent_chunks: vec!["  indented first".to_string(), "    more indented".to_string()],
		}]);
		let rendered = render_lines(&cell.display_lines(60)).join("\n");
		insta::assert_snapshot!(rendered);
	}
	...
	async fn mcp_tools_output_masks_sensitive_values() {
		...
		config.mcp_servers.set(servers).expect("test mcp servers should accept any configuration");
		...
		let cell = new_mcp_tools_output(&config, tools, HashMap::new(), HashMap::new(), &auth_statuses);
		let rendered = render_lines(&cell.display_lines(120)).join("\n");
	...
	fn prefixed_wrapped_history_cell_height_matches_wrapped_rendering() {
		...
		let cell: Box<dyn HistoryCell> =
			Box::new(PrefixedWrappedHistoryCell::new(Line::from(url_like), "✔ ".green(), "  "));
		...
		let first_row = (0..area.width)
			.map(|x| {
				...
				if symbol.is_empty() { ' ' } else { symbol.chars().next().unwrap_or(' ') }
			})
			.collect::<String>();
		assert!(
			first_row.contains("✔"),
	...
	fn unified_exec_interaction_cell_height_matches_wrapped_rendering() {
		let url_like = "example.test/api/v1/projects/alpha-team/releases/2026-02-17/builds/1234567890/artifacts/reports/performance/summary/detail/with/a/very/long/path";
		let cell: Box<dyn HistoryCell> =
			Box::new(UnifiedExecInteractionCell::new(Some("true".to_string()), url_like.to_string()));
	...
	fn web_search_history_cell_snapshot() {
		...
		let cell = new_web_search_call(
			"call-1".to_string(),
			query.clone(),
			WebSearchAction::Search { query: Some(query), queries: None },
		);
	...
	fn completed_mcp_tool_call_success_snapshot() {
		...
		assert!(cell.complete(Duration::from_millis(1420), Ok(result)).is_none());
	...
	fn completed_mcp_tool_call_image_after_text_returns_extra_cell() {
		...
		let result = CallToolResult {
			content: vec![text_block("Here is the image:"), image_block(SMALL_PNG_BASE64)],
			is_error: None,
			structured_content: None,
			meta: None,
		};
		...
		let extra_cell =
			cell.complete(Duration::from_millis(25), Ok(result)).expect("expected image cell");
	...
	fn completed_mcp_tool_call_error_snapshot() {
		...
		assert!(cell.complete(Duration::from_secs(2), Err("network timeout".into())).is_none());
	...
	fn completed_mcp_tool_call_multiple_outputs_snapshot() {
		...
		assert!(cell.complete(Duration::from_millis(640), Ok(result)).is_none());
	...
	fn completed_mcp_tool_call_wrapped_outputs_snapshot() {
		...
		assert!(cell.complete(Duration::from_millis(1280), Ok(result)).is_none());
	...
	fn completed_mcp_tool_call_multiple_outputs_inline_snapshot() {
		...
		assert!(cell.complete(Duration::from_millis(320), Ok(result)).is_none());
	...
	fn stderr_tail_more_than_five_lines_snapshot() {
		...
		cell.complete_call(
			&call_id,
			CommandOutput { exit_code: 1, formatted_output: String::new(), aggregated_output: stderr },
	...
	fn user_history_cell_height_matches_rendered_lines_with_remote_images() {
		...
		let width = 80;
		let rendered_len: u16 = cell.display_lines(width).len().try_into().unwrap_or(u16::MAX);
	...
	fn user_history_cell_trims_trailing_blank_message_lines() {
		...
		let trailing_blank_count =
			rendered.iter().rev().take_while(|line| line.trim().is_empty()).count();
	...
	fn user_history_cell_trims_trailing_blank_message_lines_with_text_elements() {
		...
		let cell = UserHistoryCell {
			message,
			text_elements: vec![TextElement::new((0..8).into(), Some("tokenized".to_string()))],
			local_image_paths: Vec::new(),
			remote_image_urls: vec!["https://example.com/one.png".to_string()],
		};
	...
	fn render_uses_wrapping_for_long_url_like_line() {
		...
		assert!(height > 1, "expected wrapped height for long URL, got {height}");
	...
	fn plan_update_without_note_snapshot() {
		let update = UpdatePlanArgs {
			explanation: None,
			plan: vec![
				PlanItemArg { step: "Define error taxonomy".into(), status: StepStatus::InProgress },
				PlanItemArg {
	...
	fn plan_update_does_not_split_url_like_tokens_in_note_or_step() {
		...
		let update = UpdatePlanArgs {
			explanation: Some(format!("Investigate failures under {note_url} immediately.")),
			plan: vec![PlanItemArg {
		...
		assert_eq!(
			rendered.iter().filter(|line| line.contains(note_url)).count(),
			1,
			"expected full note URL-like token in one rendered line, got: {rendered:?}"
		);
		assert_eq!(
			rendered.iter().filter(|line| line.contains(step_url)).count(),
			1,
	...
	fn deprecation_notice_renders_summary_with_details() {
		...
		assert_eq!(
			rendered,
			vec!["⚠ Feature flag `foo`".to_string(), "Use flag `bar` instead.".to_string(),]
		);
	}
}
