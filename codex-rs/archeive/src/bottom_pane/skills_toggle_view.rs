...
impl SkillsToggleView {
	pub(crate) fn new(items: Vec<SkillsToggleItem>, app_event_tx: AppEventSender) -> Self {
		...
		header.push(Line::from("Turn skills on or off. Your changes are saved automatically.".dim()));
	...
	fn toggle_selected(&mut self) {
		...
		self
			.app_event_tx
			.send(AppEvent::SetSkillEnabled { path: item.path.clone(), enabled: item.enabled });
	}
	...
	fn close(&mut self) {
		...
		self
			.app_event_tx
			.send(AppEvent::CodexOp(Op::ListSkills { cwds: Vec::new(), force_reload: true }));
	}
