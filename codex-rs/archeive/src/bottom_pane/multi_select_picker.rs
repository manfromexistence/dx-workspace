...
impl MultiSelectPicker {
		...
		self
			...
			.filter_map(|(visible_idx, actual_idx)| {
				self.items.get(*actual_idx).map(|item| {
					...
					GenericDisplayRow { name, description: item.description.clone(), ..Default::default() }
				})
			})
			.collect()
	}
	...
	fn confirm_selection(&mut self) {
		...
		if let Some(on_confirm) = &self.on_confirm {
			let selected_ids: Vec<String> =
				self.items.iter().filter(|item| item.enabled).map(|item| item.id.clone()).collect();
	...
	fn update_preview_line(&mut self) {
		self.preview_line = self.preview_builder.as_ref().and_then(|builder| builder(&self.items));
	}
	...
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		if search_area.height >= 2 {
			...
			let line = if self.search_query.is_empty() {
				...
				Line::from(vec![SEARCH_PROMPT_PREFIX.dim(), self.search_query.clone().into()])
			};
