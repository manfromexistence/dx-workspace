...
fn line_to_owned(line: Line<'_>) -> Line<'static> {
	Line {
		...
		spans: line
			.spans
			.into_iter()
			.map(|span| Span { style: span.style, content: Cow::Owned(span.content.into_owned()) })
			.collect(),
	}
}
...
impl Renderable for RequestUserInputOverlay {
	fn desired_height(&self, width: u16) -> u16 {
		...
		let options_height =
			if has_options { self.options_preferred_height(inner_width) as usize } else { 0 };
		let notes_visible = !has_options || self.notes_ui_visible();
		let notes_height =
			if notes_visible { self.notes_input_height(inner_width) as usize } else { 0 };
		...
		let spacer_rows = if has_options {
			if notes_visible { SPACER_ROWS_WITH_NOTES } else { DESIRED_SPACERS_BETWEEN_SECTIONS as usize }
		} else {
...
impl RequestUserInputOverlay {
	fn unanswered_confirmation_data(&self) -> UnansweredConfirmationData {
		...
		let subtitle =
			format!("{unanswered} unanswered question{}", if unanswered == 1 { "" } else { "s" });
	...
	fn unanswered_confirmation_layout(&self, width: u16) -> UnansweredConfirmationLayout {
		...
		let hint_lines =
			wrap_styled_line(&data.hint_line, content_width).into_iter().map(line_to_owned).collect();
		UnansweredConfirmationLayout { header_lines, hint_lines, rows: data.rows, state: data.state }
	}
	...
	fn unanswered_confirmation_height(&self, width: u16) -> u16 {
		...
		let height = layout.header_lines.len() as u16
			+ 1 + rows_height
			+ 1 + layout.hint_lines.len() as u16
	...
	fn render_unanswered_confirmation(&self, area: Rect, buf: &mut Buffer) {
		...
		for line in layout.header_lines {
			...
			Paragraph::new(line)
				.render(Rect { x: content_area.x, y: cursor_y, width: content_area.width, height: 1 }, buf);
		...
		let remaining = content_area.height.saturating_sub(cursor_y.saturating_sub(content_area.y));
		if remaining == 0 {
			return;
		}
		...
		let rows_area =
			Rect { x: content_area.x, y: cursor_y, width: content_area.width, height: rows_height };
		render_rows(
			rows_area,
			buf,
			&layout.rows,
		...
		for (offset, line) in layout.hint_lines.into_iter().enumerate() {
			...
			Paragraph::new(line)
				.render(Rect { x: content_area.x, y, width: content_area.width, height: 1 }, buf);
		}
	}
	...
	pub(super) fn render_ui(&self, area: Rect, buf: &mut Buffer) {
		...
		let answered = self.is_question_answered(self.current_index(), &self.composer.current_text());
		for (offset, line) in sections.question_lines.iter().enumerate() {
			...
			let question_line =
				if answered { Line::from(line.clone()) } else { Line::from(line.clone()).cyan() };
		...
		if self.has_options() {
			let mut options_state =
				self.current_answer().map(|answer| answer.options_state).unwrap_or_default();
		...
		let footer_y = sections.notes_area.y.saturating_add(sections.notes_area.height);
		...
		for (row_idx, tips) in tip_lines.into_iter().take(footer_area.height as usize).enumerate() {
	...
	fn render_notes_input(&self, area: Rect, buf: &mut Buffer) {
		...
		let is_secret = self.current_question().is_some_and(|question| question.is_secret);
		if is_secret {
...
fn render_rows_bottom_aligned(
	...
	let rendered_height =
		render_rows(scratch_area, &mut scratch, rows, state, max_results, empty_message);
...
fn truncate_line_word_boundary_with_ellipsis(
	...
	'outer: for (span_idx, span) in line.spans.iter().enumerate() {
		...
		for (byte_idx, ch) in text.char_indices() {
			...
			let bp = BreakPoint { span_idx, byte_end: byte_idx + ch.len_utf8() };
			last_fit = Some(bp);
			if ch.is_whitespace() {
	...
	while let Some(last) = spans_out.last_mut() {
		let trimmed = last.content.trim_end_matches(char::is_whitespace).to_string();
		if trimmed.is_empty() {
			spans_out.pop();
		} else {
	...
	let ellipsis_style = spans_out.last().map(|span| span.style).unwrap_or(line_style);
