...
impl ApprovalOverlay {
	...
	fn build_options(
		...
		let (options, title) = match request {
			...
			ApprovalRequest::Permissions { .. } => {
				(permissions_options(), "Would you like to grant these permissions?".to_string())
			}
			ApprovalRequest::ApplyPatch { .. } => {
				(patch_options(), "Would you like to make the following edits?".to_string())
			}
			ApprovalRequest::McpElicitation { server_name, .. } => {
				(elicitation_options(), format!("{server_name} needs your approval."))
			}
		};
	...
	fn apply_selection(&mut self, actual_idx: usize) {
		...
		if let Some(request) = self.current_request.as_ref() {
			match (request, &option.decision) {
				...
				(
					ApprovalRequest::Permissions { call_id, permissions, .. },
				...
				(
					ApprovalRequest::McpElicitation { server_name, request_id, .. },
	...
	fn handle_patch_decision(&self, id: &str, decision: ReviewDecision) {
		let Some(thread_id) = self.current_request.as_ref().map(ApprovalRequest::thread_id) else {
			return;
		};
		self.app_event_tx.send(AppEvent::SubmitThreadOp {
			thread_id,
			op: Op::PatchApproval { id: id.to_string(), decision },
		});
	}
	...
	fn try_handle_shortcut(&mut self, key_event: &KeyEvent) -> bool {
		match key_event {
			KeyEvent { kind: KeyEventKind::Press, code: KeyCode::Char('a'), modifiers, .. }
			...
			KeyEvent { kind: KeyEventKind::Press, code: KeyCode::Char('o'), .. } => {
				if let Some(request) = self.current_request.as_ref() {
			...
			e => {
				if let Some(idx) =
					self.options.iter().position(|opt| opt.shortcuts().any(|s| s.is_press(*e)))
				{
...
fn build_header(request: &ApprovalRequest) -> Box<dyn Renderable> {
	match request {
		ApprovalRequest::Exec {
			...
			{
				header.push(Line::from(vec!["Permission rule: ".into(), rule_line.cyan()]));
				header.push(Line::from(""));
			}
		...
		ApprovalRequest::ApplyPatch { thread_label, reason, cwd, changes, .. } => {
			...
			if let Some(thread_label) = thread_label {
				header.push(Box::new(Line::from(vec!["Thread: ".into(), thread_label.clone().bold()])));
			...
			{
				header.push(Box::new(
					Paragraph::new(Line::from_iter(["Reason: ".into(), reason.clone().italic()]))
		...
		ApprovalRequest::McpElicitation { thread_label, server_name, message, .. } => {
			...
			if let Some(thread_label) = thread_label {
				lines.push(Line::from(vec!["Thread: ".into(), thread_label.clone().bold()]));
...
impl ApprovalOption {
	fn shortcuts(&self) -> impl Iterator<Item = KeyBinding> + '_ {
		self.display_shortcut.into_iter().chain(self.additional_shortcuts.iter().copied())
	}
}
			...
			ReviewDecision::NetworkPolicyAmendment { network_policy_amendment } => {
				let (label, shortcut) = match network_policy_amendment.action {
					NetworkPolicyRuleAction::Allow => {
						("Yes, and allow this host in the future".to_string(), KeyCode::Char('p'))
					}
					NetworkPolicyRuleAction::Deny => {
						("No, and block this host in the future".to_string(), KeyCode::Char('d'))
					}
				};
				Some(ApprovalOption {
					label,
...
pub(crate) fn format_additional_permissions_rule(
	...
	if additional_permissions.network.as_ref().and_then(|network| network.enabled).unwrap_or(false) {
		parts.push("network".to_string());
	}
	if let Some(file_system) = additional_permissions.file_system.as_ref() {
		if let Some(read) = file_system.read.as_ref() {
			let reads =
				read.iter().map(|path| format!("`{}`", path.display())).collect::<Vec<_>>().join(", ");
			parts.push(format!("read {reads}"));
		}
		if let Some(write) = file_system.write.as_ref() {
			let writes =
				write.iter().map(|path| format!("`{}`", path.display())).collect::<Vec<_>>().join(", ");
...
mod tests {
	...
	fn o_opens_source_thread_for_cross_thread_approval() {
		...
		assert_eq!(matches!(event, AppEvent::SelectAgentThread(id) if id == thread_id), true);
	}
	...
	fn cross_thread_footer_hint_mentions_o_shortcut() {
		...
		assert_snapshot!("approval_overlay_cross_thread_prompt", render_overlay_lines(&view, 80));
	}
	...
	fn exec_prefix_option_emits_execpolicy_amendment() {
		...
		let mut saw_op = false;
		while let Ok(ev) = rx.try_recv() {
			if let AppEvent::SubmitThreadOp { op: Op::ExecApproval { decision, .. }, .. } = ev {
				assert_eq!(
					decision,
					ReviewDecision::ApprovedExecpolicyAmendment {
						proposed_execpolicy_amendment: ExecPolicyAmendment::new(vec!["echo".to_string()])
		...
		assert!(saw_op, "expected approval decision to emit an op with command prefix");
	}
	...
	fn header_includes_command_snippet() {
		...
		let rendered: Vec<String> = (0..buf.area.height)
			.map(|row| (0..buf.area.width).map(|col| buf[(col, row)].symbol().to_string()).collect())
			.collect();
		assert!(
			rendered.iter().any(|line| line.contains("echo hello world")),
	...
	fn generic_exec_options_can_offer_allow_for_session() {
		let options = exec_options(
			&[ReviewDecision::Approved, ReviewDecision::ApprovedForSession, ReviewDecision::Abort],
			None,
			None,
		);
	...
	fn additional_permissions_exec_options_hide_execpolicy_amendment() {
		...
		assert_eq!(
			labels,
			vec!["Yes, proceed".to_string(), "No, and tell Codex what to do differently".to_string(),]
		);
	}
	...
	fn permissions_options_use_expected_labels() {
		let labels: Vec<String> =
			permissions_options().into_iter().map(|option| option.label).collect();
		assert_eq!(
			labels,
			vec![
	...
	fn permissions_session_shortcut_submits_session_scope() {
		...
		assert!(saw_op, "expected permission approval decision to emit a session-scoped response");
	}
	...
	fn additional_permissions_prompt_shows_permission_rule_line() {
		...
		assert!(
			rendered.iter().any(|line| line.contains("Permission rule:")),
	...
	fn network_exec_prompt_title_includes_host() {
		...
		assert!(
			rendered
				.iter()
				.any(|line| { line.contains("Do you want to approve network access to \"example.com\"?") }),
	...
	fn enter_sets_last_selected_index_without_dismissing() {
		...
		while let Ok(ev) = rx.try_recv() {
			if let AppEvent::SubmitThreadOp { op: Op::ExecApproval { decision: d, .. }, .. } = ev {
				decision = Some(d);
				break;
			}
		}
