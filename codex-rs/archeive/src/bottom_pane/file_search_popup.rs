...
impl FileSearchPopup {
	...
	pub(crate) fn selected_match(&self) -> Option<&PathBuf> {
		self.state.selected_idx.and_then(|idx| self.matches.get(idx)).map(|file_match| &file_match.path)
	}
		...
		let rows_all: Vec<GenericDisplayRow> = if self.matches.is_empty() {
			...
			self
				...
				.map(|m| GenericDisplayRow {
					...
					match_indices: m.indices.as_ref().map(|v| v.iter().map(|&i| i as usize).collect()),
...
mod tests {
	...
	fn set_matches_keeps_only_the_first_page_of_results() {
		...
		assert_eq!(popup.matches, (0..MAX_POPUP_ROWS).map(file_match).collect::<Vec<_>>());
