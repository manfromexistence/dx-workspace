...
impl SkillPopup {
	pub(crate) fn new(mentions: Vec<MentionItem>) -> Self {
		Self { query: String::new(), mentions, state: ScrollState::new() }
	}
	...
	fn rows_from_matches(
		...
		matches
			...
			.map(|(idx, indices, _score)| {
				...
				let description = match (mention.category_tag.as_deref(), mention.description.as_deref()) {
					(Some(tag), Some(description)) if !description.is_empty() => {
						Some(format!("{tag} {description}"))
					}
					(Some(tag), _) => Some(tag.to_string()),
					(None, Some(description)) if !description.is_empty() => Some(description.to_string()),
					_ => None,
				};
				GenericDisplayRow {
					name,
...
mod tests {
	...
	fn filtered_mentions_are_capped_to_max_popup_rows() {
		...
		assert_eq!(
			filtered_names,
			(0..MAX_POPUP_ROWS).map(|idx| format!("Mention {idx:02}")).collect::<Vec<_>>()
		);
		assert_eq!(popup.calculate_required_height(72), (MAX_POPUP_ROWS as u16) + 2);
	}
}
