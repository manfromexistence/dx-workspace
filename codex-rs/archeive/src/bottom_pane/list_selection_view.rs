...
impl ListSelectionView {
	...
	pub fn new(params: SelectionViewParams, app_event_tx: AppEventSender) -> Self {
		...
		if params.title.is_some() || params.subtitle.is_some() {
			...
			header = Box::new(ColumnRenderable::with([header, Box::new(title), Box::new(subtitle)]));
		}
		let mut s = Self {
			...
			search_placeholder: if params.is_searchable { params.search_placeholder } else { None },
	...
	fn selected_actual_idx(&self) -> Option<usize> {
		self.state.selected_idx.and_then(|visible_idx| self.filtered_indices.get(visible_idx).copied())
	}
	...
	fn apply_filter(&mut self) {
		let previously_selected = self
			.selected_actual_idx()
			.or_else(|| {
				(!self.is_searchable).then(|| self.items.iter().position(|item| item.is_current)).flatten()
			})
		...
		if self.is_searchable && !self.search_query.is_empty() {
			...
			self.filtered_indices = self
				...
				.iter()
				.positions(|item| {
					item.search_value.as_ref().is_some_and(|v| v.to_lowercase().contains(&query_lower))
				})
				.collect();
		} else {
	...
	fn clear_to_terminal_bg(buf: &mut Buffer, area: Rect) {
		...
		let max_x = area.x.saturating_add(area.width).min(buf_area.x.saturating_add(buf_area.width));
		let max_y = area.y.saturating_add(area.height).min(buf_area.y.saturating_add(buf_area.height));
		for y in min_y..max_y {
			for x in min_x..max_x {
				buf[(x, y)].set_symbol(" ").set_style(ratatui::style::Style::reset());
			}
		}
	}
	...
	fn stacked_side_content(&self) -> &dyn Renderable {
		self.stacked_side_content.as_deref().unwrap_or_else(|| self.side_content.as_ref())
	}
	...
	fn side_layout_width(&self, content_width: u16) -> Option<u16> {
		side_by_side_layout_widths(content_width, self.side_content_width, self.side_content_min_width)
...
impl Renderable for ListSelectionView {
	...
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		let note_lines = self.footer_note.as_ref().map(|note| wrap_styled_line(note, note_width));
		...
		let stacked_side_h =
			if side_w.is_none() { self.stacked_side_content().desired_height(inner_width) } else { 0 };
		...
		if let Some(sw) = side_w {
			...
			let clear_w =
				outer_content_area.x.saturating_add(outer_content_area.width).saturating_sub(clear_x);
			Self::clear_to_terminal_bg(
				buf,
				Rect::new(clear_x, outer_content_area.y, clear_w, outer_content_area.height),
			);
			...
			let clear_height =
				(outer_content_area.y + outer_content_area.height).saturating_sub(stacked_side_area.y);
		...
		if footer_area.height > 0 {
			...
			if let Some(lines) = note_lines {
				...
				for (idx, line) in lines.iter().enumerate() {
					...
					let line_area =
						Rect { x: note_area.x, y: note_area.y + idx as u16, width: note_area.width, height: 1 };
...
mod tests {
	...
	fn description_col(rendered: &str, item_marker: &str, description: &str) -> usize {
		...
		line.find(description).expect("expected rendered line to contain description")
	}
	...
	fn renders_blank_line_between_title_and_items_without_subtitle() {
		...
		assert_snapshot!("list_selection_spacing_without_subtitle", render_lines(&view));
	}
	...
	fn snapshot_footer_note_wraps() {
		...
		assert_snapshot!("list_selection_footer_note_wraps", render_lines_with_width(&view, 40));
	}
	...
	fn wraps_long_option_without_overflowing_columns() {
		...
		let view = ListSelectionView::new(
			SelectionViewParams { title: Some("Approval".to_string()), items, ..Default::default() },
			tx,
		);
	...
	fn width_changes_do_not_hide_rows() {
		...
		assert!(missing.is_empty(), "third option missing at widths {missing:?}");
	}
	...
	fn narrow_width_keeps_all_rows_visible() {
		...
		let items: Vec<SelectionItem> = (1..=3)
			...
			.collect();
		let view = ListSelectionView::new(
			SelectionViewParams { title: Some("Debug".to_string()), items, ..Default::default() },
			tx,
		);
		let rendered = render_lines_with_width(&view, 24);
		assert!(rendered.contains("3."), "third option missing for width 24:\n{rendered}");
	}
	...
	fn snapshot_model_picker_width_80() {
		...
		assert_snapshot!("list_selection_model_picker_width_80", render_lines_with_width(&view, 80));
	}
	...
	fn side_layout_width_half_uses_exact_split() {
		...
		let view = ListSelectionView::new(
			SelectionViewParams {
				...
				side_content: Box::new(MarkerRenderable { marker: "W", height: 1 }),
				side_content_width: SideContentWidth::Half,
				side_content_min_width: 10,
				..Default::default()
			},
	...
	fn stacked_side_content_is_used_when_side_by_side_does_not_fit() {
		...
		let view = ListSelectionView::new(
			SelectionViewParams {
				...
				side_content: Box::new(MarkerRenderable { marker: "W", height: 1 }),
				stacked_side_content: Some(Box::new(MarkerRenderable { marker: "N", height: 1 })),
				side_content_width: SideContentWidth::Half,
		...
		assert!(rendered.contains('N'), "expected stacked marker to be rendered:\n{rendered}");
		assert!(!rendered.contains('W'), "wide marker should not render in stacked mode:\n{rendered}");
	}
	...
	fn side_content_clearing_resets_symbols_and_style() {
		...
		for y in 0..height {
			for x in 0..width {
				buf[(x, y)].set_symbol("X").set_style(Style::default().bg(Color::Red));
			}
		}
		view.render(area, &mut buf);
		...
		assert!(saw_marker, "expected side marker renderable to draw into buffer");
	}
