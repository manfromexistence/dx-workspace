...
- Use the `dx` command workflow for local execution in this repository.
- Do not run full build or test commands on this machine.
- Prefer incremental `cargo run` instead of `cargo build`.
- Run one job at a time (for example, with `-j 1`) to keep memory usage low.
