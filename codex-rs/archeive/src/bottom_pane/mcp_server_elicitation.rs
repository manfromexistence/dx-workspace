...
impl McpServerElicitationFormRequest {
	pub(crate) fn from_event(thread_id: ThreadId, request: ElicitationRequestEvent) -> Option<Self> {
		let ElicitationRequest::Form { meta, message, requested_schema } = request.request else {
			return None;
		};
			...
			&& (requested_schema.is_null() || is_empty_object_schema)
		{
			...
			(
				...
				vec![McpServerElicitationField {
					...
					prompt: String::new(),
					required: true,
					input: McpServerElicitationFieldInput::Select { options, default_idx: Some(0) },
				}],
			)
		} else {
			(McpServerElicitationResponseMode::FormContent, parse_fields_from_schema(&requested_schema)?)
		};
...
fn parse_tool_suggestion_request(meta: Option<&Value>) -> Option<ToolSuggestionRequest> {
	...
	let suggest_type = match meta.get(TOOL_SUGGEST_SUGGEST_TYPE_KEY).and_then(Value::as_str) {
	...
	Some(ToolSuggestionRequest {
		tool_type,
		suggest_type,
		suggest_reason: meta.get(TOOL_SUGGEST_REASON_KEY).and_then(Value::as_str)?.to_string(),
...
fn tool_approval_supports_persist_mode(meta: Option<&Value>, expected_mode: &str) -> bool {
	let Some(persist) =
		meta.and_then(Value::as_object).and_then(|meta| meta.get(APPROVAL_PERSIST_KEY))
	else {
		return false;
	};
	...
	match persist {
		...
		Value::Array(values) => {
			values.iter().filter_map(Value::as_str).any(|value| value == expected_mode)
		}
		_ => false,
	}
}
...
fn parse_tool_approval_display_params(meta: Option<&Value>) -> Vec<McpToolApprovalDisplayParam> {
	...
	let display_params = meta
		...
		.map(|display_params| {
			display_params.iter().filter_map(parse_tool_approval_display_param).collect::<Vec<_>>()
		})
		.unwrap_or_default();
...
fn parse_tool_approval_display_param(value: &Value) -> Option<McpToolApprovalDisplayParam> {
	...
	let display_name = value.get("display_name").and_then(Value::as_str).unwrap_or(name).trim();
...
fn format_tool_approval_display_param_line(param: &McpToolApprovalDisplayParam) -> String {
	format!("{}: {}", param.display_name, format_tool_approval_display_param_value(&param.value))
}
...
fn parse_fields_from_schema(requested_schema: &Value) -> Option<Vec<McpServerElicitationField>> {
	...
	for (id, property_schema) in properties {
		let property =
			serde_json::from_value::<McpElicitationPrimitiveSchema>(property_schema.clone()).ok()?;
		...
		McpElicitationPrimitiveSchema::Enum(McpElicitationEnumSchema::Legacy(schema)) => {
			...
			let options = schema
				...
				.map(|(idx, value)| McpServerElicitationOption {
					label: enum_names.get(idx).cloned().unwrap_or_else(|| value.clone()),
					description: None,
	...
	match schema {
		...
		McpElicitationSingleSelectEnumSchema::Titled(schema) => {
			...
			let default_idx = schema
				.default
				.as_ref()
				.and_then(|value| schema.one_of.iter().position(|entry| entry.const_.as_str() == value));
			let options = schema
				.one_of
				.into_iter()
...
impl McpServerElicitationOverlay {
	...
	fn reset_for_request(&mut self) {
		self.answers = self
			...
			.map(|field| {
				...
				McpServerElicitationAnswerState { selection, draft, answer_committed }
			})
			.collect();
		self.current_idx = 0;
	...
	fn restore_current_draft(&mut self) {
		self.composer.set_placeholder_text(self.answer_placeholder().to_string());
	...
	fn answer_placeholder(&self) -> &'static str {
		self.current_field().map_or(ANSWER_PLACEHOLDER, |field| {
			if field.required { ANSWER_PLACEHOLDER } else { OPTIONAL_ANSWER_PLACEHOLDER }
		})
	}
	...
	fn selected_option_index(&self) -> Option<usize> {
		self.current_answer().and_then(|answer| answer.selection.selected_idx)
	}
	...
	fn option_rows(&self) -> Vec<GenericDisplayRow> {
		...
		self
			...
			.enumerate()
			.map(|(idx, option)| {
				let prefix = if selected_idx.is_some_and(|selected| selected == idx) { '›' } else { ' ' };
				let number = idx + 1;
	...
	fn current_prompt_text(&self) -> String {
		...
		{
			if field.prompt.trim().is_empty() { field.label.clone() } else { field.prompt.clone() }
		} else {
			format!("{}\n{}", field.label, field.prompt)
		};
	...
	fn submit_answers(&mut self) {
		...
		if self.request.response_mode == McpServerElicitationResponseMode::ApprovalAction {
			let (decision, meta) = match self.field_value(/*idx*/ 0).as_ref().and_then(Value::as_str) {
	...
	fn render_prompt(&self, area: Rect, buf: &mut Buffer) {
		...
		for (offset, line) in self.wrapped_prompt_lines(area.width).iter().enumerate() {
			...
			let line = if answered { Line::from(line.clone()) } else { Line::from(line.clone()).cyan() };
	...
	fn render_footer(&self, area: Rect, input_area_height: u16, buf: &mut Buffer) {
		...
		for (row_idx, tips) in tip_lines.into_iter().take(area.height as usize).enumerate() {
			...
			Paragraph::new(line).render(
				Rect { x: area.x, y: area.y.saturating_add(row_idx as u16), width: area.width, height: 1 },
				buf,
			);
		}
	}
...
impl Renderable for McpServerElicitationOverlay {
	...
	fn cursor_pos(&self, area: Rect) -> Option<(u16, u16)> {
		...
		let input_area = Rect {
			x: content_area.x,
			y: content_area.y.saturating_add(1).saturating_add(prompt_height),
			width: content_area.width,
			height: input_height,
		};
...
mod tests {
	...
	fn form_request(
		...
		ElicitationRequestEvent {
			...
			request: ElicitationRequest::Form { meta, message: message.to_string(), requested_schema },
		}
	}
	...
	fn tool_approval_meta(
		...
		if !persist_modes.is_empty() {
			meta.insert(
				...
				Value::Array(persist_modes.iter().map(|mode| Value::String((*mode).to_string())).collect()),
			);
		}
	...
	fn submit_sends_accept_with_typed_content() {
		...
		let AppEvent::SubmitThreadOp { thread_id: resolved_thread_id, op } = event else {
