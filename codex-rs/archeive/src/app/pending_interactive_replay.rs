...
impl PendingInteractiveReplayState {
	...
	pub(super) fn note_outbound_op(&mut self, op: &Op) {
		match op {
			...
			Op::PatchApproval { id, .. } => {
				...
				Self::remove_call_id_from_turn_map(&mut self.patch_approval_call_ids_by_turn_id, id);
			}
			Op::ResolveElicitation { server_name, request_id, .. } => {
				self
					.elicitation_requests
					.remove(&ElicitationRequestKey::new(server_name.clone(), request_id.clone()));
			}
			Op::RequestPermissionsResponse { id, .. } => {
				self.request_permissions_call_ids.remove(id);
				Self::remove_call_id_from_turn_map(&mut self.request_permissions_call_ids_by_turn_id, id);
			}
	...
	pub(super) fn note_event(&mut self, event: &Event) {
		match &event.msg {
			...
			EventMsg::ElicitationRequest(ev) => {
				self
					.elicitation_requests
					.insert(ElicitationRequestKey::new(ev.server_name.clone(), ev.id.clone()));
			}
	...
	pub(super) fn note_evicted_event(&mut self, event: &Event) {
		match &event.msg {
			...
			EventMsg::ElicitationRequest(ev) => {
				self
					.elicitation_requests
					.remove(&ElicitationRequestKey::new(ev.server_name.clone(), ev.id.clone()));
			}
			EventMsg::RequestUserInput(ev) => {
				self.request_user_input_call_ids.remove(&ev.call_id);
				let mut remove_turn_entry = false;
				if let Some(call_ids) = self.request_user_input_call_ids_by_turn_id.get_mut(&ev.turn_id) {
				...
				if remove_turn_entry {
					self.request_user_input_call_ids_by_turn_id.remove(&ev.turn_id);
				}
			}
			EventMsg::RequestPermissions(ev) => {
				self.request_permissions_call_ids.remove(&ev.call_id);
				let mut remove_turn_entry = false;
				if let Some(call_ids) = self.request_permissions_call_ids_by_turn_id.get_mut(&ev.turn_id) {
					call_ids.retain(|call_id| call_id != &ev.call_id);
					if call_ids.is_empty() {
						remove_turn_entry = true;
					}
				...
				if remove_turn_entry {
					self.request_permissions_call_ids_by_turn_id.remove(&ev.turn_id);
				}
			}
			_ => {}
		}
	...
	pub(super) fn should_replay_snapshot_event(&self, event: &Event) -> bool {
		match &event.msg {
			EventMsg::ExecApprovalRequest(ev) => {
				self.exec_approval_call_ids.contains(&ev.effective_approval_id())
			}
			EventMsg::ApplyPatchApprovalRequest(ev) => self.patch_approval_call_ids.contains(&ev.call_id),
			EventMsg::ElicitationRequest(ev) => self
				.elicitation_requests
				.contains(&ElicitationRequestKey::new(ev.server_name.clone(), ev.id.clone())),
			EventMsg::RequestUserInput(ev) => self.request_user_input_call_ids.contains(&ev.call_id),
			EventMsg::RequestPermissions(ev) => self.request_permissions_call_ids.contains(&ev.call_id),
			_ => true,
		}
	}
...
mod tests {
	...
	fn thread_event_snapshot_keeps_pending_request_user_input() {
		...
		let request = Event {
			id: "ev-1".to_string(),
			msg: EventMsg::RequestUserInput(codex_protocol::request_user_input::RequestUserInputEvent {
