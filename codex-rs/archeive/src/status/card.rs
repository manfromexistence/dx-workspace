		...
		for row in rows {
			match &row.value {
				...
				StatusRateLimitValue::Text(text) => {
					...
					let spans = formatter.full_spans(label.as_str(), vec![Span::from(text.clone())]);
...
impl HistoryCell for StatusHistoryCell {
	fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		let note_first_line = Line::from(vec![
			...
			"https://chatgpt.com/codex/settings/usage".cyan().underlined(),
			Span::from(" for up-to-date").cyan(),
		]);
		let note_second_line =
			Line::from(vec![Span::from("information on rate limits and credits").cyan()]);
		...
		let truncated_lines: Vec<Line<'static>> =
			lines.into_iter().map(|line| truncate_line_to_width(line, inner_width)).collect();
...
fn format_model_provider(config: &Config) -> Option<String> {
	...
	let provider_name = if name.is_empty() { config.model_provider_id.as_str() } else { name };
