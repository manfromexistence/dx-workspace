...
fn test_auth_manager(config: &Config) -> AuthManager {
	AuthManager::new(config.codex_home.clone(), false, config.cli_auth_credentials_store_mode)
}
...
fn reset_at_from(captured_at: &chrono::DateTime<chrono::Local>, seconds: i64) -> i64 {
	(*captured_at + ChronoDuration::seconds(seconds)).with_timezone(&Utc).timestamp()
}
...
async fn status_snapshot_includes_reasoning_details() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 1, 2, 3, 4, 5).single().expect("timestamp");
...
async fn status_permissions_non_default_workspace_write_is_custom() {
	...
	config.model = Some("gpt-5.1-codex-max".to_string());
	config.model_provider_id = "openai".to_string();
	config.permissions.approval_policy.set(AskForApproval::OnRequest).expect("set approval policy");
	config
		.permissions
		.sandbox_policy
		.set(SandboxPolicy::WorkspaceWrite {
	...
	let permissions_line =
		rendered_lines.iter().find(|line| line.contains("Permissions:")).expect("permissions line");
...
async fn status_snapshot_includes_forked_from() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 8, 9, 10, 11, 12).single().expect("valid time");
...
async fn status_snapshot_includes_monthly_limit() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 5, 6, 7, 8, 9).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: Some(RateLimitWindow {
...
async fn status_snapshot_shows_unlimited_credits() {
	...
	let usage = TokenUsage::default();
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 2, 3, 4, 5, 6).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: None,
	...
	assert!(
		rendered.iter().any(|line| line.contains("Credits:") && line.contains("Unlimited")),
...
async fn status_snapshot_shows_positive_credits() {
	...
	let usage = TokenUsage::default();
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 3, 4, 5, 6, 7).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: None,
	...
	let rendered = render_lines(&composite.display_lines(120));
	assert!(
		rendered.iter().any(|line| line.contains("Credits:") && line.contains("13 credits")),
...
async fn status_snapshot_hides_zero_credits() {
	...
	let usage = TokenUsage::default();
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 4, 5, 6, 7, 8).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: None,
...
async fn status_snapshot_hides_when_has_no_credits_flag() {
	...
	let snapshot = RateLimitSnapshot {
		...
		primary: None,
		secondary: None,
		credits: Some(CreditsSnapshot { has_credits: false, unlimited: true, balance: None }),
		plan_type: None,
	};
	let rate_display = rate_limit_snapshot_display(&snapshot, captured_at);
	let model_slug = codex_core::test_support::get_model_offline(config.model.as_deref());
...
async fn status_card_token_usage_excludes_cached_tokens() {
	...
	let now = chrono::Local.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).single().expect("timestamp");
...
async fn status_snapshot_shows_missing_limits_message() {
	...
	let now = chrono::Local.with_ymd_and_hms(2024, 2, 3, 4, 5, 6).single().expect("timestamp");
...
async fn status_snapshot_includes_credits_and_limits() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 7, 8, 9, 10, 11).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: Some(RateLimitWindow {
...
async fn status_snapshot_shows_empty_limits_message() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 6, 7, 8, 9, 10).single().expect("timestamp");
	let rate_display = rate_limit_snapshot_display(&snapshot, captured_at);
...
async fn status_snapshot_cached_limits_hide_credits_without_flag() {
	...
	let captured_at =
		chrono::Local.with_ymd_and_hms(2024, 9, 10, 11, 12, 13).single().expect("timestamp");
	let snapshot = RateLimitSnapshot {
		limit_id: None,
		limit_name: None,
		primary: Some(RateLimitWindow {
...
async fn status_context_window_uses_last_usage() {
	...
	let now = chrono::Local.with_ymd_and_hms(2024, 6, 1, 12, 0, 0).single().expect("timestamp");
	...
	let rendered_lines = render_lines(&composite.display_lines(80));
	let context_line =
		rendered_lines.into_iter().find(|line| line.contains("Context window")).expect("context line");
