...
impl RateLimitWindowDisplay {
	fn from_window(window: &RateLimitWindow, captured_at: DateTime<Local>) -> Self {
		...
		Self { used_percent: window.used_percent, resets_at, window_minutes: window.window_minutes }
	}
}
...
pub(crate) fn compose_rate_limit_data_many(
	...
	for snapshot in snapshots {
		...
		let primary_label = snapshot
			...
			.as_ref()
			.map(|window| {
				window.window_minutes.map(get_limits_duration).unwrap_or_else(|| "5h".to_string())
			})
		...
		let secondary_label = snapshot
			...
			.as_ref()
			.map(|window| {
				window.window_minutes.map(get_limits_duration).unwrap_or_else(|| "weekly".to_string())
			})
			.map(|label| capitalize_first(&label));
		let window_count =
		...
		if let Some(secondary) = snapshot.secondary.as_ref() {
			let label = if combine_non_codex_single_limit {
				format!(
					"{} {} limit",
					limit_bucket_label,
					secondary_label.clone().unwrap_or_else(|| "weekly".to_string())
				)
			} else {
				format!("{} limit", secondary_label.clone().unwrap_or_else(|| "weekly".to_string()))
			};
			rows.push(StatusRateLimitRow {
				label,
				value: StatusRateLimitValue::Window {
...
mod tests {
	...
	fn non_codex_multi_limit_keeps_group_row() {
		...
		assert_eq!(
			labels,
			vec!["codex-other limit".to_string(), "1h limit".to_string(), "Weekly limit".to_string(),]
		);
	}
}
