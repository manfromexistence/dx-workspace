...
// Title position enum for ratatui 0.29.0 compatibility
#[derive(Clone, Debug)]
...
pub struct Border {
	...
	pub titles: Vec<(TitlePosition, ratatui::text::Line<'static>)>,
}
...
impl Widget for Border {
		...
		Self: Sized,
	{
		...
		for title in self.titles {
			block = match title {
				(TitlePosition::Top, line) => block.title_top(line),
				(TitlePosition::Bottom, line) => block.title_bottom(line),
			};
		}
