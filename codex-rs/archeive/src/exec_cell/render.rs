...
pub(crate) fn output_lines(
	...
	let CommandOutput { aggregated_output, .. } = match output {
		Some(output) if only_err && output.exit_code == 0 => {
			return OutputLines { lines: Vec::new(), omitted: None };
		}
		Some(output) => output,
		None => {
			return OutputLines { lines: Vec::new(), omitted: None };
	...
	let omitted = if show_ellipsis { Some(total - 2 * line_limit) } else { None };
	if show_ellipsis {
		let omitted = total - 2 * line_limit;
		out.push(format!("… +{omitted} lines").into());
	}

	let tail_start = if show_ellipsis { total - line_limit } else { head_end };
	...
	fn transcript_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		for (i, call) in self.iter_calls().enumerate() {
			...
			if let Some(output) = call.output.as_ref() {
				...
				let duration = call.duration.map(format_duration).unwrap_or_else(|| "unknown".to_string());
				let mut result: Line = if output.exit_code == 0 {
					Line::from("✓".green().bold())
				} else {
					Line::from(vec!["✗".red().bold(), format!(" ({})", output.exit_code).into()])
				};
...
impl ExecCell {
	fn exploring_display_lines(&self, width: u16) -> Vec<Line<'static>> {
		...
		out.push(Line::from(vec![
			...
			" ".into(),
			if self.is_active() { "Exploring".bold() } else { "Explored".bold() },
		]));
		...
		while !calls.is_empty() {
			...
			let reads_only =
				call.parsed.iter().all(|parsed| matches!(parsed, ParsedCommand::Read { .. }));
		...
		if let Some(output) = call.output.as_ref() {
			...
			if raw_output.lines.is_empty() {
				...
				for line in &raw_output.lines {
					push_owned_lines(&adaptive_wrap_line(line, output_opts.clone()), &mut wrapped_output);
				}
	...
	fn truncate_lines_middle(
		...
		if max_rows == 1 {
			...
			let extra = lines.len().saturating_sub(usize::from(omitted_hint.is_some()));
			let omitted = base + extra;
			return vec![Self::ellipsis_line_with_prefix(omitted, ellipsis_prefix.as_ref())];
		}
		...
		out.push(Self::ellipsis_line_with_prefix(base + additional, ellipsis_prefix.as_ref()));
...
impl ExecDisplayLayout {
	const fn new(
		...
		Self { command_continuation, command_continuation_max_lines, output_block, output_max_lines }
	}
}
...
mod tests {
	...
	fn user_shell_output_is_limited_by_screen_lines() {
		...
		let output = CommandOutput { exit_code: 0, aggregated_output, formatted_output: String::new() };
		let width = 20;
		let layout = EXEC_DISPLAY_LAYOUT;
		let raw_output = output_lines(
			Some(&output),
		...
		for line in &raw_output.lines {
			push_owned_lines(&adaptive_wrap_line(line, output_opts.clone()), &mut full_wrapped_output);
		}
		...
		let full_screen_lines =
			Paragraph::new(Text::from(full_prefixed_output)).wrap(Wrap { trim: false }).line_count(width);
		...
		let contains_ellipsis =
			lines.iter().any(|line| line.spans.iter().any(|span| span.content.contains("… +")));
		...
		assert!(contains_ellipsis, "expected truncated output to include an ellipsis line");
	}
