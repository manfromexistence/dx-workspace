...
impl StatusIndicatorWidget {
	...
	pub(crate) fn update_details(
		...
		self.details = details.filter(|details| !details.is_empty()).map(|details| {
	...
	pub(crate) fn update_inline_message(&mut self, message: Option<String>) {
		self.inline_message =
			message.map(|message| message.trim().to_string()).filter(|message| !message.is_empty());
	}
...
impl Renderable for StatusIndicatorWidget {
	...
	fn render(&self, area: Rect, buf: &mut Buffer) {
		...
		lines.push(truncate_line_with_ellipsis_if_overflow(Line::from(spans), usize::from(area.width)));
		if area.height > 1 {
...
mod tests {
	...
	fn details_args_can_disable_capitalization_and_limit_lines() {
		...
		let mut w = StatusIndicatorWidget::new(tx, crate::tui::FrameRequester::test_dummy(), true);
		w.update_details(
			Some("cargo test -p codex-core and then cargo test -p codex-tui-dx".to_string()),
			StatusDetailsCapitalization::Preserve,
			1,
		);

		assert_eq!(w.details(), Some("cargo test -p codex-core and then cargo test -p codex-tui-dx"));

		let lines = w.wrapped_details_lines(24);
		assert_eq!(lines.len(), 1);
		let last = lines.last().expect("expected one details line");
		assert!(
			last.spans.last().is_some_and(|span| span.content.as_ref().contains('…')),
