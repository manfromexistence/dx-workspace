...
pub(super) fn render_device_code_login(
	...
	if widget.animations_enabled {
		...
		widget.request_frame.schedule_frame_in(std::time::Duration::from_millis(100));
	...
	let verification_url = if let Some(device_code) = &state.device_code {
		...
		lines.push("".into());
		lines
			.push("  2. Enter this one-time code after you are signed in (expires in 15 minutes)".into());
		lines.push("".into());
		lines.push(Line::from(vec!["  ".into(), device_code.user_code.as_str().cyan().bold()]));
		lines.push("".into());
...
mod tests {
	...
	fn device_code_sign_in_state(cancel: Arc<Notify>) -> Arc<RwLock<SignInState>> {
		Arc::new(RwLock::new(SignInState::ChatGptDeviceCode(ContinueWithDeviceCodeState {
			device_code: None,
			cancel: Some(cancel),
		})))
	}
	...
	fn device_code_attempt_matches_only_for_matching_cancel() {
		...
		assert_eq!(device_code_attempt_matches(&state, &Arc::new(Notify::new())), false);
		assert_eq!(device_code_attempt_matches(&SignInState::PickMode, &cancel), false);
	}
	...
	fn set_device_code_state_for_active_attempt_updates_only_when_active() {
		...
		assert!(matches!(&*sign_in_state.read().unwrap(), SignInState::ChatGptDeviceCode(_)));
	}
	...
	fn set_device_code_success_message_for_active_attempt_updates_only_when_active() {
		...
		let auth_manager =
			AuthManager::shared(temp_dir.path().to_path_buf(), false, AuthCredentialsStoreMode::File);
		...
		assert!(matches!(&*sign_in_state.read().unwrap(), SignInState::ChatGptSuccessMessage));
