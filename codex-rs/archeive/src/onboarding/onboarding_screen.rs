...
impl OnboardingScreen {
	pub(crate) fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
		...
		Self { request_frame: tui.frame_requester(), steps, is_done: false, should_exit: false }
	}
	...
	pub(crate) fn is_done(&self) -> bool {
		self.is_done
			|| !self.steps.iter().any(|step| matches!(step.get_step_state(), StepState::InProgress))
	}
	...
	fn handle_key_event(&mut self, key_event: KeyEvent) {
		...
		if should_quit {
			...
			if let Some(Step::Welcome(widget)) =
				self.steps.iter_mut().find(|step| matches!(step, Step::Welcome(_)))
			{
			...
			if self.steps.iter().any(|step| {
				if let Step::TrustDirectory(widget) = step { widget.should_quit() } else { false }
			}) {
				self.should_exit = true;
				self.is_done = true;
			}
					...
					if !did_full_clear_after_success
						&& onboarding_screen.steps.iter().any(|step| {
							if let Step::Auth(w) = step {
								w.sign_in_state
									.read()
									.is_ok_and(|g| matches!(&*g, super::auth::SignInState::ChatGptSuccessMessage))
							} else {
								false
							}
						}) {
