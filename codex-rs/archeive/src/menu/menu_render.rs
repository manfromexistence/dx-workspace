...
impl Menu {
	pub fn render_in_area(
		...
		// Check if in content viewer mode
		if self.content_viewer_mode {
			self.render_content_viewer(area, buf);
			return;
		}
		...
		// Render window control dots (macOS style) on top-left
		let dots_x = content_area.x + 2;
		let dots_y = content_area.y;

		// Red dot (close)
		buf[(dots_x, dots_y)].set_char('●');
		buf[(dots_x, dots_y)].set_fg(ratatui::style::Color::Red);

		// Yellow dot (minimize)
		buf[(dots_x + 1, dots_y)].set_char('●');
		buf[(dots_x + 1, dots_y)].set_fg(ratatui::style::Color::Yellow);

		// Green dot (maximize)
		buf[(dots_x + 2, dots_y)].set_char('●');
		buf[(dots_x + 2, dots_y)].set_fg(ratatui::style::Color::Green);

		// Store the red dot area for click detection
		self.window_control_area = Rect { x: dots_x, y: dots_y, width: 1, height: 1 };

		// Render title in the center of the top border
		let title_text = format!(" {} ", menu_title);
		let title_x = content_area.x + (content_area.width.saturating_sub(title_text.len() as u16)) / 2;
		for (i, ch) in title_text.chars().enumerate() {
			let x = title_x + i as u16;
			if x < content_area.x + content_area.width {
				buf[(x, dots_y)].set_char(ch);
				buf[(x, dots_y)].set_fg(self.theme.accent);
				buf[(x, dots_y)].set_style(Style::default().add_modifier(Modifier::BOLD));
			}
		}
		...
		for idx in self.scroll_offset..end_idx {
			...
			let item_text = if description == "TOGGLE_MODE" {
				...
				format!("{} {}", title, mode_indicator)
			} else if description.starts_with("MODEL:") {
				// Extract provider name from "MODEL:id|Provider" format
				let provider = description.split('|').nth(1).unwrap_or("");
				let left_part = title.to_string();
				let right_part = provider;
				...
				&& !description.starts_with("SUBMENU:")
			{
				let left_part = title.to_string();
	...
	// Render content viewer mode
	fn render_content_viewer(&mut self, area: Rect, buf: &mut Buffer) {
		let content_bg = self.theme.card;

		// Create a centered content area with fixed height
		let content_width = (area.width * 7 / 10).min(80);
		let content_height = 20.min(area.height); // Fixed height of 20 lines
		...
		// Render title in the center of the top border
		let title = format!(" {} ", self.content_viewer_title);
		let title_x = content_area.x + (content_area.width.saturating_sub(title.len() as u16)) / 2;
		...
		let lines: Vec<Line> = self
			.content_viewer_text
			.lines()
			.map(|line| Line::from(Span::styled(line, Style::default().fg(self.theme.fg))))
			.collect();
