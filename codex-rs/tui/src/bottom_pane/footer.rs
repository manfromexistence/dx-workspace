...
///
/// Note: Status line content (model/context/path) is rendered right-aligned separately
...
pub(crate) fn render_footer_from_props(
    ...
    // If we have a status line, render it right-aligned
    if let Some(status_line) = passive_footer_status_line(props) {
        render_context_right(area, buf, &status_line.dim());
    } else if !lines.is_empty() {
        // Otherwise render instructional footer content left-aligned
        Paragraph::new(prefix_lines(
            lines,
            " ".repeat(FOOTER_INDENT_COLS).into(),
            " ".repeat(FOOTER_INDENT_COLS).into(),
...
///
/// Note: The status line (when present) should be rendered right-aligned via
/// `render_context_right` by the caller, not through this function's return value.
fn footer_from_props_lines(
    ...
    // Passive footer context (status line with model/context/path details) should be
    // rendered separately via render_context_right for right-alignment, not here.
    // This function only handles instructional footer content.
    if passive_footer_status_line(props).is_some() {
        return vec![];
    }
    match props.mode {
        FooterMode::QuitShortcutReminder => {
