...
## DX Project (codex-rs/dx)

**CRITICAL: For the `dx` project ONLY, use `cargo run` to test changes. DO NOT use `cargo build`, `cargo check`, `cargo test`, or `cargo fmt --check`.**

**NEVER BUILD OR CHECK - ONLY RUN:**
- ❌ DO NOT use: `cargo build`, `cargo check`, `cargo test`, `cargo clippy`, `cargo fmt --check`
- ✅ ONLY use: `cargo run` to test changes
- ✅ ONLY use: `cargo fmt` (without --check) to format code
- The dx binary may be locked during development, so `cargo build` will fail with "Access is denied"
- Building creates artifacts in target/ which slows down the system on low-end devices
- Always use `cargo run` which handles the binary lock gracefully and doesn't create extra artifacts

**Workflow:**
1. Make code changes
2. Run `cargo fmt` in `codex-rs/dx` directory (formats without checking)
3. Run `cargo run` in `codex-rs/dx` directory (compiles and runs in one step)
...
- `cargo run` is sufficient for testing and development
- Avoid any commands that create build artifacts
