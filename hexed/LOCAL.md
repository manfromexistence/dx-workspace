In our Codex-rust CLI please add local gRPC server directly module running functionality like this. Make sure to create a new provider and make it the default feature. Don't create another feature flag for it; it's default behavior. It should be the default behavior of our Codex-rust CLI and configure our Codex-rust CLI to work with this context window and model running capabilities correctly:

```rust//! Local LLM integration using llama.cpp
